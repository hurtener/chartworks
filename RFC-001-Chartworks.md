...
Pipelines are versioned SQL-only steps with declared inputs/output/destination, strategy and blocking checks. Bruin remains behind `PipelineRunner`, not a recreated Go orchestration platform. Preserve applicable create/replace, append, merge/incremental, interval and scd2 strategies via tested per-engine support. Pin the runner and prove telemetry disabled, secret-safe argv/environment, bounded resources and strict machine-readable validate/lineage. The qualified D-067 runner fork derives from Bruin v0.11.749. Its embedded Python parser/runtime is a supervised deployment dependency with private bounded cache/scratch; it does not widen the accepted asset contract to user Python/R, ingestr or arbitrary shell execution.

The phase 13 implementation pins D-067 fork source `5f562c2959496a04d57f5f199f5e3ad22159fa9f` and the qualified Linux arm64 Go 1.26.4 executable by SHA-256 `4fb5c4f12057c3240b7a641f408a329b15d0b2c938b786ba978938196cee5405`. It registers distinct draft, publication, run and read actions. Its [managed-pipeline contract](docs/contracts/managed-pipelines.md) keeps authority, validation, destination ownership, quality activation and reconciliation in Chartworks. Phase 13 is accepted and shipped at exact head `6883bc2103b2b870595222e623cd4d79bc01bc41` by [qualifying hosted CI run 34182486766](https://github.com/hurtener/chartworks/actions/runs/34182486766); the [current evidence ledger](docs/reviews/phase-13-current-evidence.md) records the local and hosted coverage distinction.

For phase-14 warehouse reads, [D-067](docs/decisions/2026-09-07-bruin-read-adoption.md) adopts selective in-process leaf clients from the same pinned minimal fork. The universal manager and stock query CLI are outside that read path. Chartworks retains authority, opaque plan admission, positive safety proof, limits, attempts and reconciliation. Phase 14 is accepted and shipped at the same exact head and hosted run. The [warehouse-driver contract](docs/contracts/warehouse-drivers.md) records the exact native and recorded-fixture support boundaries; recorded cloud tests do not claim live-cloud cutover evidence.
...
Preserve deterministic span hints, remote query embeddings, typed/batched authorized retrieval, published/healthy eligibility, optional Bifrost reranking, versioned evidence confidence and explicit no-route/clarify outcomes. When no topic is pinned, server selection considers only complete signed-reachable current publication manifests; reranking sees only already authorized candidates. Reviewed non-sensitive governed values, explicit categorical geography designation and temporal policy produce sealed typed interpretation constraints with locale/parser/anchor and exact source/topic pins. Instant spans bind unique reviewed-zone boundaries as RFC3339; malformed connector years and missing/folded civil boundaries clarify before provider work. Active interpretation evidence requires protected base SQL/parameters and exact receipt reconstruction on execution/replay. Ambiguity fails closed, correction/removal is replayable, and interpretation never grants authority or issues a plan. Retain English/Spanish fixtures, prior context/SQL and session-scoped follow-up deltas. Confirmed same-source multi-topic joins/cardinality and all-resource restrictions are migration scope; arbitrary federation is not implied. D-076 and the routing/interpretation contract govern the implemented boundary; live correctness calibration remains phase-24 evidence.
