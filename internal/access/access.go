...
func valid(r Resource)bool{
 if !identity.Identifier(r.Tenant)||!identity.Identifier(r.ID){return false}
...
func reaches(e identity.Envelope,r Resource)bool{
 if !valid(r)||r.Tenant!=e.Tenant(){return false}
 for _,s:=range e.Reach(){if s.Kind==r.Kind&&s.Permission==r.Permission&&(s.ID==r.ID||s.ID=="*"){return true}}
 return false
}
...
func Require(e identity.Envelope,action string,resources ...Resource)error{
 if !e.Valid(){return ErrUnauthenticated};if !e.Has(action){return ErrForbidden}
 if len(resources)==0{return ErrNotFound}
 for _,r:=range resources{if !reaches(e,r){return ErrNotFound}};return nil
}
// Tenant binds an operational requirement to the signed tenant, not a caller tenant.
func Tenant(e identity.Envelope,permission string)Resource{return Resource{e.Tenant(),"tenant",permission,e.Tenant()}}
...
func StoreScope(e identity.Envelope,action,permission string)(store.Scope,error){
 if err:=Require(e,action,Tenant(e,permission));err!=nil{return store.Scope{},err}
...
type Selection struct{tenant string;ids []string;all bool}
...
func(s Selection) Contains(tenant,id string)bool{
 if s.tenant==""||tenant!=s.tenant||!identity.Identifier(id){return false}
 if s.all{return true};i:=sort.SearchStrings(s.ids,id);return i<len(s.ids)&&s.ids[i]==id
}
...
func Constrain(e identity.Envelope,action,kind,permission string)(Selection,error){
 if !e.Valid(){return Selection{},ErrUnauthenticated};if !e.Has(action){return Selection{},ErrForbidden}
 if _,err:=identity.ParseReach("cw."+kind+"."+permission+":probe");err!=nil{return Selection{},ErrNotFound}
 s:=Selection{tenant:e.Tenant()}
 for _,r:=range e.Reach(){if r.Kind!=kind||r.Permission!=permission{continue};if r.ID=="*"{s.all=true}else if kind!="tenant"||r.ID==e.Tenant(){s.ids=append(s.ids,r.ID)}}
 sort.Strings(s.ids);if !s.all&&len(s.ids)==0{return Selection{},ErrNotFound};return s,nil
}
// Execution describes actual resolved domain references, not user-supplied metadata.
type Execution struct{Target Resource;Dependencies,Contexts []Resource}
...
func RequireExecution(e identity.Envelope,x Execution)error{
 if x.Target.Permission!="execute"||len(x.Dependencies)==0||len(x.Contexts)==0{return ErrNotFound}
 refs:=[]Resource{x.Target}
 for _,r:=range x.Dependencies{if r.Permission!="query"&&r.Permission!="execute"{return ErrNotFound};refs=append(refs,r)}
 for _,r:=range x.Contexts{if r.Kind!="execution_context"||r.Permission!="use"{return ErrNotFound};refs=append(refs,r)}
...
type Artifact struct{Tenant,RunID,ParentKind,ParentID string;Published,Private bool;Contexts []Resource}
...
func RequireArtifact(e identity.Envelope,a Artifact)error{
 if !e.Valid(){return ErrUnauthenticated};if !e.Has("reporting.read"){return ErrForbidden}
 if a.Tenant!=e.Tenant()||!identity.Identifier(a.RunID)||!identity.Identifier(a.ParentID)||(a.ParentKind!="report"&&a.ParentKind!="block")||len(a.Contexts)==0{return ErrNotFound}
 run:=Resource{a.Tenant,"run","read",a.RunID};parent:=Resource{a.Tenant,a.ParentKind,"read",a.ParentID}
 if !reaches(e,run)&&(!a.Published||a.Private||!reaches(e,parent)){return ErrNotFound}
 for _,r:=range a.Contexts{if r.Kind!="execution_context"||r.Permission!="use"||!reaches(e,r){return ErrNotFound}}
 if a.Private{return Require(e,"reporting.preview",Resource{a.Tenant,a.ParentKind,"preview",a.ParentID})};return nil
}
