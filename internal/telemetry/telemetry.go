...
func New(w io.Writer, format string) (*Reporter, error) {
	...
	if err := r.registry.Register(r.ready); err != nil {
		return nil, errors.New("telemetry: metric registration failed")
	}
	for _, e := range []Event{Started, Stopped, ConfigurationRejected, Request, DependencyChanged, AuditCommitted} {
		for _, o := range []string{"ok", "error"} {
			r.events.WithLabelValues(string(e), o).Add(0)
		}
	}
	for _, d := range []string{"store", "verification_keys"} {
		r.ready.WithLabelValues(d).Set(0)
	}
	return r, nil
}
...
func (r *Reporter) Record(ctx context.Context, event Event, success bool) error {
	switch event {
	case Started, Stopped, ConfigurationRejected, Request, DependencyChanged, AuditCommitted:
	default:
		return errors.New("telemetry: unknown event")
	}
	outcome := "error"
	if success {
		outcome = "ok"
...
func (r *Reporter) Dependency(name string, ready bool) error {
	if name != "store" && name != "verification_keys" {
		return errors.New("telemetry: unknown dependency")
	}
	value := 0.
	if ready {
		value = 1
...
func MetricNames() []string {
	return []string{"chartworks_events_total", "chartworks_dependency_ready"}
}
