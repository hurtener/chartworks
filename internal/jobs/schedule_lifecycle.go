...
func (r ScheduleHistoryRequest) Valid() bool {
	if r.Limit < 1 || r.Limit > 100 { return false }
	if r.Kind == "revisions" { return r.BeforeDue == nil && r.BeforeRevision >= 0 && r.BeforeRevision < 1<<62 }
	if r.Kind != "occurrences" || r.BeforeRevision != 0 { return false }
...
func (s *Service) RetireSchedule(ctx context.Context, e identity.Envelope, id string, expected int64) (Schedule, error) {
	if s == nil || ctx == nil || !identity.Identifier(id) || expected < 1 || expected >= 1<<62 { return Schedule{}, ErrInvalid }
	if err := access.Require(e, "scheduling.write", access.Resource{Tenant: e.Tenant(), Kind: "schedule", Permission: "write", ID: id}); err != nil { return Schedule{}, err }
	scope, err := store.NewScope(e.Tenant(), e.User())
	if err != nil { return Schedule{}, err }
	repo, ok := s.repo.(ScheduleLifecycleRepository)
	if !ok { return Schedule{}, ErrTransient }
...
func (s *Service) History(ctx context.Context, e identity.Envelope, id string, request ScheduleHistoryRequest) (ScheduleHistory, error) {
	if s == nil || ctx == nil || !identity.Identifier(id) || !request.Valid() { return ScheduleHistory{}, ErrInvalid }
	if err := access.Require(e, "scheduling.read", access.Resource{Tenant: e.Tenant(), Kind: "schedule", Permission: "read", ID: id}); err != nil { return ScheduleHistory{}, err }
	scope, err := store.NewScope(e.Tenant(), e.User())
	if err != nil { return ScheduleHistory{}, err }
	repo, ok := s.repo.(ScheduleLifecycleRepository)
	if !ok { return ScheduleHistory{}, ErrTransient }
	ctx, cancel := context.WithDeadline(ctx, e.Deadline()); defer cancel()
...
func (s *Service) TestSchedule(ctx context.Context, e identity.Envelope, id, key string, expected int64) (Job, error) {
	if s == nil || ctx == nil || !identity.Identifier(id) || !identity.Identifier(key) || expected < 1 || expected >= 1<<62 { return Job{}, ErrInvalid }
	if err := access.Require(e, "scheduling.execute", access.Resource{Tenant: e.Tenant(), Kind: "schedule", Permission: "execute", ID: id}); err != nil { return Job{}, err }
	scope, err := store.NewScope(e.Tenant(), e.User())
	if err != nil { return Job{}, err }
	ctx, cancel := context.WithDeadline(ctx, e.Deadline()); defer cancel()
	current, err := s.repo.ReadSchedule(ctx, scope, id)
	if err != nil { return Job{}, err }
	if current.Revision != expected || current.Retired || !current.Enabled { return Job{}, store.ErrConflict }
	if _, err := s.admission(ctx, e, current.Request.Target); err != nil { return Job{}, err }
