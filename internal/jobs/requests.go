...
func (r RequestInput) Require(e identity.Envelope) error {
	...
	if r.Kind=="upload.erase" { action,permission="sources.erase","erase" }
	if r.Kind=="profile.build" { action,permission="engineering.profile","write" }
	refs:=[]access.Resource{{Tenant:e.Tenant(),Kind:"source",Permission:permission,ID:r.Target}}
	if r.Context!="" { refs=append(refs,access.Resource{Tenant:e.Tenant(),Kind:"execution_context",Permission:"use",ID:r.Context}) }
...
func (t RequestTask) Require(e identity.Envelope) error {
	if !t.Valid() || t.Tenant!=e.Tenant() || t.Actor!=e.User() || t.Session!=e.Session() { return store.ErrNotFound }
...
type RequestLease struct { Task RequestTask; Owner string; Fence int64; Attempt int; Until time.Time }
...
type Invocation struct { lease RequestLease; authority identity.Envelope; owned bool }
...
func (i Invocation) Valid() bool { return i.owned && i.lease.Task.Valid() && identity.Identifier(i.lease.Owner) && i.lease.Fence>0 && i.lease.Attempt>0 && i.lease.Task.Tenant==i.authority.Tenant() && i.lease.Task.Actor==i.authority.User() && i.lease.Task.Session==i.authority.Session() }
...
func (i Invocation) Current(kind,target,hash string) (identity.Envelope,error) {
	if !i.Valid() || i.lease.Task.Input.Kind!=kind || i.lease.Task.Input.Target!=target || i.lease.Task.Input.InputHash!=hash { return identity.Envelope{},ErrAuthority }
	if err:=i.lease.Task.Require(i.authority); err!=nil { return identity.Envelope{},err }; return i.authority,nil
}
...
type RequestRunner struct { repo RequestRepository; limits Limits }
...
func NewRequestRunner(repo RequestRepository,limits Limits) (*RequestRunner,error) {
	if repo==nil || limits.Validate()!=nil { return nil,ErrInvalid }; return &RequestRunner{repo:repo,limits:limits},nil
}
...
func (r *RequestRunner) Admit(ctx context.Context,e identity.Envelope,key string,input RequestInput) (RequestTask,error) {
	if ctx==nil || !identity.Identifier(key) { return RequestTask{},ErrInvalid }
	if err:=input.Require(e); err!=nil { return RequestTask{},err }
...
func (r *RequestRunner) Inspect(ctx context.Context,e identity.Envelope,id string) (RequestTask,error) {
	if ctx==nil || !e.Valid() || !identity.Identifier(id) { return RequestTask{},ErrInvalid }
...
func (r *RequestRunner) Cancel(ctx context.Context,e identity.Envelope,id string) (RequestTask,error) {
	if _,err:=r.Inspect(ctx,e,id); err!=nil { return RequestTask{},err }; return r.repo.CancelRequest(ctx,e,id)
}
...
func (r *RequestRunner) Resume(ctx context.Context,e identity.Envelope,id string) (RequestTask,error) {
	if _,err:=r.Inspect(ctx,e,id); err!=nil { return RequestTask{},err }; return r.repo.ResumeRequest(ctx,e,id)
}
...
func (r *RequestRunner) Run(ctx context.Context,e identity.Envelope,task RequestTask,timeout time.Duration,handler func(context.Context,Invocation) error) (RequestTask,error) {
	if ctx==nil || handler==nil || timeout<time.Millisecond || timeout>time.Minute { return RequestTask{},ErrInvalid }
	if err:=task.Require(e); err!=nil { return RequestTask{},err }
	ctx,expire:=context.WithDeadline(ctx,e.Deadline()); defer expire()
	ctx,deadline:=context.WithTimeout(ctx,timeout); defer deadline()
	if err:=ctx.Err(); err!=nil { return RequestTask{},err }
	var id [16]byte; if _,err:=rand.Read(id[:]);err!=nil{return RequestTask{},store.ErrUnavailable}
	...
	work,cancel:=context.WithCancel(ctx); defer cancel()
	stop,done:=make(chan struct{}),make(chan error,1)
	go func(){
		ticker:=time.NewTicker(min(r.limits.Heartbeat,100*time.Millisecond)); defer ticker.Stop(); last:=time.Now()
		for { select {
		case <-stop:done<-nil;return
		case <-ctx.Done():done<-nil;return
		case <-ticker.C:
			pulse,end:=context.WithTimeout(work,time.Second); renew:=time.Since(last)>=r.limits.Heartbeat
			state,e:=r.repo.PulseRequest(pulse,invocation,renew,r.limits.Lease); end()
			if e!=nil { cancel();done<-e;return }
			if state=="succeeded" {done<-nil;return}
			if state!="running" {cancel();done<-context.Canceled;return}
			if renew {last=time.Now()}
		} }
	}()
	var once sync.Once; var watchErr error
	join:=func(){once.Do(func(){close(stop);watchErr=<-done})}; defer join()
	runErr:=handler(work,invocation); join()
	if watchErr!=nil {runErr=watchErr}
	cleanup,finish:=context.WithTimeout(context.WithoutCancel(ctx),2*time.Second); defer finish()
	if runErr!=nil {
		code:="attempt_failed"; if errors.Is(runErr,context.Canceled)||errors.Is(runErr,context.DeadlineExceeded){code="attempt_timeout"}
		if err=r.repo.FailRequest(cleanup,invocation,code,false,r.limits.Backoff);err!=nil&&!errors.Is(err,store.ErrConflict){return RequestTask{},err}
	}
	...
	if err!=nil {return RequestTask{},err}
	if runErr==nil && out.State!="succeeded" {return out,store.ErrConflict}
	return out,runErr
}
