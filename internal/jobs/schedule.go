...
func (s Spec) Validate() error {
	...
	switch s.Type {
	case "manual":
		if s.Cron != "" || s.IntervalSeconds != 0 || !s.Anchor.IsZero() {
			return ErrInvalid
		}
	case "interval":
		if s.Cron != "" || s.IntervalSeconds < 60 || s.IntervalSeconds > 31*86400 || s.Anchor.IsZero() || s.Anchor.Year() < 2000 || s.Anchor.Year() > 2100 || s.Anchor.Nanosecond() != 0 {
			return ErrInvalid
		}
	case "cron":
		if s.IntervalSeconds != 0 || !s.Anchor.IsZero() || len(s.Cron) > 128 || strings.ContainsAny(s.Cron, "@=\r\n") {
			return ErrInvalid
		}
		if _, err := s.parsed(); err != nil {
			return err
		}
	default:
		return ErrInvalid
...
func (s Spec) parsed() (cron.Schedule, error) {
	...
	parsed, err := cron.NewParser(cron.Minute | cron.Hour | cron.Dom | cron.Month | cron.Dow).Parse(s.Cron)
	if err != nil {
		return nil, ErrInvalid
	}
...
func (s Spec) Next(after time.Time) (time.Time, error) {
	if s.Validate() != nil || after.Year() < 2000 || after.Year() > 2100 {
	...
	if s.Type == "interval" {
		...
		n := (after.Unix() - anchor) / s.IntervalSeconds
		if after.Unix() < anchor {
			return s.Anchor.UTC(), nil
		}
		return time.Unix(anchor+(n+1)*s.IntervalSeconds, 0).UTC(), nil
	}
	schedule, err := s.parsed()
	if err != nil {
...
func (s Spec) Previous(at time.Time) (time.Time, error) {
	if s.Validate() != nil || at.Year() < 2000 || at.Year() > 2100 {
		return time.Time{}, ErrInvalid
	}
	if s.Type == "manual" {
	...
	if s.Type == "interval" {
		if at.Before(s.Anchor) {
			return s.Anchor.UTC().Add(-time.Duration(s.IntervalSeconds) * time.Second), nil
		}
		return time.Unix(s.Anchor.Unix()+((at.Unix()-s.Anchor.Unix())/s.IntervalSeconds)*s.IntervalSeconds, 0).UTC(), nil
	}
	schedule, err := s.parsed()
	if err != nil {
		return time.Time{}, err
	}
	lo, hi := at.Unix()-366*5*86400, at.Unix()
	first := schedule.Next(time.Unix(lo, 0))
	if first.IsZero() || first.After(at) {
		return time.Time{}, ErrInvalid
	}
	for lo < hi {
		mid := lo + (hi-lo)/2
		next := schedule.Next(time.Unix(mid, 0))
	...
	previous := time.Unix(lo, 0).UTC()
	if !schedule.Next(previous.Add(-time.Second)).Equal(previous) {
		return time.Time{}, ErrInvalid
	}
	return previous, nil
}
