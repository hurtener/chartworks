...
func Defaults()Limits{return Limits{Workers:4,GlobalConcurrency:16,TenantConcurrency:2,MaxPending:10000,MaxPendingPerTenant:1000,MaxAttempts:3,Batch:100,Lease:15*time.Second,Heartbeat:5*time.Second,Poll:500*time.Millisecond,AttemptTimeout:10*time.Second,Backoff:time.Second}}
func(l Limits)Validate()error{
 if l.Workers<1||l.Workers>32||l.GlobalConcurrency<l.Workers||l.GlobalConcurrency>128||l.TenantConcurrency<1||l.TenantConcurrency>l.GlobalConcurrency||l.MaxPending<1||l.MaxPending>100000||l.MaxPendingPerTenant<1||l.MaxPendingPerTenant>l.MaxPending||l.MaxAttempts<1||l.MaxAttempts>8||l.Batch<1||l.Batch>1000||l.Lease<time.Second||l.Lease>time.Minute||l.Heartbeat<10*time.Millisecond||l.Heartbeat>=l.Lease/2||l.Poll<10*time.Millisecond||l.Poll>5*time.Second||l.AttemptTimeout<100*time.Millisecond||l.AttemptTimeout>time.Minute||l.Backoff<10*time.Millisecond||l.Backoff>30*time.Second{return ErrInvalid};return nil
}
// QueueFingerprint excludes per-process worker/poll settings, but pins global admission bounds.
func(l Limits)QueueFingerprint()string{b,_:=json.Marshal([]int{l.GlobalConcurrency,l.TenantConcurrency,l.MaxPending,l.MaxPendingPerTenant});h:=sha256.Sum256(b);return hex.EncodeToString(h[:])}

// Submission is closed: there is no caller-selected actor, tenant, credential, SQL or timestamp.
type Submission struct {Kind string `json:"kind"`;BindingID string `json:"binding_id"`}
func(s Submission)Validate()error{if s.Kind!=MaintenanceKind||!BindingID(s.BindingID){return ErrInvalid};return nil}
func BindingID(s string)bool{return identity.Identifier(s)&&len(s)<=64&&!strings.Contains(s,":")}
...
type Lease struct {Job Job;Owner string;Fence int64;Attempt int;Until time.Time}

// Authority is implemented by the concrete Pengui HTTP adapter, not a local signer.
type Authority interface {Acquire(context.Context,Job)(identity.Envelope,error)}
...
func AssertExecution(e identity.Envelope,j Job)error{
 if !j.Valid()||!e.Valid()||e.Tenant()!=j.Tenant||e.User()!=j.Executor||e.Session()!=j.ID{return ErrAuthority}
 if err:=access.Require(e,"ops.maintain",access.Resource{Tenant:j.Tenant,Kind:"tenant",Permission:"erase",ID:j.Tenant},access.Resource{Tenant:j.Tenant,Kind:"execution_binding",Permission:"use",ID:j.BindingID},access.Resource{Tenant:j.Tenant,Kind:"run",Permission:"execute",ID:j.ID});err!=nil{return ErrAuthority};return nil
}
...
type Repository interface {
 ...
 FireSchedule(context.Context,store.Scope,string,string,Limits)(Job,error)
