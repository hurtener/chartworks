...
func (s *Service) clarificationPreflight(ctx context.Context, e identity.Envelope, in QuestionRequest) (*admission, error) {
	...
	if !ok {
		if len(in.Answers) > 0 {
			return nil, ErrClarificationUnsupported
		}
		return nil, nil
	}
	...
	if route.Clarification != nil || len(route.Resolutions) == 0 {
		return a, nil
	}
	if route.ClarificationPins == nil || route.ClarificationPins.Tenant != e.Tenant() || route.ClarificationPins.Actor != e.User() || route.ClarificationPins.Session != e.Session() || route.ClarificationPins.Context != in.Context {
		return nil, ErrClarificationBinding
	}
	for _, pin := range route.ClarificationPins.Sources {
		if a.source == "" {
			a.source, a.context = pin.Source, pin.Context
		}
		if pin.Source != a.source || pin.Context != a.context {
			return nil, ErrClarificationBinding
		}
	}
	if a.source == "" {
...
func (s *Service) clarificationFields(ctx context.Context, e identity.Envelope, route nlqroute.RouteResult, binding exec.Binding) ([]clarificationField, error) {
	...
	if !binding.Valid() || binding.Tenant != e.Tenant() || binding.Context != route.Request.Context {
		return nil, ErrClarificationBinding
	}
	byTopic := map[string][]semantics.ClarificationResolution{}
	for _, r := range route.Resolutions {
		if r.ID == "" || r.ID != semantics.ClarificationResolutionDigest(r) {
			return nil, ErrClarificationBinding
		}
	...
	for _, topic := range route.Topics {
		...
		for _, r := range resolutions {
			if p.State.Version != r.TopicVersion || p.Digest != r.PackDigest || !p.State.Active || p.State.Archived {
				return nil, staleClarification(route.Request.Locale)
			}
			if r.Reference != nil {
				continue
			}
			if r.Effect == nil || binding.Dialect != "postgres" {
				return nil, ErrClarificationUnsupported
			}
			ref := r.Effect.Target
			...
			for _, d := range p.Definition.Datasets {
				...
				if d.Source.Source != binding.Source || d.Source.Context != binding.Context || d.Source.SourceRevision != binding.Revision {
					return nil, ErrClarificationBinding
				}
				relationID = d.Source.Dataset
				for _, c := range d.Columns {
					if c.ID == ref.ID {
						columnName = c.SourceName
						break
					}
				}
			}
			...
			for _, relation := range binding.Relations {
				...
				for _, column := range relation.Columns {
					if column.Name == columnName && column.Safe {
						field.column = column
						found = true
						break
					}
			...
			if !found || !clarificationColumnType(field) {
				return nil, ErrClarificationUnsupported
			}
			if !r.Null {
				parameterCount++
				if r.Time != nil || r.Effect.Operator == "range" {
					parameterCount++
				}
			}
...
func clarificationColumnType(f clarificationField) bool {
	...
	case "boolean":
		return n == "boolean" || n == "bool"
	case "entity", "text":
		return n == "text" || n == "varchar" || strings.HasPrefix(n, "varchar(") || n == "character varying" || strings.HasPrefix(n, "character varying(")
	case "time_window":
		switch e.TemporalType {
		case "date":
			return n == "date"
		case "timestamp":
			return n == "timestamp" || n == "timestamp without time zone"
		case "timestamptz":
			return n == "timestamptz" || n == "timestamp with time zone"
		}
	}
	return false
}
...
func bindClarificationCandidate(a admission, candidate generatedCandidate) (generatedCandidate, error) {
	if len(a.clarificationFields) == 0 {
		return candidate, nil
	}
	if a.binding.Dialect != "postgres" || len(candidate.SQL) > maxSQLBytes {
		return generatedCandidate{}, ErrClarificationUnsupported
	}
	parsed, err := pgquery.Parse(candidate.SQL)
	if err != nil || len(parsed.Stmts) != 1 {
		return generatedCandidate{}, ErrClarificationBinding
	}
	root := parsed.Stmts[0].Stmt.GetSelectStmt()
	if root == nil || root.WithClause != nil || root.Larg != nil || root.Rarg != nil || len(root.FromClause) == 0 {
		return generatedCandidate{}, ErrClarificationUnsupported
	}
	unsupported := false
	visitClarificationMessages(parsed, func(m proto.Message) bool {
		switch m.(type) {
		case *pgtypes.RangeSubselect, *pgtypes.SubLink, *pgtypes.CommonTableExpr:
			unsupported = true
			return false
		}
		return true
	...
	visitClarificationMessages(parsed, func(m proto.Message) bool {
		...
		for _, f := range a.clarificationFields {
			if rv.Relname == f.relation.Name && (rv.Schemaname == f.relation.Schema || rv.Schemaname == "") {
				if rv.Schemaname == "" {
					count := 0
					for _, rel := range a.binding.Relations {
						if rel.Name == rv.Relname {
							count++
						}
					}
					if count != 1 {
						bindErr = ErrClarificationUnsupported
						return false
					}
				}
		...
		for _, c := range relation.Columns {
			if c.Safe {
				columns = append(columns, quoteClarificationIdentifier(c.Name))
			}
		}
		if len(columns) == 0 || len(parameters) > 64 {
			bindErr = exec.ErrLimit
			return false
		}
		...
		if err != nil || len(filter.Stmts) != 1 {
			bindErr = ErrClarificationBinding
			return false
		}
		replacement := filter.Stmts[0].Stmt.GetSelectStmt().FromClause[0]
		if rv.Alias != nil {
			replacement.GetRangeSubselect().Alias = proto.Clone(rv.Alias).(*pgtypes.Alias)
		}
	...
	if bindErr != nil {
		return generatedCandidate{}, bindErr
	}
	for _, f := range a.clarificationFields {
		if !used[f.resolution.ID] {
			return generatedCandidate{}, ErrClarificationBinding
		}
	}
	...
	if err != nil || len(statement) > maxSQLBytes {
		return generatedCandidate{}, ErrClarificationBinding
	}
...
func quoteClarificationIdentifier(value string) string {
...
func clarificationPredicate(f clarificationField, parameters *[]exec.Parameter) (string, error) {
	...
	if e == nil || r.ID != semantics.ClarificationResolutionDigest(r) {
	...
	if r.Time != nil {
		...
		if e.TemporalType == "timestamptz" {
			lower, upper = r.Time.StartUTC, r.Time.EndUTC
		}
	...
	m.Range(func(field protoreflect.FieldDescriptor, value protoreflect.Value) bool {
		if field.IsList() && field.Kind() == protoreflect.MessageKind {
			...
			for i := 0; i < list.Len(); i++ {
				visitClarificationMessages(list.Get(i).Message().Interface(), visit)
			}
		} else if field.Kind() == protoreflect.MessageKind && !field.IsMap() {
			visitClarificationMessages(value.Message().Interface(), visit)
		}
		return true
	})
}
...
func sameClarificationEvidence(a, b nlqroute.RouteResult) bool {
	...
	left, _ := json.Marshal(struct {
		Pins        *nlqroute.ClarificationPins
		Resolutions []semantics.ClarificationResolution
	}{a.ClarificationPins, a.Resolutions})
	right, _ := json.Marshal(struct {
		Pins        *nlqroute.ClarificationPins
		Resolutions []semantics.ClarificationResolution
	}{b.ClarificationPins, b.Resolutions})
