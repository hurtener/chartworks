...
func (s *Service) CaptureDefinition(ctx context.Context, e identity.Envelope, id string) (CaptureEvidence, error) {
	...
	if ctx == nil || s == nil || !identity.Identifier(id) { return out, ErrInvalid }
	if !e.Valid() { return out, access.ErrUnauthenticated }
	if !e.Has("reporting.sql.read") { return out, ErrInspectionRequired }
	...
	if err != nil { return out, err }
	if q.Session != e.Session() { return out, ErrForeignSession }
	if !slices.Contains([]string{"succeeded", "empty", "truncated"}, q.Status) || q.Result == nil || q.EvidenceStale || strings.TrimSpace(q.SQL) == "" || len(q.SQL) > maxSQLBytes || len(q.Result.Schema) == 0 { return out, ErrNoPlan }
	admitted, err := s.retainedAdmission(ctx, e, q)
	if err != nil { return out, err }
	if err := access.Require(e, "query.execute", admitted.resources...); err != nil { return out, err }
	...
	for i, topic := range q.Topics {
		...
		if err != nil { return CaptureEvidence{}, err }
		out.Publications = append(out.Publications, contract.Publication)
	}
	if err := ctx.Err(); err != nil { return CaptureEvidence{}, err }
	if !e.Valid() { return CaptureEvidence{}, access.ErrUnauthenticated }
	return out, nil
}
