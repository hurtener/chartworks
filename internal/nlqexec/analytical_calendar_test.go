...
func TestSQLRecoveryCalendarCompilerReviewedBucket(t *testing.T) {
 for _,tc := range []struct{question, grain string; columns int}{
  ...
  t.Run(tc.question,func(t *testing.T){
   a:=calendarAdmission(tc.question,"timestamptz"); before:=exec.Hash(a.route)
   c,err:=compileAnalytical(context.Background(),a)
   if err!=nil || c==nil || c.Version!=exec.AnalyticalCalendarVersion || c.Grain==nil || c.Grain.Policy!=exec.AnalyticalCalendarPolicy || len(c.Grain.Buckets)!=1 || len(c.Grain.Columns)!=tc.columns {t.Fatal("compiled bucket",err)}
   b:=c.Grain.Buckets[0]
   if b.Grain!=tc.grain || b.Column!="event_native" || b.Calendar!="gregorian" || b.Timezone!="America/New_York" || exec.Hash(a.route)!=before {t.Fatal("wrong reviewed coordinates or mutation",b)}
   if !strings.Contains(analyticalGrainGuidance(c),"date_trunc") || !strings.Contains(analyticalGrainGuidance(c),"event_native") {t.Fatal("missing generation guidance")}
  })
 }
 for _,native:=range []string{"date","timestamp without time zone"} {
  ...
  if err!=nil || c.Grain.Buckets[0].Timezone!="" {t.Fatal("civil time acquired an instant timezone",err)}
 }
}
...
func TestSQLRecoveryCalendarCompilerRefusesUnreviewedPolicy(t *testing.T) {
 for _,change:=range []func(*admission){
  ...
  func(a *admission){a.publications[0].Definition.Dimensions[2].Temporal.Grains=[]semantics.TimeGrain{"day"}},
  ...
  a:=calendarAdmission("Revenue by month of Order date","timestamptz"); change(&a); analyticalReseal(&a)
  if _,err:=compileAnalytical(context.Background(),a);!errors.Is(err,exec.ErrAnalyticalUnsupported){t.Fatal("unreviewed calendar admitted",err)}
 }
 for _,q:=range []string{"Revenue by month of Unknown","Revenue by month of","Revenue by month of Order date and","Revenue by Region and month of Unknown"} {
  if _,err:=compileAnalytical(context.Background(),calendarAdmission(q,"date"));!errors.Is(err,exec.ErrAnalyticalUnsupported){t.Fatal("partial proof",q,err)}
 }
}
...
func TestSQLRecoveryCalendarCompilerUnknownAndLiteralIntent(t *testing.T) {
 for _,q:=range []string{"Revenue by month","Revenue filtered by month of Order date","Revenue sin agrupar por mes de fecha de pedido","Revenue for “by month of Order date”","Revenue not grouped by month of Order date"} {
  ...
  if err!=nil || c==nil || c.Grain!=nil {t.Fatal("unknown/literal/negative became calendar",q,err)}
 }
 a:=calendarAdmission("Revenue by Month","date")
 a.publications[0].Definition.Dimensions[0].Name="Month"; analyticalReseal(&a)
 c,err:=compileAnalytical(context.Background(),a)
 if err!=nil || len(c.Grain.Buckets)!=0 || !reflect.DeepEqual(c.Grain.Columns,[]string{"region_native"}) {t.Fatal("direct reviewed label lost precedence",err)}
 ...
 c,err=compileAnalytical(context.Background(),a)
 if err!=nil || c.Grain!=nil {t.Fatal("private value selected a calendar",err)}
}
...
func TestSQLRecoveryCalendarRetainedVersionsAndReplay(t *testing.T) {
 ...
 for _,v:=range []int{1,2} {
  ...
  if err!=nil || c==nil || c.Grain!=nil || c.Version==exec.AnalyticalCalendarVersion {t.Fatal("retained policy reinterpreted",v,err)}
 }
 c,err:=compileAnalytical(context.Background(),a);if err!=nil{t.Fatal(err)}
 ...
 if !AnalyticalRecordValid(q){t.Fatal("new receipt shape rejected")}
 got,err:=expectedAnalytical(context.Background(),q,a);if err!=nil || !reflect.DeepEqual(c,got){t.Fatal("new replay",err)}
 for _,change:=range []func(*QueryRecord){
  ...
  bad:=q;bad.Analytical=cloneAnalyticalReceipt(q.Analytical);change(&bad)
  if _,err:=expectedAnalytical(context.Background(),bad,a);err==nil{t.Fatal("calendar proof removed or downgraded")}
 }
 changed:=calendarAdmission("Revenue by month of Order date","timestamptz")
 changed.publications[0].Definition.Dimensions[2].Temporal.Timezone="UTC";analyticalReseal(&changed)
 if _,err:=expectedAnalytical(context.Background(),q,changed);err==nil{t.Fatal("different timezone replayed")}
}
