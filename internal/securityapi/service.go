...
type Service struct{repo store.Foundation;maintenance *maintenance.Service}
// New requires the real repository; there is no ambient authority or in-memory default.
func New(repo store.Foundation)(*Service,error){m,err:=maintenance.New(repo,100,30*time.Second);if err!=nil{return nil,err};return &Service{repo,m},nil}
...
func(s *Service) Policy(ctx context.Context,e identity.Envelope)(store.Policy,error){
 scope,err:=access.StoreScope(e,"ops.read","read");if err!=nil{return store.Policy{},err};return s.repo.Policy(ctx,scope)
}
...
func(s *Service) Configure(ctx context.Context,e identity.Envelope,expected int64,p store.Policy)(store.Policy,error){
 scope,err:=access.StoreScope(e,"ops.write","write");if err!=nil{return store.Policy{},err};return s.maintenance.Configure(ctx,scope,expected,p)
}
...
func(s *Service) Audits(ctx context.Context,e identity.Envelope,limit int)([]store.Audit,error){
 scope,err:=access.StoreScope(e,"ops.audit","read");if err!=nil{return nil,err};return s.repo.Audits(ctx,scope,limit)
}
...
func(s *Service) Sweep(ctx context.Context,e identity.Envelope,key string)(store.Operation,error){
 scope,err:=access.StoreScope(e,"ops.maintain","erase");if err!=nil{return store.Operation{},err};return s.maintenance.Sweep(ctx,scope,key)
}
