...
func Handler(v *auth.Verifier, s *Service, r *telemetry.Reporter, metrics bool) http.Handler {
	return v.Middleware(auth.HTTP, http.HandlerFunc(func(w http.ResponseWriter, req *http.Request) {
		...
		e, err := identity.FromContext(req.Context())
		if err != nil {
		...
		if err = access.Require(e, op.Action, access.Tenant(e, op.Permission)); err != nil {
			_ = r.Record(req.Context(), telemetry.Request, false)
			failure(w, err)
			return
		}
		// Unknown query fields cannot become authority or silently alter a protected operation.
		query, err := parseQuery(req, op.Path)
		if err != nil {
			failure(w, store.ErrInvalid)
			return
		}
		if req.Method == "GET" && (req.ContentLength != 0 || len(req.TransferEncoding) != 0) {
			failure(w, store.ErrInvalid)
			return
		}
		...
		case "/v1/retention-policy":
			...
			p, err := s.Configure(req.Context(), e, body.Expected, store.Policy{AuditDays: body.AuditDays, OperationHours: body.OperationHours})
			if err != nil {
				failure(w, err)
				return
			}
		...
		case "/v1/audit-events":
			...
			a, err := s.Audits(req.Context(), e, limit)
			if err != nil {
				failure(w, err)
				return
			}
			out := make([]cw.Audit, 0, len(a))
			for _, v := range a {
				out = append(out, cw.Audit{ID: v.ID, Actor: v.Actor, Action: v.Action, Resource: v.Resource, CreatedAt: v.CreatedAt})
			}
			respond(w, out)
		case "/v1/retention-sweeps":
			if len(req.Header.Values("Idempotency-Key")) != 1 || !store.Identifier(req.Header.Get("Idempotency-Key")) {
				failure(w, store.ErrInvalid)
				return
			}
			var empty struct{}
			if !bodyDecode(w, req, &empty) {
				return
			}
			result, err := s.Sweep(req.Context(), e, req.Header.Get("Idempotency-Key"))
			if err != nil {
				failure(w, err)
				return
			}
...
func parseQuery(req *http.Request, path string) (string, error) {
	...
	if req.URL.RawQuery != "" {
		q, err = parseRawQuery(req.URL.RawQuery)
	}
	if err != nil {
		return "", store.ErrInvalid
	}
	for k, v := range q {
		if path != "/v1/audit-events" || k != "limit" || len(v) != 1 {
			return "", store.ErrInvalid
		}
	}
...
func failure(w http.ResponseWriter, err error) {
	...
	case errors.Is(err, access.ErrNotFound) || errors.Is(err, store.ErrNotFound):
		...
		code = "not_found"
	case errors.Is(err, store.ErrInvalid) || errors.Is(err, store.ErrScope):
