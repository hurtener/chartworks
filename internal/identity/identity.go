...
func ParseReach(s string) (Reach,error) {
 ...
 if !ok || len(p)!=3 || p[0]!="cw" || (id!="*"&&!Identifier(id)) { return r,ErrInvalid }
 if !member(p[1],"source","dataset","topic","block","report","dashboard","run","execution_context","execution_binding","tenant") || !member(p[2],"read","query","write","execute","preview","publish","certify","export","use","erase") { return r,ErrInvalid }
 return Reach{p[1],p[2],id},nil
}
func member(s string, values ...string) bool { for _, v:=range values { if s==v {return true} }; return false }
...
func FromVerified(tenant,user,session string, scopes []string, until time.Time, now func()time.Time) (Envelope,error) {
 if !Identifier(tenant)||!Identifier(user)||!Identifier(session)||until.IsZero()||len(scopes)>128 {return Envelope{},ErrInvalid}
 if strings.HasPrefix(strings.ToLower(user),"svc:") && (!strings.HasPrefix(user,"svc:")||len(user)==4) {return Envelope{},ErrInvalid}
 if now==nil {now=time.Now}
 e:=Envelope{tenant:tenant,user:user,session:session,until:until,now:now}
 seen:=map[string]bool{}; total:=0
 for _, s:=range scopes {
  total+=len(s)
  if len(s)==0||len(s)>256||total>8192||seen[s] {return Envelope{},ErrInvalid}
  for _,c:=range s {if c<33||c>126 {return Envelope{},ErrInvalid}}
  seen[s]=true
  if strings.HasPrefix(s,"cw.") {r,err:=ParseReach(s);if err!=nil{return Envelope{},err};e.reach=append(e.reach,r)}
  e.scopes=append(e.scopes,s)
 }
 if !e.Valid(){return Envelope{},ErrInvalid};return e,nil
}
...
func(e Envelope) Has(s string)bool {if !e.Valid(){return false};for _,v:=range e.scopes{if v==s{return true}};return false}
...
func(e Envelope) Context(ctx context.Context)(context.Context,error){if !e.Valid(){return nil,ErrInvalid};return context.WithValue(ctx,contextKey{},e),nil}
// FromContext returns a current envelope, never a header/body-derived fallback.
func FromContext(ctx context.Context)(Envelope,error){e,ok:=ctx.Value(contextKey{}).(Envelope);if !ok||!e.Valid(){return Envelope{},ErrInvalid};return e,nil}
