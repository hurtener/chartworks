...
func ParseReach(s string) (Reach, error) {
	...
	if !member(p[1], "schedule", "source", "dataset", "topic", "block", "report", "dashboard", "run", "onboarding", "execution_context", "execution_binding", "tenant") || !member(p[2], "read", "query", "write", "execute", "preview", "publish", "certify", "export", "use", "erase", "cancel") {
		return r, ErrInvalid
	}
