...
func (s *Service) Recheck(ctx context.Context, e identity.Envelope, in RouteRequest) (RouteResult, error) {
 if ctx == nil || s == nil || s.topics == nil || s.rules == nil || s.assembler == nil { return RouteResult{}, ErrInvalid }
 if !e.Valid() { return RouteResult{}, access.ErrUnauthenticated }
...
func (s *Service) prepareClarifications(ctx context.Context, e identity.Envelope, in RouteRequest, ids []string) ([]admittedTopic, RouteResult, []nlq.PinnedMetric, error) {
 ...
 if len(ids) > 1 { result.Outcome = nlq.StrategyMultiTopic }
 admitted := make([]admittedTopic, 0, len(ids))
 started := time.Now()
 for _, id := range ids {
  contract, err := s.topics.Contract(ctx, e, id)
  if err != nil { return nil, RouteResult{}, nil, err }
  publication := contract.Publication
  if publication.State.Topic != id || publication.Definition.Topic != id || publication.Definition.Version != publication.State.Version || publication.State.Archived || !publication.State.Active || publication.State.Version == "" || !topics.DigestValid(publication.Digest) { return nil, RouteResult{}, nil, store.ErrConflict }
 ...
 if !contextMatches(admitted, in.Context) { return nil, RouteResult{}, nil, readexec.ErrBinding }
 ...
 for i, item := range admitted {
  ...
  if item.hasRules { result.RuleVersions[i] = item.rules.State.Version }
 ...
 sort.Slice(pins.Sources, func(i,j int) bool { a,b:=pins.Sources[i],pins.Sources[j]; if a.Topic!=b.Topic{return a.Topic<b.Topic}; return a.Dataset<b.Dataset })
 byTopic := map[string][]semantics.ClarificationAnswer{}
 for _, answer := range in.Answers {
  found := false
  for _, item := range admitted { if item.id == answer.Topic { found=true; break } }
  if !found { return nil, RouteResult{}, nil, ErrInvalid }
 ...
 for _, choice := range in.Choices {
  ...
  for _, item := range admitted {
   if !item.hasRules { continue }
   for _, pattern := range item.rules.Definition.Patterns {
    if choice.Pattern!="" && choice.Pattern!=pattern.ID { continue }
    for _, slot := range pattern.Slots { if slot.ID==choice.Slot { owner,patternID,matches=item.id,pattern.ID,matches+1 } }
   }
  }
  if matches!=1 { return nil, RouteResult{}, nil, ErrInvalid }
 ...
 for i := range admitted {
  item:=&admitted[i]
  if !item.hasRules {
   if len(byTopic[item.id])!=0 { return nil,RouteResult{},nil,ErrInvalid }
   continue
  }
  ...
  if err != nil { return nil,RouteResult{},nil,err }
  if model.Digest()!=item.rules.Digest { return nil,RouteResult{},nil,store.ErrConflict }
  ...
  for _, id := range in.MetricIDs {
   for _, m := range item.publication.Definition.Measures { if m.ID==id {refs=appendUniqueRef(refs,semantics.Reference{Kind:semantics.KindMeasure,ID:id})} }
   for _, m := range item.publication.Definition.KPIs { if m.ID==id {refs=appendUniqueRef(refs,semantics.Reference{Kind:semantics.KindKPI,ID:id})} }
  }
  ...
  if evaluation.Outcome==semantics.ClarificationInvalid || evaluation.Outcome==semantics.ClarificationConflicting || evaluation.Outcome==semantics.ClarificationMissing { continue }
  refs=evaluation.References
  if len(refs)==0 {
   if len(item.publication.Definition.Datasets)==0 {return nil,RouteResult{},nil,store.ErrConflict}
  ...
  if err!=nil {return nil,RouteResult{},nil,err}
  if checked.TopicVersion!=item.publication.State.Version || checked.PackDigest!=item.publication.Digest || checked.RuleVersion!=item.rules.State.Version || checked.RuleDigest!=item.rules.Digest {return nil,RouteResult{},nil,store.ErrConflict}
  state:=&nlq.ConstraintState{Allowed:checked.Result.Allowed}
  for _, ref:=range checked.Result.Required {state.Required=append(state.Required,nlq.MandatoryConstraint{ID:referenceID(ref),Kind:"required",Text:referenceText(ref)})}
  for _, ref:=range checked.Result.Excluded {state.Excluded=append(state.Excluded,nlq.MandatoryConstraint{ID:referenceID(ref),Kind:"excluded",Text:referenceText(ref)})}
  ...
  if !state.Allowed {result.Clarification=&Clarification{Reason:"mandatory_constraint",Prompt:"Choose a metric or dimension that satisfies the published rules."}}
 ...
 for _, evaluation:=range evaluations {
  ...
  case semantics.ClarificationInvalid: outcome=semantics.ClarificationInvalid
  case semantics.ClarificationConflicting: if outcome!=semantics.ClarificationInvalid {outcome=semantics.ClarificationConflicting}
  case semantics.ClarificationMissing: if outcome==semantics.ClarificationSatisfied {outcome=semantics.ClarificationMissing}
  }
 }
 if outcome!=semantics.ClarificationSatisfied {
  ...
  for _, evaluation:=range evaluations { result.Clarification.Errors=append(result.Clarification.Errors,evaluation.Errors...) }
  for _, q:=range result.ClarificationOutcomes { if q.Outcome==semantics.ClarificationMissing && q.Prompt!="" {result.Clarification.Pattern,result.Clarification.Slot,result.Clarification.Prompt=q.Pattern,q.Slot,q.Prompt; for _, c:=range q.Choices {result.Clarification.Choices=append(result.Clarification.Choices,ClarificationChoice{ID:c.ID,Label:c.Label})};break} }
  if outcome==semantics.ClarificationInvalid || outcome==semantics.ClarificationConflicting {result.Resolutions=nil}
 }
 result.Request.Answers=semantics.CanonicalClarificationAnswers(result.Resolutions)
 if len(result.ClarificationOutcomes)>0 || len(result.Resolutions)>0 {result.ClarificationPins=pins}
 if result.Clarification==nil && len(admitted)>1 {result.Clarification=confirmJoins(admitted,in.JoinChoices)}
 result.Stages=append(result.Stages,Stage{Name:"admission",DurationMS:time.Since(started).Milliseconds()})
 if result.Clarification!=nil {result.Outcome=nlq.StrategyClarify;return admitted,result,metrics,nil}
...
func clarificationSubject(p topics.Published) (semantics.RuleSubject,error) {
 ...
 for _, dataset:=range d.Datasets {pack.Datasets=append(pack.Datasets,semantics.Dataset{ID:dataset.ID,Name:dataset.Name,Columns:append([]semantics.Column(nil),dataset.Columns...)})}
...
func resolutionContext(r semantics.ClarificationResolution) string {
 type effectView struct {Kind string `json:"kind"`;Target semantics.Reference `json:"target"`;Operator string `json:"operator"`;Nulls string `json:"nulls"`;Unit string `json:"unit,omitempty"`;Bounds string `json:"bounds,omitempty"`;Calendar string `json:"calendar,omitempty"`;TimeZone string `json:"time_zone,omitempty"`;Grain string `json:"grain,omitempty"`}
 view:=struct{ID string `json:"resolution"`;Reference *semantics.Reference `json:"reference,omitempty"`;Effect *effectView `json:"effect,omitempty"`;Binding string `json:"binding"`}{ID:r.ID,Reference:r.Reference,Binding:"Server binds reviewed values before validation. Do not invent or duplicate these filters."}
 if r.Effect!=nil {e:=r.Effect;view.Effect=&effectView{Kind:e.Kind,Target:e.Target,Operator:e.Operator,Nulls:e.Nulls,Unit:e.Unit,Bounds:e.Bounds,Calendar:e.Calendar,TimeZone:e.TimeZone};if r.Time!=nil {view.Effect.Grain=r.Time.Grain}}
...
func CloneAnswers(in []semantics.ClarificationAnswer) []semantics.ClarificationAnswer {
 ...
 raw,err:=json.Marshal(in);if err!=nil{return nil}
 var out []semantics.ClarificationAnswer
 if json.Unmarshal(raw,&out)!=nil{return nil};return out
}
