...
func (c *Clarification) Unwrap() error {
	if c.Outcome == semantics.ClarificationInvalid { return ErrInvalid }
	if c.Outcome == semantics.ClarificationConflicting { return store.ErrConflict }
...
func clarificationFailure(locale nlq.Language, field, code string) *Clarification {
	...
	if locale == nlq.LanguageSpanish { message = "Actualizá la aclaración para el tema, la política, la sesión y el contexto actuales; después ingresá una respuesta tipada." }
...
func clarificationContext(e identity.Envelope, admitted []admittedTopic) string {
	...
	for _, item := range admitted {
		...
		if item.hasRules { pin.RulesVersion, pin.RulesDigest, pin.RulesRevision = item.rules.State.Version, item.rules.Digest, item.rules.State.Revision }
		for _, dataset := range item.publication.Definition.Datasets { pin.Sources = append(pin.Sources, dataset.Source) }
		sort.Slice(pin.Sources, func(i, j int) bool { return pin.Sources[i].Dataset < pin.Sources[j].Dataset })
		if item.binding != nil { pin.BindingDigest = readexec.Hash(*item.binding) }
		pins = append(pins, pin)
	}
	sort.Slice(pins, func(i, j int) bool { return pins[i].Topic < pins[j].Topic })
	return readexec.Hash(struct { Schema int; Tenant, Actor, Session string; Topics []clarificationTopicPin }{semantics.ClarificationSchemaVersion, e.Tenant(), e.User(), e.Session(), pins})
}
...
func scopedClarificationInputs(in RouteRequest, admitted []admittedTopic) (map[string]semantics.ClarificationInput, error) {
	...
	for _, item := range admitted { inputs[item.id] = semantics.ClarificationInput{Locale: string(in.Locale), Question: in.Question, References: append([]semantics.Reference(nil), in.References...)} }
	for _, answer := range semantics.CloneClarificationAnswers(in.Answers) {
		input, ok := inputs[answer.Topic]
		if !ok { return nil, clarificationFailure(in.Locale, "answers.topic", "foreign_answer") }
	...
	for _, choice := range in.Choices {
		...
		for _, item := range admitted {
			for _, pattern := range item.rules.Definition.Patterns {
				if choice.Pattern != "" && choice.Pattern != pattern.ID { continue }
				for _, slot := range pattern.Slots {
					if slot.ID == choice.Slot { owner, matches = item.id, matches+1 }
				}
			}
		}
		if matches != 1 { return nil, clarificationFailure(in.Locale, "choices", "foreign_or_ambiguous_choice") }
		input := inputs[owner]
...
func (s *Service) prepareClarifications(ctx context.Context, e identity.Envelope, in RouteRequest, admitted []admittedTopic, result *RouteResult) error {
	...
	for _, index := range order {
		...
		if !item.hasRules {
			if len(input.Answers)+len(input.LegacyChoices) > 0 { return clarificationFailure(in.Locale, "answers", "no_reviewed_policy") }
			continue
		}
		...
		for _, state := range evaluation.Slots {
			if state.Effect != nil && (state.Outcome != semantics.ClarificationNotApplicable || state.Reason == "optional_omitted") { needsBinding = true }
		}
		if needsBinding {
			reader, ok := s.topics.(clarificationBindingReader)
			if !ok { return &readexec.BusinessConstraintError{Code: "unsupported_constraint_binding", Field: "source"} }
			for _, dataset := range item.publication.Definition.Datasets {
				...
				if err != nil { return err }
				if !binding.Valid() || binding.Tenant != e.Tenant() || binding.Context != in.Context || binding.Source != dataset.Source.Source || binding.Revision != dataset.Source.SourceRevision { return readexec.ErrBinding }
				if item.binding != nil && readexec.Hash(*item.binding) != readexec.Hash(binding) { return readexec.ErrBinding }
				copy := binding.Clone()
				item.binding = &copy
			}
		}
	...
	for _, index := range order {
		if admitted[index].clarifications != nil { resolutions = append(resolutions, admitted[index].clarifications.Resolutions...) }
	}
	...
	for _, answer := range in.Answers {
		for _, item := range admitted {
			if item.id != answer.Topic { continue }
			for _, pattern := range item.rules.Definition.Patterns {
				if pattern.ID != answer.Pattern { continue }
				for _, slot := range pattern.Slots {
					if slot.ID == answer.Slot && slot.Sensitivity == semantics.LiteralSensitive { redactions = append(redactions, semantics.ClarificationResolution{Topic: answer.Topic, Pattern: answer.Pattern, Slot: answer.Slot, Sensitivity: slot.Sensitivity}) }
				}
			}
		}
	}
	...
	for _, index := range order {
		...
		for i := range item.clarifications.Resolutions {
			...
			if r.Provenance != "reviewed_default" && r.Provenance != "typed_interpretation" { r.Provenance = "answer" }
	...
	if len(in.Choices) > 0 { result.Warnings = append(result.Warnings, "legacy_reference_choices_normalized") }
	if len(result.Resolutions) == 0 && !missing { result.Request.AnswerContext = "" }
	if missing {
		...
		if in.Locale == nlq.LanguageSpanish { clarification.Why = "Cada campo obligatorio resuelve una condición de negocio revisada. Los campos dependientes siguen a sus requisitos." }
		for _, group := range result.Clarifications {
			...
			for _, question := range group.Slots {
				if question.Outcome != semantics.ClarificationMissing { continue }
				...
				if clarification.Prompt == "" && question.Prompt != "" {
					...
					for _, choice := range question.Choices { clarification.Choices = append(clarification.Choices, ClarificationChoice{ID: choice.ID, Label: choice.Label}) }
				}
			}
		}
	...
	for _, item := range admitted {
		...
		for _, resolution := range item.clarifications.Resolutions {
			if resolution.Effect == nil { continue }
			if item.binding == nil { return readexec.ErrBinding }
			constraint, err := businessConstraint(item, resolution)
			if err != nil { return err }
			if err := readexec.ValidateBusinessConstraints(*item.binding, []readexec.BusinessConstraint{constraint}); err != nil { return err }
			result.business = append(result.business, constraint)
			digest := readexec.Hash(*item.binding)
			if result.SourceBindingDigest != "" && result.SourceBindingDigest != digest { return readexec.ErrBinding }
...
func businessConstraint(item admittedTopic, resolution semantics.ClarificationResolution) (readexec.BusinessConstraint, error) {
	...
	if effect == nil { return readexec.BusinessConstraint{}, readexec.ErrBinding }
	dataset, column, err := rulesets.ClarificationColumn(item.publication.Definition, effect.Target)
	if err != nil { return readexec.BusinessConstraint{}, err }
	...
	if effect.Target.Kind == semantics.KindMeasure {
		...
		if effect.Kind != "number" { return out, &readexec.BusinessConstraintError{Code: "unsupported_aggregate_effect", Resolution: resolution.ID, Field: "target"} }
		for _, measure := range item.publication.Definition.Measures {
			if measure.ID == effect.Target.ID {
				if effect.Unit != measure.Unit { return out, &readexec.BusinessConstraintError{Code: "conflicting_unit", Resolution: resolution.ID, Field: "unit"} }
	...
	if resolution.Time != nil {
		...
		if effect.TemporalType == "timestamptz" { out.Value, out.Upper = resolution.Time.StartUTC, resolution.Time.EndUTC }
	}
	return out, nil
}
...
func (r RouteResult) ResolvedBusinessConstraints() ([]readexec.BusinessConstraint, error) {
	if _, err := r.GenerationContext(); err != nil { return nil, err }
	if r.resolutionSeal == "" || r.resolutionSeal != r.resolutionProof() { return nil, readexec.ErrBinding }
	for _, resolution := range r.Resolutions {
		found := false
		if r.Context.Constraints != nil {
			for _, constraint := range r.Context.Constraints.Required { if constraint.ID == "clarification-"+resolution.ID { found = true } }
		}
		if !found { return nil, readexec.ErrBinding }
	}
...
func appendResolvedConstraints(state *nlq.ConstraintState, resolutions []semantics.ClarificationResolution, full bool) (*nlq.ConstraintState, error) {
	if len(resolutions) == 0 { return state, nil }
	if state == nil { state = &nlq.ConstraintState{Allowed: true} }
	for _, resolution := range resolutions {
		text, err := semantics.ProviderClarificationText(resolution)
		if full { text, err = semantics.ClarificationBudgetText(resolution) }
...
func (s *Service) preflightClarificationBudget(ctx context.Context, in RouteRequest, admitted []admittedTopic, result RouteResult, metrics []nlq.PinnedMetric) (nlq.Tier, error) {
	...
	if constraints != nil {
		kept := constraints.Required[:0]
		for _, constraint := range constraints.Required {
			if constraint.Kind != "clarification" { kept = append(kept, constraint) }
		}
	...
	for _, tier := range []nlq.Tier{nlq.TierLow, nlq.TierMedium, nlq.TierHigh} {
		if _, err := s.assembler.Assemble(ctx, input, tier); err == nil { return tier, nil } else if !errors.Is(err, nlq.ErrInsufficient) { return "", err } else { last = err }
	}
	return "", last
}
...
func selectedClarificationMetrics(ids []string, resolutions []semantics.ClarificationResolution) []string {
	...
	for _, resolution := range resolutions {
		if resolution.Reference == nil || resolution.Reference.Kind != semantics.KindMeasure && resolution.Reference.Kind != semantics.KindKPI { continue }
		found := false
		for _, id := range out { found = found || id == resolution.Reference.ID }
		if !found { out = append(out, resolution.Reference.ID) }
	}
	return out
}
...
func (s *Service) ReplayClarifications(ctx context.Context, e identity.Envelope, previous RouteResult) ([]readexec.BusinessConstraint, string, error) {
	...
	if !contextMatches(admitted, in.Context) { return nil, "", readexec.ErrBinding }
	current := RouteResult{Outcome: previous.Outcome}
	if err := s.prepareClarifications(ctx, e, in, admitted, &current); err != nil { return nil, "", err }
	if current.Clarification != nil { return nil, "", current.Clarification }
	old, _ := json.Marshal(previous.Resolutions)
	fresh, _ := json.Marshal(current.Resolutions)
	if string(old) != string(fresh) || previous.AnswerContext != current.AnswerContext || previous.SourceBindingDigest != current.SourceBindingDigest { return nil, "", clarificationFailure(in.Locale, "answers", "stale_answer_resolution") }
