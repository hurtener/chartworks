...
const usage = "usage: chartworks version | config-check [--defaults | --config PATH] | serve --config PATH [--listen IP:PORT] | mcp --config PATH [--listen IP:PORT] | client --help | eval gate|inspect --suite PATH | eval perf-inspect|perf-smoke --profile PATH [--report PATH]\n"
