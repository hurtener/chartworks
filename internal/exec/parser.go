...
func (r *sqlResolver) selectStatement(v any, inherited map[string][]string) ([]string, error) {
	...
	if !only(m, "distinctClause", "targetList", "fromClause", "whereClause", "groupClause", "groupDistinct", "havingClause", "windowClause", "valuesLists", "sortClause", "limitOffset", "limitCount", "limitOption", "op", "all", "larg", "rarg", "withClause") {
		return nil, ErrUnsafe
	}
	...
	if with := m["withClause"]; with != nil {
		...
		if !only(w, "ctes", "recursive") || truth(w["recursive"]) {
			return nil, ErrUnsupported
		}
		local := map[string]bool{}
		for _, item := range array(w["ctes"]) {
			cte := fieldObject(item, "CommonTableExpr")
			if !only(cte, "ctename", "aliascolnames", "ctematerialized", "ctequery") {
				return nil, ErrUnsafe
			}
			name := text(cte["ctename"])
			if !SQLIdentifier(name) || local[name] {
				return nil, ErrUnsafe
			}
			cols, err := r.selectStatement(cte["ctequery"], scope.ctes)
			if err != nil {
				return nil, err
			}
			aliases, ok := names(cte["aliascolnames"])
			if !ok || len(aliases) > len(cols) {
				return nil, ErrUnsafe
			}
			for i, alias := range aliases {
				if !SQLIdentifier(alias) {
					return nil, ErrUnsupported
				}
				cols[i] = alias
			}
			scope.ctes[name] = cols
			local[name] = true
		}
	}
	op := text(m["op"])
	if op != "" && op != "SETOP_NONE" {
		if op != "SETOP_UNION" && op != "SETOP_INTERSECT" && op != "SETOP_EXCEPT" {
			return nil, ErrUnsupported
		}
		if len(array(m["targetList"])) != 0 || len(array(m["fromClause"])) != 0 || m["whereClause"] != nil || m["havingClause"] != nil || len(array(m["groupClause"])) != 0 || len(array(m["valuesLists"])) != 0 {
			return nil, ErrUnsafe
		}
		left, err := r.selectStatement(map[string]any{"SelectStmt": m["larg"]}, scope.ctes)
		if err != nil {
			return nil, err
		}
		right, err := r.selectStatement(map[string]any{"SelectStmt": m["rarg"]}, scope.ctes)
		if err != nil {
			return nil, err
		}
		...
		scope.outputs = left
		if err = r.trailing(m, scope); err != nil {
			return nil, err
		}
		return left, nil
	}
	if m["larg"] != nil || m["rarg"] != nil {
		return nil, ErrUnsafe
	}
	for _, item := range array(m["fromClause"]) {
		if err := r.from(item, scope); err != nil {
			return nil, err
		}
	}
	for _, item := range array(m["windowClause"]) {
		window := fieldObject(item, "WindowDef")
		name := text(window["name"])
		if !SQLIdentifier(name) || scope.windows[name] {
			return nil, ErrUnsafe
		}
		scope.windows[name] = true
	}
	if values := array(m["valuesLists"]); len(values) > 0 {
		if len(scope.sources) > 0 || len(array(m["targetList"])) > 0 {
			return nil, ErrUnsafe
		}
		width := 0
		...
		for i := 0; i < width; i++ {
			scope.outputs = append(scope.outputs, fmt.Sprintf("column%d", i+1))
		}
	} else {
		targets := array(m["targetList"])
		if len(targets) < 1 || len(targets) > 256 {
			return nil, ErrLimit
		}
		for _, item := range targets {
			target := fieldObject(item, "ResTarget")
			if !only(target, "name", "val") {
				return nil, ErrUnsafe
			}
			if err := r.expr(target["val"], scope, false); err != nil {
				return nil, err
			}
			name := text(target["name"])
			if name != "" && !SQLIdentifier(name) {
				return nil, ErrUnsupported
			}
			if name == "" {
				name = resultName(target["val"])
			}
			scope.outputs = append(scope.outputs, name)
		}
	}
	for _, field := range []string{"whereClause", "havingClause"} {
		if err := r.optional(m[field], scope, false); err != nil {
			return nil, err
		}
	}
	...
	for _, item := range array(m["windowClause"]) {
		if err := r.window(fieldObject(item, "WindowDef"), scope); err != nil {
			return nil, err
		}
	}
	if err := r.trailing(m, scope); err != nil {
...
func resultName(v any) string {
	m := object(v)
	if col := object(m["ColumnRef"]); col != nil {
		parts, ok := names(col["fields"])
		if ok && len(parts) > 0 {
			return parts[len(parts)-1]
		}
	}
	if f := object(m["FuncCall"]); f != nil {
...
func (r *sqlResolver) trailing(m map[string]any, s *sqlScope) error {
	for _, item := range array(m["sortClause"]) {
		if err := r.expr(item, s, true); err != nil {
			return err
		}
	}
	for _, field := range []string{"limitCount", "limitOffset"} {
		if err := r.optional(m[field], s, false); err != nil {
			return err
		}
	}
	option := text(m["limitOption"])
	if option != "" && option != "LIMIT_OPTION_DEFAULT" && option != "LIMIT_OPTION_COUNT" && option != "LIMIT_OPTION_WITH_TIES" {
...
func addSource(s *sqlScope, name string, columns []string, alias any) error {
	if alias != nil {
		...
		if !only(a, "aliasname", "colnames") {
			return ErrUnsafe
		}
		name = text(a["aliasname"])
		n, ok := names(a["colnames"])
		if !ok || len(n) > len(columns) {
			return ErrUnsafe
		}
		columns = append([]string(nil), columns...)
		for i, col := range n {
			if !SQLIdentifier(col) {
				return ErrUnsupported
			}
			columns[i] = col
		}
	}
	if !SQLIdentifier(name) || s.sources[name] != nil {
		return ErrUnsafe
	}
...
func (r *sqlResolver) from(v any, s *sqlScope) error {
	...
	if value, ok := root["RangeVar"]; ok {
		m := object(value)
		if !only(m, "catalogname", "schemaname", "relname", "inh", "relpersistence", "alias") || text(m["catalogname"]) != "" {
			return ErrUnsafe
		}
	...
	if value, ok := root["RangeSubselect"]; ok {
		m := object(value)
		if !only(m, "lateral", "subquery", "alias") || truth(m["lateral"]) || m["alias"] == nil {
			return ErrUnsupported
		}
		columns, err := r.selectStatement(m["subquery"], s.ctes)
		if err != nil {
			return err
		}
	...
	if value, ok := root["JoinExpr"]; ok {
		m := object(value)
		if !only(m, "jointype", "isNatural", "larg", "rarg", "quals", "rtindex") || truth(m["isNatural"]) {
			return ErrUnsupported
		}
		switch text(m["jointype"]) {
		case "JOIN_INNER", "JOIN_LEFT", "JOIN_RIGHT", "JOIN_FULL":
		default:
			return ErrUnsupported
		}
		if err := r.from(m["larg"], s); err != nil {
			return err
		}
		if err := r.from(m["rarg"], s); err != nil {
			return err
		}
...
func columnVisible(s *sqlScope, parts []string, outputs bool) bool {
	...
	if len(parts) == 1 {
		...
		for _, c := range s.sources[parts[0]] {
			if c == parts[1] {
				count++
			}
		}
...
func (r *sqlResolver) window(m map[string]any, s *sqlScope) error {
	if !only(m, "name", "refname", "partitionClause", "orderClause", "frameOptions", "startOffset", "endOffset") {
		return ErrUnsafe
	}
	if name := text(m["refname"]); name != "" && !s.windows[name] {
		return ErrUnsafe
	}
	for _, key := range []string{"partitionClause", "orderClause"} {
		if err := r.expressions(m[key], s, false); err != nil {
			return err
		}
	}
	for _, key := range []string{"startOffset", "endOffset"} {
		if err := r.optional(m[key], s, false); err != nil {
			return err
		}
	}
	return nil
...
func (r *sqlResolver) expr(v any, s *sqlScope, outputs bool) error {
	...
	for kind, value := range root {
		...
		case "ColumnRef":
			...
			if !ok || !columnVisible(s, parts, outputs) {
				return ErrUnsafe
			}
		case "ParamRef":
			if !only(m, "number") {
			...
			if !ok || n < 1 || n > float64(r.parameterCount) || n != float64(int(n)) {
				return ErrBinding
			}
			r.parameters[int(n)] = true
		case "A_Const":
			if !only(m, "ival", "fval", "sval", "boolval", "bsval", "isnull") {
				return ErrUnsafe
			}
		case "A_Expr":
			if !only(m, "kind", "name", "lexpr", "rexpr") || !safeOperator(m["name"]) {
				return ErrUnsafe
			}
			switch text(m["kind"]) {
			...
			if err := r.optional(m["lexpr"], s, outputs); err != nil {
				return err
			}
			if err := r.optional(m["rexpr"], s, outputs); err != nil {
				return err
			}
		case "BoolExpr":
			if !only(m, "boolop", "args") {
				return ErrUnsafe
			}
			switch text(m["boolop"]) {
			case "AND_EXPR", "OR_EXPR", "NOT_EXPR":
			default:
				return ErrUnsafe
			}
		...
		case "NullTest", "BooleanTest":
			if !only(m, "arg", "nulltesttype", "argisrow", "booltesttype") {
				return ErrUnsafe
			}
			if err := r.expr(m["arg"], s, outputs); err != nil {
				return err
			}
		case "FuncCall":
			if !only(m, "funcname", "args", "agg_order", "agg_filter", "over", "agg_star", "agg_distinct", "agg_within_group", "func_variadic", "funcformat") || truth(m["func_variadic"]) {
				return ErrUnsafe
			}
			parts, ok := names(m["funcname"])
			...
			for _, key := range []string{"args", "agg_order"} {
				if err := r.expressions(m[key], s, outputs); err != nil {
					return err
				}
			}
			if err := r.optional(m["agg_filter"], s, outputs); err != nil {
				return err
			}
			if m["over"] != nil {
		...
		case "CaseExpr":
			if !only(m, "arg", "args", "defresult", "casetype", "casecollid") {
				return ErrUnsafe
			}
			if err := r.optional(m["arg"], s, outputs); err != nil {
				return err
			}
			if err := r.expressions(m["args"], s, outputs); err != nil {
				return err
			}
			if err := r.optional(m["defresult"], s, outputs); err != nil {
				return err
			}
		case "CaseWhen":
			if !only(m, "expr", "result") {
				return ErrUnsafe
			}
			if err := r.expr(m["expr"], s, outputs); err != nil {
				return err
			}
			if err := r.expr(m["result"], s, outputs); err != nil {
				return err
			}
		case "TypeCast":
			if !only(m, "arg", "typeName") {
				return ErrUnsafe
			}
			typeName := fieldObject(m["typeName"], "TypeName")
			if !only(typeName, "names", "typemod", "typmods", "setof", "pct_type") || truth(typeName["setof"]) || truth(typeName["pct_type"]) {
				return ErrUnsafe
			}
			parts, ok := names(typeName["names"])
			if !ok || !safeType(parts) {
				return ErrUnsafe
			}
			if err := r.expr(m["arg"], s, outputs); err != nil {
				return err
			}
			if err := r.expressions(typeName["typmods"], s, false); err != nil {
				return err
			}
		case "SortBy":
			if !only(m, "node", "sortby_dir", "sortby_nulls", "useOp") || len(array(m["useOp"])) != 0 || text(m["sortby_dir"]) == "SORTBY_USING" {
				return ErrUnsafe
			}
			if err := r.expr(m["node"], s, outputs); err != nil {
				return err
			}
		case "List":
			if !only(m, "items") {
			...
			if err := r.expressions(m["items"], s, outputs); err != nil {
				return err
			}
		case "A_ArrayExpr", "RowExpr":
			if !only(m, "elements", "args", "row_format") {
				return ErrUnsafe
			}
			if err := r.expressions(m["elements"], s, outputs); err != nil {
				return err
			}
			if err := r.expressions(m["args"], s, outputs); err != nil {
				return err
		...
		case "SubLink":
			if !only(m, "subLinkType", "subLinkId", "testexpr", "operName", "subselect") {
				return ErrUnsafe
			}
			switch text(m["subLinkType"]) {
			case "EXISTS_SUBLINK", "EXPR_SUBLINK", "ANY_SUBLINK", "ALL_SUBLINK":
			default:
				return ErrUnsafe
			}
			if len(array(m["operName"])) > 0 && !safeOperator(m["operName"]) {
				return ErrUnsafe
			}
			if err := r.optional(m["testexpr"], s, outputs); err != nil {
				return err
			}
			if _, err := r.selectStatement(m["subselect"], s.ctes); err != nil {
				return err
			}
		...
		case "GroupingSet":
			if !only(m, "kind", "content") {
				return ErrUnsafe
			}
			if err := r.expressions(m["content"], s, outputs); err != nil {
				return err
			}
		default:
			return ErrUnsafe
