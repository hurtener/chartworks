...
type Validator struct { adapter ReadAdapter; limits config.ReadValidation; slots chan struct{} }
...
func NewValidator(adapter ReadAdapter,limits config.ReadValidation) (*Validator,error) {
	if adapter==nil || config.ValidateReadValidation(limits)!=nil { return nil,ErrBinding }
...
func (v *Validator) Validate(ctx context.Context,e identity.Envelope,r Request) (Plan,error) {
	if !e.Valid() { return Plan{},ErrBinding }
	if !identity.Identifier(r.Source) || !identity.Identifier(r.Context) || len(r.SQL)<1 || len(r.SQL)>v.limits.MaxSQLBytes || len(r.Parameters)>v.limits.MaxParameters || strings.ContainsRune(r.SQL,0) { return Plan{},ErrLimit }
	for _,p:=range r.Parameters { if !p.Valid() { return Plan{},ErrBinding } }
	...
	if strings.Count(r.SQL,"(")>256 { return Plan{},ErrLimit }
	binding,err:=v.adapter.Binding(ctx,e,r.Source,r.Context)
	if err!=nil { return Plan{},err }
	if !binding.Valid() || binding.Source!=r.Source || binding.Context!=r.Context { return Plan{},ErrBinding }
	if binding.Dialect!="postgres" { return Plan{},ErrUnsupported }
	if err=Require(e,binding,nil); err!=nil { return Plan{},err }
	select { case v.slots<-struct{}{}: case <-ctx.Done(): return Plan{},ctx.Err() }
	raw,err:=pgquery.ParseToJSON(r.SQL)
	<-v.slots
	if err!=nil { return Plan{},ErrUnsafe }
	if err=ctx.Err(); err!=nil { return Plan{},err }
	var tree map[string]any
	if json.Unmarshal([]byte(raw),&tree)!=nil { return Plan{},ErrUnsafe }
	nodes:=0
	if !boundedTree(tree,0,&nodes,v.limits) { return Plan{},ErrLimit }
	stmts:=array(tree["stmts"])
	if len(stmts)!=1 { return Plan{},ErrUnsafe }
	...
	if err!=nil { return Plan{},err }
	if len(resolver.parameters)!=len(r.Parameters) { return Plan{},ErrBinding }
	deps:=make([]string,0,len(resolver.dependencies)); for id:=range resolver.dependencies { deps=append(deps,id) }; sort.Strings(deps)
	if err=Require(e,binding,deps); err!=nil { return Plan{},err }
	...
	if err=v.adapter.Explain(ctx,e,candidate); err!=nil { return Plan{},err }
	if !e.Valid() { return Plan{},ErrBinding }
...
func boundedTree(v any,depth int,count *int,limits config.ReadValidation) bool {
	*count++
	if depth>limits.MaxASTDepth || *count>limits.MaxASTNodes { return false }
	switch x:=v.(type) {
	case map[string]any: for _,item:=range x { if !boundedTree(item,depth+1,count,limits) { return false } }
	case []any: for _,item:=range x { if !boundedTree(item,depth+1,count,limits) { return false } }
	}
	return true
}
