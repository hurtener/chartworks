...
func BindBusinessConstraints(ctx context.Context, binding Binding, statement string, parameters []Parameter, constraints []BusinessConstraint) (BusinessBoundQuery, error) {
	...
	if err := ValidateBusinessConstraints(binding, constraints); err != nil { return BusinessBoundQuery{}, err }
	for _, parameter := range parameters { if !parameter.Valid() { return BusinessBoundQuery{}, businessSQLFailure("invalid_parameter_value") } }
	...
	if err != nil { return BusinessBoundQuery{}, err }
	...
	for _, constraint := range constraints {
		if err := ctx.Err(); err != nil { return BusinessBoundQuery{}, err }
		alias, matches := "", 0
		for _, source := range layout.ranges {
			if source.relation.ID == constraint.Dataset { alias, matches = source.alias, matches+1 }
		}
		if matches != 1 { return BusinessBoundQuery{}, businessError(constraint, "target", "unsupported_missing_or_ambiguous_target") }
		column := businessQuote(binding.Dialect, alias)+"."+businessQuote(binding.Dialect, constraint.Column)
		if constraint.Aggregation != "" {
			if !layout.hasAggregate { return BusinessBoundQuery{}, businessError(constraint, "target", "unsupported_aggregate_query") }
		...
		if err != nil { return BusinessBoundQuery{}, err }
		if constraint.Aggregation != "" { aggregates = append(aggregates, predicate) } else { rows = append(rows, predicate) }
	...
	if len(rows) > 0 {
		...
		} else { edits = append(edits, businessEdit{layout.rowInsert, layout.rowInsert, " WHERE "+predicate+" "}) }
	}
	if len(aggregates) > 0 {
		...
		} else { edits = append(edits, businessEdit{layout.aggregateInsert, layout.aggregateInsert, " HAVING "+predicate+" "}) }
	}
	...
	for _, token := range boundTokens {
		if token.kind != 'p' && token.kind != 'm' { continue }
		var value Parameter
		resolution := ""
		if token.kind == 'm' {
			index, err := strconv.Atoi(token.text)
			if err != nil || index < 0 || index >= len(scalars) { return BusinessBoundQuery{}, businessSQLFailure("invalid_parameter_marker") }
			...
			if err != nil || index < 1 || index > len(parameters) { return BusinessBoundQuery{}, businessSQLFailure("invalid_parameter_index") }
			used[index-1], value = true, parameters[index-1]
		}
		if len(outParams) >= 64 { return BusinessBoundQuery{}, businessSQLFailure("unsupported_parameter_budget") }
		...
		if resolution != "" {
			for i := range bindings { if bindings[i].Resolution == resolution { bindings[i].Parameters = append(bindings[i].Parameters, len(outParams)) } }
		}
	}
	for _, found := range used { if !found { return BusinessBoundQuery{}, businessSQLFailure("unused_parameter") } }
	bound, err = businessApplyEdits(bound, parameterEdits)
	if err != nil { return BusinessBoundQuery{}, err }
	if len(bound) > 32<<10 || strings.ContainsRune(bound, 0x1f) { return BusinessBoundQuery{}, businessSQLFailure("unsupported_statement_size") }
...
func businessAggregate(aggregation, column string) (string, error) {
	switch aggregation {
	case "sum": return "SUM("+column+")", nil
	case "average": return "AVG("+column+")", nil
	case "minimum": return "MIN("+column+")", nil
	case "maximum": return "MAX("+column+")", nil
	case "count": return "COUNT("+column+")", nil
	case "distinct_count": return "COUNT(DISTINCT "+column+")", nil
	default: return "", businessSQLFailure("unsupported_aggregation")
	}
}

func businessPredicate(dialect, column string, c BusinessConstraint, scalars *[]businessScalar) (string, error) {
	if c.Null { return "("+column+" IS NULL)", nil }
	appendValue := func(value string) (string, error) {
		kind := "text"
		if c.Kind == "boolean" { kind = "boolean" }
		index := len(*scalars)
		*scalars = append(*scalars, businessScalar{Parameter{Kind: kind, Value: value}, c.Resolution})
		...
		case "number":
			...
			if dialect == "bigquery" {
				typeName = "NUMERIC"
				if c.Precision > 38 || c.Scale > 9 { typeName = "BIGNUMERIC" }
			}
		...
		case "time_window":
			...
			if c.TemporalType == "timestamp" {
				switch dialect {
				case "sqlserver": typeName = "DATETIME2"
				case "mysql", "bigquery": typeName = "DATETIME"
				case "snowflake": typeName = "TIMESTAMP_NTZ"
				default: typeName = "TIMESTAMP"
				}
			} else if c.TemporalType == "timestamptz" {
				switch dialect {
				case "postgres": typeName = "TIMESTAMPTZ"
				case "sqlserver": typeName = "DATETIMEOFFSET"
				case "snowflake": typeName = "TIMESTAMP_TZ"
				case "bigquery", "databricks": typeName = "TIMESTAMP"
				default: return "", businessError(c, "temporal_type", "unsupported_temporal_type")
				}
			}
			return "CAST("+marker+" AS "+typeName+")", nil
		default: return marker, nil
		}
	}
	...
	if c.Operator == "range" {
		...
		if err != nil { return "", err }
		lowerOp, upperOp := ">", "<"
		if strings.HasPrefix(c.Bounds, "[") { lowerOp = ">=" }
		if strings.HasSuffix(c.Bounds, "]") { upperOp = "<=" }
		predicate = column+" "+lowerOp+" "+lower+" AND "+column+" "+upperOp+" "+upper
	} else {
		op := map[string]string{"eq":"=", "ne":"<>", "lt":"<", "lte":"<=", "gt":">", "gte":">="}[c.Operator]
		if op == "" { return "", businessError(c, "operator", "unsupported_operator") }
		predicate = column+" "+op+" "+lower
	}
	if c.Nulls == "include" { predicate = "("+predicate+") OR "+column+" IS NULL" }
...
func businessApplyEdits(statement string, edits []businessEdit) (string, error) {
	...
	for _, edit := range edits {
		if edit.start < position || edit.end < edit.start || edit.end > len(statement) { return "", businessSQLFailure("unsupported_overlapping_clause") }
...
func businessLayout(statement string, tokens []businessToken, binding Binding) (businessSQLLayout, error) {
	...
	if len(tokens) < 4 || !tokens[0].word("select") || tokens[0].depth != 0 { return out, businessSQLFailure("unsupported_select_shape") }
	from, fromEnd := -1, len(tokens)
	for i, token := range tokens {
		if i > 0 && token.word("select") { return out, businessSQLFailure("unsupported_nested_select") }
		for _, forbidden := range []string{"with", "union", "intersect", "except", "into", "values", "returning", "for", "qualify", "tablesample", "pivot", "unpivot", "connect", "match_recognize"} {
			if token.word(forbidden) { return out, businessSQLFailure("unsupported_select_shape") }
		}
		if token.text == ";" {
			if i != len(tokens)-1 || token.depth != 0 { return out, businessSQLFailure("unsupported_multiple_statements") }
		...
		if token.word("from") {
			if from >= 0 { return out, businessSQLFailure("unsupported_multiple_from") }
			from = i
		}
		if token.word("sum") || token.word("avg") || token.word("min") || token.word("max") || token.word("count") { out.hasAggregate = true }
		if from < 0 { continue }
		if token.word("where") || token.word("group") || token.word("having") || token.word("order") || token.word("limit") || token.word("offset") || token.word("fetch") {
			if fromEnd == len(tokens) { fromEnd = i }
		}
		if token.word("where") {
			if out.where >= 0 { return out, businessSQLFailure("unsupported_multiple_where") }
			out.where = token.start
		}
		if token.word("having") {
			if out.having >= 0 { return out, businessSQLFailure("unsupported_multiple_having") }
			out.having = token.start
		}
		if token.word("group") { out.hasAggregate = true }
	}
	if from < 0 || from+1 >= fromEnd { return out, businessSQLFailure("unsupported_missing_from") }
	for _, token := range tokens {
		if token.depth != 0 { continue }
		if token.word("group") || token.word("having") || token.word("order") || token.word("limit") || token.word("offset") || token.word("fetch") {
			if token.start < out.rowInsert { out.rowInsert = token.start }
			if out.where >= 0 && token.start > out.where && token.start < out.whereEnd { out.whereEnd = token.start }
		}
		if token.word("order") || token.word("limit") || token.word("offset") || token.word("fetch") {
			if token.start < out.aggregateInsert { out.aggregateInsert = token.start }
			if out.having >= 0 && token.start > out.having && token.start < out.havingEnd { out.havingEnd = token.start }
		}
	}
	aliases := map[string]bool{}
	for i := from+1; i < fromEnd; i++ {
		if i != from+1 && !(tokens[i-1].depth == 0 && (tokens[i-1].word("join") || tokens[i-1].text == ",")) { continue }
		if tokens[i].depth != 0 { return out, businessSQLFailure("unsupported_derived_relation") }
		source, next, err := businessReadRange(tokens, i, fromEnd, binding)
		if err != nil { return out, err }
		if aliases[source.alias] { return out, businessSQLFailure("unsupported_duplicate_alias") }
		aliases[source.alias] = true
		out.ranges = append(out.ranges, source)
		i = next-1
	}
	if len(out.ranges) < 1 || len(out.ranges) > 32 { return out, businessSQLFailure("unsupported_relation_count") }
	return out, nil
}
	...
	for index < end {
		...
		if token.kind == 'q' && binding.Dialect == "bigquery" && strings.Contains(token.text, ".") {
			for _, part := range strings.Split(token.text, ".") {
				if !SQLIdentifierForDialect(binding.Dialect, part) { return businessRange{}, index, businessSQLFailure("unsupported_relation_name") }
				parts = append(parts, part)
			}
		} else {
			name, ok := businessName(token, binding.Dialect)
			if !ok { return businessRange{}, index, businessSQLFailure("unsupported_relation_name") }
			parts = append(parts, name)
		}
		index++
		if index < end && tokens[index].text == "." { index++; continue }
		break
	}
	if len(parts) < 2 || len(parts) > 3 || len(parts) == 3 && parts[0] != binding.Catalog { return businessRange{}, index, businessSQLFailure("unsupported_unqualified_or_foreign_relation") }
	var relation Relation
	matches := 0
	for _, candidate := range binding.Relations {
		if candidate.Schema == parts[len(parts)-2] && candidate.Name == parts[len(parts)-1] { relation, matches = candidate, matches+1 }
	}
	if matches != 1 { return businessRange{}, index, businessSQLFailure("foreign_relation") }
	alias := relation.Name
	if index < end && tokens[index].word("as") {
		index++
		if index >= end { return businessRange{}, index, businessSQLFailure("invalid_relation_alias") }
		name, ok := businessName(tokens[index], binding.Dialect)
		if !ok { return businessRange{}, index, businessSQLFailure("invalid_relation_alias") }
		alias = name
		index++
	} else if index < end {
		token := tokens[index]
		reserved := false
		for _, word := range []string{"join", "inner", "outer", "left", "right", "full", "cross", "natural", "on", "using"} { reserved = reserved || token.word(word) }
