...
func (e *BusinessConstraintError) Error() string {
	return "exec: business constraint could not be bound"
}
func (e *BusinessConstraintError) Unwrap() error {
	if strings.HasPrefix(e.Code, "unsupported") {
...
func ValidateBusinessConstraints(binding Binding, constraints []BusinessConstraint) error {
	if !binding.Valid() || len(constraints) > 64 {
		return ErrBinding
	}
	...
	for _, c := range constraints {
		...
		for _, relation := range binding.Relations {
			...
			for _, candidate := range relation.Columns {
				if candidate.Name == c.Column && candidate.Safe {
		...
		if !found {
			return businessError(c, "target", "foreign_constraint_target")
		}
		if c.Nulls != "exclude" && c.Nulls != "include" && c.Nulls != "only" {
			return businessError(c, "nulls", "invalid_null_semantics")
		}
		if !businessColumnCompatible(binding.Dialect, column, c) {
			return businessError(c, "target", "unsupported_constraint_type")
		}
		if c.Null || c.Nulls == "only" {
			if !c.Null || c.Nulls == "exclude" || c.Value != "" || c.Upper != "" {
				return businessError(c, "value", "invalid_null_value")
			}
			continue
		}
		...
		case "number":
			...
			if c.Operator == "range" {
				...
				if !ok || (c.Bounds != "[]" && c.Bounds != "[)" && c.Bounds != "(]" && c.Bounds != "()") || lower.Cmp(upper) > 0 || lower.Cmp(upper) == 0 && c.Bounds != "[]" {
					return businessError(c, "upper", "invalid_range")
				}
			} else if c.Upper != "" || c.Bounds != "" || !businessOperator(c.Operator) {
				return businessError(c, "operator", "invalid_operator")
			}
		case "boolean":
			if c.Operator != "eq" || (c.Value != "true" && c.Value != "false") || c.Upper != "" {
				return businessError(c, "value", "invalid_boolean")
			}
		case "entity", "text":
			if c.Value == "" || len(c.Value) > 512 || c.Upper != "" || (c.Operator != "eq" && c.Operator != "ne") {
				return businessError(c, "value", "invalid_governed_value")
			}
		case "time_window":
			...
			layout := "2006-01-02"
			if c.TemporalType == "timestamptz" {
				layout = time.RFC3339
			}
			lower, err := time.Parse(layout, c.Value)
			upper, upperErr := time.Parse(layout, c.Upper)
			if err != nil || upperErr != nil || !lower.Before(upper) {
				return businessError(c, "time", "invalid_time_bounds")
			}
		default:
...
func businessOperator(op string) bool {
	return op == "eq" || op == "ne" || op == "lt" || op == "lte" || op == "gt" || op == "gte"
}

func businessDecimal(value string, precision, scale int) (*big.Rat, bool) {
	if len(value) == 0 || len(value) > 128 || !regexp.MustCompile(`^-?(0|[1-9][0-9]*)(\.[0-9]+)?$`).MatchString(value) {
		return nil, false
	}
	...
	if fraction > scale || len(strings.TrimLeft(parts[0], "0")) > precision-scale {
		return nil, false
	}
...
func businessColumnCompatible(dialect string, column Column, c BusinessConstraint) bool {
	...
	case "time_window":
		...
		if dialect == "mysql" && strings.HasPrefix(native, "timestamp") {
			return false
		}
