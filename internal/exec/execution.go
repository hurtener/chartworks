...
func (q RemoteQuery) Valid() bool { return q.PID>0 && !q.Started.IsZero() && len(q.Tag)==40 && q.Tag[:8]=="cw-read:" && hashID(q.Tag[8:]) }
func hashID(s string) bool { b,e:=hex.DecodeString(s);return e==nil && len(b)==16 && hex.EncodeToString(b)==s }
...
type NativeResult struct { Result Result; RemoteState string }
...
type Control struct { attempt Attempt; actor string; tenant string }
...
func (c Control) Target(e identity.Envelope,b Binding) (RemoteQuery,error) {
	a:=c.attempt
	if c.actor!=e.User() || c.tenant!=e.Tenant() || a.Manifest.Session!=e.Session() || a.Remote==nil || !a.Remote.Valid() || a.Manifest.Receipt.Source!=b.Source || a.Manifest.Receipt.Context!=b.Context { return RemoteQuery{},ErrBinding }
	if err:=Require(e,b,a.Manifest.Receipt.Dependencies);err!=nil { return RemoteQuery{},err }
	return *a.Remote,nil
}

// Coordinates permits a control adapter to load its current protected source binding.
func (c Control) Coordinates() (string,string) { return c.attempt.Manifest.Receipt.Source,c.attempt.Manifest.Receipt.Context }
...
type ExecutionReport struct { Attempt Attempt `json:"attempt"`; Result *Result `json:"result"` }

// Executor is one model-free read core; it never rewrites SQL or retries automatically.
type Executor struct { adapter ExecutionAdapter; repo AttemptStore; settings config.ReadValidation; slots chan struct{} }

func nilDependency(v any) bool { return v==nil || reflect.ValueOf(v).Kind()==reflect.Pointer && reflect.ValueOf(v).IsNil() }
...
func NewExecutor(adapter ExecutionAdapter,repo AttemptStore,settings config.ReadValidation) (*Executor,error) {
	if nilDependency(adapter) || nilDependency(repo) || config.ValidateReadValidation(settings)!=nil || repo.ReadCapacity()<settings.ExecutionConcurrency+2 { return nil,ErrBinding }
...
func (x *Executor) limits(o Options) (Limits,error) {
	r:=o.Rows;if r==0 { r=x.settings.RowsDefault };b:=o.Bytes;if b==0 { b=x.settings.BytesDefault }
	if !identity.Identifier(o.Operation) || o.Number<1 || o.Number>x.settings.MaxReadAttempts || r<1 || r>x.settings.RowsCeiling || b<128 || b>x.settings.BytesCeiling { return Limits{},ErrLimit }
	if o.Preview && r>x.settings.PreviewRows { r=x.settings.PreviewRows }
...
func (x *Executor) Execute(ctx context.Context,e identity.Envelope,p Plan,o Options) (ExecutionReport,error) {
	if ctx==nil || !e.Valid() { return ExecutionReport{},ErrBinding }
	if _,_,err:=p.SQL(e,p.candidate.binding);err!=nil { return ExecutionReport{},err }
	limits,err:=x.limits(o);if err!=nil { return ExecutionReport{},err }
	ctx,cancel:=context.WithTimeout(ctx,limits.Timeout);defer cancel()
	ctx,expiry:=context.WithDeadline(ctx,e.Deadline());defer expiry()
	select { case x.slots<-struct{}{}:defer func(){<-x.slots}();case <-ctx.Done():return ExecutionReport{},ctx.Err() }
	if err=ctx.Err();err!=nil { return ExecutionReport{},err }
	var random [16]byte;if _,err=rand.Read(random[:]);err!=nil { return ExecutionReport{},ErrUncertain }
	...
	scope,err:=store.NewScope(e.Tenant(),e.User());if err!=nil { return ExecutionReport{},err }
	if err=x.repo.BeginRead(ctx,scope,a,x.settings.MaxReadAttempts);err!=nil { return ExecutionReport{},err }
	...
	if runErr==nil {
		status=native.Result.Outcome;code=""
		if status!="succeeded" && status!="empty" && status!="truncated" { runErr=ErrType;status="failed";code="invalid_result" }
	} else {
		switch {
		case errors.Is(runErr,ErrCancelled),errors.Is(runErr,context.Canceled):status,code="cancelled","cancelled"
		case errors.Is(runErr,ErrTimeout),errors.Is(runErr,context.DeadlineExceeded):status,code="timed_out","timed_out"
		case errors.Is(runErr,ErrType):code="result_type_unsupported"
		case errors.Is(runErr,ErrLimit):code="limit_exceeded"
		case errors.Is(runErr,ErrBinding):code="context_changed"
		case errors.Is(runErr,ErrUnsupported):code="unsupported"
		}
	}
	if native.RemoteState=="unknown" { status,code="uncertain","remote_outcome_unknown" }
	cleanup,stop:=context.WithTimeout(context.WithoutCancel(ctx),limits.CancelGrace);defer stop()
	current,err:=x.repo.GetRead(cleanup,scope,a.ID);if err!=nil { return ExecutionReport{},ErrUncertain }
	if current.CancelRequested && status!="uncertain" { status,code="cancelled","cancelled";runErr=ErrCancelled }
	current.Status=status;current.Code=code;current.RemoteState=native.RemoteState
	if current.RemoteState=="" { current.RemoteState="not_issued" }
	if runErr==nil && status!="uncertain" { current.Rows=len(native.Result.Rows);current.Bytes=native.Result.Bytes }
	now:=time.Now().UTC();current.Finished=&now
	if err=x.repo.FinishRead(cleanup,scope,current,false);err!=nil { return ExecutionReport{},ErrUncertain }
	report:=ExecutionReport{Attempt:current}
	if runErr==nil && (status=="succeeded" || status=="empty" || status=="truncated") { report.Result=&native.Result }
	return report,nil
}

type readObserver struct { repo AttemptStore; scope store.Scope; id string }
func (o *readObserver) Dispatch(ctx context.Context,q RemoteQuery,accepted bool) error { return o.repo.DispatchRead(ctx,o.scope,o.id,q,accepted) }
func (o *readObserver) Check(ctx context.Context) error {
	a,err:=o.repo.GetRead(ctx,o.scope,o.id);if err!=nil { return err }
	if a.CancelRequested { return ErrCancelled }
	if a.Status=="uncertain" || a.Finished!=nil { return ErrUncertain }
	return nil
}

func (x *Executor) authorized(ctx context.Context,e identity.Envelope,id string) (Attempt,store.Scope,error) {
	if ctx==nil || !e.Valid() || !hashID(id) { return Attempt{},store.Scope{},ErrBinding }
	scope,err:=store.NewScope(e.Tenant(),e.User());if err!=nil { return Attempt{},scope,err }
	a,err:=x.repo.GetRead(ctx,scope,id);if err!=nil { return Attempt{},scope,err }
	if a.Manifest.Session!=e.Session() { return Attempt{},scope,store.ErrNotFound }
	b,err:=x.adapter.Binding(ctx,e,a.Manifest.Receipt.Source,a.Manifest.Receipt.Context);if err!=nil { return Attempt{},scope,err }
	if err=Require(e,b,a.Manifest.Receipt.Dependencies);err!=nil { return Attempt{},scope,err }
	return a,scope,nil
}

// Inspect reads content-free evidence with fresh actor/session/source/context reach.
func (x *Executor) Inspect(ctx context.Context,e identity.Envelope,id string) (Attempt,error) { a,_,err:=x.authorized(ctx,e,id);return a,err }

// ControlReceipt never equates delivery of a cancel request with remote termination.
type ControlReceipt struct { Attempt Attempt `json:"attempt"`; RemoteState string `json:"remote_state"` }
...
func (x *Executor) Control(ctx context.Context,e identity.Envelope,id string,cancel bool) (ControlReceipt,error) {
	a,scope,err:=x.authorized(ctx,e,id);if err!=nil { return ControlReceipt{},err }
	if a.Finished!=nil && a.Status!="uncertain" { return ControlReceipt{Attempt:a,RemoteState:a.RemoteState},nil }
	if cancel { if err=x.repo.CancelRead(ctx,scope,id);err!=nil { return ControlReceipt{},err };a.CancelRequested=true }
	state:="not_issued"
	if a.Remote!=nil {
	...
	if a.Status=="uncertain" && (state=="stopped" || state=="not_issued") {
		a.Status="interrupted";a.Code="result_not_retained";a.RemoteState=state;now:=time.Now().UTC();a.Finished=&now
		if err=x.repo.FinishRead(ctx,scope,a,true);err!=nil { return ControlReceipt{},err }
	}
