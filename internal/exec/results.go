...
func Normalize(field Field, raw []byte) (json.RawMessage, error) {
	...
	if len(raw) > 16<<20 || !utf8.Valid(raw) {
		return nil, ErrType
	}
	s := string(raw)
	switch field.Type {
...
func NewCollector(schema []Field, rows, bytes int) (*Collector, error) {
	if rows < 1 || rows > 100000 || bytes < 128 || bytes > 16<<20 || len(schema) < 1 || len(schema) > 256 {
		return nil, ErrLimit
	}
	for _, f := range schema {
		if len(f.Name) == 0 || len(f.Name) > 1024 || !utf8.ValidString(f.Name) || len(f.NativeType) == 0 || len(f.NativeType) > 128 {
			return nil, ErrType
		}
		switch f.Type {
...
func (c *Collector) Add(raw [][]byte) (bool, error) {
	if len(raw) != len(c.result.Schema) {
		return false, ErrType
	}
	if len(c.result.Rows) == c.maxRows {
		c.result.Outcome = "truncated"
		c.result.Truncation = "rows"
		return false, nil
	}
	row := make([]json.RawMessage, len(raw))
	for i, value := range raw {
		v, err := Normalize(c.result.Schema[i], value)
		if err != nil {
			return false, err
		}
		row[i] = v
	...
	if c.result.Bytes+extra > c.maxBytes {
		c.result.Outcome = "truncated"
		c.result.Truncation = "bytes"
		return false, nil
	}
	c.result.Rows = append(c.result.Rows, row)
	c.result.Bytes += extra
	c.result.Outcome = "succeeded"
	return true, nil
}
