...
func TestBusinessBindingConjunctionAndParameterReceipts(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			originalBinding := binding.Clone()
			originalConstraint := constraint
			out, err := BindBusinessConstraints(ctx, binding, tc.sql, tc.params, []BusinessConstraint{constraint})
			if err != nil || !strings.Contains(out.SQL, tc.fragment) || len(out.Parameters) != tc.count || out.Receipt.Validation != nil || len(out.Receipt.Bindings) != 1 {
				t.Fatalf("closed conjunction or receipt failed: %v", err)
			}
			if _, err := pgquery.ParseToJSON(out.SQL); err != nil {
				t.Fatal("transformation did not produce parsable PostgreSQL", err)
			}
			indexes := out.Receipt.Bindings[0].Parameters
			if len(indexes) != 1 || indexes[0] < 1 || indexes[0] > len(out.Parameters) || out.Parameters[indexes[0]-1].Value != constraint.Value {
				t.Fatal("binding receipt does not identify exact scalar")
			}
			if out.Receipt.SourceBinding != Hash(binding) || out.Receipt.Statement != Hash([]any{out.SQL, out.Parameters}) {
				t.Fatal("receipt lost exact source or parameter seal")
			}
			if constraint != originalConstraint || !reflect.DeepEqual(binding, originalBinding) {
				t.Fatal("shared inputs mutated")
			}
			wire, _ := json.Marshal(out)
			if strings.Contains(string(wire), constraint.Value) || strings.Contains(fmt.Sprintf("%v %#v", out, out), constraint.Value) {
				t.Fatal("ordinary bound-query projection leaked scalar")
			}
		})
	}
}
...
func TestBusinessBindingAggregatesAndNulls(t *testing.T) {
	...
	for _, aggregation := range []string{"sum", "average", "minimum", "maximum", "count", "distinct_count"} {
		t.Run(aggregation, func(t *testing.T) {
			...
			if err != nil || !strings.Contains(out.SQL, "HAVING (count(*)>0 OR sum(amount)>0) AND (") || strings.Contains(out.SQL, " WHERE ") || out.Receipt.Bindings[0].Aggregation != aggregation {
				t.Fatal("aggregate constraint fell into row-filter scope", err)
			}
			if _, err := pgquery.ParseToJSON(out.SQL); err != nil {
				t.Fatal(err)
			}
	...
	for _, bounds := range []string{"[]", "[)", "(]", "()"} {
		...
		if err != nil || len(out.Parameters) != 2 || out.Parameters[0].Value != "1.5" || out.Parameters[1].Value != "9.75" {
			t.Fatal("exact interval bound order lost", err)
		}
		lower, upper := ">", "<"
		if bounds[0] == '[' {
			lower = ">="
		...
		if !strings.Contains(out.SQL, `"amount" `+lower) || !strings.Contains(out.SQL, `"amount" `+upper) {
			t.Fatal("interval inclusivity changed")
		}
	}
}
...
func TestBusinessBindingRejectsAmbiguityAndForeignInputs(t *testing.T) {
	...
	for _, sql := range []string{
		...
		if err == nil || out.SQL != "" || len(out.Parameters) != 0 || out.Receipt.Validation != nil {
			t.Fatal("unsupported shape returned partial bound output")
		}
	}
	for _, mutate := range []func(*BusinessConstraint){
		...
		changed := c
		mutate(&changed)
		if err := ValidateBusinessConstraints(binding, []BusinessConstraint{changed}); err == nil {
			t.Fatal("invalid typed constraint passed admission")
		}
	}
	unsafe := binding.Clone()
	unsafe.Relations[0].Columns[1].NativeType = "double precision"
	unsafe.Relations[0].Columns[1].Category = "numeric"
	if err := ValidateBusinessConstraints(unsafe, []BusinessConstraint{c}); !errors.Is(err, ErrUnsupported) {
		t.Fatal("exact threshold admitted an approximate warehouse column", err)
	}
	cancel, cancelled := context.WithCancel(ctx)
	cancelled()
	if _, err := BindBusinessConstraints(cancel, binding, "SELECT id FROM analytics.sales", nil, []BusinessConstraint{c}); !errors.Is(err, context.Canceled) {
		t.Fatal("cancelled binding continued", err)
	}
	for _, params := range [][]Parameter{{{Kind: "number", Value: "NaN"}}, {{Kind: "integer", Value: "1"}}} {
		if _, err := BindBusinessConstraints(ctx, binding, "SELECT id FROM analytics.sales", params, []BusinessConstraint{c}); err == nil {
			t.Fatal("invalid or unused parameter accepted")
		}
	}
	var zero Plan
	if _, _, err := zero.SQL(zero.candidate.owner, binding); err == nil {
		t.Fatal("business evidence replaced validator proof")
	}
}
		...
		t.Run(dialect, func(t *testing.T) {
			...
			for i := 0; i < 12; i++ {
				...
				go func() {
					...
					if err != nil || len(out.Parameters) != 1 || out.Parameters[0].Value != c.Value || !strings.Contains(out.SQL, businessPlaceholder(dialect, 1)) {
						t.Error("dialect transport lost exact value", err)
					}
				}()
			}
			wg.Wait()
...
func FuzzBusinessConstraintStatement(f *testing.F) {
	for _, sql := range []string{"SELECT id FROM analytics.sales", "SELECT id FROM analytics.sales WHERE id=1 OR id=2", "SELECT '\\' FROM analytics.sales", "SELECT * FROM analytics.sales;SELECT 1", "", "SELECT id FROM analytics.sales WHERE id=$1"} {
		f.Add(sql)
	}
	f.Fuzz(func(t *testing.T, sql string) {
		...
		if err != nil {
			if out.SQL != "" || len(out.Parameters) != 0 {
				t.Fatal("failed transformation returned partial constraints")
			}
			return
		}
		if len(out.Parameters) != 1 || out.Parameters[0].Value != businessFixtureConstraint().Value || out.Receipt.Validation != nil || strings.ContainsRune(out.SQL, 0x1f) {
			t.Fatal("binding lost typed scalar or fabricated execution proof")
		}
	})
}
