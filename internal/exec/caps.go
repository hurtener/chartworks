...
func (c Caps) valid() bool {
	return c.Rows > 0 && c.Rows <= 100000 && c.Bytes >= 128 && c.Bytes <= 16<<20 && c.Timeout >= time.Millisecond && c.Timeout <= time.Minute && c.PlannerCost > 0 && c.PlannerCost <= 1e12
}
...
func (x *Executor) Execute(ctx context.Context, e identity.Envelope, p Plan, o Options) (ExecutionReport, error) {
	return x.execute(ctx, e, p, o, nil)
}
...
func (x *Executor) ExecuteCapped(ctx context.Context, e identity.Envelope, p Plan, o Options, c Caps) (ExecutionReport, error) {
	if !c.valid() {
		return ExecutionReport{}, ErrLimit
	}
