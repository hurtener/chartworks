...
func (p Parameter) Valid() bool {
	if len(p.Value) > 4096 || strings.ContainsRune(p.Value, 0) {
		return false
	}
	switch p.Kind {
	case "null":
	...
	case "text":
		return true
	case "boolean":
		return p.Value == "true" || p.Value == "false"
	case "integer":
		_, err := strconv.ParseInt(p.Value, 10, 64)
		return err == nil && len(p.Value) <= 20
	case "number":
...
func (p Plan) Receipt() Receipt {
	if !p.nativeChecked || !p.candidate.owner.Valid() {
		return Receipt{}
	}
	c := p.candidate
...
func Require(e identity.Envelope, b Binding, dependencies []string) error {
	...
	for _, id := range dependencies {
		refs = append(refs, access.Resource{Tenant: b.Tenant, Kind: "dataset", Permission: "query", ID: id})
	}
