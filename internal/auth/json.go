...
func Object(data []byte, limit int)(map[string]json.RawMessage,error){
 if len(data)==0||len(data)>limit||!utf8.Valid(data){return nil,ErrToken}
 d:=json.NewDecoder(bytes.NewReader(data));d.UseNumber()
 if uniqueValue(d,0)!=nil{return nil,ErrToken}
 if _,err:=d.Token();!errors.Is(err,io.EOF){return nil,ErrToken}
 var m map[string]json.RawMessage
 if json.Unmarshal(data,&m)!=nil||m==nil{return nil,ErrToken};return m,nil
}
