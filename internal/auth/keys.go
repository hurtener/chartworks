...
func NewKeyProbe(cfg config.Auth, client *http.Client) *KeyProbe {
	...
	c.Timeout = time.Duration(cfg.RequestTimeout)
	c.Jar = nil
...
func (p *KeyProbe) refresh(ctx context.Context) {
	...
	interval := time.Duration(p.auth.RefreshInterval)
	if interval <= 0 {
		interval = time.Second
	}
	p.next = p.now().Add(interval)
	...
	if err == nil {
		p.keys = keys
		p.until = p.now().Add(time.Duration(p.auth.JWKSMaxStale))
	}
	p.flight = nil
	close(ch)
	p.mu.Unlock()
...
func (p *KeyProbe) fetch(ctx context.Context) (map[string]publicKey, error) {
	ctx, cancel := context.WithTimeout(ctx, time.Duration(p.auth.RequestTimeout))
	defer cancel()
	req, err := http.NewRequestWithContext(ctx, http.MethodGet, p.auth.JWKSURL, nil)
	if err != nil {
		return nil, ErrKeys
	}
...
func (p *KeyProbe) key(ctx context.Context, kid, algorithm string) (any, error) {
	lookup := func() (publicKey, bool) {
		p.mu.Lock()
		defer p.mu.Unlock()
		k, ok := p.keys[kid]
		return k, ok && p.now().Before(p.until)
	}
	k, ok := lookup()
	if !ok {
		p.refresh(ctx)
...
func parseKeys(data []byte, allowed []string) (map[string]publicKey, error) {
	...
	for _, k := range doc.Keys {
		...
		switch str(k, "kty") {
		case "RSA":
			key.key = &rsa.PublicKey{N: new(big.Int).SetBytes(decode(str(k, "n"))), E: int(new(big.Int).SetBytes(decode(str(k, "e"))).Int64())}
		case "EC":
