...
type account struct {provider schemas.ModelProvider; key,endpoint,ca string; private bool; concurrency int; timeout int}
func (a *account)GetConfiguredProviders()([]schemas.ModelProvider,error){return []schemas.ModelProvider{a.provider},nil}
func (a *account)GetKeysForProvider(_ context.Context,p schemas.ModelProvider)([]schemas.Key,error){
 if p!=a.provider{return nil,nil};return []schemas.Key{{ID:"configured-key",Name:"configured-key",Value:*schemas.NewSecretVar(a.key),Models:schemas.WhiteList{"*"},Weight:1}},nil
}
func (a *account)GetConfigForProvider(p schemas.ModelProvider)(*schemas.ProviderConfig,error){
 if p!=a.provider{return nil,nil}
 ...
 if a.ca!=""{cfg.NetworkConfig.CACertPEM=schemas.NewSecretVar(a.ca)}
 // v1.6.2's DefaultMaxRetries is zero. Domain retry admission is the only retry owner.
 cfg.CheckAndSetDefaults();return cfg,nil
}
...
func (quietLogger)LogHTTPRequest(schemas.LogLevel,string)schemas.LogEventBuilder{return schemas.NoopLogEvent}
