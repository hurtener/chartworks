...
func (b *Budget) Check(call Call) error {
	...
	b.mu.Lock()
	defer b.mu.Unlock()
	if call.Key() != b.key || !time.Now().Before(b.deadline) || b.tokens > b.maxTokens {
		return ErrBudget
	}
	return nil
}
...
func (b *Budget) Observe(call Call, reserved, reported int) error {
	...
	b.mu.Lock()
	defer b.mu.Unlock()
	if call.Key() != b.key || reported < 0 || reserved < 1 {
		return ErrBudget
	}
	if reported > reserved {
		extra := reported - reserved
		if extra > b.maxTokens-b.tokens {
			b.tokens = b.maxTokens + 1
			return ErrBudget
		}
		b.tokens += extra
	}
