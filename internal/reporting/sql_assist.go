...
type sqlEdit struct { start, end int; text string }
...
func astInteger(v any)(int,bool){n,ok:=v.(float64);return int(n),ok && n>=0 && n<=1<<20 && float64(int(n))==n}

func walkAST(v any, depth int, budget *int, fn func(map[string]any) error) error {
	*budget--
	if depth>64 || *budget<0 { return ErrInvalid }
	switch value:=v.(type){
	case map[string]any:
		if err:=fn(value);err!=nil{return err}
		for _,child:=range value{if err:=walkAST(child,depth+1,budget,fn);err!=nil{return err}}
	case []any:
		for _,child:=range value{if err:=walkAST(child,depth+1,budget,fn);err!=nil{return err}}
	}
	return nil
...
func parseAssistance(ctx context.Context, statement string)(map[string]any,map[string]any,error){
	if ctx==nil || ctx.Err()!=nil || statement=="" || len(statement)>64<<10{return nil,nil,ErrInvalid}
	raw,err:=pgquery.ParseToJSON(statement)
	if err!=nil || len(raw)>4<<20{return nil,nil,ErrInvalid}
	var document map[string]any
	if json.Unmarshal([]byte(raw),&document)!=nil{return nil,nil,ErrInvalid}
	budget:=20000
	if err:=walkAST(document,0,&budget,func(map[string]any)error{return nil});err!=nil{return nil,nil,err}
	statements:=astArray(document["stmts"])
	if len(statements)!=1{return nil,nil,ErrInvalid}
	root:=astObject(astObject(statements[0])["stmt"])
	selectNode:=astObject(root["SelectStmt"])
	if len(root)!=1 || selectNode==nil{return nil,nil,ErrInvalid}
...
func equivalentAST(a,b any) bool {
	...
	strip:=func(m map[string]any)error{delete(m,"location");delete(m,"stmt_location");delete(m,"stmt_len");return nil}
	budget:=20000;if walkAST(left,0,&budget,strip)!=nil{return false}
	budget=20000;if walkAST(right,0,&budget,strip)!=nil{return false}
	return reflect.DeepEqual(left,right)
}

func applySQLEdits(ctx context.Context, original string, expected map[string]any, edits []sqlEdit)(string,error){
	if len(edits)==0 || len(edits)>512{return "",ErrInvalid}
	sort.SliceStable(edits,func(i,j int)bool{return edits[i].start<edits[j].start})
	var out strings.Builder
	previous:=0
	for _,edit:=range edits{
		if edit.start<previous || edit.start<0 || edit.end<edit.start || edit.end>len(original){return "",ErrInvalid}
		out.WriteString(original[previous:edit.start]);out.WriteString(edit.text);previous=edit.end
	}
	out.WriteString(original[previous:])
	statement:=out.String()
	actual,_,err:=parseAssistance(ctx,statement)
	if err!=nil || !equivalentAST(expected,actual){return "",ErrInvalid}
...
func columnNames(node any)([]string,bool){
	...
	if len(fields)==0 || len(fields)>4{return nil,false}
	out:=make([]string,len(fields))
	for i,field:=range fields{
		name:=astString(astObject(astObject(field)["String"])["sval"])
		if name=="" || !text(name,128){return nil,false}
		out[i]=name
	}
	return out,true
}
...
func literalNode(node any) map[string]any {
	m:=astObject(node)
	if constant:=astObject(m["A_Const"]);constant!=nil{return m}
	if cast:=astObject(m["TypeCast"]);cast!=nil{return literalNode(cast["arg"])}
	return nil
}

func temporalLiteral(s string) bool {
	if len(s)>64{return false}
	if date,err:=time.Parse("2006-01-02",s);err==nil{return date.Year()>0 && date.Format("2006-01-02")==s}
...
func parameterizePeriod(ctx context.Context, sql string, column []string, slot int)(string,error){
	if len(column)<1 || len(column)>4 || slot<1 || slot>63{return "",ErrInvalid}
	for _,name:=range column{if name=="" || !text(name,128){return "",ErrInvalid}}
	...
	conjunct=func(value any)error{
		...
		if b:=astObject(m["BoolExpr"]);b!=nil{
			if astString(b["boolop"])!="AND_EXPR"{return ErrInvalid}
			for _,child:=range astArray(b["args"]){if err:=conjunct(child);err!=nil{return err}}
			return nil
		}
		if len(predicates)>=256{return ErrInvalid};predicates=append(predicates,value);return nil
	}
	if err:=conjunct(root["whereClause"]);err!=nil{return "",err}
	seen:=map[string]bool{}
	edits:=[]sqlEdit{}
	for _,predicate:=range predicates{
		expression:=astObject(astObject(predicate)["A_Expr"])
		if expression==nil || astString(expression["kind"])!="AEXPR_OP"{continue}
		names:=astArray(expression["name"]);if len(names)!=1{continue}
		op:=astString(astObject(astObject(names[0])["String"])["sval"])
		if op!=">=" && op!="<"{continue}
		fields,ok:=columnNames(expression["lexpr"]);if !ok || !reflect.DeepEqual(fields,column){continue}
		...
		if !ok || !temporalLiteral(literal) || seen[op]{return "",ErrInvalid}
		end,err:=quotedLiteralSpan(sql,start,literal);if err!=nil{return "",err}
		number:=slot;if op=="<"{number++}
		edits=append(edits,sqlEdit{start:start,end:end,text:"$"+strconv.Itoa(number)})
		delete(constantNode,"A_Const");constantNode["ParamRef"]=map[string]any{"number":float64(number),"location":float64(start)}
		seen[op]=true
	}
	if len(seen)!=2{return "",ErrInvalid}
