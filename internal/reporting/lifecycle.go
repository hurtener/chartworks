...
func (s *Service) Publish(ctx context.Context, e identity.Envelope, id string, in PublishRequest) (State, error) {
	...
	defer cancel()
	if !identity.Identifier(in.Evidence) { return State{}, ErrInvalid }
	snapshot, err := s.repo.ReadBlock(ctx, e, id, Reference{Draft: true}, Publish)
	if err != nil { return State{}, err }
	if err := expected(snapshot, in.ExpectedVersion); err != nil { return State{}, err }
	if snapshot.State.Archived || snapshot.State.DraftState != "validated" || snapshot.PublishedAt != nil { return State{}, store.ErrConflict }
	if err := freshValidation(snapshot, in.Evidence, time.Now()); err != nil { return State{}, err }
...
func (s *Service) Certify(ctx context.Context, e identity.Envelope, id string, in CertifyRequest) (Attestation, error) {
	...
	if err != nil { return Attestation{}, err }
	defer cancel()
	if in.Revision < 1 || !identity.Identifier(in.Evidence) || !note(in.Note) { return Attestation{}, ErrInvalid }
	snapshot, err := s.repo.ReadBlock(ctx, e, id, Reference{Revision: in.Revision}, Certify)
	if err != nil { return Attestation{}, err }
	if err := expected(snapshot, in.ExpectedVersion); err != nil { return Attestation{}, err }
	if snapshot.State.Archived || snapshot.PublishedAt == nil { return Attestation{}, store.ErrConflict }
	if err := freshValidation(snapshot, in.Evidence, time.Now()); err != nil { return Attestation{}, err }
...
func (s *Service) Withdraw(ctx context.Context, e identity.Envelope, id string, in WithdrawRequest) (Withdrawal, error) {
	ctx, cancel, err := s.begin(ctx, e, id, Certify)
	if err != nil { return Withdrawal{}, err }
	defer cancel()
	if !identity.Identifier(in.Attestation) || !note(in.Note) || in.Revision < 0 { return Withdrawal{}, ErrInvalid }
	snapshot, err := s.repo.ReadBlock(ctx, e, id, Reference{Revision: in.Revision}, Certify)
	if err != nil { return Withdrawal{}, err }
	if err := expected(snapshot, in.ExpectedVersion); err != nil { return Withdrawal{}, err }
	if snapshot.Attestation == nil || snapshot.Attestation.ID != in.Attestation || snapshot.Withdrawal != nil { return Withdrawal{}, store.ErrConflict }
...
func (s *Service) Reject(ctx context.Context, e identity.Envelope, id string, in TransitionRequest) (State, error) {
	...
	if err != nil { return State{}, err }
	defer cancel()
	if !note(in.Note) { return State{}, ErrInvalid }
	snapshot, err := s.repo.ReadBlock(ctx, e, id, Reference{Draft: true}, Write)
	if err != nil { return State{}, err }
	if err := expected(snapshot, in.ExpectedVersion); err != nil { return State{}, err }
	if snapshot.State.Archived || snapshot.PublishedAt != nil || snapshot.State.DraftState == "rejected" { return State{}, store.ErrConflict }
...
func (s *Service) Restore(ctx context.Context, e identity.Envelope, id string, in RestoreRequest) (View, error) {
	...
	defer cancel()
	if in.Revision < 1 || !note(in.Note) { return View{}, ErrInvalid }
...
func (s *Service) Archive(ctx context.Context, e identity.Envelope, id string, in TransitionRequest) (State, error) {
	...
	if err != nil { return State{}, err }
	if err := expected(snapshot, in.ExpectedVersion); err != nil { return State{}, err }
	if snapshot.State.Archived { return State{}, store.ErrConflict }
