...
func New(repo Repository, topics TopicReader, sources SourceReader, validator Validator, executor Executor, capture QueryCapture, limits config.Reporting) (*Service, error) {
	...
	if nilValue(sources) { sources = nil }
	if nilValue(validator) { validator = nil }
	if nilValue(executor) { executor = nil }
	if nilValue(capture) { capture = nil }
...
func (s *Service) CanValidate() bool { return s != nil && s.sources != nil && s.validator != nil && s.executor != nil }
...
func (s *Service) Read(ctx context.Context, e identity.Envelope, id string, ref Reference) (View, error) {
	...
	if err != nil { return View{}, err }
	if err := ctx.Err(); err != nil { return View{}, err }
...
func (s *Service) SQL(ctx context.Context, e identity.Envelope, id string, ref Reference) (SQLView, error) {
	...
	if err != nil { return SQLView{}, err }
	defer cancel()
	if err := Require(e, id, Read); err != nil { return SQLView{}, err }
	snapshot, err := s.repo.ReadBlock(ctx, e, id, ref, SQLRead)
	if err != nil { return SQLView{}, err }
	if err := ctx.Err(); err != nil { return SQLView{}, err }
...
func (s *Service) newRevision(e identity.Envelope, number int64, d Definition, provenance Provenance) (Revision, error) {
	id, err := newID()
	if err != nil { return Revision{}, err }
...
func (s *Service) Create(ctx context.Context, e identity.Envelope, in CreateRequest) (View, error) {
	...
	if err != nil { return View{}, err }
	defer cancel()
	if err := validateDefinition(ctx, in.Definition, s.limits, false); err != nil { return View{}, err }
	if err := RequireParent(e, in.Definition.Topics[0].Topic, Write, true); err != nil { return View{}, err }
...
func (s *Service) Edit(ctx context.Context, e identity.Envelope, id string, in EditRequest) (View, error) {
	...
	if err != nil { return View{}, err }
	if err := expected(base, in.ExpectedVersion); err != nil { return View{}, err }
	if base.State.Archived { return View{}, store.ErrConflict }
	...
	if err := validateDefinition(ctx, in.Definition, s.limits, captured); err != nil { return View{}, err }
	if in.Definition.Topics[0].Topic != base.State.Topic { return View{}, ErrInvalid }
	_, refs, err := s.resolveDefinitions(ctx, e, in.Definition, false)
	if err != nil { return View{}, err }
...
func (s *Service) CaptureQuery(ctx context.Context, e identity.Envelope, in CaptureRequest) (View, error) {
	...
	if err != nil { return View{}, err }
	defer cancel()
	if !identity.Identifier(in.Query) || !metadataValid(in.Metadata, s.limits) || len(in.Outputs) == 0 || len(in.Outputs) > s.limits.MaxOutputs { return View{}, ErrInvalid }
	if !s.CanCapture() { return View{}, ErrUnavailable }
	...
	if err != nil || digest(resolved.Parameters) != digest(captured.Parameters) { return View{}, ErrInvalid }
	...
	if err := validateDefinition(ctx, d, s.limits, true); err != nil { return View{}, err }
	if err := RequireParent(e, d.Topics[0].Topic, Write, true); err != nil { return View{}, err }
...
func (s *Service) List(ctx context.Context, e identity.Envelope, in ListRequest) (Page, error) {
	if s == nil || ctx == nil || in.Limit < 1 || in.Limit > 100 || in.After != "" && !identity.Identifier(in.After) { return Page{}, ErrInvalid }
	if !e.Valid() { return Page{}, access.ErrUnauthenticated }
	ctx, cancel := context.WithDeadline(ctx, e.Deadline())
	defer cancel()
	out, err := s.repo.ListBlocks(ctx, e, in)
	if err != nil { return Page{}, err }
	for i := range out.Items { out.Items[i].State = publicState(out.Items[i].State, out.Items[i].Private) }
	if err := ctx.Err(); err != nil { return Page{}, err }
	return out, nil
}

func (s *Service) History(ctx context.Context, e identity.Envelope, id string) (History, error) {
	ctx, cancel, err := s.begin(ctx, e, id, Read)
	if err != nil { return History{}, err }
	defer cancel()
	out, err := s.repo.BlockHistory(ctx, e, id)
	if err == nil && ctx.Err() != nil { return History{}, ctx.Err() }
	return out, err
}
...
func (s *Service) AssessQuestions(ctx context.Context, e identity.Envelope, in QuestionRequest) (Assessment, error) {
	if !locale(in.Locale) || strings.TrimSpace(in.Question) == "" || !text(in.Question, 2048) || normalizeQuestion(in.Question) == "" { return Assessment{}, ErrInvalid }
	...
	for _, item := range page.Items {
		for _, localized := range item.Metadata {
			if localized.Locale != in.Locale { continue }
			for _, question := range append([]string{localized.Question}, localized.Aliases...) {
				score := questionScore(in.Question, question)
				if score < s.limits.QuestionThreshold { continue }
				kind := "similar"
				if score == 1 && normalizeQuestion(question) == normalizeQuestion(in.Question) { kind = "exact" }
	...
	if len(out.Matches) > 100 { out.Matches, out.Complete = out.Matches[:100], false }
	return out, nil
}

func acceptedResolution(in Resolution) Resolution {
	out := clone(in)
	if out.At.IsZero() { out.At = time.Now().UTC() }
	if out.Timezone == "" { out.Timezone = "UTC" }
	return out
}

func revisionReference(revision int64) Reference {
	if revision == 0 { return Reference{Draft: true} }
	return Reference{Revision: revision}
}

func successful(status string) bool { return slices.Contains([]string{"succeeded", "empty", "truncated"}, status) }
