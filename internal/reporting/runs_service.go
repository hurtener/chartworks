...
func normalizedRunRequest(in RunRequest) (RunRequest, error) {
	in = clone(in)
	if in.Policy == "" { in.Policy = "published" }
	if in.PartialPolicy == "" { in.PartialPolicy = "fail" }
	if in.Resolution.Timezone == "" { in.Resolution.Timezone = "UTC" }
	if !identity.Identifier(in.Key) || !slices.Contains([]string{"published", "certified_only", "explicit_stale", "private_preview"}, in.Policy) ||
		...
		in.ReuseMaxAgeSeconds < 0 || len(in.Outputs) > 32 || len(in.Arguments) > 64 || in.Locale != "" && !locale(in.Locale) {
		return RunRequest{}, ErrInvalid
	}
	seen := map[string]bool{}
	for _, id := range in.Outputs {
		if !identity.Identifier(id) || seen[id] { return RunRequest{}, ErrInvalid }
		seen[id] = true
	}
	return in, nil
}
...
func runEligibility(e identity.Envelope, snapshot Snapshot, policy string, now time.Time) error {
	if snapshot.State.Archived || snapshot.Validation == nil { return ErrStale }
	private := snapshot.PublishedAt == nil
	if private {
		if policy != "private_preview" || snapshot.State.DraftState == "rejected" { return ErrStale }
		if err := RequirePrivate(e, snapshot.State.ID, snapshot.Revision.Actor, Execute); err != nil { return err }
		...
		if err := Require(e, snapshot.State.ID, Preview); err != nil { return err }
	}
	...
	if policy == "certified_only" && trust.Certification != "valid" { return ErrStale }
	if policy == "explicit_stale" && (snapshot.Attestation == nil || snapshot.Withdrawal != nil || trust.Certification == "unavailable") { return ErrStale }
	return nil
}
...
func (s *Runs) selectRunOutputs(d Definition, in RunRequest) ([]Output, error) {
	...
	for _, output := range selected {
		if output.Kind != "narrative" { continue }
		n := output.Narrative
	...
	if calls > s.limits.NarrativeCalls || tokens > s.limits.NarrativeTokens { return nil, ErrBudget }
	return selected, nil
}
...
func (s *Runs) Admit(ctx context.Context, e identity.Envelope, id string, input RunRequest) (RunView, error) {
	if s == nil || ctx == nil { return RunView{}, ErrInvalid }
	in, err := normalizedRunRequest(input)
	if err != nil { return RunView{}, err }
	if err = Require(e, id, Execute); err != nil { return RunView{}, err }
	if time.Duration(in.ReuseMaxAgeSeconds)*time.Second > time.Duration(s.limits.MaxReuseAge) { return RunView{}, ErrInvalid }
	...
	if err == nil { return existing.View, nil }
	if !errors.Is(err, store.ErrNotFound) && !errors.Is(err, access.ErrNotFound) { return RunView{}, err }
	if !time.Now().Before(task.Expires) { return RunView{}, ErrExpired }
	if !s.blocks.CanValidate() { return RunView{}, ErrUnavailable }
	snapshot, err := s.blocks.repo.ReadBlock(ctx, e, id, in.Reference, Execute)
	if err != nil { return RunView{}, err }
	if err = runEligibility(e, snapshot, in.Policy, time.Now()); err != nil { return RunView{}, err }
	d := snapshot.Revision.Definition
	selected, err := s.selectRunOutputs(d, in)
	if err != nil { return RunView{}, err }
	if in.Resolution.At.IsZero() { in.Resolution.At = task.Created }
	...
	if err != nil { return RunView{}, err }
	if exec.Hash(binding) != snapshot.Validation.BindingDigest { return RunView{}, ErrStale }
	private := in.Policy == "private_preview"
	retention := s.limits.Retention
	if private { retention = s.limits.PreviewRetention }
	language := in.Locale
	if language == "" { language = d.Metadata[0].Locale }
	...
	for _, definition := range definitions { m.Definitions = append(m.Definitions, clone(definition.Definition)) }
	privacyActor := ""
	if private { privacyActor = e.User() }
	m.ReuseKey = digest([]any{FrozenVersion, charts.SpecVersion, charts.MappingVersion, m.Tenant, m.Block, m.Revision.Digest,
...
func (s *Runs) List(ctx context.Context, e identity.Envelope, after string, limit int) (ArtifactList, error) {
	if s == nil || ctx == nil || limit < 1 || limit > 100 || after != "" && !identity.Identifier(after) { return ArtifactList{}, ErrInvalid }
...
func (s *Runs) Rows(ctx context.Context, e identity.Envelope, id string, offset, limit int) (ResultPage, error) {
	if s == nil || ctx == nil || offset < 0 || limit < 1 || limit > s.limits.PageRows { return ResultPage{}, ErrInvalid }
	r, err := s.repo.ReadFrozenRun(ctx, e, id, false)
	if err != nil { return ResultPage{}, err }
	if r.View.State == "expired" { return ResultPage{}, ErrExpired }
	if r.Result == nil || !slices.Contains([]string{"succeeded", "partial"}, r.View.State) { return ResultPage{}, ErrIncomplete }
	if offset > len(r.Result.Rows) { return ResultPage{}, ErrInvalid }
	...
	out := ResultPage{ID: id, Schema: clone(r.Result.Schema), Rows: clone(r.Result.Rows[offset:end]), Offset: offset,
		TotalRows: len(r.Result.Rows), Truncated: r.Result.Truncated, Observed: clone(r.View.Observed)}
	if end < len(r.Result.Rows) { out.Next = &end }
	return out, nil
}
...
func (s *Runs) Output(ctx context.Context, e identity.Envelope, id, output string) (RetainedOutput, error) {
	if s == nil || ctx == nil || !identity.Identifier(output) { return RetainedOutput{}, ErrInvalid }
	r, err := s.repo.ReadFrozenRun(ctx, e, id, false)
	if err != nil { return RetainedOutput{}, err }
	if r.View.State == "expired" { return RetainedOutput{}, ErrExpired }
	if !slices.Contains([]string{"succeeded", "partial"}, r.View.State) { return RetainedOutput{}, ErrIncomplete }
	for _, item := range r.Outputs {
		if item.ID == output { return clone(item), nil }
	}
...
func (s *Runs) Expire(ctx context.Context, e identity.Envelope, limit int) (int64, error) {
	if s == nil || ctx == nil || limit < 1 || limit > 1000 { return 0, ErrInvalid }
