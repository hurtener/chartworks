...
func (s *Service) CanObserve() bool {
	...
	_, ok := s.sources.(CatalogObserver)
	return ok
}
...
func semanticShape(d topics.Definition, used map[string]bool) topics.Definition {
	...
	for _, dataset := range out.Datasets {
		...
		dataset.Source.SourceRevision = 0
		for i := range dataset.Columns {
			dataset.Columns[i].Name = ""
			dataset.Columns[i].SourceName = ""
		}
	...
	for i := range out.Measures {
		out.Measures[i].Name = ""
		out.Measures[i].Description = ""
	}
	for i := range out.Dimensions {
		out.Dimensions[i].Name = ""
		out.Dimensions[i].Description = ""
	}
	for i := range out.KPIs {
		out.KPIs[i].Name = ""
		out.KPIs[i].Description = ""
	}
	return out
}

func nativeColumn(identity sources.CatalogIdentity, dataset, name string) (string, string) {
	if identity.Version != "postgres-catalog-identity-v1" || !hashValid(identity.Authority) {
		return "", ""
	}
	for _, relation := range identity.Relations {
		...
		for _, column := range relation.Columns {
			if column.Name == name {
				return relation.Object, column.Object
			}
		}
	}
	return "", ""
...
func compareImpact(old ValidationRecord, binding exec.Binding, observed sources.CatalogObservation, current []topics.Published) (string, string, []Rename) {
	...
	for i, publication := range current {
		...
		if !reflect.DeepEqual(semanticShape(before, used), semanticShape(after, used)) {
			return "review_required", "semantic_contract_changed", nil
		}
		for _, beforeDataset := range before.Datasets {
			if !used[beforeDataset.ID] {
				continue
			}
			for _, afterDataset := range after.Datasets {
				if beforeDataset.ID != afterDataset.ID {
					continue
				}
				for _, beforeColumn := range beforeDataset.Columns {
					for _, afterColumn := range afterDataset.Columns {
						if beforeColumn.ID != afterColumn.ID {
							continue
						}
						...
						if beforeColumn.SourceName != afterColumn.SourceName {
							renamed = append(renamed, Rename{Dataset: beforeDataset.ID, Column: beforeColumn.ID, From: beforeColumn.SourceName, To: afterColumn.SourceName})
						}
					}
				}
			}
	...
	for _, dep := range old.Dependencies {
		...
		for _, relation := range binding.Relations {
			if relation.ID == dep.Dataset && relation.Schema == dep.Schema && relation.Name == dep.Name {
				found = true
			}
		}
		if !found {
			return "review_required", "relation_identity_changed", nil
		}
	}
	sort.Slice(renamed, func(i, j int) bool {
		if renamed[i].Dataset != renamed[j].Dataset {
			return renamed[i].Dataset < renamed[j].Dataset
		}
		return renamed[i].Column < renamed[j].Column
	})
	if len(renamed) > 0 {
		return "rename", "reviewed_columns_renamed_in_place", renamed
	}
	pins := []TopicPin{}
	for _, publication := range current {
		pins = append(pins, TopicPin{Topic: publication.Definition.Topic, Version: publication.Definition.Version, Digest: publication.Digest})
	}
	if old.BindingDigest == exec.Hash(binding) && digest(pins) == digest(old.Topics) {
		return "unchanged", "exact_dependencies_unchanged", renamed
	}
...
func (s *Service) observeImpact(ctx context.Context, e identity.Envelope, id string, snapshot Snapshot) (impactWork, error) {
	...
	observer, ok := s.sources.(CatalogObserver)
	if !ok {
	...
	for _, pin := range d.Topics {
		...
		if err != nil {
			...
			out.impact.Classification = "unavailable"
			out.impact.Reason = "semantic_catalog_unavailable"
			return out, nil
		}
		if publication.State.Archived || !publication.State.Active {
			out.impact.Reason = "topic_not_active"
			return out, nil
		}
		for _, dataset := range publication.Definition.Datasets {
			if dataset.Source.Source != d.Source {
				out.impact.Reason = "source_changed"
				return out, nil
			}
			if partition != "" && partition != dataset.Source.Context {
				out.impact.Reason = "mixed_execution_contexts"
				return out, nil
			}
			partition = dataset.Source.Context
		}
		out.publications = append(out.publications, publication)
		out.pins = append(out.pins, TopicPin{Topic: publication.Definition.Topic, Version: publication.Definition.Version, Digest: publication.Digest})
	}
	...
	if err != nil {
		...
		out.impact.Reason = "registered_context_changed"
		return out, nil
	}
	out.binding = binding
	...
	if err != nil {
		...
		out.impact.Classification = "unavailable"
		out.impact.Reason = "source_catalog_unavailable"
		if errors.Is(err, exec.ErrBinding) {
			out.impact.Classification = "review_required"
			out.impact.Reason = "unregistered_source_drift"
		}
		return out, nil
	}
	...
	ids := []string{}
	for _, dep := range snapshot.Validation.Dependencies {
		ids = append(ids, dep.Dataset)
	}
	out.dependencies, err = deriveDependencies(binding, scope, ids)
	if err != nil {
		out.impact.Reason = "dependency_missing"
		return out, nil
	}
	...
	if classification == "rename" || classification == "cosmetic" {
		out.impact.ProposalDigest = digest(struct {
			...
			Pins                       []TopicPin
			Renames                    []Rename
		}{CanonicalizationVersion, snapshot.Revision.Digest, observed.BindingDigest, out.pins, out.impact.Renames})
	}
	return out, nil
}
...
func (s *Service) RecheckImpact(ctx context.Context, e identity.Envelope, id string, in ImpactRequest) (Impact, error) {
	...
	work, err := s.observeImpact(ctx, e, id, snapshot)
	if err != nil {
		return Impact{}, err
	}
	...
	if work.impact.Classification == "unchanged" {
...
func (s *Service) ApplyImpact(ctx context.Context, e identity.Envelope, id string, in ApplyImpactRequest) (View, error) {
	...
	defer cancel()
	if in.Revision < 1 || !hashValid(in.ProposalDigest) || !note(in.Note) {
	...
	if work.impact.ProposalDigest != in.ProposalDigest || work.impact.Classification != "rename" && work.impact.Classification != "cosmetic" {
		return View{}, ErrStale
	}
	d := clone(snapshot.Revision.Definition)
	if work.impact.Classification == "rename" {
		if work.binding.Dialect != "postgres" {
			return View{}, ErrInvalid
		}
	...
	for i := range d.Parameters {
		if ref := d.Parameters[i].Dimension; ref != nil {
			for _, pin := range d.Topics {
				if pin.Topic == ref.Topic {
	...
	for i := range d.Outputs {
		if d.Outputs[i].Mapping == nil {
			continue
		}
		for j := range d.Outputs[i].Mapping.Columns {
			provenance := &d.Outputs[i].Mapping.Columns[j].Provenance
			if provenance.Source == d.Source {
				provenance.SourceRevision = work.binding.Revision
			}
			for _, pin := range d.Topics {
				if provenance.Topic == pin.Topic {
					provenance.TopicVersion = pin.Version
				}
			}
		}
	}
	...
	provenance.Kind = "dependency_amendment"
	provenance.ParentRevision = in.Revision
	provenance.ChangeDigest = in.ProposalDigest
