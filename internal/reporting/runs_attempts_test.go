...
func TestFrozenQueryAttemptClassification(t *testing.T){
	...
	makeAttempt:=func(number int,status,remote string,finished bool)exec.Attempt{
		...
		if finished{a.Finished=&now};return a
	}
	for _,test:=range []struct{name string;attempts []exec.Attempt;want int;failure error}{
	...
	}{t.Run(test.name,func(t *testing.T){got,err:=nextFrozenQueryNumber(test.attempts);if got!=test.want||!errors.Is(err,test.failure){t.Fatalf("got (%d,%v), want (%d,%v)",got,err,test.want,test.failure)}})}
}
