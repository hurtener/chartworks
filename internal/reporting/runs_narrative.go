...
func narrativeEvidence(result exec.Result, n Narrative) ([]NarrativeEvidence, []string, error) {
	if n.MaxRows < 1 || n.MaxBytes < 1 || n.MaxCharacters < 1 || len(result.Schema) == 0 {
	...
	appendItem := func(field, kind, value string, row int) bool {
		if len(items) >= 256 || !utf8.ValidString(value) {
			return false
		}
...
func groundedText(answer NarrativeAnswer, evidence []NarrativeEvidence, n Narrative) (string, error) {
	...
	for _, claim := range answer.Claims {
		...
		case "difference":
			if len(claim.Evidence) != 2 || claim.Evidence[0] == claim.Evidence[1] {
				return "", gateway.ErrOutput
			}
	...
	if n.RequireCaveats {
		...
		if spanish {
			caveat = "La evidencia es una observación retenida y acotada, no un total completo ni una consulta en vivo."
		}
		text += "\n" + caveat
	}
	if utf8.RuneCountInString(text) > n.MaxCharacters {
		return "", ErrBudget
	}
	return text, nil
}
...
func (s *Runs) generateNarrative(ctx context.Context, e identity.Envelope, m RunManifest, output string, result exec.Result, n Narrative) (NarrativeResult, error) {
	...
	if json.Unmarshal(generated.JSON, &answer) != nil {
		return NarrativeResult{Receipt: generated.Receipt}, gateway.ErrOutput
	}
