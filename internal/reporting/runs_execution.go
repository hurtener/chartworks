...
func (s *Runs) pinnedPlan(ctx context.Context, e identity.Envelope, m RunManifest) (exec.Plan, error) {
	if !s.blocks.CanValidate() { return exec.Plan{}, ErrUnavailable }
	if err := RequireRunManifest(e, m); err != nil { return exec.Plan{}, err }
	snapshot, err := s.blocks.repo.ReadBlock(ctx, e, m.Block, Reference{Revision: m.Revision.Number}, Execute)
	if err != nil { return exec.Plan{}, err }
	if snapshot.Revision.Digest != m.Revision.Digest { return exec.Plan{}, ErrStale }
	if err = runEligibility(e, snapshot, m.Policy, time.Now()); err != nil { return exec.Plan{}, err }
	...
	if err != nil { return exec.Plan{}, err }
	if exec.Hash(binding) != exec.Hash(m.Binding) { return exec.Plan{}, ErrStale }
	...
	if err != nil || statement != d.SQL || parameterDigest(parameters) != parameterDigest(m.Resolved.Parameters) { return exec.Plan{}, ErrStale }
	dependencies, err := deriveDependencies(binding, scope, plan.Receipt().Dependencies)
	if err != nil { return exec.Plan{}, err }
	if DependencyDigest(dependencies, d.Topics) != DependencyDigest(m.Dependencies, d.Topics) { return exec.Plan{}, ErrStale }
	return plan, nil
}
...
func frozenOutcome(outputs []RetainedOutput, policy string) (string, string) {
	failed := false
	for _, output := range outputs { failed = failed || output.State != "succeeded" }
	if !failed { return "succeeded", "" }
	if policy == "allow_partial" { return "partial", "output_failed" }
...
func (s *Runs) queryFrozen(ctx context.Context, e identity.Envelope, inv jobs.Invocation, m RunManifest) (RunRecord, error) {
	...
	if err = checkResult(ctx, m.Revision.Definition, *report.Result, limits); err != nil { return RunRecord{}, err }
...
func (s *Runs) makeOutput(ctx context.Context, e identity.Envelope, inv jobs.Invocation, m RunManifest, result exec.Result, saved Output) (RetainedOutput, error) {
	...
	if saved.Kind != "narrative" {
		data, err := chartdata.FromResult(result)
		...
		if n == nil || s.model == nil || m.Model != s.modelVersion || n.ModelVersion != s.modelVersion {
			...
			if _, err := s.writeRun(ctx, e, inv, RunWrite{Kind: "output_start", Manifest: m, Output: &start}); err != nil { return out, err }
...
func (s *Runs) continueFrozen(ctx context.Context, e identity.Envelope, inv jobs.Invocation, initial RunRecord) error {
	if initial.Manifest == nil { return ErrExpired }
	m := *initial.Manifest
	if err := RequireRunManifest(e, m); err != nil { return err }
	if !time.Now().Before(m.Expires) { return ErrExpired }
	...
	for _, saved := range m.Outputs {
		...
		if existing, ok := byID[saved.ID]; ok {
			...
			if _, err = s.writeRun(ctx, e, inv, RunWrite{Kind: "output", Manifest: m, Output: &existing}); err != nil { return err }
			byID[saved.ID] = existing
			continue
		}
		output, buildErr := s.makeOutput(ctx, e, inv, m, *current.Result, saved)
		if buildErr != nil { return buildErr }
		if _, err = s.writeRun(ctx, e, inv, RunWrite{Kind: "output", Manifest: m, Output: &output}); err != nil { return err }
		byID[saved.ID] = output
	}
	ordered := make([]RetainedOutput, 0, len(m.Outputs))
	for _, saved := range m.Outputs { ordered = append(ordered, byID[saved.ID]) }
...
func (s *Runs) Run(ctx context.Context, e identity.Envelope, id string, resume bool) (RunView, error) {
	...
	switch r.View.State {
	case "expired": return r.View, ErrExpired
	case "succeeded", "partial": return r.View, nil
	case "failed": return r.View, ErrIncomplete
	}
	if r.Manifest == nil { return r.View, ErrIncomplete }
	if err = RequireRunManifest(e, *r.Manifest); err != nil { return RunView{}, err }
	task, err := s.runner.Inspect(ctx, e, id)
	if err != nil { return r.View, err }
	if resume {
	...
	if readErr != nil {
		if runErr != nil { return r.View, runErr }
		return r.View, readErr
	}
	if runErr != nil { return current.View, runErr }
	if current.View.State == "failed" { return current.View, ErrIncomplete }
	if current.View.State != "succeeded" && current.View.State != "partial" { return current.View, ErrIncomplete }
...
func (s *Runs) RebuildOutput(ctx context.Context, e identity.Envelope, id, output string) (RetainedOutput, error) {
	...
	if r.Manifest == nil || r.Result == nil || r.View.State != "succeeded" && r.View.State != "partial" { return RetainedOutput{}, ErrIncomplete }
	var retained *RetainedOutput
	for i := range r.Outputs { if r.Outputs[i].ID == output { retained = &r.Outputs[i] } }
	if retained == nil { return RetainedOutput{}, store.ErrNotFound }
	if retained.Kind == "narrative" || retained.State != "succeeded" { return clone(*retained), nil }
	for _, saved := range r.Manifest.Outputs {
		if saved.ID != output { continue }
		data, err := chartdata.FromResult(*r.Result)
		if err != nil { return RetainedOutput{}, err }
		chart, err := charts.Build(ctx, data, *saved.Mapping)
		if err != nil { return RetainedOutput{}, err }
		out := clone(*retained)
		...
		out.Digest = digest(out)
		if out.Digest != retained.Digest { return RetainedOutput{}, ErrStale }
		return out, nil
	}
