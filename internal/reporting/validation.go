...
func (s *Service) validateWork(ctx context.Context, e identity.Envelope, id string, snapshot Snapshot, in ValidateRequest) (ValidationRecord, exec.Result, Resolved, []ResourceReference, error) {
	...
	if !s.CanValidate() { return record, result, resolved, nil, ErrUnavailable }
	if snapshot.State.Archived || snapshot.State.DraftState == "rejected" && snapshot.PublishedAt == nil { return record, result, resolved, nil, store.ErrConflict }
	if err := expected(snapshot, in.ExpectedVersion); err != nil { return record, result, resolved, nil, err }
	d := snapshot.Revision.Definition
	if err := access.Require(e, "sources.query", access.Resource{Tenant: e.Tenant(), Kind: "source", Permission: "query", ID: d.Source}, access.Resource{Tenant: e.Tenant(), Kind: "execution_context", Permission: "use", ID: d.Context}); err != nil { return record, result, resolved, nil, err }
	...
	if err != nil { return record, result, resolved, nil, err }
	if binding.Tenant != e.Tenant() || binding.Source != d.Source || binding.Context != d.Context { return record, result, resolved, nil, ErrStale }
	...
	if err != nil || statement != d.SQL || digest(parameters) != digest(resolved.Parameters) { return record, result, resolved, nil, ErrStale }
...
func (s *Service) Validate(ctx context.Context, e identity.Envelope, id string, in ValidateRequest) (ValidationResult, error) {
	...
	if err != nil { return ValidationResult{}, err }
	defer cancel()
...
func freshValidation(snapshot Snapshot, evidenceID string, now time.Time) error {
	...
	if v == nil || v.Evidence.ID != evidenceID || !hashValid(v.Evidence.DefinitionDigest) || v.Evidence.DefinitionDigest != r.Digest || v.Evidence.ExecutionDigest != r.ExecutionDigest || v.Evidence.Revision != r.Number || v.Evidence.RevisionID != r.ID || v.Evidence.CanonicalizationVersion != CanonicalizationVersion || v.Evidence.DependencyDigest != DependencyDigest(v.Dependencies, r.Definition.Topics) || !now.Before(v.Evidence.ExpiresAt) || !snapshot.Current || !successful(v.Evidence.Attempt.Status) { return ErrStale }
	return nil
}
