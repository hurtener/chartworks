...
func identifierSpan(sql string, start int) (int, string, error) {
	...
	for i < len(sql) {
		c := sql[i]
		if c >= 'a' && c <= 'z' || c >= 'A' && c <= 'Z' || c == '_' || i > start && (c >= '0' && c <= '9' || c == '$') {
			i++
			continue
		}
		break
...
func columnSpan(sql string, column map[string]any, names []string) (int, int, error) {
	position, ok := astInteger(column["location"])
	if !ok {
	...
	for i, wanted := range names {
		...
		if position >= len(sql) || sql[position] != '.' {
			return 0, 0, ErrInvalid
		}
		position++
	}
...
func renameSQL(ctx context.Context, sql string, renames []Rename, dependencies []Dependency) (string, error) {
	if len(renames) == 0 || len(renames) > 256 {
		return "", ErrInvalid
	}
	...
	if len(from) != 1 || root["withClause"] != nil || root["larg"] != nil || root["rarg"] != nil {
		return "", ErrInvalid
	}
	...
	if alias == "" {
		alias = astString(astObject(astObject(rangeVar["alias"])["Alias"])["aliasname"])
	}
	if schema == "" || table == "" {
		return "", ErrInvalid
	}
	...
	for _, dep := range dependencies {
		if dep.Schema == schema && dep.Name == table {
			if dataset != "" && dataset != dep.Dataset {
				return "", ErrInvalid
			}
			dataset = dep.Dataset
		}
	...
	for _, rename := range renames {
		if rename.Dataset != dataset || rename.From == "" || rename.To == "" || rename.From == rename.To || !text(rename.To, 128) {
			return "", ErrInvalid
		}
		if old, ok := mapping[rename.From]; ok && old != rename.To {
			return "", ErrInvalid
		}
	...
	if err := walkAST(document, 0, &budget, func(m map[string]any) error {
		...
		if m["SubLink"] != nil || m["A_Star"] != nil {
			return ErrInvalid
		}
		return nil
	}); err != nil || selects != 1 || ranges != 1 {
		return "", ErrInvalid
	}
	qualifies := func(names []string) bool {
		...
		case 2:
			...
			return names[0] == table
		case 3:
			return alias == "" && names[0] == schema && names[1] == table
		default:
			return false
		}
	}
	...
	for _, target := range astArray(root["targetList"]) {
		...
		if outputName != "" && mapping[outputName] != "" && (!plainColumn || names[len(names)-1] != outputName) {
			return "", ErrInvalid
		}
		if !plainColumn || !qualifies(names) || mapping[names[len(names)-1]] == "" || outputName != "" {
			continue
		}
		_, end, err := columnSpan(sql, astObject(astObject(res["val"])["ColumnRef"]), names)
		if err != nil {
			return "", err
		}
	...
	err = walkAST(document, 0, &budget, func(m map[string]any) error {
		column := astObject(m["ColumnRef"])
		if column == nil {
			return nil
		}
		names, ok := columnNames(m)
		if !ok || !qualifies(names) {
			return ErrInvalid
		}
		replacement := mapping[names[len(names)-1]]
		if replacement == "" {
			return nil
		}
		start, end, err := columnSpan(sql, column, names)
		if err != nil {
			return err
		}
