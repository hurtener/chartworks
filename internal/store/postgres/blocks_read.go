...
const blockReferenceEligibility = `NOT EXISTS (
...
func blockReadArgs(e identity.Envelope, id string, ref reporting.Reference, a reporting.Access) ([]any, error) {
	if ref.Revision < 0 || ref.Revision > 256 || ref.Draft && ref.Revision != 0 { return nil, store.ErrInvalid }
	if err := reporting.Require(e, id, a); err != nil { return nil, err }
	if a == reporting.SQLRead {
		if err := reporting.Require(e, id, reporting.Read); err != nil { return nil, err }
	}
	grants, err := blockGrants(e)
	if err != nil { return nil, err }
	private := a == reporting.Write || a == reporting.Validate || a == reporting.Publish
	if !private { private = reporting.Require(e, id, reporting.Preview) == nil }
	parent := "read"
	if a == reporting.Write { parent = "write" }
...
func blockTx(ctx context.Context, tx pgx.Tx, e identity.Envelope, id string, ref reporting.Reference, a reporting.Access) (out reporting.Snapshot, err error) {
 ...
 AND `+blockReferenceEligibility, args...).Scan(
	...
	if err != nil { return out, err }
	if json.Unmarshal(definition,&out.Revision.Definition)!=nil || json.Unmarshal(provenance,&out.Revision.Provenance)!=nil || json.Unmarshal(references,&out.References)!=nil { return reporting.Snapshot{}, store.ErrInvalid }
	if args[8].(bool) && (reporting.DefinitionDigest(out.Revision.Definition)!=out.Revision.Digest || reporting.ExecutionDigest(out.Revision.Definition)!=out.Revision.ExecutionDigest) { return reporting.Snapshot{}, store.ErrInvalid }
	if len(validation)>0 && json.Unmarshal(validation,&out.Validation)!=nil || len(attestation)>0 && json.Unmarshal(attestation,&out.Attestation)!=nil || len(withdrawal)>0 && json.Unmarshal(withdrawal,&out.Withdrawal)!=nil || len(health)>0 && json.Unmarshal(health,&out.Health)!=nil { return reporting.Snapshot{}, store.ErrInvalid }
	if out.Validation != nil && (out.Validation.Evidence.Revision != out.Revision.Number || out.Validation.Evidence.RevisionID != out.Revision.ID || out.Validation.Evidence.DefinitionDigest != out.Revision.Digest) { return reporting.Snapshot{}, store.ErrInvalid }
	if !out.Current {
	...
	if !e.Valid() { return reporting.Snapshot{}, access.ErrUnauthenticated }
	return out, ctx.Err()
}
...
func (d *DB) ReadBlock(ctx context.Context, e identity.Envelope, id string, ref reporting.Reference, a reporting.Access) (out reporting.Snapshot, err error) {
	if _, err = blockReadArgs(e,id,ref,a); err != nil { return out, err }
	ctx,cancel,err := requestContext(ctx,e)
	if err != nil { return out,err }
	defer cancel()
	err = d.transaction(ctx,func(ctx context.Context,tx pgx.Tx) error { var err error; out,err=blockTx(ctx,tx,e,id,ref,a);return err })
	return
}
...
func (d *DB) ListBlocks(ctx context.Context, e identity.Envelope, in reporting.ListRequest) (out reporting.Page, err error) {
	...
	if in.Limit<1 || in.Limit>100 || in.After!="" && !identity.Identifier(in.After) {return out,store.ErrInvalid}
 ...
 AND `+blockReferenceEligibility+` ORDER BY h.block_id LIMIT $3`,e.Tenant(),in.After,in.Limit+1,in.IncludeDrafts,e.User(),grants,selection.All(),selection.IDs(),e.Has(reporting.Preview.Action()))
		...
		for rows.Next(){
			...
			var metadata []byte
			if err:=rows.Scan(&item.State.ID,&item.State.Topic,&item.State.Version,&item.State.DraftRevision,&item.State.PublishedRevision,&item.State.DraftState,&item.State.Archived,&item.State.CreatedAt,&item.State.UpdatedAt,&item.Revision,&metadata,&item.Private);err!=nil{return err}
			if json.Unmarshal(metadata,&item.Metadata)!=nil{return store.ErrInvalid}
			if len(out.Items)==in.Limit{out.Next=out.Items[len(out.Items)-1].State.ID;break}
			out.Items=append(out.Items,item)
		}
		return rows.Err()
	})
	if err==nil && !e.Valid(){err=access.ErrUnauthenticated}
	if err!=nil{return reporting.Page{},err}
	return out,nil
}
...
func (d *DB) BlockHistory(ctx context.Context,e identity.Envelope,id string)(out reporting.History,err error){
	if err=reporting.Require(e,id,reporting.Read);err!=nil{return out,err}
	grants,err:=blockGrants(e);if err!=nil{return out,err}
 ...
 AND `+blockReferenceEligibility+` ORDER BY ev.version LIMIT 4096`,args...)
		...
		for rows.Next(){var event reporting.Event;if err:=rows.Scan(&event.Version,&event.Kind,&event.Revision,&event.Actor,&event.Note,&event.CreatedAt);err!=nil{rows.Close();return err};out.Events=append(out.Events,event);count++}
		err=rows.Err();rows.Close();if err!=nil{return err}
		if count==0{return store.ErrNotFound}
		...
		if err!=nil{return err}
		if reporting.Require(e,id,reporting.Preview)!=nil {out.State.DraftRevision=0;out.State.DraftState=""}
		return nil
	})
	return
}
...
func blockConsistency(s reporting.Snapshot) error {
	if s.State.ID=="" || s.Revision.Number<1 || s.Revision.ID=="" || s.State.DraftRevision<s.Revision.Number {return fmt.Errorf("%w: block coordinate",store.ErrInvalid)}
	return nil
}
