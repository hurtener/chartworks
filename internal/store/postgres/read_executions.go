...
func scanRead(row pgx.Row) (a readexec.Attempt,err error) {
	...
	if json.Unmarshal(manifest,&a.Manifest)!=nil || !a.Manifest.Valid() { return readexec.Attempt{},store.ErrInvalid }
	if remote!=nil {
		a.Remote=&readexec.RemoteQuery{}
		if json.Unmarshal(remote,a.Remote)!=nil || !a.Remote.Valid() { return readexec.Attempt{},store.ErrInvalid }
	}
	if a.Finished==nil && time.Now().After(a.Deadline.Add(a.Manifest.Limits.CancelGrace)) { a.Status="uncertain";a.RemoteState="unknown";a.Code="remote_outcome_unknown" }
	return a,nil
}
...
func (d *DB) BeginRead(ctx context.Context,s store.Scope,a readexec.Attempt,maximum int) error {
	if !s.Valid() { return store.ErrScope }
	if !identity.Identifier(a.ID) || len(a.ID)!=32 || !a.Manifest.Valid() || a.Number<1 || maximum<1 || maximum>3 || a.Number>maximum || a.Created.IsZero() || !a.Deadline.After(a.Created) || a.Deadline.Sub(a.Created)>time.Minute+time.Second { return store.ErrInvalid }
	return d.transaction(ctx,func(ctx context.Context,tx pgx.Tx) error {
		if _,err:=tx.Exec(ctx,`SELECT pg_advisory_xact_lock(hashtextextended($1,7214061010))`,s.Tenant());err!=nil { return err }
		...
		if _,err:=tx.Exec(ctx,`DELETE FROM chartworks.read_attempts WHERE tenant_id=$1 AND finished_at IS NOT NULL AND status<>'uncertain' AND created_at<clock_timestamp()-interval '24 hours'`,s.Tenant());err!=nil { return err }
		...
		if err==nil {
			if readexec.Hash(previous.Manifest)!=readexec.Hash(a.Manifest) { return store.ErrConflict }
			if a.Number<=previous.Number { return readexec.ErrReplay }
			if a.Number!=previous.Number+1 { return store.ErrConflict }
			switch previous.Status { case "failed","cancelled","timed_out","interrupted":default:return store.ErrConflict }
		} else if !errors.Is(err,pgx.ErrNoRows) { return err } else if a.Number!=1 { return store.ErrConflict }
		var total,actor int
		if err=tx.QueryRow(ctx,`SELECT count(*),count(*) FILTER(WHERE actor_id=$2) FROM chartworks.read_attempts WHERE tenant_id=$1`,s.Tenant(),s.Actor()).Scan(&total,&actor);err!=nil { return err }
		if total>=10000 || actor>=1000 { return readexec.ErrLimit }
...
func (d *DB) DispatchRead(ctx context.Context,s store.Scope,id string,q readexec.RemoteQuery,accepted bool) error {
	if !s.Valid() { return store.ErrScope }
	if !identity.Identifier(id) || !q.Valid() || q.Tag!="cw-read:"+id { return store.ErrInvalid }
	return d.transaction(ctx,func(ctx context.Context,tx pgx.Tx) error {
		remote,_:=json.Marshal(q)
		from,to:="accepted","dispatching";if accepted { from,to="dispatching","running" }
		...
		if err!=nil { return err };if tag.RowsAffected()!=1 { return readexec.ErrCancelled };return nil
	})
}
...
func (d *DB) GetRead(ctx context.Context,s store.Scope,id string) (a readexec.Attempt,err error) {
	if !s.Valid() { return a,store.ErrScope };if !identity.Identifier(id) { return a,store.ErrInvalid }
	err=d.transaction(ctx,func(ctx context.Context,tx pgx.Tx) error { var e error;a,e=scanRead(tx.QueryRow(ctx,`SELECT `+readAttemptColumns+` FROM chartworks.read_attempts WHERE tenant_id=$1 AND actor_id=$2 AND attempt_id=$3`,s.Tenant(),s.Actor(),id));return e })
...
func (d *DB) CancelRead(ctx context.Context,s store.Scope,id string) error {
	if !s.Valid() { return store.ErrScope };if !identity.Identifier(id) { return store.ErrInvalid }
	return d.transaction(ctx,func(ctx context.Context,tx pgx.Tx) error {
...
func (d *DB) FinishRead(ctx context.Context,s store.Scope,a readexec.Attempt,reconcile bool) error {
	if !s.Valid() { return store.ErrScope }
	if !identity.Identifier(a.ID) || !a.Manifest.Valid() || a.Finished==nil || a.Rows<0 || a.Rows>a.Manifest.Limits.Rows || a.Bytes<0 || a.Bytes>a.Manifest.Limits.Bytes { return store.ErrInvalid }
	switch a.Status { case "succeeded","empty","truncated","cancelled","timed_out","failed","uncertain","interrupted":default:return store.ErrInvalid }
	if reconcile && (a.Status!="interrupted" || a.Rows!=0 || a.Bytes!=0 || a.RemoteState!="stopped" && a.RemoteState!="not_issued") { return store.ErrInvalid }
	return d.transaction(ctx,func(ctx context.Context,tx pgx.Tx) error {
