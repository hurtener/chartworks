...
func (d *DB) ReuseFrozenRun(ctx context.Context, inv jobs.Invocation, id string) (out reporting.RunRecord, reused bool, err error) {
	...
	err = d.transaction(ctx, func(ctx context.Context, tx pgx.Tx) error {
		...
		h, readErr := frozenReadHeadTx(ctx, tx, e, id, true, true)
		if readErr != nil {
			return readErr
		}
		out, readErr = frozenReadTx(ctx, tx, e, id, true, true)
		if readErr != nil {
			return readErr
		}
		if out.Manifest == nil || !time.Now().Before(h.view.Expires) {
		...
		args, readErr := frozenArgs(e, id, false)
		if readErr != nil {
			return readErr
		}
		...
		previous, readErr := frozenReadTx(ctx, tx, e, candidate, false, true)
		if readErr != nil {
			return readErr
		}
		...
		result, marshalErr := json.Marshal(previous.Result)
		if marshalErr != nil || len(result) > m.Limits.MaxResultBytes {
			return reporting.ErrBudget
		}
		payloads := make([][]byte, len(previous.Outputs))
		added := int64(len(result))
		for index, output := range previous.Outputs {
			payloads[index], marshalErr = json.Marshal(output)
			if marshalErr != nil {
				return store.ErrInvalid
			}
		...
		if _, readErr = tx.Exec(ctx, `UPDATE chartworks.frozen_run_payloads SET result=$3,result_digest=$4 WHERE tenant_id=$1 AND operation_id=$2 AND result IS NULL`, e.Tenant(), id, result, readexec.Hash(*previous.Result)); readErr != nil {
			return readErr
		}
		for index, output := range previous.Outputs {
			if _, readErr = tx.Exec(ctx, `INSERT INTO chartworks.frozen_run_outputs(tenant_id,operation_id,output_id,ordinal,kind,state,payload) VALUES($1,$2,$3,$4,$5,$6,$7)`, e.Tenant(), id, output.ID, index, output.Kind, output.State, payloads[index]); readErr != nil {
				return readErr
			}
		}
		if _, readErr = tx.Exec(ctx, `UPDATE chartworks.frozen_runs SET state='succeeded',reused_from=$3,observed_at=$4,finished_at=clock_timestamp(),reserved_bytes=0,payload_expires_at=LEAST(payload_expires_at,$5) WHERE tenant_id=$1 AND operation_id=$2`, e.Tenant(), id, candidate, *previous.View.Observed, previous.View.Expires); readErr != nil {
			return readErr
		}
		if readErr = completeRequestTx(ctx, tx, inv); readErr != nil {
			return readErr
		}
		if readErr = frozenAudit(ctx, tx, e, "reporting.run_reused", id); readErr != nil {
			return readErr
		}
		out, readErr = frozenReadTx(ctx, tx, e, id, true, true)
		reused = readErr == nil
	...
	if err != nil {
		return reporting.RunRecord{}, false, err
	}
...
func expireFrozenRows(ctx context.Context, tx pgx.Tx, scope store.Scope, asOf time.Time, limit int) (int64, error) {
	if !scope.Valid() || asOf.IsZero() || limit < 1 || limit > 1000 {
		return 0, store.ErrInvalid
	}
	if err := frozenQuotaLock(ctx, tx, scope.Tenant()); err != nil {
		return 0, err
	}
...
func (d *DB) ExpireFrozenArtifacts(ctx context.Context, e identity.Envelope, limit int) (count int64, err error) {
	...
	err = d.transaction(ctx, func(ctx context.Context, tx pgx.Tx) error {
		var expireErr error
		count, expireErr = expireFrozenRows(ctx, tx, scope, time.Now(), limit)
		return expireErr
	})
	if err != nil {
		return 0, err
