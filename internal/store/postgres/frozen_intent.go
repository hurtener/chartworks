...
import("context";"encoding/json";"github.com/hurtener/chartworks/internal/reporting";"github.com/hurtener/chartworks/internal/store";"github.com/jackc/pgx/v5")
...
func frozenIntentTx(ctx context.Context,tx pgx.Tx,tenant,id string,view *reporting.RunView)error{
 ...
 if err!=nil{return err};var metadata reporting.RunView
 if len(raw)>1<<20||json.Unmarshal(raw,&metadata)!=nil{return store.ErrInvalid}
 ...
 for i:=range view.ExpectedSchema{view.ExpectedSchema[i].Sensitivity="";if i<len(view.ResultPolicy)&&view.ResultPolicy[i].Name==view.ExpectedSchema[i].Name{view.ExpectedSchema[i].Sensitivity=view.ResultPolicy[i].Sensitivity}}
 return nil
}
