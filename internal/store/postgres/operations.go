...
func scanOperation(row pgx.Row) (o store.Operation, err error) {
	err = row.Scan(&o.ID, &o.Status, &o.PolicyRevision, &o.Cutoff, &o.Limit, &o.DeletedEvents, &o.DeletedOperations)
	return o, err
}
...
func (d *DB) ReserveSweep(ctx context.Context, s store.Scope, key, hash string, limit int) (out store.Operation, err error) {
	...
	if !store.Identifier(key) || e != nil || len(b) != 32 || len(hash) != 64 || limit < 1 || limit > 1000 {
 ...
 ON CONFLICT(tenant_id,actor_id,kind,client_key) DO NOTHING`, s.Tenant(), operationID, s.Actor(), key, hash, limit)
		if e != nil {
			return e
		}
		...
		if storedHash != hash || out.Limit != limit {
			return store.ErrConflict
		}
		if expired || out.Status == "expired" {
...
func validLease(l store.Lease) bool {
	return store.Identifier(l.OperationID) && store.Identifier(l.Owner) && l.Fence > 0
}
...
func (d *DB) Claim(ctx context.Context, s store.Scope, opID, owner string, ttl time.Duration) (out store.Lease, err error) {
	...
	if !store.Identifier(opID) || !store.Identifier(owner) || !validTTL(ttl) {
		return out, store.ErrInvalid
	}
...
func (d *DB) Renew(ctx context.Context, s store.Scope, l store.Lease, ttl time.Duration) error {
	if e := checkScope(s); e != nil {
		return e
	}
	if !validLease(l) || !validTTL(ttl) {
...
func (d *DB) CommitSweep(ctx context.Context, s store.Scope, l store.Lease) (out store.Operation, err error) {
	...
	err = d.transaction(ctx, func(ctx context.Context, tx pgx.Tx) error {
		...
		var current int64
		if e = tx.QueryRow(ctx, `SELECT current_revision FROM chartworks.policies WHERE tenant_id=$1 FOR SHARE`, s.Tenant()).Scan(&current); e != nil {
			return e
		}
		if current != out.PolicyRevision {
			return store.ErrConflict
		}
		tag, e := tx.Exec(ctx, `DELETE FROM chartworks.audit_events WHERE (tenant_id,event_id) IN (SELECT tenant_id,event_id FROM chartworks.audit_events WHERE tenant_id=$1 AND created_at<$2 ORDER BY created_at,event_id LIMIT $3 FOR UPDATE SKIP LOCKED)`, s.Tenant(), out.Cutoff, out.Limit)
		if e != nil {
			return e
		}
		out.DeletedEvents = tag.RowsAffected()
		tag, e = tx.Exec(ctx, `UPDATE chartworks.operations SET status='expired',deleted_events=0,deleted_operations=0 WHERE (tenant_id,operation_id) IN (SELECT o.tenant_id,o.operation_id FROM chartworks.operations o WHERE o.tenant_id=$1 AND o.operation_id<>$2 AND o.status='succeeded' AND o.expires_at<clock_timestamp() AND NOT EXISTS (SELECT 1 FROM chartworks.audit_events a WHERE a.tenant_id=o.tenant_id AND a.operation_id=o.operation_id) ORDER BY o.expires_at,o.operation_id LIMIT $3 FOR UPDATE SKIP LOCKED)`, s.Tenant(), out.ID, out.Limit)
		if e != nil {
			return e
		}
		...
		_, e = tx.Exec(ctx, `INSERT INTO chartworks.audit_events(tenant_id,event_id,actor_id,action,resource_id,operation_id) VALUES($1,$2,$3,'retention.sweep',$4,$4)`, s.Tenant(), eventID, s.Actor(), out.ID)
		if e != nil {
			return e
		}
