...
func (d *DB) sealFrozenRun(ctx context.Context, e identity.Envelope, task jobs.RequestTask, proof reporting.PreparedRun, invocation *jobs.Invocation) (out reporting.RunRecord, err error) {
	...
	if task.Input.Kind != "reporting.run" || task.ID != m.ID || task.Input.Target != m.Block || task.Input.InputHash != m.RequestHash || task.ManifestHash != m.TaskHash || !task.Created.Equal(m.Created) {
		return out, store.ErrInvalid
	}
	body, err := json.Marshal(m)
	if err != nil || int64(len(body)) > int64(m.Limits.MaxArtifactBytes) {
		return out, reporting.ErrBudget
	}
	...
	err = d.transaction(ctx, func(ctx context.Context, tx pgx.Tx) error {
		...
		if actual.ManifestHash != m.TaskHash || actual.Input.InputHash != m.RequestHash {
		...
		if readErr == nil {
			...
			out, readErr = frozenReadTx(ctx, tx, e, m.ID, true, true)
			return readErr
		}
		...
		if readErr = frozenQuotaLock(ctx, tx, e.Tenant()); readErr != nil {
			return readErr
		}
		var count int
		var charged int64
 ...
 UNION ALL SELECT retained_bytes,reserved_bytes FROM chartworks.composition_runs WHERE tenant_id=$1) usage`, e.Tenant()).Scan(&count, &charged); readErr != nil {
		...
		if _, readErr = tx.Exec(ctx, `INSERT INTO chartworks.frozen_run_payloads(tenant_id,operation_id,manifest) VALUES($1,$2,$3)`, e.Tenant(), m.ID, body); readErr != nil {
			return readErr
		}
		if readErr = frozenAudit(ctx, tx, e, "reporting.run_sealed", m.ID); readErr != nil {
			return readErr
		}
		out, readErr = frozenReadTx(ctx, tx, e, m.ID, true, true)
		return readErr
...
func frozenJournalTx(ctx context.Context, tx pgx.Tx, e identity.Envelope, h frozenHead, a readexec.Attempt) error {
	...
	if !frozenAttemptMatches(h, actual) || readexec.Hash(actual) != readexec.Hash(a) {
...
func frozenBytesTx(ctx context.Context, tx pgx.Tx, e identity.Envelope, h frozenHead, delta int64) error {
	if delta > h.maximum-h.view.RetainedBytes || delta < -h.view.RetainedBytes {
		return reporting.ErrBudget
	}
...
func frozenResultTx(ctx context.Context, tx pgx.Tx, e identity.Envelope, h frozenHead, w reporting.RunWrite) error {
	if w.Result == nil || w.Attempt == nil {
		return store.ErrInvalid
	}
	if err := reporting.CheckFrozenResult(ctx, w.Manifest, *w.Result, *w.Attempt); err != nil {
		return err
	}
	if err := frozenJournalTx(ctx, tx, e, h, *w.Attempt); err != nil {
		return err
	}
	var oldHash *string
	if err := tx.QueryRow(ctx, `SELECT result_digest FROM chartworks.frozen_run_payloads WHERE tenant_id=$1 AND operation_id=$2 FOR UPDATE`, e.Tenant(), h.view.ID).Scan(&oldHash); err != nil {
		return err
	}
	...
	raw, err := json.Marshal(w.Result)
	if err != nil {
		return store.ErrInvalid
	}
	if err = frozenBytesTx(ctx, tx, e, h, int64(len(raw))); err != nil {
		return err
	}
	if _, err = tx.Exec(ctx, `UPDATE chartworks.frozen_run_payloads SET result=$3,result_digest=$4 WHERE tenant_id=$1 AND operation_id=$2 AND result IS NULL`, e.Tenant(), h.view.ID, raw, hash); err != nil {
		return err
	}
...
func frozenOutputTx(ctx context.Context, tx pgx.Tx, e identity.Envelope, h frozenHead, w reporting.RunWrite) error {
	if w.Output == nil || h.view.State != "normalized" {
		return store.ErrConflict
	}
	o := *w.Output
	starting := w.Kind == "output_start"
	if err := reporting.CheckFrozenOutput(w.Manifest, o, starting); err != nil {
		return err
	}
	ordinal := -1
	for index, output := range w.Manifest.Outputs {
		if output.ID == o.ID {
			ordinal = index
			break
		}
	...
	if !starting && exists {
		...
		if oldState != "indeterminate" {
			if old.Digest == o.Digest {
				return nil
			}
			return store.ErrConflict
		}
		if o.Kind != "narrative" || o.ReservedCalls != old.ReservedCalls || o.ReservedTokens != old.ReservedTokens {
			return store.ErrInvalid
		}
	}
	...
	if starting {
		...
		if _, err = tx.Exec(ctx, `UPDATE chartworks.frozen_runs SET reserved_calls=reserved_calls+$3,reserved_tokens=reserved_tokens+$4 WHERE tenant_id=$1 AND operation_id=$2`, e.Tenant(), h.view.ID, o.ReservedCalls, o.ReservedTokens); err != nil {
			return err
		}
	}
...
func (d *DB) CheckpointFrozenRun(ctx context.Context, inv jobs.Invocation, proof reporting.PreparedRunWrite) (out reporting.RunRecord, err error) {
	...
	defer stop()
	err = d.transaction(ctx, func(ctx context.Context, tx pgx.Tx) error {
		if _, fenceErr := requestFenceTx(ctx, tx, inv); fenceErr != nil {
			return fenceErr
		}
		h, readErr := frozenReadHeadTx(ctx, tx, e, w.Manifest.ID, true, true)
		if readErr != nil {
			return readErr
		}
		if h.view.ManifestDigest != w.Manifest.Digest() || h.taskHash != inv.Lease().Task.ManifestHash {
			return store.ErrConflict
		}
		...
		if w.Kind != "attempt" {
			if readErr = frozenCurrentTx(ctx, tx, e, w.Manifest); readErr != nil {
				return readErr
			}
		}
		...
		case "result":
			readErr = frozenResultTx(ctx, tx, e, h, w)
		case "output_start", "output":
			readErr = frozenOutputTx(ctx, tx, e, h, w)
			action = "reporting.output_checkpoint"
			if w.Kind == "output_start" {
				action = "reporting.output_started"
			}
		case "complete":
			current, currentErr := frozenReadTx(ctx, tx, e, w.Manifest.ID, true, true)
			if currentErr != nil {
				return currentErr
			}
			...
			state, code, completionErr := reporting.FrozenCompletion(w.Manifest, current.Outputs)
			if completionErr != nil {
				return completionErr
			}
			if state != w.Outcome || code != w.Code {
				return store.ErrInvalid
			}
			...
			if readErr == nil {
				readErr = completeRequestTx(ctx, tx, inv)
			}
		...
		out, readErr = frozenReadTx(ctx, tx, e, w.Manifest.ID, true, true)
		return readErr
	})
	if err != nil {
		return reporting.RunRecord{}, err
...
func (d *DB) CancelFrozenRun(ctx context.Context, e identity.Envelope, id string) (out reporting.RunView, err error) {
	...
	defer stop()
	err = d.transaction(ctx, func(ctx context.Context, tx pgx.Tx) error {
		task, readErr := readOwnedRequestTx(ctx, tx, e, id, true)
		if readErr != nil {
			return readErr
		}
		if task.Input.Kind != "reporting.run" {
			return store.ErrNotFound
		}
		if readErr = access.Require(e, "jobs.cancel", access.Resource{Tenant: e.Tenant(), Kind: "block", Permission: "execute", ID: task.Input.Target}); readErr != nil {
			return readErr
		}
		...
		var private bool
		if readErr = tx.QueryRow(ctx, `SELECT context_id,private FROM chartworks.frozen_runs WHERE tenant_id=$1 AND operation_id=$2 AND actor_id=$3 AND session_id=$4`, e.Tenant(), id, e.User(), e.Session()).Scan(&contextID, &private); readErr != nil {
			return readErr
		}
		if readErr = access.Require(e, "jobs.cancel", access.Resource{Tenant: e.Tenant(), Kind: "execution_context", Permission: "use", ID: contextID}); readErr != nil {
			return readErr
		}
		if private {
			if readErr = reporting.Require(e, task.Input.Target, reporting.Preview); readErr != nil {
				return readErr
			}
		}
		if _, readErr = cancelRequestTaskTx(ctx, tx, e, task); readErr != nil {
			return readErr
		}
		if _, readErr = tx.Exec(ctx, `UPDATE chartworks.read_attempts SET cancel_requested=true WHERE tenant_id=$1 AND actor_id=$2 AND operation_id=$3 AND finished_at IS NULL`, e.Tenant(), e.User(), id); readErr != nil {
			return readErr
		}
		if readErr = frozenAudit(ctx, tx, e, "reporting.run_cancelled", id); readErr != nil {
			return readErr
		}
