-- Forward-only grouping continuity: retain all v0-v4 query receipts unchanged.
ALTER TABLE chartworks.nlq_queries
 ...
 ADD CONSTRAINT nlq_queries_analytical_version_check CHECK (analytical_version IN (0,1,2,3,4,5)),
 ADD CONSTRAINT nlq_analytical_shape CHECK (
  (analytical_version=0 AND analytical IS NULL) OR
  (analytical_version IN (1,2,3,4,5) AND (analytical IS NULL OR COALESCE(
   ...
   AND (NOT analytical ? 'query_population' OR
    (analytical_version IN (4,5) AND analytical->>'query_population'='owned-query-predicates-v1'))
   AND (
    ...
    OR
    (analytical_version IN (3,4,5) AND analytical->>'version'=CASE analytical_version WHEN 3 THEN 'analytical-metrics-v3' WHEN 4 THEN 'analytical-metrics-v4' ELSE 'analytical-metrics-v5' END AND (
     (analytical->>'scope'='selected_metric_expression_and_population;single_base_relation' AND NOT analytical ? 'grouping') OR
     (analytical_version=5 AND analytical->>'scope'='selected_metric_expression_population_and_scalar_total;single_base_relation' AND NOT analytical ? 'grouping') OR
