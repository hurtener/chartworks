...
type operationLease struct {
	tenant, id, owner, manifest, mode string
	fence                             int64
}
...
func claimOperationLease(ctx context.Context, tx pgx.Tx, tenant, id, owner string, ttl time.Duration) (fence int64, attempt int, until time.Time, err error) {
	if _, err = tx.Exec(ctx, `UPDATE chartworks.operation_attempts SET state='abandoned',finished_at=clock_timestamp(),error_code='lease_lost' WHERE tenant_id=$1 AND operation_id=$2 AND state='acquiring'`, tenant, id); err != nil {
		return
	}
