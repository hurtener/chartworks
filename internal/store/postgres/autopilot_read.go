...
func proposalReadArgs(e identity.Envelope,id,action string)([]any,error){
	...
	default:return nil,access.ErrForbidden
	}
	if !e.Has(action){return nil,access.ErrForbidden}
	grants,err:=blockGrants(e);if err!=nil{return nil,err}
	permission:="write";if action=="engineering.autopilot.read"{permission="read"}
...
func proposalHardLimits()config.Autopilot{
	l:=config.DefaultAutopilot();l.MaxSteps=16;l.MaxCalls=8;l.MaxTokens=128<<10;l.MaxProposalBytes=1<<20;l.MaxEvidenceBytes=128<<10
	return l
}
...
func proposalTx(ctx context.Context,tx pgx.Tx,e identity.Envelope,id,action string,lock bool)(out engineering.AutopilotProposal,err error){
	args,err:=proposalReadArgs(e,id,action);if err!=nil{return out,err}
 ...
 WHERE `+proposalEligibility
	...
	if json.Unmarshal(material,&out.Material)!=nil||out.Material.Digest()!=out.Digest||out.Material.Request.ID!=out.ID||out.Material.Binding.Tenant!=out.Tenant||engineering.ValidateAutopilotMaterial(out.Material,proposalHardLimits())!=nil{return engineering.AutopilotProposal{},store.ErrInvalid}
	if review!=nil{
		if json.Unmarshal(review,&out.Review)!=nil||out.Review==nil||out.Review.Revision!=out.Revision||out.Review.Digest!=out.Digest{return engineering.AutopilotProposal{},store.ErrInvalid}
		if out.Review.Decision=="approve"&&(out.Review.Actor==out.OriginAuthor||out.Review.Actor==out.Material.Author){return engineering.AutopilotProposal{},store.ErrInvalid}
	}
	if err=engineering.RequireAutopilotMaterial(e,out.Material,action);err!=nil{return engineering.AutopilotProposal{},err}
	...
	if err!=nil{return engineering.AutopilotProposal{},err};defer rows.Close()
	for rows.Next(){
		var raw []byte;var effect engineering.ProposalEffect
		if rows.Scan(&raw)!=nil||json.Unmarshal(raw,&effect)!=nil||!identity.Identifier(effect.Target)||effect.Observed.IsZero(){return engineering.AutopilotProposal{},store.ErrInvalid}
		out.Effects=append(out.Effects,effect)
		if len(out.Effects)>40{return engineering.AutopilotProposal{},store.ErrInvalid}
	}
	if err=rows.Err();err!=nil{return engineering.AutopilotProposal{},err}
	if !e.Valid(){return engineering.AutopilotProposal{},access.ErrUnauthenticated}
	return out,ctx.Err()
}
...
func(d *DB) ReadAutopilotProposal(ctx context.Context,e identity.Envelope,id,action string)(out engineering.AutopilotProposal,err error){
	if _,err=proposalReadArgs(e,id,action);err!=nil{return out,err}
	ctx,stop,err:=requestContext(ctx,e);if err!=nil{return out,err};defer stop()
	err=d.transaction(ctx,func(ctx context.Context,tx pgx.Tx)error{var readErr error;out,readErr=proposalTx(ctx,tx,e,id,action,false);return readErr})
...
func proposalSourceFence(ctx context.Context,tx pgx.Tx,e identity.Envelope,m engineering.ProposalMaterial)error{
	var revision int64;var contextID string
	...
	if revision!=m.Binding.Revision||contextID!=m.Binding.Context{return engineering.ErrProposalDrift}
	return nil
}
...
func proposalApplyTx(ctx context.Context,tx pgx.Tx,e identity.Envelope,v engineering.ProposalApply)(engineering.AutopilotProposal,error){
	p,err:=proposalTx(ctx,tx,e,v.ID,v.Action,true);if err!=nil{return p,err}
	if p.Revision!=v.Revision||p.Digest!=v.Digest||readexec.Hash(p.Material)!=readexec.Hash(v.Material){return engineering.AutopilotProposal{},store.ErrConflict}
	if v.Action!="engineering.autopilot.drift"{
		if p.Review==nil||p.Review.Decision!="approve"||p.Review.Actor==p.OriginAuthor||p.Review.Actor==p.Material.Author{return engineering.AutopilotProposal{},engineering.ErrProposalReview}
		if p.ApplyActor!=""&&(p.ApplyActor!=e.User()||p.ApplySession!=e.Session()){return engineering.AutopilotProposal{},access.ErrForbidden}
	}
	return p,nil
}
