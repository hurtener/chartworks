...
func (d *DB) RetireSchedule(ctx context.Context, scope store.Scope, id string, expected int64) (out jobs.Schedule, err error) {
	...
	err = d.transaction(ctx, func(ctx context.Context, tx pgx.Tx) error {
		...
		if e != nil { return e }
		if current.Revision != expected || current.Retired { return store.ErrConflict }
		if _, e = tx.Exec(ctx, `UPDATE chartworks.job_schedules SET revision=revision+1,enabled=false,retired=true WHERE tenant_id=$1 AND schedule_id=$2`, scope.Tenant(), id); e != nil { return e }
		if e = auditJob(ctx, tx, scope, "schedule.updated", id); e != nil { return e }
		current.Revision++
		current.Enabled, current.Retired = false, true
		out = current
		return nil
	})
	if err != nil { return jobs.Schedule{}, err }
	return out, nil
}
...
func (d *DB) ScheduleHistory(ctx context.Context, scope store.Scope, id string, request jobs.ScheduleHistoryRequest) (out jobs.ScheduleHistory, err error) {
	if !scope.Valid() || !identity.Identifier(id) || !request.Valid() { return out, jobs.ErrInvalid }
	...
	err = d.transaction(ctx, func(ctx context.Context, tx pgx.Tx) error {
		var exists bool
		if err := tx.QueryRow(ctx, `SELECT true FROM chartworks.job_schedules WHERE tenant_id=$1 AND schedule_id=$2 FOR SHARE`, scope.Tenant(), id).Scan(&exists); err != nil { return err }
		if request.Kind == "revisions" { return scheduleRevisionHistory(ctx, tx, scope.Tenant(), id, request, &out) }
		return scheduleOccurrenceHistory(ctx, tx, scope.Tenant(), id, request, &out)
	})
	if err != nil { return jobs.ScheduleHistory{}, err }
	return out, nil
}
...
func scheduleRevisionHistory(ctx context.Context, tx pgx.Tx, tenant, id string, request jobs.ScheduleHistoryRequest, out *jobs.ScheduleHistory) error {
 ...
 WHERE tenant_id=$1 AND schedule_id=$2 AND ($3::bigint=0 OR revision<$3) ORDER BY revision DESC LIMIT $4`, tenant, id, request.BeforeRevision, request.Limit+1)
	...
	for rows.Next() {
		...
		var raw []byte
		if err = rows.Scan(&revision.Revision, &revision.Enabled, &revision.Retired, &raw, &revision.RecordedAt); err != nil { return err }
		if len(raw) > 8192 || json.Unmarshal(raw, &revision.Request) != nil || revision.Request.Validate() != nil { return store.ErrUnavailable }
...
func scheduleOccurrenceHistory(ctx context.Context, tx pgx.Tx, tenant, id string, request jobs.ScheduleHistoryRequest, out *jobs.ScheduleHistory) error {
 ...
 WHERE tenant_id=$1 AND schedule_id=$2 AND ($3::timestamptz IS NULL OR due_at<$3) ORDER BY due_at DESC LIMIT $4`, tenant, id, request.BeforeDue, request.Limit+1)
	...
	for rows.Next() {
		...
		if err = rows.Scan(&occurrence.DueAt, &occurrence.WindowStart, &occurrence.WindowEnd, &occurrence.SkippedThrough, &occurrence.Disposition, &occurrence.JobID); err != nil { return err }
