...
func (d *DB) AdmitRequest(ctx context.Context, e identity.Envelope, key string, input jobs.RequestInput, l jobs.Limits) (out jobs.RequestTask, err error) {
	...
	err = d.transaction(ctx, func(ctx context.Context, tx pgx.Tx) error {
		...
		if err == nil {
			if previous.Input != input || previous.Session != e.Session() {
		...
		var revision int64
		var hours int
		if err = tx.QueryRow(ctx, `SELECT clock_timestamp(),p.current_revision,r.operation_hours FROM chartworks.policies p JOIN chartworks.policy_revisions r ON(r.tenant_id,r.revision)=(p.tenant_id,p.current_revision) WHERE p.tenant_id=$1 FOR SHARE OF p`, e.Tenant()).Scan(&now, &revision, &hours); err != nil {
			return err
		}
		id, err := newID()
		if err != nil {
		...
		return auditJob(ctx, tx, scope, "request.accepted", out.ID)
	})
	if err != nil {
		return jobs.RequestTask{}, err
	}
...
func requestCoordinates(i jobs.Invocation) operationLease {
	l := i.Lease()
	return operationLease{tenant: l.Task.Tenant, id: l.Task.ID, owner: l.Owner, manifest: l.Task.ManifestHash, mode: "request", fence: l.Fence}
}
...
func (d *DB) PulseRequest(ctx context.Context, i jobs.Invocation, renew bool, ttl time.Duration) (state string, err error) {
	...
	err = d.transaction(ctx, func(ctx context.Context, tx pgx.Tx) error {
		...
		if renew {
			return renewOperationLease(ctx, tx, requestCoordinates(i), ttl)
		}
		return nil
	})
	return state, err
...
func (d *DB) FailRequest(ctx context.Context, i jobs.Invocation, code string, permanent bool, delay time.Duration) error {
	if !i.Valid() || delay < 0 || delay > time.Minute {
		return jobs.ErrInvalid
	}
	switch code {
...
func requestFenceTx(ctx context.Context, tx pgx.Tx, i jobs.Invocation) (jobs.RequestTask, error) {
	...
	var live bool
	if err = tx.QueryRow(ctx, `SELECT status='running' AND lease_owner=$3 AND fence=$4 AND lease_until>clock_timestamp() AND expires_at>clock_timestamp() AND manifest_hash=$5 FROM chartworks.operations WHERE tenant_id=$1 AND operation_id=$2`, task.Tenant, task.ID, l.Owner, l.Fence, l.Task.ManifestHash).Scan(&live); err != nil {
		return jobs.RequestTask{}, err
	}
	if !live {
		return jobs.RequestTask{}, store.ErrConflict
...
func completeRequestTx(ctx context.Context, tx pgx.Tx, i jobs.Invocation) error {
	...
	if _, err = tx.Exec(ctx, `UPDATE chartworks.operation_attempts SET state='succeeded',executor_id=$4,finished_at=clock_timestamp() WHERE tenant_id=$1 AND operation_id=$2 AND fence=$3 AND state='acquiring'`, l.Task.Tenant, l.Task.ID, l.Fence, e.User()); err != nil {
		return err
	}
	scope, _ := store.NewScope(e.Tenant(), e.User())
	return auditJob(ctx, tx, scope, "request.succeeded", l.Task.ID)
}
