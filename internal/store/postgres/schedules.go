...
func scanSchedule(row pgx.Row) (jobs.Schedule, error) {
	...
	if json.Unmarshal(raw, &s.Request) != nil || s.Request.Validate() != nil {
...
func (d *DB) CreateSchedule(ctx context.Context, scope store.Scope, session, key string, request jobs.ScheduleRequest, l jobs.Limits) (out jobs.Schedule, err error) {
	...
	err = d.transaction(ctx, func(ctx context.Context, tx pgx.Tx) error {
		...
		var count, total int
		if e = tx.QueryRow(ctx, `SELECT count(*) FILTER(WHERE tenant_id=$1),count(*) FROM chartworks.job_schedules`, scope.Tenant()).Scan(&count, &total); e != nil {
			return e
		}
		if count >= 1000 || total >= 10000 {
			return jobs.ErrBusy
		}
		var now time.Time
...
func (d *DB) ReadSchedule(ctx context.Context, scope store.Scope, id string) (out jobs.Schedule, err error) {
	...
	err = d.transaction(ctx, func(ctx context.Context, tx pgx.Tx) error {
		var e error
		out, e = scanSchedule(tx.QueryRow(ctx, `SELECT `+scheduleColumns+` FROM chartworks.job_schedules WHERE tenant_id=$1 AND schedule_id=$2`, scope.Tenant(), id))
		return e
	})
	return out, err
}
func (d *DB) SetSchedule(ctx context.Context, scope store.Scope, id string, expected int64, enabled bool) (out jobs.Schedule, err error) {
	...
	err = d.transaction(ctx, func(ctx context.Context, tx pgx.Tx) error {
		var e error
		out, e = scanSchedule(tx.QueryRow(ctx, `SELECT `+scheduleColumns+` FROM chartworks.job_schedules WHERE tenant_id=$1 AND schedule_id=$2 FOR UPDATE`, scope.Tenant(), id))
		if e != nil {
			return e
		}
...
func (d *DB) FireSchedule(ctx context.Context, scope store.Scope, session, id, key string, l jobs.Limits) (out jobs.Job, err error) {
	...
	err = d.transaction(ctx, func(ctx context.Context, tx pgx.Tx) error {
		...
		schedule, e := scanSchedule(tx.QueryRow(ctx, `SELECT `+scheduleColumns+` FROM chartworks.job_schedules WHERE tenant_id=$1 AND schedule_id=$2 FOR UPDATE`, scope.Tenant(), id))
		if e != nil {
			return e
		}
		if !schedule.Enabled {
		...
		_, e = tx.Exec(ctx, `INSERT INTO chartworks.job_occurrences(tenant_id,schedule_id,due_at,window_start,window_end,disposition,operation_id) VALUES($1,$2,$3,$4,$3,'queued',$5) ON CONFLICT DO NOTHING`, scope.Tenant(), id, out.DueAt, out.WindowStart, out.ID)
		return e
	})
	return out, err
}
func activeSchedule(ctx context.Context, tx pgx.Tx, tenant, id string) (bool, error) {
	var active bool
	err := tx.QueryRow(ctx, `SELECT EXISTS(SELECT 1 FROM chartworks.operations WHERE tenant_id=$1 AND schedule_id=$2 AND dispatch_mode='queued' AND status IN ('pending','retry','running'))`, tenant, id).Scan(&active)
	return active, err
}
...
func (d *DB) TickSchedules(ctx context.Context, l jobs.Limits) (admitted int, err error) {
	...
	err = d.transaction(ctx, func(ctx context.Context, tx pgx.Tx) error {
		...
		rows, e := tx.Query(ctx, `SELECT `+scheduleColumns+` FROM chartworks.job_schedules WHERE enabled AND next_due<=$1 ORDER BY next_due,schedule_id LIMIT 32 FOR UPDATE SKIP LOCKED`, now)
		if e != nil {
			return e
		}
		list := []jobs.Schedule{}
		...
		for _, s := range list {
			scope, e := store.NewScope(s.Tenant, s.Initiator)
			if e != nil {
				return e
			}
			due, previous := *s.NextDue, *s.PreviousDue
			maximum := 1
			if s.Request.Spec.Missed == "catch_up" {
				maximum = s.Request.Spec.MaxCatchUp
			}
			processed := 0
			for !due.After(now) {
				...
				if s.Request.Spec.Missed == "skip" && now.Sub(due) > 5*time.Second || processed >= maximum {
					last, e := s.Request.Spec.Previous(now)
					if e != nil {
						return e
					}
					next, e := s.Request.Spec.Next(now)
					if e != nil {
						return e
					}
					if e = skippedOccurrence(ctx, tx, s, due, previous, last, last, "missed_skipped"); e != nil {
						return e
					}
				...
				if active && s.Request.Spec.Overlap == "skip" {
					if e = skippedOccurrence(ctx, tx, s, due, previous, due, due, "overlap_skipped"); e != nil {
						return e
					}
				} else {
					...
					if errors.Is(e, jobs.ErrBusy) {
						break
					}
					if e != nil {
						return e
					}
					if _, e = tx.Exec(ctx, `INSERT INTO chartworks.job_occurrences(tenant_id,schedule_id,due_at,window_start,window_end,disposition,operation_id) VALUES($1,$2,$3,$4,$3,'queued',$5) ON CONFLICT DO NOTHING`, s.Tenant, s.ID, due, previous, job.ID); e != nil {
						return e
					}
					admitted++
				}
			...
			if _, e = tx.Exec(ctx, `UPDATE chartworks.job_schedules SET previous_due=$3,next_due=$4 WHERE tenant_id=$1 AND schedule_id=$2`, s.Tenant, s.ID, previous, due); e != nil {
				return e
			}
		}
		return nil
