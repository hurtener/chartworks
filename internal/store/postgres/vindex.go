...
func vectorCoordinates(s store.Scope, g vindex.Generation) error {
	...
	if !g.Valid() { return store.ErrInvalid }
	return nil
}
...
func checkedGeneration(ctx context.Context,tx pgx.Tx,tenant string,g vindex.Generation) (string,error) {
	...
	if err != nil { return "",err }
	if hash != vindex.Digest(g) { return "",store.ErrConflict }
	return state,nil
}
...
func (d *DB) BeginGeneration(ctx context.Context,s store.Scope,g vindex.Generation) error {
	if err := vectorCoordinates(s,g); err != nil { return err }
	return d.transaction(ctx,func(ctx context.Context,tx pgx.Tx) error {
		if err := vectorHeadLock(ctx,tx,s.Tenant(),g.Topic,g.Context); err != nil { return err }
...
func (d *DB) UpsertFacets(ctx context.Context,s store.Scope,g vindex.Generation,batch []vindex.Facet) error {
	if err := vectorCoordinates(s,g); err != nil { return err }
	if err := vindex.CheckBatch(g,batch); err != nil { return err }
	return d.transaction(ctx,func(ctx context.Context,tx pgx.Tx) error {
		if err := vectorHeadLock(ctx,tx,s.Tenant(),g.Topic,g.Context); err != nil { return err }
		state,err := checkedGeneration(ctx,tx,s.Tenant(),g)
		if err != nil { return err }
		if state != "staging" { return store.ErrConflict }
		for _,f := range batch {
...
func (d *DB) PublishGeneration(ctx context.Context,s store.Scope,g vindex.Generation,expected int64) (out vindex.Publication,err error) {
	if err = vectorCoordinates(s,g); err != nil { return out,err }
	if expected < 0 || expected >= 1<<62 { return out,store.ErrInvalid }
	err = d.transaction(ctx,func(ctx context.Context,tx pgx.Tx) error {
		if e := vectorHeadLock(ctx,tx,s.Tenant(),g.Topic,g.Context); e != nil { return e }
		if _,e := checkedGeneration(ctx,tx,s.Tenant(),g); e != nil { return e }
		var count,missing int
		if e := tx.QueryRow(ctx,`SELECT count(*) FROM chartworks.vector_facets WHERE tenant_id=$1 AND topic_id=$2 AND context_id=$3 AND generation_id=$4`,s.Tenant(),g.Topic,g.Context,g.ID).Scan(&count); e != nil { return e }
		if e := tx.QueryRow(ctx,`SELECT count(*) FROM chartworks.vector_generations g CROSS JOIN LATERAL jsonb_to_recordset(g.manifest) AS m(id text,kind text,source_id text,text_hash text) LEFT JOIN chartworks.vector_facets f ON (f.tenant_id,f.topic_id,f.context_id,f.generation_id,f.facet_id)=(g.tenant_id,g.topic_id,g.context_id,g.generation_id,m.id) WHERE g.tenant_id=$1 AND g.topic_id=$2 AND g.context_id=$3 AND g.generation_id=$4 AND (f.facet_id IS NULL OR f.kind<>m.kind OR f.source_id<>m.source_id OR encode(sha256(convert_to(f.body,'UTF8')),'hex')<>m.text_hash)`,s.Tenant(),g.Topic,g.Context,g.ID).Scan(&missing); e != nil { return e }
		if count != len(g.Expected) || missing != 0 { return store.ErrConflict }
		if _,e := tx.Exec(ctx,`UPDATE chartworks.vector_generations SET state='ready' WHERE tenant_id=$1 AND topic_id=$2 AND context_id=$3 AND generation_id=$4`,s.Tenant(),g.Topic,g.Context,g.ID); e != nil { return e }
		...
		if e != nil { return e }
		if tag.RowsAffected()!=1 { return store.ErrConflict }
	...
	if err != nil { return vindex.Publication{},err }
	return out,nil
}
...
func (d *DB) ArchiveGeneration(ctx context.Context,s store.Scope,topic,partition string,expected int64) (out vindex.Publication,err error) {
	if !s.Valid() { return out,store.ErrScope }
	if !identity.Identifier(topic) || !identity.Identifier(partition) || expected < 1 || expected >= 1<<62 { return out,store.ErrInvalid }
	err = d.transaction(ctx,func(ctx context.Context,tx pgx.Tx) error {
...
func vectorPublication(ctx context.Context,tx pgx.Tx,tenant string,q vindex.Query) (p vindex.Publication,source string,err error) {
	...
	if errors.Is(err,pgx.ErrNoRows) { return vindex.Publication{},"",nil }
	if err != nil { return p,"",err }
	if p.Archived || p.Generation=="" { return p,source,nil }
	if state!="ready" || key!=q.Space.Key() || dimensions!=q.Space.Dimensions { return vindex.Publication{},"",store.ErrConflict }
	return p,source,nil
}
...
func (d *DB) SearchFacets(ctx context.Context,s store.Scope,queries []vindex.Query) (out []vindex.Result,err error) {
	if !s.Valid() { return nil,store.ErrScope }
	if len(queries)<1 || len(queries)>8 { return nil,store.ErrInvalid }
	for _,q := range queries { if !q.Valid() { return nil,store.ErrInvalid } }
	...
	err = d.transactionOptions(ctx,pgx.TxOptions{IsoLevel:pgx.RepeatableRead,AccessMode:pgx.ReadOnly},func(ctx context.Context,tx pgx.Tx) error {
		...
		for _,q := range queries {
			...
			if p.Archived || p.Generation=="" { out=append(out,r); continue }
			...
			for rows.Next() {
				...
				if e=rows.Scan(&h.ID,&h.Kind,&h.SourceID,&h.Text,&h.Distance); e!=nil { rows.Close(); return e }
				bytes += len(h.Text)+len(h.ID)+len(h.SourceID)+len(h.Generation)+len(h.Version)+len(h.SourceGeneration)+128
				if bytes>2<<20 { rows.Close(); return store.ErrInvalid }
...
func (d *DB) DeleteFacets(ctx context.Context,s store.Scope,r vindex.Removal) error {
	if !s.Valid() { return store.ErrScope }
	if r.Topic=="" && (r.Context!="" || r.Version!="") || r.Topic!="" && (!identity.Identifier(r.Topic) || !identity.Identifier(r.Context)) || r.Version!="" && !identity.Identifier(r.Version) { return store.ErrInvalid }
	return d.transaction(ctx,func(ctx context.Context,tx pgx.Tx) error {
		...
		if err!=nil { return err }
		for rows.Next() { var id string; if err=rows.Scan(&id); err!=nil { rows.Close(); return err } }
		...
		if err!=nil { return err }
		target:=r.Topic; if target=="" { target=s.Tenant() }
...
func (d *DB) ExplainFacets(ctx context.Context,s store.Scope,q vindex.Query) (out json.RawMessage,err error) {
	if !s.Valid() { return nil,store.ErrScope }
	if !q.Valid() { return nil,store.ErrInvalid }
	err=d.transactionOptions(ctx,pgx.TxOptions{IsoLevel:pgx.RepeatableRead,AccessMode:pgx.ReadOnly},func(ctx context.Context,tx pgx.Tx) error {
		p,_,e:=vectorPublication(ctx,tx,s.Tenant(),q); if e!=nil { return e }
		if p.Archived || p.Generation=="" { return store.ErrNotFound }
		vector,e:=vindex.VectorLiteral(q.Vector,q.Space.Dimensions); if e!=nil { return e }
