...
func frozenArgs(e identity.Envelope, id string, execution bool) ([]any, error) {
	if !e.Valid() { return nil, access.ErrUnauthenticated }
	if !identity.Identifier(id) { return nil, store.ErrInvalid }
	action := "reporting.read"
	if execution { action = "reporting.execute" }
	if !e.Has(action) { return nil, access.ErrForbidden }
...
func scanFrozenHead(row pgx.Row) (h frozenHead, err error) {
	var reused *string
	v := &h.view
	err = row.Scan(&v.ID,&v.Block,&v.Revision,&v.RevisionDigest,&v.ManifestDigest,&v.State,&v.Code,&v.Private,&v.Source,&v.Context,&v.PartitionDigest,&v.Locale,&v.Timezone,&v.Created,&v.Expires,&v.Observed,&v.Finished,&reused,&v.RetainedBytes,&v.ReservedCalls,&v.ReservedTokens,&v.FrozenVersion,&h.actor,&h.session,&h.requestHash,&h.taskHash,&h.reuseKey,&h.reserved,&h.maximum,&h.operationState,&v.Attempts,&h.published)
	if reused != nil { v.ReusedFrom = *reused }
...
func frozenAttemptsTx(ctx context.Context, tx pgx.Tx, tenant string, h frozenHead) ([]readexec.Attempt, error) {
	...
	if err != nil { return nil, err }
	for rows.Next() {
		var raw []byte; var a readexec.Attempt
		if rows.Scan(&raw) != nil || json.Unmarshal(raw,&a) != nil || !frozenAttemptMatches(h,a) { rows.Close(); return nil, store.ErrInvalid }
		byNumber[a.Number] = a
	}
	...
	for rows.Next() {
		...
		if !frozenAttemptMatches(h,a) { return nil, store.ErrInvalid }
		if old, found := byNumber[a.Number]; found && (old.ID != a.ID || readexec.Hash(old.Manifest) != readexec.Hash(a.Manifest)) { return nil, store.ErrInvalid }
		byNumber[a.Number]=a
	}
	if err=rows.Err(); err != nil { return nil, err }
	out:=make([]readexec.Attempt,0,len(byNumber))
	for _,a:=range byNumber { out=append(out,a) }
...
func frozenValuesTx(ctx context.Context, tx pgx.Tx, e identity.Envelope, h frozenHead, out *reporting.RunRecord) error {
	var manifest, result []byte; var resultHash *string
	if err:=tx.QueryRow(ctx,`SELECT manifest,result,result_digest FROM chartworks.frozen_run_payloads WHERE tenant_id=$1 AND operation_id=$2`,e.Tenant(),h.view.ID).Scan(&manifest,&result,&resultHash);err!=nil{return err}
	...
	if json.Unmarshal(manifest,&m)!=nil || m.Digest()!=h.view.ManifestDigest || m.Tenant!=e.Tenant() || m.ID!=h.view.ID || m.Actor!=h.actor || m.Session!=h.session ||
		...
		!m.Created.Equal(h.view.Created) || !m.Expires.Equal(h.view.Expires) || m.Limits.Validate()!=nil {return store.ErrInvalid}
	out.Manifest=&m
	out.View.Parameters=append([]reporting.BoundValue{},m.Resolved.Values...)
	out.View.Trust=m.Trust
	if result!=nil {
		var r readexec.Result
		if json.Unmarshal(result,&r)!=nil || resultHash==nil || readexec.Hash(r)!=*resultHash || len(r.Rows)>m.Limits.MaxRows || len(result)>m.Limits.MaxResultBytes {return store.ErrInvalid}
		out.Result=&r
	}
	...
	for rows.Next(){
		var raw []byte;var output reporting.RetainedOutput
		if rows.Scan(&raw)!=nil || json.Unmarshal(raw,&output)!=nil || reporting.CheckFrozenOutput(m,output,output.State=="indeterminate")!=nil{return store.ErrInvalid}
...
func frozenReadTx(ctx context.Context, tx pgx.Tx, e identity.Envelope, id string, execution, values bool) (reporting.RunRecord,error) {
	...
	if !execution {if err=access.RequireArtifact(e,out.Reach);err!=nil{return reporting.RunRecord{},err}}
	...
	if h.view.State=="sealed" || h.view.State=="normalized" {out.View.State=h.operationState}
	if values {
		if err=frozenValuesTx(ctx,tx,e,h,&out);err!=nil{return reporting.RunRecord{},err}
		if execution {if err=reporting.RequireRunManifest(e,*out.Manifest);err!=nil{return reporting.RunRecord{},err}}
	}
	if !e.Valid(){return reporting.RunRecord{},access.ErrUnauthenticated}
	return out,ctx.Err()
}
...
func (d *DB) ReadFrozenRun(ctx context.Context,e identity.Envelope,id string,execution bool)(out reporting.RunRecord,err error){
	if _,err=frozenArgs(e,id,execution);err!=nil{return out,err}
	ctx,stop,err:=requestContext(ctx,e);if err!=nil{return out,err};defer stop()
	err=d.transaction(ctx,func(ctx context.Context,tx pgx.Tx)error{var readErr error;out,readErr=frozenReadTx(ctx,tx,e,id,execution,true);return readErr})
...
func(d *DB) ListFrozenArtifacts(ctx context.Context,e identity.Envelope,after string,limit int)(out reporting.ArtifactList,err error){
	...
	if limit<1||limit>100||after!=""&&!identity.Identifier(after){return out,store.ErrInvalid}
	args,err:=frozenArgs(e,"list",false);if err!=nil{return out,err};args[1]=after
	ctx,stop,err:=requestContext(ctx,e);if err!=nil{return out,err};defer stop()
	err=d.transaction(ctx,func(ctx context.Context,tx pgx.Tx)error{
		...
		ids:=[]string{}
		for rows.Next(){var id string;if queryErr=rows.Scan(&id);queryErr!=nil{rows.Close();return queryErr};ids=append(ids,id)}
		queryErr=rows.Err();rows.Close();if queryErr!=nil{return queryErr}
		if len(ids)>limit{ids=ids[:limit];out.Next=ids[len(ids)-1]}
		for _,id:=range ids{r,readErr:=frozenReadTx(ctx,tx,e,id,false,false);if readErr!=nil{return readErr};out.Items=append(out.Items,r.View)}
		return nil
	})
	if err!=nil{return reporting.ArtifactList{},err};return out,nil
}
