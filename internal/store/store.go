...
func NewScope(tenant,actor string)(Scope,error) {
	if !Identifier(tenant)||!Identifier(actor) {return Scope{},ErrScope};return Scope{tenant:tenant,actor:actor},nil
}
...
func Identifier(s string) bool {
	...
	for _,c:=range s {if !((c>='a'&&c<='z')||(c>='A'&&c<='Z')||(c>='0'&&c<='9')||c=='_'||c=='-'||c==':'||c=='.'){return false}}
	return true
}
...
func (p Policy) Validate() error {
	if p.AuditDays<1||p.AuditDays>3650||p.OperationHours<1||p.OperationHours>8760 {return ErrInvalid};return nil
}
...
type Audit struct { ID, Actor, Action, Resource string; CreatedAt time.Time }
// Audits provides tenant-scoped, bounded discovery without arbitrary SQL.
type Audits interface { Audits(context.Context,Scope,int)([]Audit,error) }
...
type Lease struct { OperationID,Owner string; Fence int64 }
...
type Foundation interface { Policies; Audits; Retention }
