...
func (s *Service) ObserveCatalog(ctx context.Context,e identity.Envelope,id,partition string)(out CatalogObservation,err error){
	if _,err=s.ContextBinding(ctx,e,id,partition);err!=nil{return out,err}
	...
	err=s.call(ctx,e,true,func(ctx context.Context)error{
		return s.repo.WithSource(ctx,scope,id,func(ctx context.Context,record Record)error{
			...
			connection,err:=s.recordConnection(record);if err!=nil{return err}
			if record.Source.Dialect!="postgres"{
				binding,err:=s.probe(ctx,connection,id,record.Source.Revision,nil);if err!=nil{return err}
				if !observedBindingMatches(record.Binding,binding){return readexec.ErrBinding}
	...
	if err!=nil{return CatalogObservation{},err}
	if !e.Valid(){return CatalogObservation{},access.ErrUnauthenticated}
	return out,nil
}
...
func postgresCatalogIdentity(ctx context.Context,tx readTransaction,c config.SourceConnection,binding readexec.Binding)(CatalogIdentity,error){
	...
	if err:=tx.QueryRow(ctx,`SELECT COALESCE(inet_server_addr()::text,''),COALESCE(inet_server_port(),0),current_database(),d.oid::bigint,current_user,r.oid::bigint FROM pg_catalog.pg_database d CROSS JOIN pg_catalog.pg_roles r WHERE d.datname=current_database() AND r.rolname=current_user`).Scan(&address,&port,&database,&databaseOID,&user,&roleOID);err!=nil{return CatalogIdentity{},safe(err)}
	...
	for _,relation:=range binding.Relations{
		...
		var oid int64
		if err:=tx.QueryRow(ctx,`SELECT c.oid::bigint FROM pg_catalog.pg_class c JOIN pg_catalog.pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname=$1 AND c.relname=$2`,relation.Schema,relation.Name).Scan(&oid);err!=nil{return CatalogIdentity{},safe(err)}
		item.Object=readexec.Hash([]any{out.Authority,oid})
		names:=make([]string,len(relation.Columns));for i,column:=range relation.Columns{names[i]=column.Name}
		...
		if err!=nil{return CatalogIdentity{},safe(err)}
		for rows.Next(){var name string;var number int;if err:=rows.Scan(&name,&number);err!=nil{rows.Close();return CatalogIdentity{},safe(err)};item.Columns=append(item.Columns,ColumnIdentity{Name:name,Object:readexec.Hash([]any{item.Object,number})})}
		err=rows.Err();rows.Close();if err!=nil{return CatalogIdentity{},safe(err)}
		if len(item.Columns)!=len(relation.Columns){return CatalogIdentity{},readexec.ErrBinding}
