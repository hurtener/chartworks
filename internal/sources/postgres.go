...
func (s *Service) pool(ctx context.Context,c config.SourceConnection) (*pgxpool.Pool,string,error) {
	...
	if !ok || len(dsn)>16384 { return nil,"",store.ErrUnavailable }
	u,err:=url.Parse(dsn)
	...
	if !has || password=="" || len(password)>8192 { return nil,"",store.ErrInvalid }
	...
	s.mu.Lock()
	if entry,ok:=s.pools[key]; ok && entry.material==material { s.mu.Unlock(); return entry.pool,u.Host+u.EscapedPath(),nil }
	cfg,err:=pgxpool.ParseConfig(dsn)
	if err!=nil { s.mu.Unlock(); return nil,"",store.ErrInvalid }
	...
	cfg.ConnConfig.BuildFrontend=func(reader io.Reader,writer io.Writer) *pgproto3.Frontend { frontend:=pgproto3.NewFrontend(reader,writer); frontend.SetMaxBodyLen(maximum); return frontend }
	pool,err:=pgxpool.NewWithConfig(ctx,cfg)
	if err!=nil { s.mu.Unlock(); return nil,"",safe(err) }
	old:=s.pools[key]
	s.pools[key]=poolEntry{material:material,pool:pool}
	s.mu.Unlock()
	if old.pool!=nil { old.pool.Close() }
...
func (s *Service) probe(ctx context.Context,c config.SourceConnection,id string,revision int64,consume func(context.Context,readTransaction,readexec.Binding) error) (out readexec.Binding,err error) {
	...
	ctx,cancel:=context.WithTimeout(ctx,time.Duration(s.settings.QueryTimeout)); defer cancel()
	tx,err:=pool.BeginTx(ctx,pgx.TxOptions{IsoLevel:pgx.ReadCommitted,AccessMode:pgx.ReadOnly})
	if err!=nil { return out,safe(err) }
	defer func(){ cleanup,stop:=context.WithTimeout(context.WithoutCancel(ctx),time.Second); defer stop(); _=tx.Rollback(cleanup) }()
	if _,err=tx.Exec(ctx,`SELECT set_config('search_path','pg_catalog',true),set_config('row_security','on',true),set_config('statement_timeout',$1,true),set_config('lock_timeout',$1,true)`,strconv.FormatInt(time.Duration(s.settings.QueryTimeout).Milliseconds(),10)); err!=nil { return out,safe(err) }
	relations:=append([]config.SourceRelation(nil),c.Relations...)
	sort.Slice(relations,func(i,j int) bool { return relations[i].Schema+"."+relations[i].Name<relations[j].Schema+"."+relations[j].Name })
	for _,relation:=range relations {
		if _,err=tx.Exec(ctx,"LOCK TABLE ONLY "+pgx.Identifier{relation.Schema,relation.Name}.Sanitize()+" IN ACCESS SHARE MODE"); err!=nil { return out,safe(err) }
	}
	...
	if err=tx.QueryRow(ctx,`SELECT current_user,session_user,r.oid::bigint,r.rolsuper,r.rolbypassrls,r.rolcreaterole,r.rolcreatedb,current_setting('transaction_read_only') FROM pg_catalog.pg_roles r WHERE r.rolname=current_user`).Scan(&evidence.User,&session,&evidence.RoleOID,&super,&bypass,&createRole,&createDB,&readonly); err!=nil { return out,safe(err) }
	if evidence.User!=session || super || bypass || createRole || createDB || readonly!="on" { return out,readexec.ErrUnsafe }
	...
	for _,relation:=range relations {
		...
		if e!=nil { return readexec.Binding{},e }
	...
	if !out.Valid() { return readexec.Binding{},readexec.ErrBinding }
	if consume!=nil { if err=consume(ctx,tx,out.Clone()); err!=nil { return readexec.Binding{},safe(err) } }
	if err=tx.Commit(ctx); err!=nil { return readexec.Binding{},safe(err) }
	return out,nil
}
...
func discoverRelation(ctx context.Context,tx readTransaction,relation config.SourceRelation) (out tableEvidence,columns []readexec.Column,err error) {
	...
	if err!=nil { return out,nil,safe(err) }
	...
	if kind!="r" || method!="heap" || persistence=="t" || rls || forceRLS || partition || children { return out,nil,readexec.ErrUnsupported }
	if !selectable || writable || columnWritable { return out,nil,readexec.ErrUnsafe }
	...
	for rows.Next() {
		...
		if err=rows.Scan(&proof.Name,&native,&namespace,&proof.Native,&proof.OID,&proof.Modifier,&proof.Nullable,&kind,&base,&baseNamespace); err!=nil { return out,nil,safe(err) }
		proof.Base=baseNamespace+"."+base
		proof.Safe=namespace=="pg_catalog" && kind=="b" && safeOID(proof.OID)
		category:=Classify(native)
		if kind=="d" && baseNamespace=="pg_catalog" { category=Classify(base) }
	...
	if err=rows.Err(); err!=nil { return out,nil,safe(err) }
	if len(columns)!=len(relation.Columns) { return out,nil,readexec.ErrBinding }
	return out,columns,nil
}
func safeOID(oid int64) bool {
	switch oid { case 16,17,20,21,23,25,114,700,701,790,1042,1043,1082,1083,1114,1184,1186,1266,1700,2950,3802: return true }; return false
}

// Classify is a lossless category label, not authorization to execute a custom type.
func Classify(native string) string {
	switch native {
	case "int2","int4","int8","numeric","decimal","float4","float8","money","smallint","integer","bigint","real","double precision": return "numeric"
	case "date","timestamp","timestamptz","time","timetz","interval": return "temporal"
	case "text","varchar","bpchar","char","name","uuid": return "text"
	case "bool","boolean": return "boolean"
	case "json","jsonb","xml": return "structured"
...
func arguments(parameters []readexec.Parameter) ([]any,error) {
	...
	for _,p:=range parameters {
		if !p.Valid() { return nil,readexec.ErrBinding }
		switch p.Kind {
		case "null": out=append(out,nil)
		case "text": out=append(out,p.Value)
		case "boolean": out=append(out,p.Value=="true")
		case "integer": n,_:=strconv.ParseInt(p.Value,10,64); out=append(out,n)
		case "number": var n pgtype.Numeric; if err:=n.Scan(p.Value); err!=nil { return nil,readexec.ErrBinding }; out=append(out,n)
		}
	}
	return out,nil
}
func explain(ctx context.Context,tx readTransaction,statement string,parameters []readexec.Parameter) error {
	args,err:=arguments(parameters); if err!=nil { return err }
	var plan json.RawMessage
	// The candidate has already passed the positive grammar and dependency checks.
	if err=tx.QueryRow(ctx,"EXPLAIN (FORMAT JSON) "+statement,args...).Scan(&plan); err!=nil { return safe(err) }
	if !json.Valid(plan) || len(plan)>1<<20 { return readexec.ErrUnsafe }
	return nil
}
func readRows(ctx context.Context,tx readTransaction,statement string,parameters []readexec.Parameter,maxRows,maxBytes int) (out Rows,err error) {
	...
	rows,err:=tx.Query(ctx,statement,args...); if err!=nil { return out,safe(err) }
	defer rows.Close()
	bytes:=0
	for _,column:=range rows.FieldDescriptions() { out.Columns=append(out.Columns,column.Name); bytes+=len(column.Name)+16 }
	out.Values=[][]*string{}
	for rows.Next() {
		if len(out.Values)>=maxRows { cancel(); return Rows{},readexec.ErrLimit }
		values:=rows.RawValues()
		for _,value:=range values { bytes+=len(value)+16 }
		if bytes>maxBytes { cancel(); return Rows{},readexec.ErrLimit }
		row:=make([]*string,len(values))
		for i,value:=range values { if value!=nil { text:=string(value); row[i]=&text } }
		out.Values=append(out.Values,row)
	}
	if err=rows.Err(); err!=nil { return Rows{},safe(err) }
	return out,nil
}
