...
type poolEntry struct { material string; pool *pgxpool.Pool }
...
func New(repo Repository,settings config.Sources,lookup func(string)(string,bool)) (*Service,error) {
	if repo==nil || reflect.ValueOf(repo).Kind()==reflect.Ptr && reflect.ValueOf(repo).IsNil() || lookup==nil || config.ValidateSources(settings)!=nil { return nil,store.ErrInvalid }
...
func (s *Service) call(ctx context.Context,e identity.Envelope,warehouse bool,fn func(context.Context) error) error {
	...
	if s.closed || warehouse && !s.settings.Enabled { return store.ErrUnavailable }
	ctx,cancel:=context.WithDeadline(ctx,e.Deadline()); defer cancel()
	ctx,bound:=context.WithTimeout(ctx,4*time.Second); defer bound()
	return fn(ctx)
}
...
func sourceScope(e identity.Envelope,action,permission,id string) (store.Scope,error) {
	if err:=access.Require(e,action,access.Resource{Tenant:e.Tenant(),Kind:"source",Permission:permission,ID:id}); err!=nil { return store.Scope{},err }
...
func (s *Service) connection(tenant,id string) (config.SourceConnection,error) {
	for _,c:=range s.settings.Connections { if c.Tenant==tenant && c.ID==id { return c,nil } }
...
func (s *Service) Create(ctx context.Context,e identity.Envelope,r CreateRequest) (out Source,err error) {
	scope,err:=access.StoreScope(e,"sources.write","write"); if err!=nil { return out,err }
	...
	connection,err:=s.connection(e.Tenant(),r.Connection); if err!=nil { return out,err }
	err=s.call(ctx,e,true,func(ctx context.Context) error {
		binding,e2:=s.probe(ctx,connection,r.ID,1,nil); if e2!=nil { return e2 }
		...
		if e2=s.repo.PutSource(ctx,scope,0,record); e2!=nil { return e2 }
		out=record.Source; return nil
	})
	return out,err
}
...
func (s *Service) Get(ctx context.Context,e identity.Envelope,id string) (out Source,err error) {
	scope,err:=sourceScope(e,"sources.read","read",id); if err!=nil { return out,err }
	err=s.call(ctx,e,false,func(ctx context.Context) error { record,e2:=s.repo.ReadSource(ctx,scope,id); if e2!=nil { return e2 }; out=record.Source; return nil })
	return out,err
}
...
func (s *Service) List(ctx context.Context,e identity.Envelope,limit int) (out []Source,err error) {
	selection,err:=access.Constrain(e,"sources.read","source","read"); if err!=nil { return nil,err }
	if limit<1 || limit>100 { return nil,store.ErrInvalid }
	scope,err:=store.NewScope(e.Tenant(),e.User()); if err!=nil { return nil,err }
	err=s.call(ctx,e,false,func(ctx context.Context) error { var e2 error; out,e2=s.repo.ListSources(ctx,scope,selection,limit); return e2 })
...
func (s *Service) Rotate(ctx context.Context,e identity.Envelope,id string,expected int64) (out Source,err error) {
	scope,err:=sourceScope(e,"sources.rotate","write",id); if err!=nil { return out,err }
	if expected<1 || expected>=1<<62-1 { return out,store.ErrInvalid }
	err=s.call(ctx,e,true,func(ctx context.Context) error {
		old,e2:=s.repo.ReadSource(ctx,scope,id); if e2!=nil { return e2 }
		if old.Source.Revision!=expected { return store.ErrConflict }
		if e2=actualContext(e,"sources.rotate",old); e2!=nil { return e2 }
		connection,e2:=s.connection(e.Tenant(),old.Connection); if e2!=nil { return e2 }
		binding,e2:=s.probe(ctx,connection,id,expected+1,nil); if e2!=nil { return e2 }
		next:=old; next.Source.Revision++; next.Source.ContextID=contextID(id,next.Source.Revision); next.Binding=binding
		if e2=s.repo.PutSource(ctx,scope,expected,next); e2!=nil { return e2 }
		out=next.Source; return nil
	})
	return out,err
}

func (s *Service) observed(ctx context.Context,e identity.Envelope,id string,fn func(Record,readexec.Binding) error) error {
	scope,err:=sourceScope(e,"sources.read","read",id); if err!=nil { return err }
	return s.call(ctx,e,true,func(ctx context.Context) error {
		return s.repo.WithSource(ctx,scope,id,func(ctx context.Context,record Record) error {
			if err:=actualContext(e,"sources.read",record); err!=nil { return err }
			connection,err:=s.connection(e.Tenant(),record.Connection); if err!=nil { return err }
			binding,err:=s.probe(ctx,connection,id,record.Source.Revision,nil); if err!=nil { return err }
			if readexec.Hash(binding)!=readexec.Hash(record.Binding) { return readexec.ErrBinding }
...
func (s *Service) Test(ctx context.Context,e identity.Envelope,id string) (out Status,err error) {
	err=s.observed(ctx,e,id,func(record Record,_ readexec.Binding) error { out=Status{Available:true,Revision:record.Source.Revision,ContextID:record.Source.ContextID,ObservedAt:time.Now().UTC()}; return nil })
	return out,err
}
...
func (s *Service) Discover(ctx context.Context,e identity.Envelope,id string) (out Discovery,err error) {
	err=s.observed(ctx,e,id,func(record Record,b readexec.Binding) error { out=Discovery{SourceID:id,ContextID:record.Source.ContextID,Revision:record.Source.Revision,Relations:b.Clone().Relations}; return nil })
	return out,err
}
...
func (s *Service) Binding(ctx context.Context,e identity.Envelope,id,partition string) (out readexec.Binding,err error) {
	scope,err:=sourceScope(e,"sources.query","query",id); if err!=nil { return out,err }
	if err=access.Require(e,"sources.query",access.Resource{Tenant:e.Tenant(),Kind:"execution_context",Permission:"use",ID:partition}); err!=nil { return out,err }
	err=s.call(ctx,e,true,func(ctx context.Context) error {
		record,e2:=s.repo.ReadSource(ctx,scope,id); if e2!=nil { return e2 }
		if record.Source.ContextID!=partition { return readexec.ErrBinding }
		out=record.Binding.Clone(); return nil
	})
	return out,err
}
...
func (s *Service) Explain(ctx context.Context,e identity.Envelope,candidate readexec.Candidate) error {
	...
	scope,err:=sourceScope(e,"sources.query","query",id); if err!=nil { return err }
	return s.call(ctx,e,true,func(ctx context.Context) error {
		return s.repo.WithSource(ctx,scope,id,func(ctx context.Context,record Record) error {
			if record.Source.ContextID!=partition { return readexec.ErrBinding }
			if _,_,err:=candidate.SQL(e,record.Binding); err!=nil { return err }
			connection,err:=s.connection(e.Tenant(),record.Connection); if err!=nil { return err }
			_,err=s.probe(ctx,connection,id,record.Source.Revision,func(ctx context.Context,tx readTransaction,b readexec.Binding) error {
				statement,parameters,err:=candidate.SQL(e,b); if err!=nil { return err }
...
func (s *Service) Read(ctx context.Context,e identity.Envelope,plan readexec.Plan) (out Rows,err error) {
	...
	if id=="" || partition=="" { return out,readexec.ErrBinding }
	scope,err:=sourceScope(e,"sources.query","query",id); if err!=nil { return out,err }
	err=s.call(ctx,e,true,func(ctx context.Context) error {
		return s.repo.WithSource(ctx,scope,id,func(ctx context.Context,record Record) error {
			if record.Source.ContextID!=partition { return readexec.ErrBinding }
			if _,_,err:=plan.SQL(e,record.Binding); err!=nil { return err }
			connection,err:=s.connection(e.Tenant(),record.Connection); if err!=nil { return err }
			_,err=s.probe(ctx,connection,id,record.Source.Revision,func(ctx context.Context,tx readTransaction,b readexec.Binding) error {
				statement,parameters,err:=plan.SQL(e,b); if err!=nil { return err }
	...
	if err!=nil { return Rows{},err }; return out,nil
}

func safe(err error) error {
	if err==nil { return nil }
	for _,known:=range []error{context.Canceled,context.DeadlineExceeded,readexec.ErrUnsafe,readexec.ErrUnsupported,readexec.ErrBinding,readexec.ErrLimit,store.ErrInvalid,store.ErrNotFound,store.ErrConflict,store.ErrUnavailable,access.ErrUnauthenticated,access.ErrForbidden,access.ErrNotFound} { if errors.Is(err,known) { return known } }
