...
func TestAssemblerPreservesMandatoryLanesAndDetachesInput(t *testing.T) {
	...
	if assembled.Question == input.Question || assembled.Unpruned.Question == input.Question {
		t.Fatal("assembled question aliases caller input")
	}
	if assembled.Constraints.Required[0].Text == input.Constraints.Required[0].Text || assembled.Unpruned.Constraints.Required[0].Text == input.Constraints.Required[0].Text {
		t.Fatal("assembled constraints alias caller input")
	}
	if assembled.Evidence[0].Text == input.Evidence[0].Text || assembled.Unpruned.Evidence[0].Text == input.Evidence[0].Text {
...
func TestAssemblerCapsOmissionDetailsAndExamples(t *testing.T) {
	...
	if assembled.OmittedCount < len(input.Examples)-MaxExamples {
		t.Fatalf("omitted count %d did not include capped examples", assembled.OmittedCount)
	}
	if len(assembled.Omitted) > MaxOmissions {
		t.Fatalf("recorded %d omissions, max is %d", len(assembled.Omitted), MaxOmissions)
	}
}
