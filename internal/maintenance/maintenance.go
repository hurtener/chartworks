...
type Service struct { store store.Foundation;batch int;lease time.Duration }
...
func New(repository store.Foundation,batch int,lease time.Duration)(*Service,error){
	if repository==nil||batch<1||batch>1000||lease<time.Second||lease>5*time.Minute{return nil,store.ErrInvalid};return &Service{store:repository,batch:batch,lease:lease},nil
}
// Configure is a business-setting change with CAS, not an access-policy decision.
func (s *Service) Configure(ctx context.Context,scope store.Scope,expected int64,p store.Policy)(store.Policy,error){return s.store.SetPolicy(ctx,scope,expected,p)}
...
func (s *Service) Sweep(ctx context.Context,scope store.Scope,key string)(store.Operation,error){
	...
	op,err:=s.store.ReserveSweep(ctx,scope,key,hex.EncodeToString(h[:]),s.batch);if err!=nil{return store.Operation{},err}
	if op.Status=="succeeded"{return op,nil}
	var owner [16]byte
	if _,err=rand.Read(owner[:]);err!=nil{return store.Operation{},errors.New("maintenance: randomness unavailable")}
	lease,err:=s.store.Claim(ctx,scope,op.ID,hex.EncodeToString(owner[:]),s.lease);if err!=nil{return store.Operation{},err}
