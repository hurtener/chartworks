...
func (g Generation) Valid() bool {
	for _, s := range []string{g.ID, g.Topic, g.Version, g.Context, g.SourceGeneration} {
		if !identity.Identifier(s) { return false }
	}
	if !g.Space.Valid() || len(g.Expected) < 1 || len(g.Expected) > 4096 { return false }
	seen := map[string]bool{}
	for _, o := range g.Expected {
		b, err := hex.DecodeString(o.TextHash)
		if !identity.Identifier(o.ID) || !identity.Identifier(o.SourceID) || !Kind(o.Kind) || err != nil || len(b) != 32 || strings.ToLower(o.TextHash) != o.TextHash || seen[o.ID] { return false }
		seen[o.ID] = true
	}
	return true
}
...
func VectorLiteral(v []float32, dimensions int) (string, error) {
	...
	b.WriteByte(']')
	if !nonzero { return "", store.ErrInvalid }
	return b.String(), nil
}
...
func CheckBatch(g Generation, facets []Facet) error {
	if !g.Valid() || len(facets) < 1 || len(facets) > 64 { return store.ErrInvalid }
	origins := make(map[string]Origin, len(g.Expected))
	for _, o := range g.Expected { origins[o.ID] = o }
	seen := map[string]bool{}
	size := 0
	for _, f := range facets {
		o, ok := origins[f.ID]
		if !ok || seen[f.ID] || f.Kind != o.Kind || f.SourceID != o.SourceID || len(f.Text) < 1 || len(f.Text) > 4096 || TextHash(f.Text) != o.TextHash { return store.ErrInvalid }
		if _, err := VectorLiteral(f.Vector, g.Space.Dimensions); err != nil { return err }
		seen[f.ID] = true
		size += len(f.Text) + 4*len(f.Vector)
	}
	if size > 4<<20 { return store.ErrInvalid }
	return nil
}
...
func (q Query) Valid() bool {
	if !identity.Identifier(q.ID) || !identity.Identifier(q.Topic) || !identity.Identifier(q.Context) || !q.Space.Valid() || len(q.Kinds) < 1 || len(q.Kinds) > 8 || q.LimitPerKind < 1 || q.LimitPerKind > 10 { return false }
	seen := map[string]bool{}
	for _, k := range q.Kinds { if !Kind(k) || seen[k] { return false }; seen[k] = true }
...
func New(repo Repository) (*Service, error) {
	if repo == nil { return nil, store.ErrInvalid }
	return &Service{repo: repo}, nil
}

func coordinate(e identity.Envelope, action, permission, topic, partition string) (store.Scope, error) {
	if err := access.Require(e, action, access.Resource{Tenant:e.Tenant(), Kind:"topic", Permission:permission, ID:topic}, access.Resource{Tenant:e.Tenant(), Kind:"execution_context", Permission:"use", ID:partition}); err != nil { return store.Scope{}, err }
...
func deadline(ctx context.Context, e identity.Envelope) (context.Context, context.CancelFunc) {
	...
	if e.Deadline().Before(until) { until = e.Deadline() }
...
func (s *Service) Begin(ctx context.Context, e identity.Envelope, g Generation) error {
	scope, err := coordinate(e,"topics.write","write",g.Topic,g.Context); if err != nil { return err }
	if !g.Valid() { return store.ErrInvalid }
	ctx, cancel := deadline(ctx,e); defer cancel()
...
func (s *Service) Upsert(ctx context.Context, e identity.Envelope, g Generation, batch []Facet) error {
	scope, err := coordinate(e,"topics.write","write",g.Topic,g.Context); if err != nil { return err }
	if err = CheckBatch(g,batch); err != nil { return err }
	copyBatch := append([]Facet(nil),batch...)
	for i := range copyBatch { copyBatch[i].Vector = append([]float32(nil),batch[i].Vector...) }
	ctx, cancel := deadline(ctx,e); defer cancel()
...
func (s *Service) Publish(ctx context.Context, e identity.Envelope, g Generation, expected int64) (Publication,error) {
	scope, err := coordinate(e,"topics.publish","publish",g.Topic,g.Context); if err != nil { return Publication{},err }
	if !g.Valid() || expected < 0 || expected >= 1<<62 { return Publication{},store.ErrInvalid }
	ctx, cancel := deadline(ctx,e); defer cancel()
...
func (s *Service) Archive(ctx context.Context,e identity.Envelope,topic,partition string,expected int64) (Publication,error) {
	scope, err := coordinate(e,"topics.publish","publish",topic,partition); if err != nil { return Publication{},err }
	if expected < 1 || expected >= 1<<62 { return Publication{},store.ErrInvalid }
	ctx,cancel := deadline(ctx,e); defer cancel()
...
func (s *Service) Search(ctx context.Context,e identity.Envelope,queries []Query) ([]Result,error) {
	...
	for i,q := range queries {
		var err error
		scope,err = coordinate(e,"topics.read","read",q.Topic,q.Context); if err != nil { return nil,err }
		if !q.Valid() || seen[q.ID] { return nil,store.ErrInvalid }; seen[q.ID] = true
...
func (s *Service) Delete(ctx context.Context,e identity.Envelope,r Removal) error {
	...
	var err error
	if r.Topic == "" {
		if r.Context != "" || r.Version != "" { return store.ErrInvalid }
		scope,err = access.StoreScope(e,"ops.maintain","erase")
	} else {
		scope,err = coordinate(e,"topics.write","erase",r.Topic,r.Context)
		if r.Version != "" && !identity.Identifier(r.Version) { return store.ErrInvalid }
	}
