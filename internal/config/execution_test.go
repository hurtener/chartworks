...
func TestReadExecutionConfigBounds(t *testing.T) {
 ...
 if baseline.RowsDefault!=10000||baseline.RowsCeiling!=100000||baseline.PreviewRows!=200||baseline.Timeout!=Duration(time.Minute)||ValidateReadValidation(baseline)!=nil{t.Fatal("documented read defaults differ")}
 ...
 }{v:=baseline;change(&v);if ValidateReadValidation(v)==nil{t.Fatal("invalid read execution bound accepted")}}
}
