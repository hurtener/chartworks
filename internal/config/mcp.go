...
func DefaultMCP() MCP {
	return MCP{MaxRequestBytes: 10 << 20, MaxResponseBytes: 16 << 20, MaxConcurrent: 16, Timeout: Duration(65 * time.Second), Groups: []string{"discovery", "query", "byo", "charts", "reporting", "onboarding", "migration"}, AllowedHosts: []string{"localhost", "127.0.0.1", "::1"}}
}
...
func ValidateMCP(m MCP) error {
	...
	if len(m.Groups) < 1 || len(m.Groups) > 7 {
		return invalid("mcp.groups", "one or more implemented groups required")
	}
	seen := map[string]bool{}
	for _, g := range m.Groups {
		if seen[g] || (g != "discovery" && g != "query" && g != "byo" && g != "charts" && g != "reporting" && g != "onboarding" && g != "migration") {
