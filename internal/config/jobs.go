...
func ValidateJobs(j Jobs, a Auth) error {
	...
	if j.BrokerURL != "" {
		u, err := url.Parse(j.BrokerURL)
		if err != nil || !secureURL(j.BrokerURL) || !strings.HasSuffix(u.Path, "/exchange/execution-authority") {
			return invalid("jobs.broker_url", "trusted Pengui execution endpoint required")
		}
	}
	if len(j.Credentials) > 128 {
		return invalid("jobs.credentials", "too many tenant credentials")
	}
	seen := map[string]bool{}
	for _, c := range j.Credentials {
		if c.Tenant == "" || len(c.Tenant) > 128 || seen[c.Tenant] {
		...
		if _, err := reference(c.ClientID); err != nil {
			return invalid("jobs.credentials.client_id", "explicit environment reference required")
		}
		if _, err := reference(c.ClientSecret); err != nil {
			return invalid("jobs.credentials.client_secret", "explicit environment reference required")
		}
		seen[c.Tenant] = true
	}
	if j.Enabled && (j.BrokerURL == "" || len(j.Credentials) == 0 || a.Audiences.Jobs == "" || a.MaxTokenLifetime < Duration(30*time.Second)) {
		return invalid("jobs", "Pengui broker, credentials and execution audience required")
	}
	return nil
}
