...
func (a Auth) HTTPAudience() string {
	if a.Audiences.HTTP != "" {
		return a.Audiences.HTTP
	}
	return a.Audience
}

// MCPAudience resolves the explicit MCP audience, not the HTTP value implicitly.
func (a Auth) MCPAudience() string {
	if a.Audiences.MCP != "" {
		return a.Audiences.MCP
	}
	return a.Audience
}
...
func ValidateAuth(a Auth) error {
	if !secureURL(a.Issuer) || !secureURL(a.JWKSURL) {
		return invalid("auth", "trusted HTTPS issuer and key URL required")
	}
	if a.Audience != "" && (a.Audiences.HTTP != "" || a.Audiences.MCP != "") {
		return invalid("auth.audiences", "cannot mix shorthand and per-surface audiences")
	}
	for _, s := range []string{a.HTTPAudience(), a.MCPAudience()} {
	...
	for _, s := range a.Algorithms {
		...
		default:
			return invalid("auth.algorithms", "unsupported algorithm")
		}
		if seen[s] {
			return invalid("auth.algorithms", "duplicate algorithm")
		}
		seen[s] = true
	}
	if a.RequestTimeout <= 0 || a.RequestTimeout > Duration(time.Minute) || a.RefreshInterval <= 0 || a.RefreshInterval >= a.JWKSMaxStale || a.JWKSMaxStale > Duration(time.Hour) {
		return invalid("auth", "invalid key refresh bounds")
	}
