...
func TestAuthConfigurationBounds(t *testing.T) {
	...
	base.JWKSURL = "https://issuer.example/keys"
	base.Audience = "chartworks"
	...
	for i, change := range changes {
		a := base
		change(&a)
		if ValidateAuth(a) == nil {
			t.Fatal("invalid auth config", i)
		}
	}
	base.Audience = ""
	base.Audiences = Audiences{HTTP: "chartworks:http", MCP: "chartworks:mcp"}
	if ValidateAuth(base) != nil || base.HTTPAudience() == base.MCPAudience() {
		t.Fatal("per-surface audience contract")
	}
}
