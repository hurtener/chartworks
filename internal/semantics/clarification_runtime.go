...
func CloneClarificationAnswers(in []ClarificationAnswer) []ClarificationAnswer {
	...
	for i := range out {
		if in[i].Value == nil { continue }
		v := *in[i].Value
		if v.Time != nil { x := *v.Time; v.Time = &x }
		if v.Number != nil { x := *v.Number; v.Number = &x }
		if v.Boolean != nil { x := *v.Boolean; v.Boolean = &x }
		if v.Text != nil { x := *v.Text; v.Text = &x }
		out[i].Value = &v
	}
	return out
}
...
func MergeClarificationAnswers(base, delta []ClarificationAnswer) ([]ClarificationAnswer, error) {
	if len(base) > 64 || len(delta) > 64 { return nil, invalid(CodeLimit, "answers") }
	...
	for _, answer := range CloneClarificationAnswers(base) {
		...
		if answer.Remove || answer.Value == nil { return nil, invalid(CodeInvalidValue, "answers") }
		if _, exists := values[key]; exists { return nil, invalid(CodeDuplicateID, "answers") }
		values[key] = answer
	}
	seen := map[clarificationAnswerKey]bool{}
	for _, answer := range CloneClarificationAnswers(delta) {
		key := clarificationAnswerKey{answer.Topic, answer.Pattern, answer.Slot}
		if seen[key] || answer.Remove == (answer.Value != nil) { return nil, invalid(CodeInvalidValue, "answers") }
		seen[key] = true
		...
		if answer.Remove {
			if !exists { return nil, invalid(CodeMissingReference, "answers.remove") }
			delete(values, key)
		} else {
			values[key] = answer
		}
	}
	if len(values) > 64 { return nil, invalid(CodeLimit, "answers") }
	out := make([]ClarificationAnswer, 0, len(values))
	for _, answer := range values { out = append(out, answer) }
...
func ProviderClarificationText(resolution ClarificationResolution) (string, error) {
	if resolution.ID == "" || resolution.ID != ClarificationResolutionDigest(resolution) { return "", invalid(CodeEvidenceMismatch, "clarification.resolution") }
	var effect *ClarificationEffect
	if resolution.Effect != nil { x := *resolution.Effect; x.Values = nil; effect = &x }
	grain := ""
	if resolution.Time != nil { grain = resolution.Time.Grain }
	view := struct {
	...
	if err != nil || len(raw) > 16<<10 { return "", invalid(CodeLimit, "clarification.context") }
...
func ClarificationBudgetText(resolution ClarificationResolution) (string, error) {
	if resolution.ID == "" || resolution.ID != ClarificationResolutionDigest(resolution) { return "", invalid(CodeEvidenceMismatch, "clarification.resolution") }
	if resolution.Effect != nil { x := *resolution.Effect; x.Values = nil; resolution.Effect = &x }
...
func RedactClarificationText(text string, answers []ClarificationAnswer, resolutions []ClarificationResolution) string {
	...
	for _, r := range resolutions {
		if r.Sensitivity != LiteralSensitive { continue }
		literals = append(literals, r.Value, r.Upper)
		if r.Time != nil { literals = append(literals, r.Time.StartUTC, r.Time.EndUTC, r.Time.LocalStart, r.Time.LocalEnd) }
		for _, answer := range answers {
			if answer.Topic != r.Topic || answer.Pattern != r.Pattern || answer.Slot != r.Slot || answer.Value == nil { continue }
			v := answer.Value
			literals = append(literals, v.OptionID)
			if v.Text != nil { literals = append(literals, *v.Text) }
			if v.Boolean != nil { literals = append(literals, *v.Boolean) }
			if v.Number != nil { literals = append(literals, v.Number.Value, v.Number.Upper) }
			if v.Time != nil { literals = append(literals, v.Time.Start, v.Time.End, v.Time.Period) }
		}
	}
	sort.Slice(literals, func(i, j int) bool { return len(literals[i]) > len(literals[j]) })
	for _, literal := range literals {
		if literal == "" || len(literal) > 4096 { continue }
...
func clarificationPresentationOrder(slots []ClarificationSlot) []ClarificationSlot {
	...
	sort.Slice(out, func(i, j int) bool {
		if out[i].Required != out[j].Required { return out[i].Required }
