...
func profileSummarySchema() (*gateway.Schema, error) {
	return gateway.NewSchema("profile_summary", []byte(`{"type":"object","additionalProperties":false,"required":["summary"],"properties":{"summary":{"type":"string","minLength":1,"maxLength":1024}}}`))
}
...
func SummaryContext(p Profile) (string, error) {
	...
	out := struct {
		...
		Complete    bool     `json:"complete_sample"`
		Freshness   string   `json:"freshness"`
		ScanBounded bool     `json:"physical_scan_bounded"`
	}{Columns: make([]column, len(p.Columns)), Complete: p.Sampling.Complete, Freshness: p.Freshness.State, ScanBounded: false}
	for i, c := range p.Columns {
		category := "other"
		switch c.Category {
		case "integer", "decimal", "number", "boolean", "text", "temporal", "binary", "json", "structured", "numeric", "string", "date", "timestamp":
			category = c.Category
		}
		families := map[string]int{}
		for family, count := range c.Families {
			switch family {
...
func (s *Service) summarize(ctx context.Context, e identity.Envelope, r ProfileRecord, p Profile) ProfileSummary {
	...
	if !r.Settings.Summaries || s.gateway == nil {
		out.Status = "disabled"
		return out
	}
	input, err := SummaryContext(p)
	if err != nil {
		out.Status = "input_rejected"
		return out
	}
	...
	budget, err := gateway.NewBudget(call, gateway.Limits{Calls: 1, Tokens: 128 << 10, Duration: time.Duration(r.Settings.Timeout)})
	if err != nil {
		return out
	}
	...
	if s.summarySchema.Validate(generated.JSON, 8192) != nil {
	...
	if json.Unmarshal(generated.JSON, &value) != nil || len(value.Summary) > 2048 {
		out.Status = "invalid_output"
		return out
	}
	out.Status = "available"
	out.Text = value.Summary
	return out
}
