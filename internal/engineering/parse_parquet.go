...
func (p *parser) parquet(raw []byte) error {
	columns, err := p.parquetPreflight(raw)
	if err != nil {
		return err
	}
		...
		for {
			...
			if n == 1 {
				...
				for i, v := range row {
					...
					if err != nil {
						_ = rows.Close()
						return &ParseError{Code: "parquet_type", Row: p.receipt.Rows + 2, Column: i + 1}
					}
				}
	...
	if int64(p.receipt.Rows) != file.NumRows() {
		return ErrFormat
	}
	return nil
}
...
func parquetCell(v parquet.Value, c parquetColumn, declared UploadColumn) (Cell, error) {
	...
	if c.decimal {
		...
		case 1:
			if declared.Type != "integer" && declared.Type != "decimal" {
				return Cell{}, ErrFormat
			}
