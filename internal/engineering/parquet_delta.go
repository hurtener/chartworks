...
func guardDeltaLengths(data []byte, count, maxCell int) error {
	if count < 0 || count > 1000000 || maxCell < 1 || maxCell > 65536 { return ErrLimit }
	r := compactReader{data: data}
	block, err := r.unsigned()
	if err != nil || block < 128 || block > 65536 || block%128 != 0 { return ErrFormat }
	mini, err := r.unsigned()
	if err != nil || mini < 1 || mini > 256 || block%mini != 0 || (block/mini)%32 != 0 { return ErrFormat }
	total, err := r.unsigned()
	if err != nil || total != uint64(count) { return ErrFormat }
	...
	produced := 0
	if count > 0 {
		if current < 0 || current > int64(maxCell) { return ErrLimit }
		size = current
		produced = 1
	}
	...
	for produced < count {
		...
		if minimum < -int64(maxCell) || minimum > int64(maxCell) { return ErrFormat }
		widths, e := r.take(int(mini))
		if e != nil { return ErrFormat }
		for _, width := range widths {
			if produced >= count { break }
			if width > 32 { return ErrLimit }
			...
			for i := 0; i < valuesPerMini && produced < count; i++ {
				...
				if current < 0 || current > int64(maxCell) { return ErrLimit }
				size += current
				if size > int64(len(data)) { return ErrFormat }
				produced++
			}
		}
	}
	if size != int64(len(data)-r.at) { return ErrFormat }
	return nil
}
