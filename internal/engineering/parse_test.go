...
func TestXLSXSelectedSheetsAndNulls(t *testing.T) {
	raw := xlsxFixture(t, nil, 0)
	spec := parseSpec("xlsx", raw, UploadColumn{"id", "integer", false}, UploadColumn{"note", "text", true})
	cells, r := parsed(t, raw, spec)
	if r.Rows != 2 || cells[0].Text != "9007199254740993" || cells[1].Null || !cells[3].Null {
		t.Fatal("XLSX types/nulls", r, cells)
	}
	spec.Sheet = "Other"
	cells, r = parsed(t, raw, spec)
	if r.Rows != 1 || cells[0].Text != "7" || cells[1].Text != "second" {
		t.Fatal("selected wrong sheet")
	}
	for _, c := range []struct {
		value        string
		epoch, stamp bool
		want         string
	}{{"1", false, false, "1900-01-01"}, {"61", false, false, "1900-03-01"}, {"0", true, false, "1904-01-01"}, {"1.5", true, true, "1904-01-02T12:00:00Z"}} {
		got, e := excelTemporal(c.value, c.epoch, c.stamp)
		if e != nil || got != c.want {
			t.Fatal("serial date", got, e)
		}
	}
	for _, s := range []string{"60", "NaN", "-1", "1.25", "9999999999"} {
		if _, e := excelTemporal(s, false, false); e == nil {
...
func parquetFixture(t *testing.T, codec compress.Codec) []byte {
	t.Helper()
	var b bytes.Buffer
	w := parquet.NewGenericWriter[parquetFixtureRow](&b, parquet.Compression(codec))
	name := "ordinary"
	if _, e := w.Write([]parquetFixtureRow{{9007199254740993, &name, true, 1.25}, {-2, nil, false, 0}}); e != nil {
		t.Fatal(e)
	}
	if e := w.Close(); e != nil {
		t.Fatal(e)
...
func TestParquetFlatExactTypesAndCompression(t *testing.T) {
	for name, codec := range map[string]compress.Codec{"snappy": &pqsnap.Codec{}, "gzip": &pqgzip.Codec{}, "zstd": &pqzstd.Codec{}} {
		t.Run(name, func(t *testing.T) {
			...
			cells := []Cell{}
			r, e := Parse(context.Background(), raw, spec, config.DefaultUploads(), func(row []Cell) error { cells = append(cells, row...); return nil })
			if e != nil {
				size := int(binary.LittleEndian.Uint32(raw[len(raw)-8 : len(raw)-4]))
				meta, _, _ := compact(raw[len(raw)-8-size : len(raw)-8])
				for _, g := range meta.fields[4].list {
					t.Log("row group", g.number(2), g.number(3))
					for _, c := range g.fields[1].list {
						m := c.fields[3]
						at := int(m.number(9))
						if m.has(11) && m.number(11) < int64(at) {
							at = int(m.number(11))
						}
						h, _, _ := compact(raw[at:])
						t.Log("column", m.number(1), m.number(4), m.number(5), m.number(6), m.number(7), "page", h)
					}
				}
				t.Fatal("qualified Parquet rejected", e)
			}
			if r.Rows != 2 || cells[0].Text != "9007199254740993" || cells[2].Text != "true" || cells[3].Text != "1.25" || !cells[5].Null {
				t.Fatal("Parquet type transport", r, cells)
			}
		})
	}
	if scaledInteger(big.NewInt(-123), 4) != "-0.0123" || scaledInteger(big.NewInt(0), 2) != "0.00" {
		t.Fatal("decimal scale")
	}
}
func TestUploadParserAdversarial(t *testing.T) {
	one := []UploadColumn{{"id", "integer", false}}
	for _, raw := range [][]byte{[]byte("id\n9223372036854775808\n"), []byte("id\nNaN\n"), []byte("id\n\\N\n"), []byte("id,id\n1,2\n"), []byte("id\n\"bad\n"), {0xff, 0}, []byte("../id\n1\n")} {
		s := parseSpec("csv", raw, one...)
		if _, e := Parse(context.Background(), raw, s, config.DefaultUploads(), func([]Cell) error { return nil }); e == nil {
			t.Fatal("malformed CSV accepted")
		}
	}
	for _, value := range []string{"=1+1", "+cmd", "-cmd", "@SUM(1)", "\t＝1"} {
		if _, e := normalizeCell(UploadColumn{Type: "text"}, Cell{Text: value}); e == nil {
			t.Fatal("executable text accepted")
		}
	}
	for _, edit := range []func(map[string]string){
		...
		func(m map[string]string) {
			m["xl/worksheets/sheet2.xml"] = strings.Replace(m["xl/worksheets/sheet2.xml"], "<v>7</v>", "<f>1+1</f><v>2</v>", 1)
		},
		func(m map[string]string) {
			m["xl/_rels/workbook.xml.rels"] = strings.Replace(m["xl/_rels/workbook.xml.rels"], "Target=", "TargetMode=\"External\" Target=", 1)
		},
		func(m map[string]string) {
			m["xl/worksheets/sheet1.xml"] = strings.Replace(m["xl/worksheets/sheet1.xml"], "A2", "XFD9999999", 1)
		},
		func(m map[string]string) {
			m["xl/sharedStrings.xml"] = "<!DOCTYPE a [<!ENTITY x SYSTEM 'file:///x'>]><a/>"
		},
		func(m map[string]string) {
			m["xl/worksheets/sheet1.xml"] = strings.Replace(m["xl/worksheets/sheet1.xml"], "<v>0</v>", "<v>999999999</v>", 1)
		},
	} {
		raw := xlsxFixture(t, edit, fs.ModeSymlink|0600)
		s := parseSpec("xlsx", raw, UploadColumn{"id", "integer", false}, UploadColumn{"note", "text", true})
		if _, e := Parse(context.Background(), raw, s, config.DefaultUploads(), func([]Cell) error { return nil }); e == nil {
	...
	raw := []byte("id\n1\n2\n")
	s := parseSpec("csv", raw, one...)
	l := config.DefaultUploads()
	l.MaxRows = 1
	if _, e := Parse(context.Background(), raw, s, l, func([]Cell) error { return nil }); !errors.Is(e, ErrLimit) {
		t.Fatal("row bound", e)
	}
	s.SHA256 = strings.Repeat("0", 64)
	if _, e := Parse(context.Background(), raw, s, l, func([]Cell) error { return nil }); !errors.Is(e, ErrChecksum) {
		t.Fatal("checksum", e)
	}
	s = parseSpec("csv", raw, one...)
	ctx, cancel := context.WithCancel(context.Background())
	cancel()
	if _, e := Parse(ctx, raw, s, l, func([]Cell) error { return nil }); !errors.Is(e, context.Canceled) {
...
func TestParquetAllocationGuards(t *testing.T) {
	for _, raw := range [][]byte{{0x18, 0xff, 0xff, 0xff, 0xff, 0x7f}, bytes.Repeat([]byte{0x1c}, 32), {0x19, 0xf5, 0xff, 0xff, 0xff, 0x7f}, {0x16, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff}} {
		if _, _, e := compact(raw); e == nil {
			t.Fatal("unbounded compact structure accepted")
		}
	}
	if _, e := guardRLE([]byte{0xff, 0xff, 0xff, 0xff, 0x7f}, 1, 1, 2, false); e == nil {
		t.Fatal("unbounded RLE run accepted")
	}
	if _, e := boundedDecompress(0, []byte("large"), 1); e == nil {
		t.Fatal("declared expansion not enforced")
	}
	if e := guardPlain([]byte{255, 255, 255, 127}, parquetColumn{physical: 6}, 1, 64); e == nil {
		t.Fatal("cell allocation not bounded")
	}
}
...
func FuzzUploadCSV(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, b []byte) {
		...
		s := parseSpec("csv", b, UploadColumn{"id", "text", true})
		l := config.DefaultUploads()
		l.MaxBytes = 8192
		l.MaxRows = 64
		l.MaxCells = 64
		l.MaxCellBytes = 128
		_, _ = Parse(context.Background(), b, s, l, func(row []Cell) error {
			if len(row) != 1 {
				t.Fatal("shape")
			}
			return nil
...
func FuzzUploadArchiveAndCompact(f *testing.F) {
	...
	f.Add([]byte{0x18, 0xff, 0xff, 0xff, 0xff, 0x7f})
	f.Add([]byte("PAR1PAR1"))
	f.Fuzz(func(t *testing.T, b []byte) {
		...
		l := config.DefaultUploads()
		l.MaxBytes = 32768
		l.MaxExpandedBytes = 32768
		l.MaxArchiveEntries = 16
		l.MaxRows = 64
		l.MaxCells = 256
		l.MaxCellBytes = 256
		for _, format := range []string{"xlsx", "parquet"} {
			s := parseSpec(format, b, UploadColumn{"id", "text", true})
			_, _ = Parse(context.Background(), b, s, l, func([]Cell) error { return nil })
		}
	})
}
