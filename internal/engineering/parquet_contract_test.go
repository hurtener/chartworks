...
func TestParquetMetadataAndPageContracts(t *testing.T) {
 ...
 file,used,err:=compact(raw[len(raw)-8-size:len(raw)-8]);if err!=nil||used!=size{t.Fatal("bounded footer",err,used,size)}
 t.Log("file version",file.number(1),"rows",file.number(3),"root",file.fields[2].list[0])
 for i,node:=range file.fields[2].list[1:] {t.Log("schema",i,node)}
 for _,group:=range file.fields[4].list {
  for i,chunk:=range group.fields[1].list {
   m:=chunk.fields[3]; start:=m.number(9);if m.has(11)&&m.number(11)<start{start=m.number(11)}
   ...
   if e!=nil {
    h,n,_:=compact(raw[start:]);body:=raw[start+int64(n):start+int64(n)+h.number(3)];v2:=h.fields[8];def:=v2.number(5)
 ...
 if _,err=p.parquetPreflight(raw);err!=nil{t.Fatalf("complete metadata preflight: %v",err)}
}
