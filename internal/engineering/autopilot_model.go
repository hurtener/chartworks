...
func validateAutopilotGoal(g AutopilotGoal) error {
	if !identity.Identifier(g.ID)||!identity.Identifier(g.Pipeline)||len(g.Pipeline)>48||g.Pipeline==g.Source||
		...
		!proposalText(g.Goal,4096)||g.ExpectedPipelineVersion<0||g.ExpectedPipelineVersion>4096||g.MaxStalenessSeconds<60||g.MaxStalenessSeconds>604800{return ErrInvalid}
	return nil
}
...
func RequireAutopilotMaterial(e identity.Envelope,m ProposalMaterial,action string)error{
	...
	if m.Binding.Tenant!=e.Tenant(){return access.ErrForbidden}
	...
	for _,r:=range m.References{refs=append(refs,access.Resource{Tenant:e.Tenant(),Kind:r.Kind,Permission:r.Permission,ID:r.ID})}
...
func ValidateAutopilotMaterial(m ProposalMaterial,l config.Autopilot)error{
	if l.Validate()!=nil||m.Version!=AutopilotVersion||m.PromptVersion!=AutopilotPromptVersion||validateAutopilotGoal(m.Request)!=nil||
		...
		len(m.Plan.Requirements)<1||len(m.Plan.Requirements)>l.MaxSteps||!proposalText(m.Plan.Rationale,4096)||len(m.Objects)!=len(m.Pipeline.Steps)+1{return ErrInvalid}
	for _,r:=range m.Plan.Requirements{if !proposalText(r,1024){return ErrInvalid}}
	pl:=config.DefaultPipelines();pl.MaxSteps=l.MaxSteps
	if ValidatePipelineDefinition(m.Pipeline,pl)!=nil{return ErrInvalid}
	relations:=map[string]bool{}
	wantEvidence:=map[string]string{"binding":readexec.Hash(m.Binding)}
	for _,r:=range m.Binding.Relations{relations[r.ID]=true;wantEvidence["relation:"+r.ID]=readexec.Hash(r)}
	if len(m.Evidence)!=len(wantEvidence){return ErrInvalid}
	...
	for _,v:=range m.Evidence{
		if seenEvidence[v.ID]||wantEvidence[v.ID]!=v.Digest||v.Source!=m.Binding.Source||v.Context!=m.Binding.Context||
			...
			(v.ID!="binding"&&(v.Kind!="relation"||v.ID!="relation:"+v.Dataset||!relations[v.Dataset])){return ErrInvalid}
	...
	for _,r:=range m.Binding.Relations{wantRefs=append(wantRefs,ProposalReference{Kind:"dataset",Permission:"query",ID:r.ID})}
	if readexec.Hash(m.References)!=readexec.Hash(wantRefs){return ErrInvalid}
	for index,o:=range m.Objects{
		kind,id:="pipeline",m.Pipeline.ID
		if index>0{kind,id="dataset",m.Pipeline.ID+"."+m.Pipeline.Steps[index-1].ID}
		if o.Kind!=kind||o.ID!=id||o.Decision!="build_managed"||o.Author!=m.Author||o.ModelVersion!=m.ModelVersion||
			!proposalText(o.Rationale,4096)||len(o.Evidence)<1||len(o.Evidence)>33||len(o.Alternatives)<1||len(o.Alternatives)>16{return ErrInvalid}
		seen:=map[string]bool{}
		for _,id:=range o.Evidence{if !seenEvidence[id]||seen[id]{return ErrInvalid};seen[id]=true}
		for _,alternative:=range o.Alternatives{if !relations[alternative.Dataset]||!proposalText(alternative.Rationale,2048){return ErrInvalid}}
	}
	for _,step:=range m.Pipeline.Steps{
		if len(step.FromSteps)!=0||step.Source!=m.Binding.Source||step.Context!=m.Binding.Context||step.Strategy!="replace"{return ErrInvalid}
		for _,id:=range step.Inputs{if !relations[id]{return ErrInvalid}}
	}
	if len(m.Usage.Calls)<1||len(m.Usage.Calls)>l.MaxCalls{return ErrInvalid}
	for _,u:=range m.Usage.Calls{if u.Attempts<0||u.Attempts>l.MaxCalls||!slices.Contains([]string{"pipeline_draft"},u.Role){return ErrInvalid}}
	raw,err:=json.Marshal(m);if err!=nil||len(raw)>l.MaxProposalBytes{return ErrLimit}
	return nil
}
