...
func(r UploadRecord) Require(e identity.Envelope,action,permission string)error{
	if !r.Valid()||r.Tenant!=e.Tenant()||r.Actor!=e.User()||r.Session!=e.Session(){return store.ErrNotFound}
...
func(r UploadRecord) Public()UploadStatus{
	...
	if r.Receipt!=nil{out.Rows=r.Receipt.Rows}
	if r.State=="active"{out.Source=&sources.Source{ID:r.Spec.ID,Name:r.Spec.Name,Dialect:"postgres",Revision:1,ContextID:r.Spec.ID+":v1",Status:"registered"}}
	return out
}
...
type UploadRun struct { Upload UploadStatus `json:"upload"`; Operation jobs.RequestTask `json:"operation"`; Code string `json:"code"` }
...
func New(repo Repository,source *sources.Service,validator *readexec.Validator,executor *readexec.Executor,model *gateway.Engine,values config.Values,lookup func(string)(string,bool))(*Service,error){
	if repo==nil||source==nil||lookup==nil||config.ValidateUploads(values.Uploads)!=nil||config.ValidateProfiling(values.Profiling)!=nil{return nil,ErrInvalid}
	if (values.Uploads.Enabled||values.Profiling.Enabled)&&(validator==nil||executor==nil||!values.Sources.Enabled||repo.ReadCapacity()<values.Uploads.Concurrency+values.Exec.ExecutionConcurrency+2){return nil,ErrInvalid}
	limits:=jobs.Defaults()
	limits.GlobalConcurrency=values.Jobs.GlobalConcurrency;limits.TenantConcurrency=values.Jobs.TenantConcurrency;limits.MaxPending=values.Jobs.MaxPending;limits.MaxPendingPerTenant=values.Jobs.MaxPendingPerTenant;limits.MaxAttempts=values.Jobs.MaxAttempts
	limits.Workers=min(limits.Workers,limits.GlobalConcurrency)
	runner,err:=jobs.NewRequestRunner(repo,limits);if err!=nil{return nil,err}
	values.Sources=values.Sources.Clone();values.Uploads=values.Uploads.Clone();values.Profiling=values.Profiling.Clone()
...
func(s *Service)Close(){s.cancel();s.lifecycle.Lock();defer s.lifecycle.Unlock();s.closed=true}
func(s *Service)call(ctx context.Context,e identity.Envelope,work bool,fn func(context.Context)error)error{
	...
	s.lifecycle.RLock();defer s.lifecycle.RUnlock();if s.closed{return store.ErrUnavailable}
	ctx,cancel:=context.WithDeadline(ctx,e.Deadline());defer cancel();stop:=context.AfterFunc(s.ctx,cancel);defer stop()
	if err:=ctx.Err();err!=nil{return err}
	if work{ctx,finish:=context.WithTimeout(ctx,time.Duration(s.values.Uploads.Timeout));defer finish();select{case s.slots<-struct{}{}:defer func(){<-s.slots}();case<-ctx.Done():return ctx.Err()};return fn(ctx)}
	return fn(ctx)
}
func(s *Service)workspace(e identity.Envelope,alias string)(config.SourceConnection,error){
	for _,c:=range s.values.Sources.Connections{if c.Tenant==e.Tenant()&&c.ID==alias&&c.ManagedSchema!=""&&c.WriteDSN!=""{return c,nil}}
...
func(s *Service)ReserveUpload(ctx context.Context,e identity.Envelope,spec UploadSpec)(out UploadStatus,err error){
	...
	if !s.values.Uploads.Enabled{return out,store.ErrUnavailable}
	if !spec.Valid(s.values.Uploads){return out,ErrInvalid}
	if _,err=s.workspace(e,spec.Connection);err!=nil{return out,err}
	err=s.call(ctx,e,false,func(ctx context.Context)error{record,e2:=s.repo.ReserveUpload(ctx,e,spec,s.values.Uploads);if e2==nil{out=record.Public()};return e2})
	return out,err
}
...
func(s *Service)InspectUpload(ctx context.Context,e identity.Envelope,id string)(out UploadStatus,err error){
	if err=access.Require(e,"sources.read",access.Resource{Tenant:e.Tenant(),Kind:"source",Permission:"read",ID:id});err!=nil{return out,err}
	err=s.call(ctx,e,false,func(ctx context.Context)error{record,e2:=s.repo.ReadUpload(ctx,e,id,"sources.read","read");if e2==nil{out=record.Public()};return e2});return out,err
}
...
func(s *Service)StageUpload(ctx context.Context,e identity.Envelope,id string,body io.Reader)(out UploadStatus,err error){
	if err=access.Require(e,"sources.upload",access.Resource{Tenant:e.Tenant(),Kind:"source",Permission:"write",ID:id});err!=nil{return out,err}
	if body==nil||!s.values.Uploads.Enabled{return out,ErrInvalid}
	err=s.call(ctx,e,true,func(ctx context.Context)error{
		r,e2:=s.repo.StageUpload(ctx,e,id,func(ctx context.Context,r UploadRecord)error{
			c,e2:=s.workspace(e,r.Spec.Connection);if e2!=nil{return e2}
			raw,e2:=io.ReadAll(io.LimitReader(body,r.Spec.Bytes+1));if e2!=nil{return ErrFormat}
			if int64(len(raw))!=r.Spec.Bytes||contentHash(raw)!=r.Spec.SHA256{return ErrChecksum}
			if e2=ctx.Err();e2!=nil{return e2}
			return s.stageWorkspace(ctx,e,c,r,raw)
		});if e2==nil{out=r.Public()};return e2
	});return out,err
}
...
func(s *Service)runUpload(ctx context.Context,e identity.Envelope,id,key string,resume,erase bool)(out UploadRun,err error){
	...
	if err=access.Require(e,action,access.Resource{Tenant:e.Tenant(),Kind:"source",Permission:permission,ID:id});err!=nil{return out,err}
	if !s.values.Uploads.Enabled||!identity.Identifier(key){return out,ErrInvalid}
	err=s.call(ctx,e,true,func(ctx context.Context)error{
		r,err:=s.repo.ReadUpload(ctx,e,id,action,permission);if err!=nil{return err}
		if !erase&&r.State=="active"||erase&&r.State=="erased"{out.Upload=r.Public();out.Code="already_complete";return nil}
		if !erase&&r.State!="staged"{return ErrState}
		if !erase&&time.Now().After(r.Expires){return store.ErrExpired}
		c,err:=s.workspace(e,r.Spec.Connection);if err!=nil{return err}
		partition:="";if r.Receipt!=nil{partition=id+":v1"}
		input:=jobs.RequestInput{Kind:kind,Target:id,InputHash:r.SpecHash,Context:partition}
		task,err:=s.runner.Admit(ctx,e,key,input);if err!=nil{return err}
		if resume{task,err=s.runner.Resume(ctx,e,task.ID);if err!=nil{return err}}
		r,err=s.repo.AttachUpload(ctx,e,id,task,erase);if err!=nil{return err}
		task,runErr:=s.runner.Run(ctx,e,task,time.Duration(s.values.Uploads.Timeout),func(ctx context.Context,i jobs.Invocation)error{
			if _,err:=i.Current(kind,id,r.SpecHash);err!=nil{return err}
			if erase{if err:=s.eraseWorkspace(ctx,e,c,r);err!=nil{return err};return s.repo.FinishUploadErasure(ctx,i,r)}
			receipt,err:=s.loadWorkspace(ctx,e,c,r);if err!=nil{return err}
			columns:=make([]string,len(r.Spec.Columns));for i,c:=range r.Spec.Columns{columns[i]=c.Name}
			source,err:=s.sources.DescribeManaged(ctx,e,id,r.Spec.Name,r.Spec.Connection,columns);if err!=nil{return err}
			return s.repo.ActivateUpload(ctx,i,r,receipt,source)
		})
		if task.ID==""{return runErr}
		current,readErr:=s.repo.ReadUpload(ctx,e,id,action,permission);if readErr!=nil{return readErr}
		out=UploadRun{Upload:current.Public(),Operation:task}
		if runErr!=nil{out.Code=engineeringCode(runErr)}
		return nil
	});return out,err
}

// RequestOperation inspects only an actor/session-bound task with current reach.
func(s *Service)RequestOperation(ctx context.Context,e identity.Envelope,id string)(jobs.RequestTask,error){return s.runner.Inspect(ctx,e,id)}

// CancelOperation persists intent; stopping the actual owner is separately observed.
func(s *Service)CancelOperation(ctx context.Context,e identity.Envelope,id string)(jobs.RequestTask,error){return s.runner.Cancel(ctx,e,id)}
...
func(s *Service)SweepUploads(ctx context.Context,e identity.Envelope,limit int)(out []UploadRun,err error){
	...
	if limit<1||limit>32{return nil,ErrInvalid}
	records,err:=s.repo.ExpiredUploads(ctx,e,limit);if err!=nil{return nil,err}
	out=[]UploadRun{}
	for _,r:=range records{result,e2:=s.EraseUpload(ctx,e,r.Spec.ID,"staging-expiry-"+r.SpecHash[:32],true);if e2!=nil{return out,e2};out=append(out,result)}
	return out,nil
}
func engineeringCode(err error)string{
	switch{
	case errors.Is(err,context.Canceled):return "cancelled"
	case errors.Is(err,context.DeadlineExceeded),errors.Is(err,readexec.ErrTimeout):return "timed_out"
	case errors.Is(err,ErrLimit),errors.Is(err,readexec.ErrLimit):return "limit_exceeded"
	case errors.Is(err,ErrFormat),errors.Is(err,ErrChecksum),errors.Is(err,ErrInvalid):return "invalid_file"
	case errors.Is(err,ErrOwnership):return "workspace_ownership_unproven"
	case errors.Is(err,readexec.ErrBinding):return "context_changed"
	case errors.Is(err,access.ErrUnauthenticated),errors.Is(err,access.ErrForbidden),errors.Is(err,jobs.ErrAuthority):return "authority_blocked"
	case errors.Is(err,ErrState):return "reconciliation_required"
	default:return "dependency_unavailable"
	}
}

// ProtectedString rejects accidental control characters in retained display text.
func ProtectedString(s string)bool{return len(s)>0&&len(s)<=128&&strings.TrimSpace(s)==s&&!strings.ContainsAny(s,"\x00\r\n\t")}
