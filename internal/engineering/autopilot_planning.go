...
func NewAutopilot(repo AutopilotRepository,pipelines *PipelineService,limits config.Autopilot)(*Autopilot,error){
	if repo==nil||pipelines==nil||limits.Validate()!=nil{return nil,ErrInvalid}
	if limits.Enabled&&(!pipelines.Enabled()||pipelines.model==nil||pipelines.validator==nil){return nil,ErrUnavailable}
...
func(s *Autopilot) begin(ctx context.Context,e identity.Envelope,mutate bool)(context.Context,func(),error){
	if ctx==nil{return nil,nil,ErrInvalid};if !e.Valid(){return nil,nil,access.ErrUnauthenticated}
	s.mu.RLock()
	if s.closed||mutate&&!s.limits.Enabled{s.mu.RUnlock();return nil,nil,ErrUnavailable}
	deadline:=minTime(e.Deadline(),time.Now().Add(time.Duration(s.limits.Timeout)))
...
func(s *Autopilot) Get(ctx context.Context,e identity.Envelope,id string)(AutopilotProposal,error){
	ctx,stop,err:=s.begin(ctx,e,false);if err!=nil{return AutopilotProposal{},err};defer stop()
...
func proposalRefs(b readexec.Binding)[]ProposalReference{
	...
	for _,relation:=range b.Relations{out=append(out,ProposalReference{Kind:"dataset",Permission:"query",ID:relation.ID})}
	return out
}
...
func proposalFacts(b readexec.Binding)[]ProposalEvidence{
	...
	for _,r:=range b.Relations{out=append(out,ProposalEvidence{ID:"relation:"+r.ID,Kind:"relation",Source:b.Source,Context:b.Context,Dataset:r.ID,Digest:readexec.Hash(r)})}
	return out
}
...
func(s *Autopilot) Propose(ctx context.Context,e identity.Envelope,g AutopilotGoal)(AutopilotProposal,error){
	...
	if validateAutopilotGoal(g)!=nil{return AutopilotProposal{},ErrInvalid}
	if err=access.Require(e,"engineering.autopilot.propose",access.Resource{Tenant:e.Tenant(),Kind:"source",Permission:"write",ID:g.Pipeline},access.Resource{Tenant:e.Tenant(),Kind:"source",Permission:"query",ID:g.Source},access.Resource{Tenant:e.Tenant(),Kind:"execution_context",Permission:"use",ID:g.Context});err!=nil{return AutopilotProposal{},err}
	previous,err:=s.repo.ReadAutopilotProposal(ctx,e,g.ID,"engineering.autopilot.propose")
	if err==nil{if readexec.Hash(previous.Material.Request)!=readexec.Hash(g){return AutopilotProposal{},store.ErrConflict};return previous,nil}
	if !errors.Is(err,store.ErrNotFound){return AutopilotProposal{},err}
	select{case s.planning<-struct{}{}:defer func(){<-s.planning}();case <-ctx.Done():return AutopilotProposal{},ctx.Err()}
	binding,err:=s.pipelines.source.Binding(ctx,e,g.Source,g.Context);if err!=nil{return AutopilotProposal{},err}
	if binding.Dialect!="postgres"||!binding.Valid(){return AutopilotProposal{},ErrInvalid}
	...
	if err=RequireAutopilotMaterial(e,material,"engineering.autopilot.propose");err!=nil{return AutopilotProposal{},err}
	refs:=[]access.Resource{{Tenant:e.Tenant(),Kind:"source",Permission:"write",ID:g.Pipeline}}
	for _,r:=range material.References{refs=append(refs,access.Resource{Tenant:e.Tenant(),Kind:r.Kind,Permission:r.Permission,ID:r.ID})}
	call,err:=gateway.Authorize(e,"engineering.autopilot.propose",readexec.Hash([]any{g,binding,s.limits.ModelVersion}),refs...);if err!=nil{return AutopilotProposal{},err}
	budget,err:=gateway.NewBudget(call,gateway.Limits{Calls:s.limits.MaxCalls,Tokens:s.limits.MaxTokens,Duration:time.Duration(s.limits.Timeout)});if err!=nil{return AutopilotProposal{},err}
	blindSchema,err:=gateway.NewSchema("engineering_blind_plan",[]byte(blindPlanSchema));if err!=nil{return AutopilotProposal{},err}
	blindInput,_:=json.Marshal(struct{Goal string `json:"goal"`;Maximum int `json:"maximum_requirements"`}{g.Goal,s.limits.MaxSteps})
	...
	if err!=nil{return AutopilotProposal{},err}
	if blindSchema.Validate(blind.JSON,32<<10)!=nil||json.Unmarshal(blind.JSON,&material.Plan)!=nil||len(material.Plan.Requirements)>s.limits.MaxSteps{return AutopilotProposal{},gateway.ErrOutput}
	...
	if err=RequireAutopilotMaterial(e,material,"engineering.autopilot.propose");err!=nil{return AutopilotProposal{},err}
	input,err:=json.Marshal(struct{Goal string `json:"goal"`;Plan AutopilotPlan `json:"blind_plan"`;Relations []readexec.Relation `json:"relations"`;Evidence []ProposalEvidence `json:"evidence"`}{g.Goal,material.Plan,binding.Relations,material.Evidence})
	if err!=nil||len(input)>s.limits.MaxEvidenceBytes{return AutopilotProposal{},ErrLimit}
	schema,err:=gateway.NewSchema("engineering_match_build",[]byte(matchedProposalSchema));if err!=nil{return AutopilotProposal{},err}
	...
	if schema.Validate(generated.JSON,128<<10)!=nil||json.Unmarshal(generated.JSON,&matched)!=nil{return AutopilotProposal{},gateway.ErrOutput}
	plan,err:=s.pipelines.validator.Validate(ctx,e,readexec.Request{Source:g.Source,Context:g.Context,SQL:matched.SQL});if err!=nil{return AutopilotProposal{},err}
	...
	if err=s.pipelines.validateInputs(ctx,e,material.Pipeline);err!=nil{return AutopilotProposal{},err}
	latest,err:=s.pipelines.source.Binding(ctx,e,g.Source,g.Context);if err!=nil{return AutopilotProposal{},err}
	if readexec.Hash(latest)!=readexec.Hash(binding){return AutopilotProposal{},ErrProposalDrift}
	proof,err:=prepareProposal(e,material,s.limits);if err!=nil{return AutopilotProposal{},err}
...
func(s *Autopilot) Edit(ctx context.Context,e identity.Envelope,id string,r AutopilotEditRequest)(AutopilotProposal,error){
	ctx,stop,err:=s.begin(ctx,e,true);if err!=nil{return AutopilotProposal{},err};defer stop()
	p,err:=s.repo.ReadAutopilotProposal(ctx,e,id,"engineering.autopilot.propose");if err!=nil{return AutopilotProposal{},err}
	if r.ExpectedVersion!=p.Version||!proposalText(r.Reason,2048)||p.State=="applying"||p.State=="applied"||p.State=="compensated"{return AutopilotProposal{},store.ErrConflict}
	m:=p.Material
	if r.Definition.ID!=m.Pipeline.ID||r.Definition.Connection!=m.Pipeline.Connection||r.Definition.Name!=m.Pipeline.Name||len(r.Definition.Steps)!=len(m.Pipeline.Steps){return AutopilotProposal{},ErrInvalid}
	for index,step:=range r.Definition.Steps{if step.ID!=m.Pipeline.Steps[index].ID||step.Source!=m.Binding.Source||step.Context!=m.Binding.Context||len(step.FromSteps)!=0{return AutopilotProposal{},ErrInvalid}}
	if err=s.pipelines.validateInputs(ctx,e,r.Definition);err!=nil{return AutopilotProposal{},err}
	binding,err:=s.pipelines.source.Binding(ctx,e,m.Binding.Source,m.Binding.Context);if err!=nil{return AutopilotProposal{},err}
	if readexec.Hash(binding)!=readexec.Hash(m.Binding){return AutopilotProposal{},ErrProposalDrift}
	m.Pipeline=r.Definition;m.Author=e.User();m.Session=e.Session();m.Created=time.Now().UTC().Truncate(time.Microsecond)
	for index:=range m.Objects{m.Objects[index].Author=e.User();m.Objects[index].Rationale=r.Reason}
	proof,err:=prepareProposal(e,m,s.limits);if err!=nil{return AutopilotProposal{},err}
...
func(s *Autopilot) Review(ctx context.Context,e identity.Envelope,id string,r AutopilotReviewRequest)(AutopilotProposal,error){
	ctx,stop,err:=s.begin(ctx,e,true);if err!=nil{return AutopilotProposal{},err};defer stop()
	if r.ExpectedVersion<1||r.Revision<1||!slices.Contains([]string{"approve","reject"},r.Decision)||!proposalText(r.Reason,2048){return AutopilotProposal{},ErrInvalid}
