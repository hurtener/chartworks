...
func (s *Autopilot) Apply(ctx context.Context, e identity.Envelope, id string, r AutopilotApplyRequest) (AutopilotProposal, error) {
	...
	defer stop()
	p, err := s.repo.ReadAutopilotProposal(ctx, e, id, "engineering.autopilot.apply")
	if err != nil {
		return AutopilotProposal{}, err
	}
	if !proposalRequestMatches(p, r) {
		return AutopilotProposal{}, store.ErrConflict
	}
	if p.State == "compensated" || p.State == "rejected" {
		return AutopilotProposal{}, ErrProposalConflict
	}
	if p.State == "approved" && p.Version != r.ExpectedVersion {
		return AutopilotProposal{}, store.ErrConflict
	}
	if p.ApplyActor != "" && (p.ApplyActor != e.User() || p.ApplySession != e.Session()) {
		return AutopilotProposal{}, access.ErrForbidden
	}
	if p.State == "applied" {
		return p, nil
	}
	...
	if err = s.pipelines.validateInputs(ctx, e, p.Material.Pipeline); err != nil {
		return p, err
	}
	for _, stage := range []string{"draft", "publish"} {
		proof, proofErr := prepareProposalApply(e, p, "engineering.autopilot.apply", "", nil)
		if proofErr != nil {
			return p, proofErr
		}
		p, err = s.repo.StageAutopilotPipeline(ctx, e, proof, stage)
		if err != nil {
			return p, err
		}
	}
	...
	p, err = s.repo.RecordAutopilotRun(ctx, e, proof, run)
	if err != nil {
		return p, err
	}
	if p.State == "applied" {
...
func (s *Autopilot) Compensate(ctx context.Context, e identity.Envelope, id string, r AutopilotApplyRequest) (AutopilotProposal, error) {
	...
	defer stop()
	p, err := s.repo.ReadAutopilotProposal(ctx, e, id, "engineering.autopilot.compensate")
	if err != nil {
		return AutopilotProposal{}, err
	}
	if !proposalRequestMatches(p, r) {
	...
	if err = pipelineAuthority(e, p.Material.Pipeline, "engineering.pipeline.run", "write"); err != nil {
		return p, err
	}
	x, err := s.pipelines.repo.ReadPipelineExecution(ctx, e, p.Operation)
	if err != nil {
		return p, err
	}
	if x.State != "published" || x.Operation.State != "succeeded" || x.Digest != readexec.Hash(p.Material.Pipeline) {
		return p, ErrCompensationBlocked
	}
	steps := make([]string, 0, len(x.Stages))
	for _, stage := range x.Stages {
		if stage.Previous != nil || stage.State != "checked" || stage.Stage.Revision != 1 || !stage.Stage.Valid() {
			return p, ErrCompensationBlocked
		}
		if err = access.Require(e, "engineering.autopilot.compensate", access.Resource{Tenant: e.Tenant(), Kind: "source", Permission: "write", ID: stage.Stage.Source}); err != nil {
			return p, err
		}
		steps = append(steps, stage.Stage.Step)
	}
	if len(steps) != len(p.Material.Pipeline.Steps) {
		return p, ErrCompensationBlocked
	}
	...
	err = s.pipelines.source.WithPipelineOutputs(ctx, e, p.Operation, steps, func(held context.Context, records []sources.Record) error {
		if len(records) != len(x.Stages) {
			return ErrOwnership
		}
		for _, record := range records {
			if record.Pipeline == nil || record.Pipeline.Operation != p.Operation {
				return ErrOwnership
			}
		}
		proof, proofErr := prepareProposalApply(e, p, "engineering.autopilot.compensate", readexec.Hash(x), nil)
		if proofErr != nil {
			return proofErr
		}
...
func (s *Autopilot) DetectDrift(ctx context.Context, e identity.Envelope, id string) (AutopilotDrift, error) {
	...
	defer stop()
	p, err := s.repo.ReadAutopilotProposal(ctx, e, id, "engineering.autopilot.drift")
	if err != nil {
		return AutopilotDrift{}, err
	}
	...
	default:
		if p.Operation == "" {
			return AutopilotDrift{}, store.ErrNotFound
		}
		x, readErr := s.pipelines.repo.ReadPipelineExecution(ctx, e, p.Operation)
		if readErr != nil {
			return AutopilotDrift{}, readErr
		}
	...
	proof, err := prepareProposalApply(e, p, "engineering.autopilot.drift", "", &drift)
	if err != nil {
		return AutopilotDrift{}, err
	}
