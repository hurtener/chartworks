...
func testPhase30Transport(t *testing.T) {
	t.Run("real-http-sdk-lifecycle-and-denial-inventory", func(t *testing.T) {
		...
		if len(scopes) > 32 {
			t.Fatal("wire fixture widened signed scope ceiling", len(scopes))
		}
		...
		if err != nil || schedule.Retired || schedule.Request.Target.Reporting == nil {
			t.Fatal("SDK reporting schedule create", schedule, err)
		}
		read, err := client.Schedule(t.Context(), schedule.ID)
		if err != nil || read.Revision != schedule.Revision || phase30Wire(t, read.Request) != phase30Wire(t, request) {
			t.Fatal("SDK schedule projection diverged", read, err)
		}
		job, err := client.TestSchedule(t.Context(), schedule.ID, "wire-test", 1)
		if err != nil || job.Reporting == nil || job.Reporting.Revision != 1 {
			t.Fatal("SDK test admission dropped accepted pins", job, err)
		}
		if err := f.queue.RunOnce(t.Context()); err != nil {
			t.Fatal(err)
		}
		finished, err := client.Job(t.Context(), job.ID)
		if err != nil || finished.State != "succeeded" || finished.Delivery == nil || finished.Delivery.Catalog != "available" || finished.Delivery.Notification != "not_requested" {
			t.Fatal("SDK discarded truthful delivery receipt", finished, err)
		}
		if schedule, err = client.SetScheduleState(t.Context(), schedule.ID, 1, false); err != nil || schedule.Revision != 2 {
			t.Fatal("SDK pause", schedule, err)
		}
		request.Target.Reporting.Budget.MaxRows = 500
		if schedule, err = client.ReplaceSchedule(t.Context(), schedule.ID, 2, "wire-replacement", request); err != nil || schedule.Revision != 3 || schedule.Enabled {
			t.Fatal("SDK CAS replacement", schedule, err)
		}
		if schedule, err = client.SetScheduleState(t.Context(), schedule.ID, 3, true); err != nil || schedule.Revision != 4 {
			t.Fatal("SDK resume", schedule, err)
		}
		pending, err := client.FireSchedule(t.Context(), schedule.ID, "wire-fire")
		if err != nil {
			t.Fatal(err)
		}
		if cancelled, err := client.CancelJob(t.Context(), pending.ID); err != nil || cancelled.State != "cancelled" {
			t.Fatal("SDK accepted occurrence cancellation", cancelled, err)
		}
		if schedule, err = client.RetireSchedule(t.Context(), schedule.ID, 4); err != nil || !schedule.Retired || schedule.Enabled {
			t.Fatal("SDK retirement", schedule, err)
		}
		history, err := client.ScheduleHistory(t.Context(), schedule.ID, cw.ScheduleHistoryRequest{Kind: "revisions", Limit: 2})
		if err != nil || len(history.Revisions) != 2 || history.Revisions[0].Revision != 5 || history.Revisions[0].Request.Target.Reporting == nil || history.NextBeforeRevision != 4 {
			t.Fatal("SDK immutable bounded history", history, err)
		}
		...
		for _, operation := range workapi.Registry(false, true) {
			...
			if denied.Code != http.StatusForbidden {
				t.Fatal("operation missing action-first denial", operation, denied.Code, denied.Body.String())
			}
			if strings.Contains(operation.Path, "event") || strings.Contains(operation.Path, "condition") || strings.Contains(operation.Path, "custom") {
				t.Fatal("unsupported scheduling capability advertised", operation)
			}
		}
		...
		if response.Code != http.StatusNotFound || strings.Contains(response.Body.String(), "p30-wire") {
			t.Fatal("history crossed the tenant boundary", response.Code, response.Body.String())
		}
		...
		for _, path := range [][]string{{"executor"}, {"reporting", "sql"}, {"reporting", "authorization"}, {"reporting", "budget", "provider_key"}, {"reporting", "arguments", "value", "execution_binding"}} {
			...
			for _, key := range path[:len(path)-1] {
				if key == "arguments" {
					cursor = cursor[key].([]any)[0].(map[string]any)
				} else {
			...
			if response.Code != http.StatusBadRequest || strings.Contains(response.Body.String(), "SYNTHETIC_FORGED_AUTHORITY") {
				t.Fatal("closed nested reporting request accepted executable/authority fields", path, response.Code, response.Body.String())
			}
		}
		for _, body := range []string{strings.Replace(base, `"kind":"reporting.scheduled"`, `"kind":"reporting.scheduled","kind":"retention.sweep"`, 1), strings.Repeat(" ", 8193) + base} {
			...
			if response.Code != http.StatusBadRequest && response.Code != http.StatusRequestEntityTooLarge {
				t.Fatal("duplicate/oversized request reached admission", response.Code)
			}
		}
		invalid := target
		invalid.Outputs = []string{"missing-output"}
		if _, err := client.SubmitJob(t.Context(), "invalid-target-contract", cw.JobTarget{Kind: jobs.ReportingKind, BindingID: "reporting", Reporting: &invalid}); err == nil {
			t.Fatal("invalid target output was admitted")
		} else {
			var status *cw.StatusError
			if !errors.As(err, &status) || status.Status != http.StatusBadRequest {
				t.Fatal("invalid reporting target lost its typed client error", err)
			}
		}
		if f.domain.attemptCount(t) != beforeQueries || f.domain.f.model.requests.Load() != beforeModels || f.calls.Load() != beforeBroker {
			t.Fatal("denied/malformed transport performed protected execution")
		}
		...
		for _, definition := range metadataRegistry.Definitions() {
			if definition.ID == "testSchedule" || definition.ID == "replaceSchedule" {
				t.Fatal("disabled dispatch advertised admission", definition.ID)
			}
		}
		...
		if response.Code != http.StatusOK {
			t.Fatal("broker availability disabled retained history", response.Code, response.Body.String())
		}
	})
	t.Run("unsupported-kinds-never-admit-work", func(t *testing.T) {
		...
		for _, kind := range []string{"event", "condition", "condition_check", "custom", "shell", "webhook"} {
			...
			if request.Validate() == nil {
				t.Fatal("unsupported trigger validated", kind)
			}
			if _, err := f.queue.CreateSchedule(t.Context(), f.actor, "unsupported-"+kind, request); !errors.Is(err, jobs.ErrInvalid) {
				t.Fatal("unsupported trigger reached persistence", kind, err)
			}
			if _, err := f.queue.Submit(t.Context(), f.actor, "unsupported-"+kind, jobs.Submission{Kind: kind, BindingID: "reporting", Reporting: &target}); !errors.Is(err, jobs.ErrInvalid) {
				t.Fatal("unsupported executor reached persistence", kind, err)
			}
		}
		if f.calls.Load() != 0 {
			t.Fatal("unsupported kinds reached execution broker")
		}
	})
	t.Run("shared-bounded-maintenance-consumer-remains-real", func(t *testing.T) {
		...
		if err := q.service.RunOnce(t.Context()); err != nil {
			t.Fatal(err)
		}
		done, err := q.service.Get(t.Context(), q.actor, job.ID)
		if err != nil || done.State != "succeeded" || done.Batch != 1 || done.DeletedEvents != 1 || q.oldAuditCount(t) != 0 {
			t.Fatal("existing bounded cleanup was replaced by a stub", done, err)
		}
	})
}
