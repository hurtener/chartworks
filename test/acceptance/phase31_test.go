...
func newPhase31Fixture(t *testing.T, dynamic bool) *phase31Fixture {
	...
	if len(scopes) > 32 {
		t.Fatal("fixture exceeds real signed scope ceiling")
	}
	return &phase31Fixture{domain: d, service: service, registry: registry, mcp: server, handler: handler, wire: wire, scopes: scopes}
}

func (f *phase31Fixture) token(t *testing.T, user, tenant string, scopes []string, mcpAudience bool) string {
	t.Helper()
	authority := f.domain.f.model.token
	claims := authority.claims(tenant, user, scopes)
	claims["session"] = f.domain.execute.Session()
	if mcpAudience {
		claims["aud"] = authority.cfg.MCPAudience()
	}
	return authority.sign(t, claims, nil)
}

func (f *phase31Fixture) client(t *testing.T, scopes []string, mcpAudience bool) *cw.Client {
	t.Helper()
...
func (f *phase31Fixture) run(t *testing.T, kind, id, key string) reporting.ReportingRunResult {
	...
	if err != nil || out.Run == "" || (out.State != "succeeded" && out.State != "completed") {
		t.Fatalf("actual reporting run: %+v %v", out, err)
	}
	return out
}

func phase31Wire(t *testing.T, value any) []byte {
	t.Helper()
