...
func phase23Parity(t *testing.T,f *phase23Fixture) {
	...
	for _,surface:=range f.surfaces(t) {
		t.Run(surface.name,func(t *testing.T){
			...
			if len(listed)<2{t.Fatal("missing real published topics")}
			...
			if published.Definition.Topic!=f.domain.pack.Topic{t.Fatal("wrong published topic")}
			canonical,err:=json.Marshal(published)
			if err!=nil{t.Fatal(err)}
			if expectedPublished==nil{expectedPublished=canonical}else if !bytes.Equal(canonical,expectedPublished){t.Fatal("published core value differs by surface")}
			...
			if len(datasets)==0{t.Fatal("missing retained source datasets")}
			...
			if dataset.Relation.ID!=ref.Dataset||len(dataset.Relation.Columns)==0{t.Fatal("wrong actual dataset")}
			...
			if preflight.QueryID==""||preflight.Route.Context==nil{t.Fatal("preflight did not persist governed routing")}
			...
			if plan.Status!="planned"||plan.SQL!="SELECT id, amount FROM analytics.sales ORDER BY id"{t.Fatal("validated SQL differs by surface",plan.Status)}
			...
			if run.Status!="succeeded"||run.Execution.Result==nil||len(run.Execution.Result.Rows)!=2{t.Fatal("actual read did not execute")}
			data,err:=json.Marshal(run.Execution.Result.Rows)
			if err!=nil{t.Fatal(err)}
			if expectedData==nil{expectedData=data}else if !bytes.Equal(data,expectedData){t.Fatal("exact returned values differ by surface")}
			...
			if refined.QueryID==""||refined.QueryID==plan.QueryID||refined.Status!="planned"{t.Fatal("refinement lost child lineage")}
			...
			if !feedback.Accepted{t.Fatal("feedback was not retained")}
			...
			if created.Bundle==nil||created.Bundle.Reference.Context!=ref.Context{t.Fatal("missing exact opaque BYO context")}
			...
			if !step.ValuesAvailable||step.Result==nil||len(step.Result.Rows)!=2{t.Fatal("missing BYO values")}
			data,err=json.Marshal(step.Result.Rows)
			if err!=nil||!bytes.Equal(data,expectedData){t.Fatal("BYO differs from governed read",err)}
			replay:=phase23Call[nlqbyo.SubmitResult](t,surface,"submitSQL","",submission,nil)
			if !replay.Replayed||replay.ValuesAvailable||replay.Result!=nil||replay.Step.Operation!=submission.Operation{t.Fatal("receipt replay reran or invented retained values")}
			view:=phase23Call[nlqbyo.View](t,surface,"readQueryContext","",created.Bundle.Reference,nil)
			if len(view.Steps)!=1||view.Steps[0].Operation!=submission.Operation{t.Fatal("durable BYO evidence missing")}
		})
	}
}
...
func phase23Inventory(t *testing.T,f *phase23Fixture) {
	...
	if err!=nil||len(rows)!=len(registry.Definitions()){t.Fatal("full registry cannot be called by generated clients",len(rows),err)}
	byID:=map[string]cw.OperationInfo{}
	for _,row:=range rows{
		byID[row.ID]=row
		if row.Audience=="mcp" {if row.SDKMethod!="MCP"||row.CLICommand!="client mcp"{t.Fatal("MCP protocol command missing")}} else if row.SDKMethod!="Invoke"||row.CLICommand!="client call "+row.ID{t.Fatal("ordinary operation has no SDK/CLI path")}
		for _,forbidden:=range []string{"/reports","/artifacts","/dashboards","/auth","/admin","/credentials","/grants","/users"}{if strings.Contains(row.Path,forbidden){t.Fatal("unbuilt/identity route exposed",row.ID)}}
	}
	for _,definition:=range registry.Definitions(){
		row,ok:=byID[definition.ID]
		if !ok||row.Method!=definition.Method||row.Path!=definition.Path||row.Action!=definition.Action||row.Effect!=definition.Effect||row.Audit!=definition.Audit||row.ResourceLoader!=definition.ResourceLoader||row.MaxBodyBytes!=definition.MaxBodyBytes{t.Fatal("operation metadata drift",definition.ID)}
	}
	...
	if err!=nil{t.Fatal(err)}
	tools:=0
	for _,row:=range matrix{if row.MCPTool!=""{tools++}}
	if tools!=len(f.registry.Manifest()){t.Fatal("installed MCP/HTTP/SDK/CLI matrix incomplete",tools)}
	...
	for _,future:=range []string{"runReport","readArtifact","exportArtifact","createGrant"}{
		if _,err:=client.Invoke(t.Context(),future,cw.CallOptions{});!errors.Is(err,cw.ErrUnknownOperation){t.Fatal("future command synthesized",future,err)}
		code,_,_:=f.cli(t.Context(),f.httpToken,[]string{"call",future,"--execute"},nil)
		if code!=2{t.Fatal("CLI future command synthesized",future,code)}
	}
	if f.domain.model.requests.Load()!=beforeModel||f.domain.f.lookups.Load()!=beforeSource{t.Fatal("inventory or unbuilt commands performed source/model work")}
}
...
func phase23Authority(t *testing.T,f *phase23Fixture) {
	...
	if _,err:=client.DescribeDataset(ambient,request);err!=nil{t.Fatal("valid in-process authority",err)}
	beforeModel,beforeSource:=f.domain.model.requests.Load(),f.domain.f.lookups.Load()
	...
	for _,token:=range []string{"invalid",mcpToken,bare}{
		...
		if !errors.As(err,&rejected)||(rejected.Status!=401&&rejected.Status!=403){t.Fatal("ambient envelope bypassed fresh verification",err)}
	}
	if f.domain.model.requests.Load()!=beforeModel||f.domain.f.lookups.Load()!=beforeSource{t.Fatal("authority denial reached a domain dependency")}
	supplied.Store(f.httpToken)
	if _,err:=client.DescribeDataset(t.Context(),request);err!=nil{t.Fatal("caller-provided renewal not honored",err)}
	...
	for _,inProcess:=range []bool{false,true}{
		...
		var rejected *cw.StatusError
		if !errors.As(err,&rejected)||rejected.Status!=404{t.Fatal("cross-tenant data escaped",err)}
	}
	...
	if _,err:=f.sdk(t,token,true).DescribeDataset(t.Context(),request);err==nil{t.Fatal("in-process resource reach widened")}
	mcpLocal,err:=f.server.Client(func(context.Context)(string,error){return f.httpToken,nil})
	if err!=nil{t.Fatal(err)}
	if _,err:=mcpLocal.ListTools(ambient);err==nil{t.Fatal("HTTP audience used for in-process MCP")}
}
...
func phase23Command(t *testing.T,f *phase23Fixture) {
	...
	for _,args:=range [][]string{{"config"},{"operations"},{"operations","--schemas"}}{
		...
		if code!=0||!json.Valid(output)||diagnostic!=""||strings.Contains(string(output),f.httpToken){t.Fatal("injected CLI read/configuration",args,code)}
	}
	for _,args:=range [][]string{{"bootstrap"},{"users"},{"grants"},{"keys"},{"config","--token","PRIVATE_ARG"},{"call","getPublishedTopic","--id",f.domain.pack.Topic}}{
		code,output,diagnostic:=f.cli(t.Context(),f.httpToken,args,nil)
		if code!=2||strings.Contains(string(output)+diagnostic,"PRIVATE_ARG"){t.Fatal("unsafe CLI command or diagnostics",code)}
	}
	...
	if code!=3||len(output)!=0||strings.Contains(diagnostic,"not-a-token"){t.Fatal("CLI denial exit/projection",code)}
	ctx,cancel:=context.WithCancel(t.Context());cancel()
	code,_,_ = f.cli(ctx,f.httpToken,[]string{"config"},nil)
	if code!=130{t.Fatal("canceled command reported success",code)}
	if f.domain.model.requests.Load()!=before{t.Fatal("configuration/diagnostics invoked a model")}
}
...
func phase23Replay(t *testing.T,f *phase23Fixture) {
	...
	if _,err:=client.SetRetentionPolicy(t.Context(),0,7,24);err!=nil{t.Fatal(err)}
	...
	interrupted:=httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter,r *http.Request){
		if r.URL.Path==phase23Mount+"/v1/retention-sweeps"&&attempts.Add(1)==1{
			committed:=httptest.NewRecorder();ops.handler.ServeHTTP(committed,r)
			if committed.Code!=200{t.Error("fault injection did not follow an actual commit",committed.Code)}
	...
	if err!=nil||attempts.Load()!=2{t.Fatal("safe post-commit replay",attempts.Load(),err)}
	var operation cw.Operation
	if json.Unmarshal(result.Body,&operation)!=nil||operation.Status!="succeeded"{t.Fatal("missing retained completion receipt")}
	replayed,err:=client.Sweep(t.Context(),"logical-once")
	if err!=nil||replayed.ID!=operation.ID{t.Fatal("post-commit replay changed operation",err)}
	audits,err:=client.AuditEvents(t.Context())
	if err!=nil{t.Fatal(err)}
	completed:=0
	for _,event:=range audits{if event.Action=="retention.sweep"{completed++}}
	if completed!=1{t.Fatal("retries committed repeated erasure",completed)}
	var forbiddenCalls atomic.Int64
	denied:=httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter,r *http.Request){if r.URL.Path==phase23Mount+"/v1/retention-policy"{forbiddenCalls.Add(1)};ops.handler.ServeHTTP(w,r)}))
	...
	var rejected *cw.StatusError
	if !errors.As(err,&rejected)||rejected.Status!=401||forbiddenCalls.Load()!=1{t.Fatal("authentication denial was replayed",err)}
	ctx,cancel:=context.WithCancel(t.Context());cancel()
	if _,err=client.Invoke(ctx,"sweepRetention",cw.CallOptions{IdempotencyKey:"canceled",Attempts:3});!errors.Is(err,context.Canceled){t.Fatal("cancellation was not preserved",err)}
	...
	limits:=config.DefaultQueryBundles();limits.TTL=config.Duration(time.Second)
	...
	if err!=nil{t.Fatal(err)}
	if _,err=expired.ReadQueryContext(t.Context(),bundle.Reference);!errors.As(err,&rejected)||rejected.Status!=409{t.Fatal("expired reference did not require explicit replan",err)}
	if _,err=expired.SubmitSQL(t.Context(),cw.SQLSubmission{Reference:bundle.Reference,Operation:"expired",SQL:"SELECT id FROM analytics.sales"});!errors.As(err,&rejected)||rejected.Status!=409{t.Fatal("expired submission revived work",err)}
	if f.domain.model.requests.Load()!=beforeModel||f.reader.calls.Load()!=beforeReads{t.Fatal("expiry denial incurred fresh work")}
}

func phase23Operational(t *testing.T,f *phase23Fixture)(*phase23Fixture,string){
	t.Helper()
	dsn:=support.Database(t);db:=support.Open(t,dsn)
	service,err:=securityapi.New(db);if err!=nil{t.Fatal(err)}
	public,err:=foundation.PublicRegistry();if err!=nil{t.Fatal(err)}
	security,err:=securityapi.APIRegistry(true);if err!=nil{t.Fatal(err)}
	registry,err:=api.Compose(public,security);if err!=nil{t.Fatal(err)}
	reporter,err:=telemetry.New(io.Discard,"json",true);if err!=nil{t.Fatal(err)}
	...
	outer,err:=foundation.NewServerWithRegistry(cfg,reporter,func(ctx context.Context)foundation.Dependency{return foundation.Dependency{Ready:db.Check(ctx)==nil}},f.authority.verifier.Check,registry,handler)
	if err!=nil{t.Fatal(err)}
	copy:=*f;copy.handler=outer.Handler();copy.security=service;copy.httpRegistry=registry;copy.wire=httptest.NewServer(copy.handler)
...
func phase23Erasure(t *testing.T,f *phase23Fixture){
	...
	for _,tenant:=range []string{"erase-a","erase-b"}{
		...
		tokens[tenant]=token;clients[tenant]=ops.sdk(t,token,tenant=="erase-a")
		if _,err:=clients[tenant].SetRetentionPolicy(t.Context(),0,7,24);err!=nil{t.Fatal(err)}
	}
	raw:=support.Raw(t,dsn)
	for _,tenant:=range []string{"erase-a","erase-b"}{
		if _,err:=raw.Exec(t.Context(),`INSERT INTO chartworks.audit_events(tenant_id,event_id,actor_id,action,resource_id,created_at) VALUES($1,repeat('a',32),'synthetic-seed','retention_policy.updated','retention',clock_timestamp()-interval '30 days')`,tenant);err!=nil{t.Fatal(err)}
	}
	beforeA,err:=clients["erase-a"].AuditEvents(t.Context());if err!=nil{t.Fatal(err)}
	beforeB,err:=clients["erase-b"].AuditEvents(t.Context());if err!=nil{t.Fatal(err)}
	for _,tenant:=range []string{"erase-a","erase-b"}{
		code,output,_:=ops.cli(t.Context(),tokens[tenant],[]string{"diagnostics"},nil)
		if code!=0||!json.Valid(output)||strings.Contains(string(output),tokens[tenant]){t.Fatal("scoped read-only diagnostics",code)}
	}
	afterA,_:=clients["erase-a"].AuditEvents(t.Context());afterB,_:=clients["erase-b"].AuditEvents(t.Context())
	if !reflect.DeepEqual(beforeA,afterA)||!reflect.DeepEqual(beforeB,afterB){t.Fatal("diagnostics changed durable audit state")}
	...
	if code!=3{t.Fatal("explicit CLI erasure manufactured authority",code)}
	foreign:=f.token(t,"erase-a","svc:erase-a","phase23-erasure",operationalScopes("erase-b"),false)
	if _,err:=ops.sdk(t,foreign,true).Sweep(t.Context(),"foreign");err==nil{t.Fatal("cross-tenant erasure accepted")}
	...
	if code!=0{t.Fatal("explicit authorized CLI erasure",code,diagnostic)}
	var receipt cw.Operation
	if json.Unmarshal(output,&receipt)!=nil||receipt.DeletedEvents!=1||receipt.Status!="succeeded"{t.Fatal("actual bounded erasure receipt",receipt)}
	afterB,err=clients["erase-b"].AuditEvents(t.Context())
	if err!=nil||!reflect.DeepEqual(beforeB,afterB){t.Fatal("erasure affected foreign tenant",err)}
	var remaining int
	if err:=raw.QueryRow(t.Context(),`SELECT count(*) FROM chartworks.audit_events WHERE tenant_id='erase-a' AND event_id=repeat('a',32)`).Scan(&remaining);err!=nil||remaining!=0{t.Fatal("authorized row was not erased",err)}
	replayed,err:=clients["erase-a"].Sweep(t.Context(),"erase-once")
	if err!=nil||replayed.ID!=receipt.ID{t.Fatal("CLI/SDK logical erasure key drift",err)}
	other,err:=clients["erase-b"].Sweep(t.Context(),"erase-once")
	if err!=nil||other.ID==receipt.ID||other.DeletedEvents!=1{t.Fatal("logical keys not tenant isolated",err)}
}
