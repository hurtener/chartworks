...
func (f *phase30Fixture) manager(t *testing.T) identity.Envelope {
	...
	if len(scopes) > 32 {
		t.Fatalf("schedule fixture exceeds real signed scope ceiling: %d", len(scopes))
	}
...
func (f *phase30Fixture) schedule(t *testing.T, key string, request jobs.ScheduleRequest) jobs.Schedule {
	...
	if err != nil {
		t.Fatal("create reporting schedule", err)
	}
	return s
}
...
func (f *phase30Fixture) occurrence(t *testing.T, schedule string, due time.Time) jobs.Job {
	t.Helper()
	var id string
	if err := support.Raw(t, f.domain.f.f.dsn).QueryRow(t.Context(), `SELECT operation_id FROM chartworks.job_occurrences WHERE tenant_id=$1 AND schedule_id=$2 AND due_at=$3`, f.actor.Tenant(), schedule, due).Scan(&id); err != nil {
		t.Fatal(err)
	}
	return f.get(t, id)
}
...
func testPhase30Calendars(t *testing.T) {
	t.Run("real-cron-and-interval-calendar", func(t *testing.T) {
		for _, test := range []struct{ name, cron, zone, after, first, second string }{
			...
			t.Run(test.name, func(t *testing.T) {
				...
				if err != nil || !first.Equal(phase30Time(t, test.first)) {
					t.Fatal("first calendar occurrence", first, err)
				}
				second, err := spec.Next(first)
				if err != nil || !second.Equal(phase30Time(t, test.second)) || !second.After(first) {
					t.Fatal("second calendar occurrence", second, err)
				}
				previous, err := spec.Previous(second.Add(-time.Microsecond))
				if err != nil || !previous.Equal(first) {
					t.Fatal("half-open calendar predecessor", previous, err)
				}
			})
		}
		...
		if err != nil || !at.Equal(phase30Time(t, "2024-03-10T07:30:00Z")) {
			t.Fatal("elapsed interval was treated as a civil hour", at, err)
		}
	})
	t.Run("parameter-core-calendar-contract", func(t *testing.T) {
		for _, test := range []struct {
			name, mode, unit, zone, at, start, end string
			invalid                                bool
		}{
			...
			t.Run(test.name, func(t *testing.T) {
				...
				if test.invalid {
					if !errors.Is(err, reporting.ErrInvalid) {
						t.Fatal("ambiguous/gap policy ignored", err)
					}
					return
				}
				if err != nil || len(resolved.Parameters) != 2 || len(resolved.Values) != 1 || resolved.Values[0].Window == nil {
					t.Fatal("period not bound by existing core", resolved, err)
				}
				w := resolved.Values[0].Window
				if !w.Start.Equal(phase30Time(t, test.start)) || !w.End.Equal(phase30Time(t, test.end)) {
					t.Fatal("calendar window", w)
				}
			})
		}
	})
	t.Run("accepted-window-survives-delay-retry-and-read", func(t *testing.T) {
		...
		if err != nil || admitted != 1 {
			t.Fatal("real bounded catch-up", admitted, err)
		}
		job := f.occurrence(t, schedule.ID, due)
		if !job.WindowStart.Equal(start) || !job.WindowEnd.Equal(due) || job.WindowEnd.Sub(job.WindowStart) != 23*time.Hour {
			t.Fatal("accepted window lost civil DST semantics", job)
		}
		f.mode.Store(2)
		if err := f.queue.RunOnce(t.Context()); err == nil {
			t.Fatal("issuer outage was hidden")
		}
		f.retryNow(t, job.ID)
		f.mode.Store(0)
		if err := f.queue.RunOnce(t.Context()); err != nil {
			t.Fatal("delayed real report", err, f.get(t, job.ID))
		}
		done := f.get(t, job.ID)
		if done.State != "succeeded" || done.ManifestHash != job.ManifestHash || !done.WindowStart.Equal(start) || !done.DueAt.Equal(due) {
			t.Fatal("retry floated the accepted window", done)
		}
		...
		if err != nil || record.Manifest == nil || len(record.Manifest.Resolved.Values) != 1 || record.Manifest.Resolved.Values[0].Window == nil {
			t.Fatal("missing actual retained bind proof", err)
		}
		w := record.Manifest.Resolved.Values[0].Window
		if !w.Start.Equal(start) || !w.End.Equal(due) || !record.Manifest.Resolved.At.Equal(due) {
			t.Fatal("domain consumer re-resolved against retry time", w)
		}
		history, err := f.queue.History(t.Context(), f.controls(t), schedule.ID, jobs.ScheduleHistoryRequest{Kind: "occurrences", Limit: 100})
		if err != nil || len(history.Occurrences) < 2 {
			t.Fatal("missed range was not retained", history, err)
		}
		skipped := false
		for _, occurrence := range history.Occurrences {
			if occurrence.SkippedThrough != nil {
				skipped = true
			}
		}
		if !skipped {
			t.Fatal("catch-up truncation silently discarded missed time")
		}
	})
	t.Run("first-occurrence-is-explicit", func(t *testing.T) {
		...
		if err := f.queue.RunOnce(t.Context()); err == nil {
			t.Fatal("first window was invented")
		}
		if done := f.get(t, job.ID); done.State != "blocked" || done.ErrorCode != "reporting_attention" || f.domain.attemptCount(t) != before {
			t.Fatal("first policy reached SQL", done)
		}
		...
		if err := f.queue.RunOnce(t.Context()); err != nil {
			t.Fatal("explicit first-window execution", err)
		}
		if done := f.get(t, job.ID); done.State != "succeeded" || f.domain.attemptCount(t) != before+1 {
			t.Fatal("explicit fallback was not executed", done)
		}
	})
}
