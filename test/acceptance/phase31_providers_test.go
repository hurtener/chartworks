...
func testPhase31AppsDiscovery(t *testing.T) {
	...
	for _, tool := range tools.Tools {
		...
		if tool.Name == "reporting_run" || tool.Name == "reporting_view" {
			...
			if err := json.Unmarshal(tool.Meta["ui"], &ui); err != nil || ui.URI != reportviewer.URI || !reflect.DeepEqual(ui.Visibility, []string{"model", "app"}) {
				t.Fatalf("established Apps metadata: %+v %v", ui, err)
			}
		}
	}
	...
	if err := json.Unmarshal(phase22RPC(t, client, "resources/list", map[string]any{}), &resources); err != nil {
		t.Fatal(err)
	}
	found := 0
	for _, resource := range resources.Resources {
		if resource.URI == reportviewer.URI {
			found++
			if resource.MIME != mcpserver.AppMIME {
				t.Fatal("wrong Apps profile")
			}
		}
	}
	if found != 1 {
		t.Fatal("bundled app was missing or duplicated", found)
	}
	var resource struct {
		Contents []struct {
			URI  string `json:"uri"`
	...
	if err := json.Unmarshal(phase22RPC(t, client, "resources/read", map[string]any{"uri": reportviewer.URI}), &resource); err != nil {
		t.Fatal(err)
	}
	if len(resource.Contents) != 1 || resource.Contents[0].Text != reportviewer.HTML() || resource.Contents[0].MIME != mcpserver.AppMIME || resource.Contents[0].URI != reportviewer.URI {
		t.Fatal("provider did not serve the actual versioned component")
	}
	for _, domains := range resource.Contents[0].Meta.UI.CSP {
		if len(domains) != 0 {
			t.Fatal("bundled component unexpectedly requests network origins")
		}
	}
	if len(resource.Contents[0].Meta.UI.CSP) != 4 || len(resource.Contents[0].Meta.UI.Permissions) != 0 {
		t.Fatal("CSP or permissions metadata missing")
	}
	unprivileged := f.client(t, []string{"mcp.use"}, true)
	if err := json.Unmarshal(phase22RPC(t, unprivileged, "tools/list", map[string]any{}), &tools); err != nil || len(tools.Tools) != 0 {
		t.Fatal("tool visibility bypassed current actions", err)
	}
	for _, uri := range []string{reportviewer.URI, reportviewer.URI + "?token=ignored", "ui://chartworks/report-viewer/../../secret"} {
		...
		if json.Unmarshal(raw, &denial) != nil || denial.Error == nil || denial.Result != nil || !strings.Contains(denial.Error.Message, "not_found") {
			t.Fatalf("unauthorized/aliased resource read: %s", raw)
		}
	}
	...
	if beforeSource != f.domain.f.f.lookups.Load() || beforeModels != f.domain.f.model.requests.Load() {
		t.Fatal("tool/resource discovery executed data or model work")
	}
}
...
func testPhase31CatalogAndRetainedReads(t *testing.T) {
	...
	for _, payload := range []any{search, description} {
		...
		for _, forbidden := range []string{"P31_PRIVATE_SQL_CANARY", "SELECT id", `"sql":`, `"credentials":`, `"prompt":`} {
			if strings.Contains(wire, forbidden) {
				t.Fatal("catalog leaked executable/private data", forbidden)
			}
		}
	}
	if beforeSource != f.domain.f.f.lookups.Load() || beforeModels != f.domain.f.model.requests.Load() {
		t.Fatal("catalog reads resolved a source/model")
	}
	beforeAttempts := f.domain.attemptCount(t)
	blockRun := f.run(t, "block", "p31-visible", "p31-block-run")
	if f.domain.attemptCount(t) != beforeAttempts+1 {
		t.Fatal("block run did not execute exactly one actual bounded query")
	}
	beforeSource, beforeModels = f.domain.f.f.lookups.Load(), f.domain.f.model.requests.Load()
	for _, output := range []string{"table-second", "table-main"} {
		...
		if first.Trust == nil || first.Observed == nil || first.Timezone != "UTC" {
	...
	if beforeSource != f.domain.f.f.lookups.Load() || beforeModels != f.domain.f.model.requests.Load() {
		t.Fatal("paging or catalog read re-executed source/model work")
	}
	...
	if beforeSource != f.domain.f.f.lookups.Load() || beforeModels != f.domain.f.model.requests.Load() {
		t.Fatal("composition projection called source/model")
	}
...
func testPhase31ArtifactStates(t *testing.T) {
	...
	if beforeSource != f.domain.f.f.lookups.Load() || beforeModels != f.domain.f.model.requests.Load() {
		t.Fatal("private/expired state reads executed work")
	}
}
