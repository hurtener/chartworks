...
func testPhase31ExplicitExecution(t *testing.T) {
	...
	if err != nil || first.State != "succeeded" || f.domain.attemptCount(t) != before+1 {
		t.Fatal("initial actual execution", first, err)
	}
	...
	if err != nil || second.Run == first.Run || f.domain.attemptCount(t) != before+1 {
	...
	before = f.domain.attemptCount(t)
	if replay, err := f.service.Run(t.Context(), f.domain.execute, request); err != nil || replay.Run != second.Run || f.domain.attemptCount(t) != before {
		t.Fatal("idempotent receipt re-executed", replay, err)
	}
	request.Key = "p31-read-only-attempt"
	if _, err := f.service.Run(t.Context(), reader, request); !errors.Is(err, access.ErrForbidden) || f.domain.attemptCount(t) != before {
		t.Fatal("read-only/viewer authority could execute", err)
	}
	request.Key = "p31-invalid-filter"
	request.Arguments[0].Value.Literal = "3"
	if _, err := f.service.Run(t.Context(), f.domain.execute, request); !errors.Is(err, reporting.ErrInvalid) || f.domain.attemptCount(t) != before {
		t.Fatal("out-of-domain filter reached warehouse", err)
	}
	...
	if err != nil || oldRun.Target.Revision != 1 || f.domain.f.model.requests.Load() != beforeModels {
		t.Fatal("publication race widened dynamic execution", oldRun, err)
	}
	pinned.Target.Revision = 2
	pinned.Key = "p31-consent-required"
	before = f.domain.attemptCount(t)
	if _, err := f.service.Run(t.Context(), f.domain.execute, pinned); !errors.Is(err, reporting.ErrInvalid) || f.domain.attemptCount(t) != before || f.domain.f.model.requests.Load() != beforeModels {
		t.Fatal("dynamic model generation lacked explicit consent", err)
	}
	pinned.Dynamic = true
	pinned.Key = "p31-consent-granted"
	dynamic, err := f.service.Run(t.Context(), f.domain.execute, pinned)
	if err != nil || dynamic.State != "completed" || f.domain.f.model.requests.Load() <= beforeModels {
	...
	if beforeSource != f.domain.f.f.lookups.Load() || beforeModels != f.domain.f.model.requests.Load() {
		t.Fatal("view regenerated a dynamic question")
	}
}
...
func phase31Equivalent(t *testing.T, first, second any, label string) {
	t.Helper()
	var a, b any
	if err := json.Unmarshal(phase31Wire(t, first), &a); err != nil {
		t.Fatal(err)
	}
	if err := json.Unmarshal(phase31Wire(t, second), &b); err != nil {
		t.Fatal(err)
	}
	if !reflect.DeepEqual(a, b) {
		t.Fatalf("%s provider contract diverged\n%s\n%s", label, phase31Wire(t, a), phase31Wire(t, b))
	}
}
...
func testPhase31ProviderParity(t *testing.T) {
	...
	if err := json.Unmarshal(phase22RPC(t, mcpClient, "tools/call", map[string]any{"name": "reporting_view", "arguments": selection}), &fallback); err != nil {
		t.Fatal(err)
	}
	if len(fallback.Content) != 1 || fallback.Content[0].Type != "text" {
		t.Fatal("text-only host fallback absent")
	}
	...
	if !failure.IsError || !bytes.Contains(failure.Structured, []byte(`"code":"not_found"`)) {
		t.Fatal("MCP missing-selection code", string(failure.Structured))
	}
	if _, err = httpClient.ViewReporting(t.Context(), selection); err == nil {
		t.Fatal("SDK missing selection unexpectedly succeeded")
	} else {
		var status *cw.StatusError
		if !errors.As(err, &status) || status.Status != http.StatusNotFound {
			t.Fatal("SDK missing-selection class", err)
		}
	}
	if beforeSource != f.domain.f.f.lookups.Load() || beforeModels != f.domain.f.model.requests.Load() {
		t.Fatal("portable view/list/error reads invoked a source or model")
	}
}
...
func testPhase31ProviderBounds(t *testing.T) {
	...
	for _, selection := range []reporting.ReportingViewRequest{
		...
		if !errors.Is(err, reporting.ErrInvalid) || view.Output != nil || view.Text != nil {
			t.Fatal("invalid selection produced data", view, err)
		}
	}
	...
	if beforeSource != f.domain.f.f.lookups.Load() || beforeModels != f.domain.f.model.requests.Load() {
		t.Fatal("provider failure path executed a source/model")
	}
}
