...
func publicDocument(t testing.TB, key *ecdsa.PrivateKey, kid string) []byte {
	t.Helper()
	b, err := json.Marshal(map[string]any{"keys": []any{map[string]any{"kty": "EC", "use": "sig", "alg": "ES256", "kid": kid, "crv": "P-256", "x": base64.RawURLEncoding.EncodeToString(key.X.FillBytes(make([]byte, 32))), "y": base64.RawURLEncoding.EncodeToString(key.Y.FillBytes(make([]byte, 32)))}}})
	if err != nil {
		t.Fatal(err)
	}
	return b
...
func newTokenFixture(t testing.TB) *tokenFixture {
	...
	f := &tokenFixture{private: key, status: 200}
	f.clock.Store(time.Now().Unix())
	f.doc = publicDocument(t, key, "test-key")
	...
	f.cfg = config.Defaults().Auth
	f.cfg.Issuer = f.server.URL + "/issuer"
	f.cfg.JWKSURL = f.server.URL + "/keys"
	f.cfg.Audience = ""
	f.cfg.Audiences = config.Audiences{HTTP: "chartworks:http", MCP: "chartworks:mcp"}
	f.cfg.RefreshInterval = config.Duration(time.Second)
	f.cfg.JWKSMaxStale = config.Duration(10 * time.Second)
	f.verifier, err = auth.New(f.cfg, f.server.Client(), func() time.Time { return time.Unix(f.clock.Load(), 0) })
	if err != nil {
		t.Fatal(err)
	}
...
func (f *tokenFixture) set(doc []byte, status int) {
	f.mu.Lock()
	defer f.mu.Unlock()
	f.doc = append([]byte(nil), doc...)
	f.status = status
}
...
func (f *tokenFixture) sign(t testing.TB, claims jwt.MapClaims, header map[string]any) string {
	...
	s, err := token.SignedString(f.private)
	if err != nil {
		t.Fatal(err)
	}
	return s
...
func (f *tokenFixture) envelope(t testing.TB, tenant, user string, scopes ...string) identity.Envelope {
	t.Helper()
	s := f.sign(t, f.claims(tenant, user, scopes), nil)
	e, err := f.verifier.Verify(context.Background(), s, auth.HTTP)
	if err != nil {
		t.Fatal(err)
	}
	return e
