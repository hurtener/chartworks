...
func TestReadStoreBoundsAndAtomicAdmission(t *testing.T) {
	...
	for _, call := range []func() error{
		...
		if !errors.Is(call(), store.ErrScope) {
			t.Fatal("unscoped attempt store access")
		}
	}
	for _, call := range []func() error{
		...
		if !errors.Is(call(), store.ErrInvalid) {
			t.Fatal("invalid attempt coordinates accepted")
		}
	}
	...
	if err = f.db.BeginRead(ctx, scope, a, 3); err == nil {
		t.Fatal("failed audit allowed partial admission")
	}
	if _, err = f.db.GetRead(ctx, scope, a.ID); !errors.Is(err, store.ErrNotFound) {
		t.Fatal("partial attempt remained", err)
	}
	sql(t, meta, `DROP TRIGGER fail_read_admission ON chartworks.audit_events`)
	if err = f.db.BeginRead(ctx, scope, a, 3); err != nil {
		t.Fatal(err)
	}
	if err = f.db.DispatchRead(ctx, scope, a.ID, q, false); err != nil {
		t.Fatal(err)
	}
	if err = f.db.DispatchRead(ctx, scope, a.ID, q, true); err != nil {
		t.Fatal(err)
	}
	if err = f.db.DispatchRead(ctx, scope, a.ID, q, true); !errors.Is(err, readexec.ErrCancelled) {
		t.Fatal("dispatch acknowledgment replayed", err)
	}
	bad := a
	bad.Status = "invented"
	bad.Finished = &now
	if err = f.db.FinishRead(ctx, scope, bad, false); !errors.Is(err, store.ErrInvalid) {
		t.Fatal("invented terminal state")
	}
	bad.Status = "succeeded"
	bad.RemoteState = "stopped"
	if err = f.db.FinishRead(ctx, scope, bad, true); !errors.Is(err, store.ErrInvalid) {
		t.Fatal("reconciliation invented successful values")
	}
	bad.Status = "failed"
	bad.Code = "source_unavailable"
	if err = f.db.FinishRead(ctx, scope, bad, false); err != nil {
		t.Fatal(err)
	}
	if err = f.db.CancelRead(ctx, scope, a.ID); !errors.Is(err, store.ErrConflict) {
		t.Fatal("terminal cancellation reopened receipt", err)
	}
	if err = f.db.FinishRead(ctx, scope, bad, false); !errors.Is(err, store.ErrConflict) {
		t.Fatal("terminal receipt overwritten", err)
	}
	gap := a
	gap.Number = 3
	gap.ID = strings.Repeat("d", 32)
	if err = f.db.BeginRead(ctx, scope, gap, 3); !errors.Is(err, store.ErrConflict) {
		t.Fatal("attempt numbering skipped", err)
	}
	first := a
	first.Manifest.Operation = "missing-first"
	first.Number = 2
	first.ID = strings.Repeat("e", 32)
	if err = f.db.BeginRead(ctx, scope, first, 3); !errors.Is(err, store.ErrConflict) {
		t.Fatal("attempt without predecessor", err)
	}
}
