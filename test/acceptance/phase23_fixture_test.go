...
func newPhase23Fixture(t *testing.T) *phase23Fixture {
	...
	protected = api.Guard(f.authority.verifier, registry, http.HandlerFunc(func(w http.ResponseWriter,r *http.Request) {
		if r.URL.Path == mcpserver.Path { f.server.Handler().ServeHTTP(w,r); return }
	...
	outer, err := foundation.NewServerWithRegistry(cfg, reporter,
		func(ctx context.Context) foundation.Dependency { return foundation.Dependency{Ready:f.domain.f.db.Check(ctx)==nil} },
...
func (f *phase23Fixture) cli(ctx context.Context, token string, args []string, input []byte) (int,[]byte,string) {
	...
	lookup := func(key string)(string,bool) {
		switch key {
		case "CHARTWORKS_CLIENT_URL":return f.wire.URL+phase23Mount,true
		case "CHARTWORKS_TOKEN":return token,true
		}
		return "",false
	}
	code := foundation.CommandWithInput(ctx,append([]string{"client"},args...),lookup,bytes.NewReader(input),&stdout,&stderr,foundation.Build{},func(context.Context,config.Config,io.Writer)error{return errors.New("client unexpectedly started a service")})
	return code,stdout.Bytes(),stderr.String()
}

type phase23Surface struct { name string; call func(context.Context,string,cw.CallOptions,any)(json.RawMessage,error) }
...
func phase23Typed(ctx context.Context,client *cw.Client,id string,options cw.CallOptions)(json.RawMessage,error) {
	switch id {
	case "listTopics":return phase23Encode(ctx,options.Body,client.ListTopics)
	case "listDatasets":return phase23Encode(ctx,options.Body,client.ListDatasets)
	case "describeDataset":return phase23Encode(ctx,options.Body,client.DescribeDataset)
	case "getPublishedTopic":
		out,err:=client.PublishedTopic(ctx,options.ResourceID);if err!=nil{return nil,err};return json.Marshal(out)
	case "preflightNLQ":return phase23Encode(ctx,options.Body,client.PreflightNLQ)
	case "planNLQ":return phase23Encode(ctx,options.Body,client.PlanNLQ)
	case "runNLQ":return phase23Encode(ctx,options.Body,client.RunNLQ)
	case "refineNLQ":return phase23Encode(ctx,options.Body,client.RefineNLQ)
	case "feedbackNLQ":
	...
	case "getQueryContext":return phase23Encode(ctx,options.Body,client.GetQueryContext)
	case "readQueryContext":return phase23Encode(ctx,options.Body,client.ReadQueryContext)
	case "submitSQL":return phase23Encode(ctx,options.Body,client.SubmitSQL)
	}
...
func (f *phase23Fixture) surfaces(t *testing.T) []phase23Surface {
	...
	if err!=nil{t.Fatal("live operation matrix",err)}
	rows:=map[string]cw.OperationInfo{}
	for _,row:=range matrix{rows[row.ID]=row}
	rawHTTP:=func(ctx context.Context,id string,options cw.CallOptions,_ any)(json.RawMessage,error){
		row:=rows[id]
		if row.ID==""{return nil,cw.ErrUnknownOperation}
		...
		if response.StatusCode!=200{return nil,&cw.StatusError{Status:response.StatusCode}}
		return io.ReadAll(io.LimitReader(response.Body,32<<20))
	}
	unpack:=func(structured []byte,failed bool)(json.RawMessage,error){
		if failed{return nil,errors.New("MCP owner rejected acceptance request: "+string(structured))}
		var result struct{Result json.RawMessage `json:"result"`}
		if err:=json.Unmarshal(structured,&result);err!=nil{return nil,err}
		if result.Result==nil{return nil,errors.New("missing MCP core result")}
		return result.Result,nil
	}
	out:=[]phase23Surface{{name:"http",call:rawHTTP}}
	for _,entry:=range []struct{name string;client *cw.Client}{{"sdk-http",network},{"sdk-local",local}}{
		out=append(out,phase23Surface{name:entry.name,call:func(ctx context.Context,id string,options cw.CallOptions,_ any)(json.RawMessage,error){return phase23Typed(ctx,entry.client,id,options)}})
		out=append(out,phase23Surface{name:entry.name+"-generic",call:func(ctx context.Context,id string,options cw.CallOptions,_ any)(json.RawMessage,error){result,err:=entry.client.Invoke(ctx,id,options);return result.Body,err}})
	}
	out=append(out,phase23Surface{name:"mcp-http",call:func(ctx context.Context,id string,_ cw.CallOptions,args any)(json.RawMessage,error){
		...
		if err!=nil{return nil,err}
		raw,err:=mcpNetwork.MCP(ctx,body);if err!=nil{return nil,err}
		var rpc struct{Result phase22ToolResult `json:"result"`;Error json.RawMessage `json:"error"`}
		if json.Unmarshal(raw,&rpc)!=nil||rpc.Error!=nil{return nil,errors.New("MCP protocol rejected acceptance request")}
		...
		body,err:=json.Marshal(args);if err!=nil{return nil,err}
		result,err:=mcpLocal.CallTool(ctx,rows[id].MCPTool,body);if err!=nil{return nil,err}
		raw,err:=json.Marshal(result.StructuredContent);if err!=nil{return nil,err}
		...
		if options.ResourceID!=""{args=append(args,"--id",options.ResourceID)}
		if options.Body!=nil{args=append(args,"--input","-")}
		code,body,diagnostic:=f.cli(ctx,f.httpToken,args,options.Body)
		if code!=0{return nil,errors.New("CLI acceptance failed: "+diagnostic)}
		return body,nil
	}})
	return out
}
func phase23Call[Output any](t *testing.T,surface phase23Surface,id,resource string,input,mcpInput any) Output {
	...
	var raw []byte
	var err error
	if input!=nil{raw,err=json.Marshal(input);if err!=nil{t.Fatal(err)}}
	if mcpInput==nil{mcpInput=input}
	body,err:=surface.call(t.Context(),id,cw.CallOptions{ResourceID:resource,Body:raw},mcpInput)
	if err!=nil{t.Fatalf("%s/%s: %v",surface.name,id,err)}
	var output Output
	if err=json.Unmarshal(body,&output);err!=nil{t.Fatal("core output decode",err)}
	return output
}
