...
func configBytes(t testing.TB, change func(map[string]any)) []byte {
	t.Helper()
	m := map[string]any{"auth": map[string]any{"issuer": "https://issuer.example", "jwks_url": "https://issuer.example/keys", "audience": "chartworks-test"}}
	if change != nil {
		change(m)
	}
	b, e := json.Marshal(m)
...
func environment(dsn string) func(string) (string, bool) {
	return func(k string) (string, bool) {
		if k == "CHARTWORKS_STORE_URL" {
			return dsn, true
		}
		return "", false
	}
}
func loaded(t testing.TB, b []byte, dsn string) config.Config {
	t.Helper()
	c, e := config.Load(bytes.NewReader(b), environment(dsn), config.Overrides{})
	if e != nil {
		t.Fatal(e)
	}
	return c
...
func keyDocument(t testing.TB) []byte {
	...
	b, e := json.Marshal(map[string]any{"keys": []any{map[string]any{"kty": "EC", "kid": "fixture", "crv": "P-256", "alg": "ES256", "use": "sig", "x": base64.RawURLEncoding.EncodeToString(k.X.FillBytes(make([]byte, 32))), "y": base64.RawURLEncoding.EncodeToString(k.Y.FillBytes(make([]byte, 32)))}}})
	if e != nil {
		t.Fatal(e)
	}
	return b
...
func response(t testing.TB, client *http.Client, address, path string) (int, string) {
	t.Helper()
	r, e := client.Get(address + path)
	if e != nil {
		return 0, ""
	}
	defer func() { _ = r.Body.Close() }()
	b, e := io.ReadAll(io.LimitReader(r.Body, 8192))
	if e != nil {
		t.Fatal("read health response failed")
	}
...
func TestPhase01(t *testing.T) {
	t.Run("AC01", func(t *testing.T) {
		...
		for _, b := range [][]byte{[]byte(`{"auth":{"mode":"self_issue"}}`), []byte(`{"server":{"listen":"` + canary + `","unknown":true}}`), []byte(`{"auth":null}`), []byte(`{"auth":{},"auth":{}}`), []byte(`[]`), []byte(`{} {}`), []byte(`{"server":{"read_timeout":123}}`), []byte(`{"server":{"read_timeout":"` + canary + `"}}`), bytes.Repeat([]byte("x"), (1<<20)+1)} {
			_, e := config.Load(bytes.NewReader(b), environment(canary), config.Overrides{})
			if e == nil || strings.Contains(e.Error(), canary) {
				t.Fatal("unsafe or accepted invalid configuration")
			}
		}
		if _, e := config.Load(badReader{}, environment(canary), config.Overrides{}); e == nil || strings.Contains(e.Error(), canary) {
		...
		if strings.Contains(string(b)+fmt.Sprintf("%v %#v", c, c), canary) {
			t.Fatal("configuration printed a resolved credential")
		}
	})
	t.Run("AC02", func(t *testing.T) {
		...
		defer keys.Close()
		b := configBytes(t, func(m map[string]any) {
			m["auth"].(map[string]any)["jwks_url"] = keys.URL
			m["auth"].(map[string]any)["refresh_interval"] = "20ms"
			m["auth"].(map[string]any)["jwks_max_stale"] = "100ms"
		...
		probe := foundation.NewKeyProbe(c.Values().Auth, keys.Client())
		s, e := foundation.NewServer(c, r, func(ctx context.Context) foundation.Dependency {
		...
		fail.Store(true)
		waitFor(t, func() bool {
			status, body := response(t, client, address, "/readyz")
			return status == 503 && strings.Contains(body, "verification_keys")
		})
		if status, _ := response(t, client, address, "/healthz"); status != 200 {
			t.Fatal("dependency outage killed liveness")
		}
		fail.Store(false)
		waitFor(t, func() bool { status, _ := response(t, client, address, "/readyz"); return status == 200 })
		db.Close()
		waitFor(t, func() bool {
			status, body := response(t, client, address, "/readyz")
			return status == 503 && strings.Contains(body, `"store":"unavailable"`)
		})
		cancel()
		select {
		case e := <-done:
		...
		if status, _ := response(t, client, address, "/healthz"); status != 0 {
			t.Fatal("listener survived shutdown")
		}
		if _, e = foundation.NewServer(config.Config{}, r, nil, nil); e == nil {
	...
	t.Run("AC03", func(t *testing.T) {
		...
		for _, event := range []telemetry.Event{telemetry.Started, telemetry.Stopped, telemetry.ConfigurationRejected, telemetry.DependencyChanged, telemetry.AuditCommitted, telemetry.Request} {
			for _, ok := range []bool{true, false} {
				if e = r.Record(context.Background(), event, ok); e != nil {
					t.Fatal(e)
				}
			}
		}
		if e = r.Record(context.Background(), telemetry.Event(canary), true); e == nil || strings.Contains(e.Error(), canary) {
			t.Fatal("unbounded event accepted/leaked")
		}
		if e = r.Dependency(canary, true); e == nil || strings.Contains(e.Error(), canary) {
			t.Fatal("unbounded metric label")
		}
		if e = r.Dependency("store", true); e != nil {
			t.Fatal(e)
		}
		...
		for _, name := range telemetry.MetricNames() {
			if !strings.Contains(w.Body.String(), name) {
				t.Fatalf("registered metric not exported: %s", name)
			}
		}
	...
	t.Run("AC04", func(t *testing.T) {
		...
		s, e := foundation.NewServer(c, r, func(context.Context) foundation.Dependency { return foundation.Dependency{} }, func(context.Context) foundation.Dependency { return foundation.Dependency{} })
		if e != nil {
			t.Fatal(e)
		}
		for _, test := range []struct {
			...
			code               int
		}{{"GET", "/healthz", "", 200}, {"HEAD", "/healthz", "", 200}, {"GET", "/readyz", "", 503}, {"GET", "/capabilities", "", 200}, {"POST", "/healthz", "", 405}, {"GET", "/healthz", "x", 400}, {"GET", "/v1/admin/keys", "", 404}, {"GET", "/metrics", "", 404}, {"GET", "/mcp", "", 404}} {
			w := httptest.NewRecorder()
			q := httptest.NewRequest(test.method, test.path, strings.NewReader(test.body))
			q.Header.Set("Authorization", "Bearer "+canary)
			...
			if test.path == "/capabilities" && !strings.Contains(w.Body.String(), `"business_api":false`) {
				t.Fatal("unimplemented capability advertised")
			}
		}
		q := httptest.NewRequest("GET", "/healthz", nil)
		q.ContentLength = 100 << 20
		w := httptest.NewRecorder()
		s.Handler().ServeHTTP(w, q)
		if w.Code != 413 {
			t.Fatal("body limit not applied")
		}
		if e = s.Serve(context.Background(), nil); e == nil {
	...
	t.Run("AC05", func(t *testing.T) {
		...
		if e := os.WriteFile(path, configBytes(t, nil), 0600); e != nil {
			t.Fatal(e)
		}
		lookup := func(k string) (string, bool) {
			switch k {
			case "CHARTWORKS_STORE_URL":
				return canary, true
			case "CHARTWORKS_CONFIG":
				return path, true
			}
			return "", false
		}
		...
		for _, tc := range []struct {
			...
			if code != tc.code {
				t.Fatalf("command %v: %d; %s", tc.args, code, errout.String())
			}
		...
		if foundation.Command(context.Background(), []string{"serve"}, lookup, &out, &out, build, func(context.Context, config.Config, io.Writer) error { return errors.New(canary) }) != 1 || strings.Contains(out.String(), canary) {
			t.Fatal("startup error leaked")
		}
		if foundation.Command(context.Background(), []string{"version"}, lookup, badWriter{}, &out, build, start) != 1 {
			t.Fatal("write failure ignored")
		}
		b := configBytes(t, func(m map[string]any) { m["server"] = map[string]any{"listen": "env:LISTEN"} })
		c, e := config.Load(bytes.NewReader(b), func(k string) (string, bool) {
			if k == "LISTEN" {
				return "127.0.0.1:9000", true
			}
			return lookup(k)
		}, config.Overrides{Listen: "127.0.0.1:9001"})
		if e != nil || c.Values().Server.Listen != "127.0.0.1:9001" {
			t.Fatal("source precedence failed")
		}
	})
	t.Run("AC06", func(t *testing.T) {
		...
		for i := 0; i < 100; i++ {
			...
			go func() {
				defer wg.Done()
				v := c.Values()
				v.Auth.Algorithms[0] = "HS256"
				if _, e := config.Load(bytes.NewReader(b), environment(canary), config.Overrides{}); e != nil {
					t.Error(e)
				}
				if e := r.Record(context.Background(), telemetry.Request, true); e != nil {
					t.Error(e)
				}
				w := httptest.NewRecorder()
				r.Handler().ServeHTTP(w, httptest.NewRequest("GET", "/metrics", nil))
		...
		wg.Wait()
		if c.Values().Auth.Algorithms[0] != "RS256" {
			t.Fatal("mutable live configuration")
		}
...
func BenchmarkConfigurationStartup(b *testing.B) {
	data := configBytes(b, nil)
	b.ReportAllocs()
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		if _, e := config.Load(bytes.NewReader(data), environment("fixture"), config.Overrides{}); e != nil {
			b.Fatal(e)
		}
	}
}
func BenchmarkIdleHealth(b *testing.B) {
	...
	s, e := foundation.NewServer(cfg, r, func(context.Context) foundation.Dependency { return foundation.Dependency{} }, func(context.Context) foundation.Dependency { return foundation.Dependency{} })
	if e != nil {
		b.Fatal(e)
	}
	handler := s.Handler()
