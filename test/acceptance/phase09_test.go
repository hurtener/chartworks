...
type unsupportedReadAdapter struct {
	binding  readexec.Binding
	explains int
}

func (a *unsupportedReadAdapter) Binding(context.Context, identity.Envelope, string, string) (readexec.Binding, error) {
	return a.binding.Clone(), nil
}
func (a *unsupportedReadAdapter) Explain(context.Context, identity.Envelope, readexec.Candidate) error {
	a.explains++
	return nil
}
...
func TestPhase09(t *testing.T) {
	t.Run("AC01", func(t *testing.T) {
		...
		if !receipt.Validated || receipt.Source != source.ID || receipt.Context != source.ContextID || len(receipt.Dependencies) != 1 || len(receipt.Manifest) != 64 {
			t.Fatal("missing validated binding receipt")
		}
		for _, typ := range []reflect.Type{reflect.TypeOf(readexec.Plan{}), reflect.TypeOf(readexec.Candidate{})} {
			for i := 0; i < typ.NumField(); i++ {
				if typ.Field(i).IsExported() {
					t.Fatal("caller-constructible proof field")
				}
			}
		}
		...
		statement, parameters, err := plan.SQL(f.e, binding)
		if err != nil || statement == "" || len(parameters) != 1 {
			t.Fatal("sealed arguments missing", err)
		}
		parameters[0].Value = "2"
		rows, err := f.s.Read(ctx, f.e, plan)
		if err != nil || len(rows.Values) != 1 || *rows.Values[0][0] != "1" {
			t.Fatal("caller mutated bound parameters", err)
		}
		changed := binding.Clone()
		changed.Relations[0].Columns[0].Name = "secret"
		if _, _, err := plan.SQL(f.e, changed); !errors.Is(err, readexec.ErrBinding) {
			t.Fatal("changed semantic contract accepted", err)
		}
		other := f.actor(t, f.e.Tenant(), "different-actor")
		if _, err := f.s.Read(ctx, other, plan); !errors.Is(err, readexec.ErrBinding) {
			t.Fatal("other actor reused plan", err)
		}
		...
		if _, err := f.s.Read(ctx, narrow, plan); !errors.Is(err, readexec.ErrBinding) {
			t.Fatal("narrowed envelope reused broader plan", err)
		}
		encoded, _ := json.Marshal(plan)
		if strings.Contains(string(encoded), statement) || strings.Contains(string(encoded), "parameters") {
			t.Fatal("opaque plan serialized executable SQL")
		}
	})
	t.Run("AC02", func(t *testing.T) {
		...
		for _, sql := range []string{
			...
			if p, err := f.validator.Validate(ctx, f.e, readexec.Request{Source: source.ID, Context: source.ContextID, SQL: sql}); err == nil || p.Receipt().Validated {
				t.Fatalf("unproven SQL accepted: %s", sql)
			}
		}
		if f.lookups.Load() != before {
			t.Fatal("unsafe SQL reached native planner")
		}
		...
		if _, err := f.validator.Validate(ctx, missingDataset, readexec.Request{Source: source.ID, Context: source.ContextID, SQL: `SELECT id FROM analytics.sales`}); err == nil || f.lookups.Load() != before {
			t.Fatal("dataset denial happened after protected planning")
		}
	})
	t.Run("AC03", func(t *testing.T) {
		...
		for _, sql := range []string{
			...
			if _, err := f.s.Read(ctx, f.e, plan); err != nil {
				t.Fatal("qualified grammar failed native read", sql, err)
			}
		}
		for _, sql := range []string{`SELECT id FROM analytics.sales s JOIN analytics.sales t ON s.id=t.id`, `WITH q AS (SELECT id FROM analytics.sales) SELECT secret FROM q`, `SELECT items.sale_id FROM analytics.sales`} {
			if _, err := f.validator.Validate(ctx, f.e, readexec.Request{Source: source.ID, Context: source.ContextID, SQL: sql}); err == nil {
				t.Fatal("ambiguous or hidden visibility guessed")
			}
		}
	})
	t.Run("AC04", func(t *testing.T) {
		...
		if _, err = f.admin.Exec(ctx, "GRANT EXECUTE ON FUNCTION analytics.side_effect() TO "+pgx.Identifier{f.role}.Sanitize()); err != nil {
			t.Fatal(err)
		}
		before := f.lookups.Load()
		for _, sql := range []string{`SELECT analytics.side_effect()`, `EXPLAIN ANALYZE SELECT id FROM analytics.sales`, `EXPLAIN SELECT id FROM analytics.sales`, `BEGIN; SELECT id FROM analytics.sales; COMMIT`, `SELECT id FROM analytics.sales FOR SHARE`} {
			if _, err := f.validator.Validate(ctx, f.e, readexec.Request{Source: source.ID, Context: source.ContextID, SQL: sql}); err == nil {
				t.Fatal("native planning bypass")
			}
		}
		if f.lookups.Load() != before {
			t.Fatal("bypass sent to warehouse")
		}
		var count int
		if err = f.admin.QueryRow(ctx, `SELECT count(*) FROM analytics.effects`).Scan(&count); err != nil || count != 0 {
			t.Fatal("planning executed side effect", err, count)
		}
		if p := f.plan(t, source, `SELECT id FROM analytics.sales`); !p.Receipt().Validated {
			t.Fatal("native safe planning failed")
		}
	})
	t.Run("AC05", func(t *testing.T) {
		...
		for _, dialect := range []string{"duckdb", "unknown"} {
			...
			if _, err = validator.Validate(ctx, f.e, readexec.Request{Source: source.ID, Context: source.ContextID, SQL: `SELECT id FROM analytics.sales`}); !errors.Is(err, readexec.ErrUnsupported) || adapter.explains != 0 {
				t.Fatal("unqualified dialect fell back", dialect, err)
			}
		}
		// A source with unproven custom types never acquires an executable proof for them.
		if _, err = f.validator.Validate(ctx, f.e, readexec.Request{Source: source.ID, Context: source.ContextID, SQL: `SELECT domain_value,mood FROM analytics.sales`}); err == nil {
			t.Fatal("unqualified type executed")
		}
	})
	t.Run("AC06", func(t *testing.T) {
		...
		ctx := context.Background()
		source := f.create(t, "sales")
		if !strings.Contains(readexec.ParserIdentity, "wasm-b511bb3bfd6e") {
			t.Fatal("parser pin missing")
		}
		before := f.lookups.Load()
		for _, request := range []readexec.Request{
			...
			if _, err := f.validator.Validate(ctx, f.e, request); err == nil || strings.Contains(err.Error(), request.SQL) {
				t.Fatal("unbounded or unsafe error response")
			}
		}
		if f.lookups.Load() != before {
			t.Fatal("bad parse input performed source work")
		}
		cancelled, cancel := context.WithCancel(ctx)
		cancel()
		if _, err := f.validator.Validate(cancelled, f.e, readexec.Request{Source: source.ID, Context: source.ContextID, SQL: `SELECT 1`}); err == nil {
			t.Fatal("cancelled parser work admitted")
		}
	})
}
