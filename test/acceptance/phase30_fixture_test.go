...
func newPhase30Fixture(t *testing.T, dynamic bool) *phase30Fixture {
	...
	server := httptest.NewTLSServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		f.mu.Unlock()
		mode := f.mode.Load()
		if mode == 1 { w.WriteHeader(http.StatusForbidden); return }
		if mode == 2 { w.WriteHeader(http.StatusServiceUnavailable); return }
		if mode == 9 { <-r.Context().Done(); return }
		scopes := append(slices.Clone(f.runtimeScopes), "cw.execution_binding.use:"+in.Binding, "cw.run.execute:"+in.Job)
		if mode == 4 { scopes = slices.DeleteFunc(scopes, func(s string) bool { return strings.HasPrefix(s, "cw.execution_context.use:") }) }
		claims := tokens.claims(d.execute.Tenant(), jobs.Executor(in.Binding), scopes)
		now := time.Now().Unix()
		claims["iat"], claims["nbf"], claims["exp"] = now, now-1, now+30
		claims["aud"], claims["session"] = cfg.JobAudience(), in.Job
		...
		switch mode {
		case 3: claims["iat"], claims["exp"] = now-31, now-1
		case 5: claims["execution_manifest"] = strings.Repeat("0", 64)
		case 6: claims["execution_binding"] = "higher"
		case 7: claims["aud"] = cfg.HTTPAudience()
		case 8: claims["user"], claims["sub"] = "svc:other", "svc:other"
		}
		token := tokens.sign(t, claims, nil)
		f.mu.Lock(); f.tokens = append(f.tokens, token); f.mu.Unlock()
...
func phase30Target(kind, id string) jobs.ReportingTarget {
	...
	if t.ResourceKind() == "block" { t.Outputs = []string{"table-second", "table-main"} }
	if kind == "saved_question" { t.Dynamic, t.Widget, t.Budget.ModelCalls, t.Budget.ModelTokens = true, "dynamic", 16, 1 << 20 }
	return t
}
...
func (f *phase30Fixture) submit(t *testing.T, key string, target jobs.ReportingTarget) jobs.Job {
	...
	if err != nil || !j.Valid() || j.Reporting == nil { t.Fatalf("reporting admission: %+v %v", j, err) }
	return j
}
