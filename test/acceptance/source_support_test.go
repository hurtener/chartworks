...
INSERT INTO analytics.items VALUES(1,3),(2,4);`)
	...
	if _, err = f.admin.Exec(ctx, "CREATE ROLE "+quoted+" LOGIN PASSWORD '"+sourcePassword+"' NOSUPERUSER NOCREATEDB NOCREATEROLE NOBYPASSRLS"); err != nil {
		t.Fatal(err)
	}
	...
	if _, err = f.admin.Exec(ctx, "GRANT USAGE ON SCHEMA analytics TO "+quoted+"; GRANT SELECT ON ALL TABLES IN SCHEMA analytics TO "+quoted); err != nil {
		t.Fatal(err)
	}
...
func (f *sourceFixture) lookup(name string) (string, bool) {
	f.lookups.Add(1)
	if name == "CHARTWORKS_SOURCE_WRITE" {
		f.writeLookups.Add(1)
	}
	f.mu.RLock()
	defer f.mu.RUnlock()
	v, ok := f.values[name]
...
func (f *sourceFixture) create(t *testing.T, id string) sources.Source {
	t.Helper()
	out, err := f.s.Create(context.Background(), f.e, sources.CreateRequest{ID: id, Name: "Synthetic warehouse", Connection: "warehouse"})
	if err != nil {
		t.Fatal("create source", err)
	}
	return out
}
func (f *sourceFixture) plan(t *testing.T, source sources.Source, sql string, parameters ...readexec.Parameter) readexec.Plan {
	t.Helper()
	p, err := f.validator.Validate(context.Background(), f.e, readexec.Request{Source: source.ID, Context: source.ContextID, SQL: sql, Parameters: parameters})
	if err != nil {
		t.Fatal("validate", sql, err)
	}
	return p
}
func (f *sourceFixture) setReadDSN(value string) {
	f.mu.Lock()
	f.values["CHARTWORKS_SOURCE_READ"] = value
	f.mu.Unlock()
}
func (f *sourceFixture) readDSN() string {
	f.mu.RLock()
	defer f.mu.RUnlock()
	return f.values["CHARTWORKS_SOURCE_READ"]
}
