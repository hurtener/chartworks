...
func TestCW08PopulatedExampleUpgrade(t *testing.T) {
	...
	if err != nil || len(manifest) != 42 || manifest[40].Name != "migrations/041_reporting_output_locale_bounds.sql" || manifest[41].Name != "migrations/042_learning_templates.sql" {
 ...
 VALUES('cw08-upgrade',repeat('1',32),'topic','Active question','SELECT 1',repeat('a',64),'active',0.8,4,'synthetic-active'),
	...
	if count(t, raw, `SELECT count(*) FROM chartworks.schema_migrations WHERE version=41 AND name='migrations/041_reporting_output_locale_bounds.sql'`) != 1 || count(t, raw, `SELECT count(*) FROM chartworks.schema_migrations WHERE version=42 AND name='migrations/042_learning_templates.sql'`) != 1 {
