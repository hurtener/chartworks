...
func expectedRows(out charts.Output) [][]*string {
	...
	for _, p := range out.Points {
		...
		for _, c := range out.Columns {
			switch c.ID {
			case b.Category:
				row = append(row, cell(p.Category))
			case b.Series:
				row = append(row, cell(p.Series))
			case b.Parent:
				row = append(row, cell(p.Parent))
			case b.X:
				row = append(row, value(p.X))
			case b.Y:
				row = append(row, value(p.Y))
			case b.Value:
				row = append(row, value(p.Value))
			}
		}
		rows = append(rows, row)
	}
...
func build(t *testing.T, kind charts.Kind, edit func(*charts.Data)) charts.Output {
	...
	if err != nil {
		t.Fatal("actual fixture binding", err)
	}
	out, err := charts.Build(context.Background(), d, mapping, charts.Defaults())
	if err != nil {
		t.Fatal("actual fixture normalization", err)
	}
	return out
}
...
func produce(t *testing.T) fixture {
	...
	for _, entry := range charts.Catalog() {
		for _, scenario := range chartfixtures.Cases() {
			...
			if g.Output != nil {
				c.ExpectedRows = expectedRows(*g.Output)
			}
...
func Run(t *testing.T, suite string) {
	t.Helper()
	if suite != "charts" && suite != "interaction" && suite != "security" && suite != "all" {
		t.Fatal("unknown component suite")
	}
	...
	if err := os.WriteFile(htmlPath, []byte(reportviewer.HTML()), 0600); err != nil {
		t.Fatal(err)
	}
