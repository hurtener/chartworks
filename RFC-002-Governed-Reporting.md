...
Narrative definitions pin type/instructions, allowed/redacted evidence fields, deterministic reduction, claim/row/byte/character/call/token/time limits, prompt/model/schema versions, locale/tone and evidence/caveat requirements. They have no query/write tools, use the remote SDK gateway, validate claims and retain exact output. Later rendering never regenerates text. Failure/omission is reflected in report policy.

CW-03 / D-073 versions output-level intent in definition v2: stable IDs, localized names/descriptions, enabled/default-selected state and explicit display order. V2 omitted selection resolves enabled defaults; explicit empty/duplicate/unknown/disabled selections reject before work. V1 preserves empty-means-all; published JSON/hashes are never rewritten. Display order is independent of accepted execution order. Native authored export uses the existing separately authorized SQL-read surface; metadata is not an authoring or data-access grant.

Reviewed sensitivity and manual restrictions compose before evidence reduction/model input. Unknown/conflicting sensitivity fails closed. Without proven expression lineage the inherited policy conservatively covers query dependencies, not guessed aliases. V2 narrative policies use closed claim types and deterministic locale/tone templates; arbitrary instructions and unsupported mappings reject. Authored/requested source-query caps bind admission/reuse and intersect current deployment/authorized ceilings again at execution. Composition, schedule retries and retained selectors preserve exact accepted revisions/selections/caps. See the [v2 contract](docs/contracts/reporting-output-intent-v2.md) and [review evidence](docs/reviews/cw-03-adversarial.md); no broader parity or new authority is implied.
