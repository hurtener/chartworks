...
Bound tools additionally publish canonical input/output/result schema, resource
loader and sorted error-contract metadata; remote discovery must match all of it.
...
distinct and cannot be overwritten by delayed progress, cancel or refinement.
Transport and accepted feedback observations remain separate from that terminal
outcome. The shared ordering envelope contains only IDs and lifecycle metadata;
