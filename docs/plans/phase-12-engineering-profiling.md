...
The recovered branch contains named acceptance tests plus source-backed checkpoint, cancellation, provider-redaction, schema-diff and inspection fixtures. The new `TestUploadErasureRemovesDerivedProfileValuesAcrossActors` additionally covers complete and unpublished evidence, another actor, interrupted erasure, same-operation resume and an unrelated surviving source. These current-head tests still require execution; their existence does not establish acceptance. See the [adversarial review and verification ledger](../reviews/phase-11-12-adversarial.md) for committed findings and the observed execution failures before runner allocation.
