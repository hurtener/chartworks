...
manifest is `docs/contracts/chartworks-onboarding-operations.json`; D-084 fixes the
...
tests. The reservation uses the complete remaining configured call allowance, so
gateway batching can make several provider calls without either exceeding the run
budget or failing under an invented single-call ceiling.
...
execution or certification authority. Stable source/context/dataset/column and
depends-on coordinates drive the affected closure, including the query intent.
Closure of private intents without executable definitions is retained as explicit
conservative impact evidence rather than represented as direct column impact.

GET, cancellation and replay reauthorize the onboarding resource plus the
server-resolved current source and execution context before returning a run.
Transformed output references also require their exact source/context reach, so a
revoked caller cannot recover dataset/profile evidence from the progress ledger.
...
`TestPhase33/AC06` creates, independently reviews and applies a real managed
engineering proposal, proves the onboarding profile is built over its exact
published output source/context/dataset, and rejects that applied proposal for an
unrelated source binding.
`TestPhase33/AC10` adds a real PostgreSQL pre-effect lease/cancellation race beyond
the eight planned acceptance rows.

## Glossary, decisions and deviations

Setup progress and semantic confidence are evidence, not permission or certification. D-052, D-082 and D-084 apply. Runtime submission is not a shipped-status claim.
...
consumer for rich column/KPI/relationship proposals. The implementation submission
above now composes that substrate through the resumable journey; shipped status
still depends on the named review and release gates.
