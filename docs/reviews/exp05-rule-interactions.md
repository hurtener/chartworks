...
| A relationship rule silently follows a removed or meaningfully rebound relationship. | The relationship regression removes a reviewed join and receives `missing_reference`. Reusing the old rule publication against a changed relationship digest receives `evidence_mismatch`; an explicit newly reviewed topic/digest pin is required before the rule can compile. |
...
| Replay relies on historical authority. | Replay rereads the exact topic/ruleset under the current verified envelope. A token with the correct topic but the wrong execution-context reach is denied before comparison evidence is written. |
