...
- **Source evidence:** Unassigned; this is an unassessed frontier and requires current owner evidence.
- **Current repository evidence:** The immutable manifest carries full dependency/provenance/retention state, dry run enumerates loss, replay is digest/CAS fenced and certificates remain historical. AC05 imports a synthetic block through its owner, creates a real PostgreSQL retained output and rendition, previews without mutation, applies due owner erasure, and proves an unrelated run/rendition survives. It covers missing context reach, stale batch revision, cross-tenant denial and replay. Private owner inventory, live comparison and backup/WAL operator evidence remain external closure gates. [retention drill](../internal/store/postgres/migration_retention.go); [Phase34 acceptance](../test/acceptance/phase34_retention_drill_test.go).
