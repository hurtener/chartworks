...
- **Disposition and boundary:** The current runtime uses the retained canonical route request as the semantic base, typed clarification answer deltas for correction/removal, governed interpretation edits for filter replacement/removal, and explicit reference/metric edits for add/replace/remove. A sixteen-refinement ancestry bound returns `new_question_required`; semantic republication returns `context_changed` and requires a freshly routed question. The service injects protected prior SQL only as edit context and reauthorizes exact current authority. See [conversational continuity v1](contracts/conversational-continuity-v1.md).
...
- **Implemented validation:** The real PostgreSQL/service journey asks, adds/removes a dimension, changes a metric, reads the stored canonical child selections, rejects cross-session/context reuse and verifies the semantic-republication fence. Existing CW-01/CW-07 tests cover clarification correction/removal and governed filter replacement/removal; Phase 24 records all six bilingual journey categories by protected semantic digest. Explicit removal tests prove stale selections are absent rather than merely hidden in generated SQL.
