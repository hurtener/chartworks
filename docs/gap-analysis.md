...
- **Source evidence:** Unassigned; this is an unassessed frontier and requires current owner evidence.
- **Current repository evidence:** Public Phase 16 lifecycle tests retain simultaneously matching rule selections, required/excluded references and attributable violations; contradictory exclusions of a pinned metric dependency reject before persistence. Closed template matching records exact selection/mismatch reasons and refuses overlapping require/exclude scopes. Semantic relationship edits must retire or explicitly repin rules: a removed relationship is missing and an old publication cannot bind to a changed topic digest. Exact historical replay survives replacement and rollback but revalidates current signed source/context reach. Reporting capture tests prove clear-via-edit, rejected edit-based re-add and fresh source-backed recapture. The populated migration-040 fixture preserves a legacy singular template revision byte-for-byte and rejects mixed singular/plural representation. See [EXP-05 adversarial evidence](reviews/exp05-rule-interactions.md), [rule evaluator](../internal/semantics/rules_evaluate.go), [Phase 16 acceptance](../test/acceptance/phase16_test.go), [relationship edit regression](../internal/semantics/rules_test.go), [reporting capture](../test/acceptance/cw06_rules_reporting_test.go) and [migration fixture](../test/acceptance/cw06_migration_test.go). Representative-user explanation comprehension, live-model behavior and held-out semantic quality remain Phase 24/final qualification rather than being inferred from synthetic fixtures.
