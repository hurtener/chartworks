...
| EXP-08 | Safe topic portability, Phase 32 rendition lifecycle, Phase 34 dependent-object manifest and exact imported-revision retention drill exist. The bounded preview/apply drill erases due Chartworks-owned outputs and renditions through their owner retention paths. | Approved private owner inventory/comparison and backup/replica/WAL operator retention evidence remain open. |
...
- **Status:** neutral bundle, exact-origin downstream online retention drill and bounded migration-record erasure implemented; private owner and backup/operator evidence pending.
- **Boundary:** Historical imported artifacts and renditions remain private quarantine, not installed domain records. The separate `migrationRetentionDrill` previews or applies due retained outputs and renditions generated from exact imported block/report/dashboard revisions. It requires current migration and reporting retention authority plus target/context read reach, a complete batch revision, no active cutover or legal hold, and uses the owning frozen/composition expiry paths. It does not shorten an output's retention or erase unrelated revisions. Phase 34 Erase refuses live descendants before deleting migration checkpoints. This online result cannot prove backup, replica or WAL expiry.
...
- **Source evidence:** Unassigned; this is an unassessed frontier and requires current owner evidence.
- **Current repository evidence:** The immutable manifest carries full dependency/provenance/retention state, dry run enumerates loss, replay is digest/CAS fenced and certificates remain historical. AC05 imports synthetic block and text-only report revisions through their owners, creates real PostgreSQL retained runs and renditions, previews without mutation, applies due owner erasure, and proves unrelated runs survive. It covers exact rendition-set fencing against a concurrent writer, legal-hold preview/apply denial, report read authority, missing block context reach, stale batch/preview revisions, cross-tenant denial and replay. HTTP tests exercise the verified-envelope route and reporting-retention scope denial. Private owner inventory, live comparison and backup/WAL operator evidence remain external closure gates. [retention drill](../internal/store/postgres/migration_retention.go); [Phase34 acceptance](../test/acceptance/phase34_retention_drill_test.go).
...
| Q10 | Evidence/report-artifact/output retention cleanup | shared foundation, Phase 32 rendition lifecycle and Phase 34 exact-origin online drill implemented | Phase 34 retention drill previews/applies due imported-revision descendants through frozen/composition owner expiry and preserves unrelated objects; private owner inventory and backup/WAL operator evidence remain | 06.AC03, 28.AC07, 32.AC07, 34.AC05 |
