...
- **Current behavior:** Definition v2 pins exact topic/rule versions and digests. Validation, certification health, frozen manifests, reuse, compositions and schedules retain the same pins. Completed-query capture also transfers the canonical reviewed template selection that produced the plan. Rule replacement/retirement marks current health stale and blocks revalidation/refresh while published definitions, retained rule versions and historical attestations stay immutable.
...
- **Persistence and migration:** Migration 038 adds tenant-composite immutable pin rows. Migration 039 adds bounded immutable template selection evidence to query and comparison rows. Existing unpinned definitions and queries remain explicit no-snapshot/no-selection compatibility records. Native protected export/import carries exact pins; mappings without provable coordinates reject instead of inventing dependencies.
- **Concurrency:** Query capture locks current rule heads in its block commit transaction using the same head ordering as publication. A replacement that wins after exact query evidence was read returns typed stale and rolls back block, revision and pin writes.
