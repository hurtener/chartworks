...
This is the current disposition at `ffffb27bab4dc2fdd67c5f138fa32cfaf452f968`
(PR #47). The September 15 inspection and per-kind comparison below remain
...
23, 24, 26–31 and 33 in progress; and 25 and 34 planned. Phase 24 has six
...
([review](reviews/phase-32-adversarial.md)). Status labels do not prove final
source/provider qualification. The in-flight Phase 34 import/cutover PR and
Phase 25 performance/acceptance PRs are excluded from this merged baseline.
...
| BLK-01, BLK-03, BLK-04, BLK-05, BLK-06, BLK-07 | CW-03 native output intent, sensitivity and bounded narrative/query policy; CW-09 bilingual period/overlap and reviewed parameter assistance. Unsupported mappings reject. | Phase 34 foreign-field disposition and representative authoring/review acceptance. |
| VIS-01, VIS-02, VIS-03, VIS-04 | CW-02 repeated measures, real series, bubble size, bounded eight-level hierarchy and pre-seal question-aware selection; CW-05 KPI/table/display formatting; Phase 32 static/retained consumers including full report/dashboard HTML/SVG pages. | Calibrated chart choice, foreign mapping, live/browser/export matrix and explicit PNG/PDF or paginated-document scope decision. Fourteen kind names alone do not prove these variants. |
| REP-01, REP-02 | CW-10 revision-bound selectable filter options and CW-11 governed report/dashboard deletion. | Foreign migration, retained/rendition cleanup and full release journeys. |
...
| MIG-01, EVAL-01, PERF-01 | Safe topic portability, merged Phase 24 evaluation and a correctness-first PERF-01 synthetic harness. | Complete calibrated environment import/cutover, real source/recorded-model measurements, six live engine cohorts and Phase 25 stress/release. PR #48 remains pending and cannot be credited here. |
...
| EXP-02 | Pending owner-calibrated state inventory. | Approved prompts/settings/example weights/model versions, explicit import/rollback and held-out before/after evidence. |
...
| EXP-04 | Bounded upload/parser and merged Phase 33 workflow exist. | Format/locale matrix through report, derived-retention erasure and unsupported-variant diagnostics. |
...
| EXP-06 | Report/filter/viewer and Phase 32 geometry exist. | Global/local/default/explicit precedence, distinct parameter runs, empty/partial/private-preview cases across interactive and static paths. |
| EXP-07 | Queue and reporting schedule cores exist. | DST/window/retry/crash matrix, separated delivery receipts and one deduplicated cutover occurrence stream. |
| EXP-08 | Safe topic portability and Phase 32 rendition lifecycle exist. | Complete dependent-object import/retention/provenance inventory, dry-run loss report and erasure verification. |
...
| EXP-11 | Generated installed-operation conformance and consumer interaction matrix exist. | Re-run after Phase 34 operations land, with deployment-specific MCP visibility and protected end-user journeys. |
| EXP-12 | Fourteen-kind interactive viewer, Phase 32 static worker, renditions and JSON/CSV/HTML/SVG exports, including full report/dashboard HTML/SVG pages, exist. | Data-shape × locale × viewer/static/export agreement, browser/live boundary and explicit PNG/PDF or paginated-document scope. |
...
interpreted with this rebaseline. The remaining completion order is Phase 34
import/cutover and its exact-source checks, then Phase 25 full acceptance,
calibrated comparisons, stress and release gates.
...
composition and the embed/BFF example. Phase 34 still owns foreign import mapping, and final migration,
...
- **Dated target baseline:** `09ef2de969317458753838d51e775ad8a21fcff1` (2026-09-15). This was the merged implementation head used for that recheck. Current disposition is pinned above to `ffffb27`; links must be read against their dated evidence.
...
## Sept 15 rebaseline and recheck

This update rechecked every original finding against the merged target code and the dated reference inventory. The semantic, context, routing, learning, clarification, rules and data reference locations were unchanged in the per-file comparison; changed reporting locations were rechecked through the existing source IDs and the current phase consumers. “Inspected” means source and acceptance registrations were read, not that this document reran the referenced suite or measured a live workload.
...
| VIS-01, VIS-02, VIS-03, VIS-04 | **CW-02/CW-05 implementation merged; final qualification remains.** | Versioned mappings retain rich binding, selection, KPI/table and per-column display intent. Phase 32 later shipped the worker, composition, renditions and static/export consumers; Phase 34 foreign import remains open. [chart mapping](../internal/charts/model.go), [retained renderer](../internal/rendering/render.go), [viewer formatter](../web/report-viewer/app.js) |
...
- `TMPDIR=/private/tmp make planning-check check-mirror` passed 60 script tests, planning coherence for 224 criteria/63 features/34 phases, and mirror validation.
...
| REP-03 | Reporting catalog | Readable actor labels and delivery relationships are richer in the reference catalog | partially implemented by CW-11; permission-filtered relationships and non-identifying fallbacks ship, readable platform labels remain open pending a real public adapter contract | 23/29/31 |
...
- **Disposition:** implemented by CW-06; broad final-gap qualification remains pending.
- **Owner / phases:** 27/28 with rule and capture consumers in 16/18.
...
- **Current behavior:** Definition v2 pins exact topic/rule versions and digests. Validation, certification health, frozen manifests, reuse, compositions and schedules retain the same pins. Completed-query capture transfers a bounded topic-ordered selection set with complete topic/ruleset coordinates. Edits cannot retain template provenance across a rule change; validation, certification and frozen sealing reject an orphan. Rule replacement/retirement marks current health stale and blocks revalidation/refresh while published definitions, retained rule versions and historical attestations stay immutable.
- **Authority and frozen boundary:** Every exact ruleset read uses the current verified envelope and cannot widen topic/source/context reach. Frozen refresh only checks the sealed pins; it performs no rule selection, NLQ or model work.
- **Persistence and migration:** Migration 038 adds tenant-composite immutable pin rows. Migration 039 adds bounded immutable template selection evidence to query and comparison rows. Migration 040 constrains reporting revisions to one legacy singular selection or the bounded per-topic representation. Existing singular records remain compatible only when their sole rule pin supplies the exact missing coordinates. Native protected export/import carries exact pins; mappings without provable coordinates reject instead of inventing dependencies.
- **Concurrency:** Query capture and frozen sealing lock current rule heads in topic order using the same ordering as publication. A replacement that wins after exact query or manifest evidence was read returns typed stale and rolls back block/revision/pin or frozen-manifest writes.
- **Evidence:** [rule dependency service](../internal/reporting/dependencies.go), [capture adapter](../internal/reporting/query_capture.go), [transaction fence](../internal/store/postgres/blocks_write.go), [store invalidation](../internal/store/postgres/rulesets.go), [unit coverage](../internal/reporting/rule_snapshots_test.go) and [real-PostgreSQL acceptance](../test/acceptance/cw06_rules_reporting_test.go). The local database URL was unavailable during this fix round, so the expanded real-database case is registered and compile-checked but awaits the D-074 manual final workflow.
...
- **Current behavior:** Topic and legacy any-entity behavior is preserved. Compound scopes apply only when all exact targets are present. Template scopes use a canonical reviewed selection pinned to current topic/ruleset coordinates; omission clarifies before generation and stale or substituted coordinates fail closed. Preflight, plan, refinement/reconstruction, saved selection replay, query persistence, reporting capture and replay/shadow evidence retain that pin. Evaluation, replay and shadow expose sorted applied/reason evidence, and selection-only advisory changes count as changed.
- **Safety:** Scope shapes are closed and mutually exclusive. Hard constraints remain typed reference presence/absence; no SQL, regex or executable predicate is accepted. Distinct template scopes avoid false conflicts, while overlapping require/exclude dependency conflicts fail compilation.
- **Migration:** Existing publications/digests do not change. Migration 039 preserves an explicit empty selection for old query/comparison rows. Native scopes round-trip; foreign records require an explicit reviewed transformation or unsupported disposition and cannot infer a scope from prose or SQL.
- **Evidence:** [scope model](../internal/semantics/rules.go), [compile/conflict checks](../internal/semantics/rules_compile.go), [truth-table evaluator](../internal/semantics/rules_evaluate.go), [ruleset replay/shadow](../internal/semantics/rulesets/service.go), [routing consumer](../internal/nlqroute/service.go), and [NLQ persistence](../internal/store/postgres/nlq_runtime.go).
...
- **Disposition:** implemented by CW-05; Phase 32 static/rendition consumer subsequently shipped. Final live/export matrix and foreign import remain open.
...
- **Disposition:** authority-aware measurement harness and bounded test-only synthetic smoke implemented; final integration/live stress evidence remains unmeasured.
...
- **Current boundary:** Current routing/vector evidence explicitly has no equivalent evidence cache, while the gateway has separate cache/usage behavior. Execution has durable idempotency and reuse boundaries of its own. The PERF-01 harness now pins the complete reuse identity, gates correctness before timing, retains raw attribution, distinguishes synthetic/real-PostgreSQL/recorded/live modes, and refuses the final profile from the local smoke command.
...
- **Closure requirements:** Run the defined post-Phase-34 integration/live final profile through Phase 25; compare cold/warm/repeated/concurrent requests and rule/source/context/topic/runtime-pack changes; retain raw call/latency/cost evidence and prove no stale or broader-context reuse.
- **Source evidence IDs:** REF-PERF-01-A, REF-PERF-01-B.
- **Current repository evidence:** [performance evidence contract](contracts/performance-evidence-v1.md); [measurement harness](../internal/evaluation/performance.go); [bounded smoke profile](../examples/evaluation-performance-smoke.json); [internal/nlqroute/service.go:192](../internal/nlqroute/service.go#L192); [docs/contracts/vector-sources-validation.md:111](../docs/contracts/vector-sources-validation.md#L111); [docs/plans/phase-24-eval.md:29](../docs/plans/phase-24-eval.md#L29).

### REP-01 — Revision-bound selectable filter options

- **Disposition:** implemented by CW-10; final integrated release evidence remains pending.
- **Owner / phases:** 29/31 with safe-source 09/10/14 and import 34; related EXP-06 and R04.
- **Reference behavior (neutral):** The reference has an actual report/revision/filter options consumer. It verifies visible revision and SELECT type, requires an explicit governed topic/table/column source within report scope, pages distinct values, returns values/labels and continuation, and binds cursors to tenant/report/revision/filter/source/search/limit and keyset. Its source topic metadata comes from the active version; the cursor is not cryptographically signed. These are observed behaviors, not automatic target requirements.
- **Current boundary:** `ReportFilter` now optionally carries a version-one exact block/topic/dataset/column source. The document API plus shared SDK/MCP/viewer facade execute bounded distinct reads through the common validator/executor with current authority and source-revision fences. Historical filters without a source remain ordinary typed inputs.
- **Consequence:** Clients can request typed searchable warehouse-backed choices only through the explicit operation; retained artifact navigation and profile samples remain separate and source-free.
- **Contract, storage and import impact:** [D-082](decisions/2026-09-22-report-filter-options.md) and the [v1 contract](contracts/report-filter-options-v1.md) define the immutable binding, closed query intent, typed/null behavior, database collation, escaped string search, 200-value/byte/deadline caps, no-cache freshness and authenticated five-minute stale cursor behavior. Existing document JSON import/export preserves the optional field.
- **Closure requirements:** Focused real-PostgreSQL acceptance proves multi-page completeness without duplicates, text search restart, typed NULL and exact decimal values, oversized-value rejection, tamper/request drift/expiry/superseded-revision denial, wrong context and cross-tenant denial before access, concurrent reuse, cancellation, and HTTP/SDK parity. Phase 31 provider discovery proves the executable MCP/Apps registration and effect classification; Phase 34/25 retains the full release matrix as an integrated gate.
- **Source evidence IDs:** REF-REP-01-A, REF-REP-01-B.
- **Current repository evidence:** [internal/reporting/documents_model.go:89](../internal/reporting/documents_model.go#L89); [internal/reportingapi/documents.go:49](../internal/reportingapi/documents.go#L49); [internal/reportingapi/delivery.go:25](../internal/reportingapi/delivery.go#L25).
...
- **Disposition:** partially implemented by CW-11; relationships and safe presentations ship, readable platform labels remain open pending a real public adapter contract.
...
- **Current boundary:** Authorized document summaries now include bounded schedule/topic/block relations and safe current/unknown/service actor presentations that omit stable actor IDs. The repository has no public platform label-resolution adapter to wire, so readable external actor names remain open rather than being simulated by an acceptance-only seam. Schedule identifiers enter the SQL result only with current scheduling-read action and resource reach. Presentations and relations are never consulted by authority checks; readers without explicit preview/write reach derive the editor from the published revision and cannot observe a private draft actor.
- **Consequence:** Clients receive authorized relationship context and bounded current/unknown/service presentations. External readable actor names are not claimed yet.
- **Contract, storage and import impact:** Stable actor IDs stay in protected audit/storage while the public DTO carries only bounded non-identifying presentations until the owner supplies a real descriptive-label contract. Delivery relations are composed in the permission-filtered list query and do not create identity or permission records.
- **Closure requirements:** Focused tests cover safe fallback, published-editor privacy and schedule relationship filtering. Final integration must first adopt a real public descriptive-label adapter, then exercise deleted actors, service actors, tenant-colliding identifiers, localization and stable paging without changing authority.
...
- **Status:** deterministic runtime and bilingual Phase 24 comparison cases implemented; live calibrated comparison remains Phase 34/25 evidence.
- **Disposition and boundary:** The current runtime uses the retained canonical route request as the semantic base, typed clarification answer deltas for correction/removal, governed interpretation edits for filter replacement/removal, and explicit reference/metric edits for add/replace/remove. Measure/KPI edits must end coherent with pinned metric IDs, and the final references/metric IDs are deterministically ordered before every downstream consumer. A sixteen-refinement ancestry bound returns `new_question_required`; each child is transactionally fenced to the exact observed parent revision/digest. Semantic republication returns `context_changed` and requires a freshly routed question. The service injects protected prior SQL only as edit context and reauthorizes exact current authority. See [conversational continuity v1](contracts/conversational-continuity-v1.md).
...
- **Implemented validation:** The real PostgreSQL/service journey asks, rejects an unpaired measure/metric change before any model request, adds/removes a dimension, changes a paired metric, reads the stored canonical child selections and parent fence, rejects a stale fork after a concurrent parent update, rejects cross-session/context reuse and verifies the semantic-republication fence. Unit permutations prove identical routing digests after canonicalization. Existing CW-01/CW-07 tests cover clarification correction/removal and governed filter replacement/removal; Phase 24 records all six bilingual journey categories by protected semantic digest. Explicit removal tests prove stale selections are absent rather than merely hidden in generated SQL.
- **Remaining closure evidence:** Phase 34 must retain the exact implementation/configuration/semantic/source snapshots and compare the complete journey on approved live workloads. Phase 25 must qualify its correctness, calls/tokens/cost and latency separately; fixture success is not a parity percentage.
- **Source evidence:** neutral behavioral comparison disposition; no private identifiers, prompts, schemas, data or cache design are imported.
- **Current repository evidence:** [continuity service](../internal/nlqexec/service.go); [continuity acceptance](../test/acceptance/exp01_test.go); [Phase 24 evaluator](../internal/evaluation); [contract](contracts/conversational-continuity-v1.md).
...
- **Status:** deterministic contract/runtime behavior and Phase 24 replay/shadow evaluation are implemented; live semantic-edit cohort comparison and human explanation calibration remain final qualification work.
...
- **Source evidence:** Unassigned; this is an unassessed frontier and requires current owner evidence.
- **Current repository evidence:** Public Phase 16 lifecycle tests retain simultaneously matching rule selections, required/excluded references and attributable violations; contradictory exclusions of a pinned metric dependency reject before persistence. Closed template matching records exact selection/mismatch reasons and refuses overlapping require/exclude scopes. Compiler-level relationship regressions prove that a removed relationship is missing and an old rule definition cannot bind to a changed topic digest without explicit reviewed repinning; the public lifecycle separately proves exact topic/ruleset replacement and rollback, while a relationship-specific published lifecycle remains final qualification evidence. Exact historical replay survives replacement and rollback but revalidates current signed source/context reach through the protected HTTP/SDK path without writing denied comparison evidence. Reporting capture tests prove clear-via-edit, rejected edit-based re-add and fresh source-backed recapture. The populated migration-040 fixture preserves a legacy singular template revision byte-for-byte, reads it through the production PostgreSQL repository with matching definition/execution digests, and rejects mixed singular/plural representation. See [EXP-05 adversarial evidence](reviews/exp05-rule-interactions.md), [rule evaluator](../internal/semantics/rules_evaluate.go), [Phase 16 acceptance](../test/acceptance/phase16_test.go), [relationship edit regression](../internal/semantics/rules_test.go), [reporting capture](../test/acceptance/cw06_rules_reporting_test.go) and [migration fixture](../test/acceptance/cw06_migration_test.go). Representative-user explanation comprehension, live-model behavior and held-out semantic quality remain Phase 24/final qualification rather than being inferred from synthetic fixtures.

### EXP-06 — Report filter and layout semantics; 28/29/31/34

- **Status:** merged report/filter/viewer and Phase 32 static composition exist; field-level equivalence remains unassessed.
- **Boundary:** Phase 28/29/31 consumers and Phase 32 page geometry are present. Exact global/local/default/explicit precedence, distinct parameter-run reuse, empty states and private-preview behavior still need a cohesive cross-surface matrix.
...
- **Source evidence:** Unassigned; this is an unassessed frontier and requires current owner evidence.
- **Current repository evidence:** [Phase 29 composition](../internal/reporting), [Phase 32 acceptance](../test/acceptance/phase32_test.go); owner comparison evidence is still required.
...
- **Status:** safe topic portability and shipped Phase 32 rendition retention exist; complete migration inventory remains pending.
- **Boundary:** Calibrated dependent-object import remains Phase 34 work. Phase 32 retention/deletion covers its own renditions, but does not prove a whole-environment erasure or lossless import.
...
- **Status:** Phase 32 static renderer, whole-page composition, rendition lifecycle and JSON/CSV/HTML/SVG exports shipped; full interactive/static/export equivalence and live browser evidence remain pending.
- **Boundary:** The specification layer has a Phase 31 selector/viewer consumer and a shipped Phase 32 isolated worker. Full report/dashboard HTML/SVG page export is supported; PNG/PDF and paginated-document formats remain outside the advertised matrix.
...
- **Source evidence:** Unassigned; this is an unassessed frontier and requires current owner evidence.
- **Current repository evidence:** [chart model](../internal/charts/model.go), [Phase 32 acceptance](../test/acceptance/phase32_test.go), [Phase 32 review](reviews/phase-32-adversarial.md).
...
The current Go package remains a sealed specification/data transformation boundary: it accepts caller-provided qualified result data and produces typed output with exact labels. The merged Phase 31 Apps viewer is now a real consumer of that output: it dispatches all fourteen kinds, draws locally, supports page/widget/output selection, retained table paging, accessibility tables and consented filter reruns. It is an inspector/selector rather than a whole-page grid renderer; static SSR/export remains Phase 32 scope. The predecessor presentation path built renderer-oriented options for most kinds, so authored fields still need to survive the specification boundary.
...
| C-PLAN | current phase ownership with an implemented viewer and planned static/render/export boundary |
...
- **Viewer interaction is now present, while export parity remains open.** The merged Phase 31 viewer dispatches all fourteen kinds, supports selector navigation, retained table paging, accessible exact-value tables and consented filter-triggered reruns. The predecessor frontend has browser CSV download, best-effort chart PNG download and a table-view toggle. Current Phase 32 explicitly plans JSON/CSV/HTML/SVG exports while excluding advertised PNG/PDF/page-layout support. PNG is therefore an intentional current scope difference requiring an equivalence decision, not a Phase 20 or Phase 31 defect. Full drilldown remains unestablished in this bounded review.
...
| Renderer boundary | Build returns sealed, bounded typed data and no source/model action; Phase 31 consumes it in the Apps viewer. | The viewer does not consume whole-page grid/presentation layout, and static renderer/export contracts remain Phase 32. This is a consumer-scope decision, not a Phase 20 chart-core defect. |
...
| Stacked bar | Category, one value, and required series; values aggregate by category/series into horizontal stacks. | Same three scalar slots and current exact typed values. | **Basic shape retained.** The Phase 31 viewer draws the basic stack; richer authored legend/options and static/export agreement remain Phase 32 decisions. Current validation still differs where source rows are duplicate/invalid. |
| Stacked column | Same as stacked bar, vertical orientation. | Same three scalar slots. | **Basic shape retained** at the sealed-data level; the Phase 31 viewer draws basic orientation/legend, while richer authored options remain pending. |
...
| Output enabled state, localized output name/description, display order | Not represented on `reporting.Output`; block-level metadata is not a substitute. | Definition gap in Phase 27; the Phase 28 frozen consumer currently seals only available fields. |
| KPI comparison, delta, percent delta, trend, target, thresholds, sparkline | Not representable by the shared `charts.Mapping`, which only has scalar chart slots. | Definition gap in Phase 27; current Phase 28/31 consumers need the extension and Phase 32 must carry it into static/export paths. |
...
- Current Phase 31 owns a real Apps viewer with locale/theme/resize/accessibility, result paging, one page/widget/output selector and filter-triggered authorized reruns. It does not consume the stored grid/presentation fields for whole-page layout. Phase 32 owns static HTML/SVG, exact-value agreement and explicitly scoped JSON/CSV/HTML/SVG exports. It expressly does **not** advertise PNG/PDF/page-layout export.
...
No phase runtime acceptance or browser tests were run for this document. The bounded Go resource test and planning/mirror checks are recorded above; they do not establish predecessor equivalence, interactive UI behavior, pixel output, live-provider quality, performance, or deployment acceptance. The predecessor source snapshot may contain incomplete or transitional behavior; its source presence alone does not make it a mandatory requirement. Any imported behavior needs a current owner, authority-preserving contract, migration disposition, and executable acceptance case.

### Stable chart subitems under the existing findings

These are stable subitems of VIS-02 and VIS-04, not additional finding IDs. The original 26-finding count and the 63-feature ledger remain unchanged; this dated refresh adds REP-01 through REP-03 as three separately tracked findings.
...
1. **Complete the two planned gates in dependency order.** Phase 34 must import and revalidate the full calibrated environment, compare owner-approved cohorts and prove schedule handoff/rollback. Phase 25 then runs the six actual acceptance criteria, final stress and release gates. Keep phases 23, 24, 26–31 and 33 honestly in progress until their own closure evidence supports a status change; Phase 32 is already shipped.
...
| Q04 | Cron, interval, timezone and prior-occurrence window | core retained at phase30 over shared06 foundation; live/release evidence pending | The recurrence contract validates timezone/cron/interval bounds and computes anchored next/previous UTC occurrences; the reporting target carries its accepted half-open window. [schedule validation](../internal/jobs/schedule.go#L25); [reporting input](../internal/jobs/reporting_runtime.go#L15) | 30.AC03 |
| Q05 | Pause/resume/retire, test runs and run history | core retained at phase30 over shared06 foundation; live/release evidence pending | Schedule revisions preserve enabled/retired state and bounded revision/occurrence cursors; pause/resume, retirement and manual test occurrences use revision checks and ordinary admission. [schedule lifecycle](../internal/jobs/schedule_lifecycle.go#L35); [test occurrence](../internal/jobs/schedule_lifecycle.go#L118) | 30.AC04 |
| Q06 | Retry, backoff, timeout and final-failure policy | core retained at phase30 over shared06 foundation; live/release evidence pending | The worker classifies budget, authority, conflict and timeout failures, bounds exponential backoff and retains recoverable leases; reporting budgets independently cap query/model attempts. [worker policy](../internal/jobs/service.go#L343); [reporting budget](../internal/jobs/reporting_target.go#L45) | 30.AC05 |
...
| Q10 | Evidence/report-artifact/output retention cleanup | shared foundation and Phase 32 rendition lifecycle implemented; whole-environment erasure evidence pending | Bounded composition/run expiry and retained rendition deletion exist; Phase 34/25 must verify dependent-object cleanup | 06.AC03, 28.AC07, 32.AC07 |
