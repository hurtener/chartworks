...
type ReadLimits struct {
	...
	CancelGrace int64   `json:"cancel_grace_ns"`
...
type ReadControlReceipt struct {
	Attempt     ReadAttempt `json:"attempt"`
	RemoteState string      `json:"remote_state"`
}
...
func (c *Client) ExecuteRead(ctx context.Context, id string, input ReadExecutionRequest) (out ReadExecutionReport, err error) {
	if !wireID(id) || !wireID(input.Context) || !wireID(input.Execution.Operation) || input.Execution.Number < 1 || input.Execution.Number > 3 {
		return out, errors.New("chartworks: invalid execution coordinates")
	}
...
func (c *Client) ReadExecution(ctx context.Context, id string) (out ReadAttempt, err error) {
	if !wireID(id) {
		return out, errors.New("chartworks: invalid attempt identifier")
	}
	err = c.call(ctx, "GET", "/v1/read-executions/"+id, "", nil, &out)
	return out, err
}
...
func (c *Client) CancelRead(ctx context.Context, id string) (out ReadControlReceipt, err error) {
	...
	err = c.call(ctx, "POST", "/v1/read-executions/"+id+"/cancel", "", struct{}{}, &out)
	return out, err
}
...
func (c *Client) ReconcileRead(ctx context.Context, id string) (out ReadControlReceipt, err error) {
	...
	err = c.call(ctx, "POST", "/v1/read-executions/"+id+"/reconcile", "", struct{}{}, &out)
	return out, err
}
