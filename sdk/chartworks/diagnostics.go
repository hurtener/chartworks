...
type Check struct { Operation RegisteredOperation `json:"operation"`;Allowed bool `json:"allowed"` }
// Diagnostics needs ops.inspect and exact tenant read reach.
func(c *Client) Diagnostics(ctx context.Context)([]Check,error){var checks []Check;err:=c.call(ctx,"GET","/v1/access/diagnostics","",nil,&checks);return checks,err}
...
func(c *Client) Metrics(ctx context.Context)(string,error){var text string;err:=c.call(ctx,"GET","/metrics","",nil,&text);return text,err}
