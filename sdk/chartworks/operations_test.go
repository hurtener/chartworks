...
func unitOperationServer(t *testing.T, prefix string, serve http.HandlerFunc) (*Client, *httptest.Server, []byte) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if !strings.HasPrefix(r.URL.Path, prefix+"/") { t.Error("base prefix escaped"); w.WriteHeader(404); return }
...
func TestOperationCatalogAndInvocation(t *testing.T) {
	...
	client, _, document := unitOperationServer(t, "/charts", func(w http.ResponseWriter, r *http.Request) {
		calls.Add(1)
		if r.Header.Get("Authorization") != "Bearer synthetic-current" { t.Error("missing current provider token") }
		switch r.URL.Path {
		case "/charts/v1/reads/item":
			if r.Method == "HEAD" { return }
			if r.URL.Query().Get("limit") != "2" { t.Error("query lost") }
		case "/charts/v1/writes":
			body,_ := io.ReadAll(r.Body)
			if string(body) != "{}" { t.Error("empty contract changed") }
		case "/charts/v1/uploads":
			body,_ := io.ReadAll(r.Body)
			if string(body) != "raw\x00bytes" || r.Header.Get("Content-Type") != "application/octet-stream" { t.Error("binary upload changed") }
		case "/charts/v1/text":
			_,_ = io.WriteString(w,"fixture_total 2\n")
			return
		default: t.Error("unregistered path",r.URL.Path)
		}
		_,_ = io.WriteString(w,unitResult)
	})
	rows, err := client.Operations(t.Context())
	if err != nil || len(rows) != 8 { t.Fatal("catalog",len(rows),err) }
	parsed, err := ParseOperations(document)
	if err != nil || len(parsed) != len(rows) { t.Fatal("offline matrix",err) }
	for i,row := range rows {
		if i>0 && rows[i-1].ID >= row.ID { t.Fatal("unstable ordering") }
		if row.ID=="mcpFixture" && (row.SDKMethod!="MCP" || row.CLICommand!="client mcp") { t.Fatal("MCP projection") }
		if row.ID=="eraseFixture" && row.Replay!="keyed" { t.Fatal("logical key not registered") }
		rows[i].Path="/v1/evil"
	...
	if err != nil || string(result.Body)!=unitResult { t.Fatal("lossless read",string(result.Body),err) }
	if _,err=client.Invoke(t.Context(),"writeFixture",CallOptions{});err!=nil { t.Fatal("empty request",err) }
	if _,err=client.Invoke(t.Context(),"uploadFixture",CallOptions{Body:[]byte("raw\x00bytes")});err!=nil { t.Fatal("raw request",err) }
	result,err=client.Invoke(t.Context(),"readHead",CallOptions{ResourceID:"item"})
	if err!=nil || len(result.Body)!=0 { t.Fatal("bodyless HEAD",err) }
	result,err=client.Invoke(t.Context(),"textFixture",CallOptions{})
	if err!=nil || string(result.Body)!="fixture_total 2\n" || !strings.HasPrefix(result.ContentType,"text/plain") { t.Fatal("text response",err) }
	if calls.Load()!=5 { t.Fatal("unexpected core calls",calls.Load()) }
}
...
func TestInvocationRejectsUnsafeCoordinatesAndReplays(t *testing.T) {
	...
	cases:=[]struct{id string; options CallOptions; want error}{
	...
	for _,test:=range cases {
		...
		if !errors.Is(err,test.want) { t.Errorf("%s: %v, want %v",test.id,err,test.want) }
		if err!=nil && strings.Contains(err.Error(),"PRIVATE") { t.Fatal("input echoed") }
	}
	if calls.Load()!=0 { t.Fatal("invalid invocation reached a core",calls.Load()) }
}
...
func TestExplicitRetryPreservesKeyBodyAndFreshProvider(t *testing.T) {
	...
	client,_,_:=unitOperationServer(t,"",func(w http.ResponseWriter,r *http.Request){
		n:=calls.Add(1)
		body,_:=io.ReadAll(r.Body)
		if string(body)!=`{"value":1}` || r.Header.Get("Idempotency-Key")!="logical-once" { t.Error("replay changed intent") }
		if n==1 { copy(original,[]byte(`{"value":2}`)); w.WriteHeader(503); return }
		_,_=io.WriteString(w,unitResult)
	})
	...
	if err!=nil || string(result.Body)!=unitResult || calls.Load()!=2 || providers.Load()!=3 { t.Fatal("bounded replay",calls.Load(),providers.Load(),err) }
	for _,status:=range []int{401,403,404,409,410,500} {
		var attempts atomic.Int64
		c,_,_:=unitOperationServer(t,"",func(w http.ResponseWriter,_ *http.Request){attempts.Add(1);w.WriteHeader(status);_,_=io.WriteString(w,"PRIVATE_SERVER_ERROR")})
		_,err=c.Invoke(t.Context(),"readFixture",CallOptions{ResourceID:"item",Attempts:3})
		var rejected *StatusError
		if !errors.As(err,&rejected) || rejected.Status!=status || attempts.Load()!=1 || strings.Contains(err.Error(),"PRIVATE") { t.Fatal("terminal status retried",status,attempts.Load(),err) }
	}
	...
	if !errors.Is(err,context.Canceled) || attempts.Load()!=1 { t.Fatal("cancellation lost during replay",err,attempts.Load()) }
}
...
func TestCatalogRejectsAmbiguousOrForeignMetadata(t *testing.T) {
	...
	mutations:=[]func(map[string]any){
		...
		func(root map[string]any){root["paths"].(map[string]any)["https://outside.example/v1/read"]=map[string]any{}},
		func(root map[string]any){root["paths"].(map[string]any)["/v1/../escape"]=map[string]any{}},
		func(root map[string]any){op:=root["paths"].(map[string]any)["/v1/writes"].(map[string]any)["post"].(map[string]any);op["operationId"]="readFixture"},
		func(root map[string]any){op:=root["paths"].(map[string]any)["/v1/writes"].(map[string]any)["post"].(map[string]any);op["x-chartworks-auth"]="local_admin"},
		func(root map[string]any){op:=root["paths"].(map[string]any)["/v1/writes"].(map[string]any)["post"].(map[string]any);op["x-chartworks-max-body-bytes"]=0},
		func(root map[string]any){op:=root["paths"].(map[string]any)["/v1/writes"].(map[string]any)["post"].(map[string]any);delete(op,"requestBody")},
		func(root map[string]any){op:=root["paths"].(map[string]any)["/v1/writes"].(map[string]any)["post"].(map[string]any);delete(op,"responses")},
		func(root map[string]any){op:=root["paths"].(map[string]any)["/v1/reads/{id}"].(map[string]any)["get"].(map[string]any);delete(op,"parameters")},
		func(root map[string]any){op:=root["paths"].(map[string]any)["/v1/reads/{id}"].(map[string]any)["get"].(map[string]any);op["parameters"]=[]any{map[string]any{"name":"id","in":"cookie","required":true,"schema":map[string]any{"type":"string"}}}},
	}
	for i,mutate:=range mutations {
		var value map[string]any
		if json.Unmarshal(document,&value)!=nil { t.Fatal("fixture JSON") }
		mutate(value)
		bad,_:=json.Marshal(value)
		if _,err=ParseOperations(bad);!errors.Is(err,ErrInvalidCatalog) { t.Errorf("mutation %d accepted: %v",i,err) }
	}
	for _,bad:=range [][]byte{nil,[]byte(`{"paths":{},"paths":{}}`),[]byte(strings.Repeat("x",operationCatalogLimit+1))} {
		if _,err=ParseOperations(bad);!errors.Is(err,ErrInvalidCatalog) { t.Fatal("unbounded/ambiguous catalog",err) }
	}
}
...
func TestOperationMatrixMatchesBothSuppliedAudiences(t *testing.T) {
	...
	client,server,_:=unitOperationServer(t,"",func(w http.ResponseWriter,r *http.Request){
		if r.URL.Path!="/v1/mcp" || r.Header.Get("Authorization")!="Bearer synthetic-mcp" || r.Header.Get("Mcp-Protocol-Version")=="" { t.Error("MCP audience or mount lost") }
		id,action:="readFixture","ops.read"
		if mode.Load()==1 { id="unbuiltReport" }
		if mode.Load()==2 { action="ops.maintain" }
		if mode.Load()==3 { _,_=io.WriteString(w,`{"jsonrpc":"2.0","id":1,"error":{"code":-32603}}`);return }
	...
	if err!=nil { t.Fatal(err) }
	found:=false
	for _,row:=range rows { if row.ID=="readFixture" { found=row.MCPTool=="read_fixture" && row.SDKMethod=="Invoke" } }
	if !found { t.Fatal("missing generated surface link") }
	if _,err=client.OperationMatrix(t.Context(),nil);err!=nil { t.Fatal("optional MCP lookup",err) }
	for _,bad:=range []int64{1,2,3} { mode.Store(bad);if _,err=client.OperationMatrix(t.Context(),mcp);!errors.Is(err,ErrInvalidCatalog) { t.Fatal("MCP contract drift accepted",bad,err) } }
}
...
func TestInvocationRejectsMalformedResponseAndDeadline(t *testing.T) {
	...
	if _,err:=client.Invoke(t.Context(),"readFixture",CallOptions{ResourceID:"item"});err==nil || strings.Contains(err.Error(),"PRIVATE") { t.Fatal("response schema ignored",err) }
	ctx,cancel:=context.WithCancel(t.Context());cancel()
	if _,err:=client.Invoke(ctx,"readFixture",CallOptions{ResourceID:"item"});!errors.Is(err,context.Canceled) { t.Fatal("cancellation lost",err) }
	...
	if _,err:=client.Operations(t.Context());!errors.Is(err,context.DeadlineExceeded) { t.Fatal("provider deadline lost",err) }
}
